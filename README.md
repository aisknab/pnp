...
| **What is the current verification status?** | Formal reconstruction is in progress. Lean now has an axiom-free, universally correct finite raw-machine verifier for canonically encoded CNF formula/certificate pairs, with an explicit polynomial bound; this establishes `PNP.Concrete.CNFSAT ∈ NP`. A separate local simulator lifts every supplied exact chain of `n` successful raw transitions from an already represented boundary frame to exactly `3 * n` successful work transitions. It also extracts from an ordinary raw run with fuel `F` an exact prefix of length `k ≤ F` reaching the same endpoint. The full-budget theorem additionally requires that endpoint to be designated halting: `workRun` with fuel `3 * F` and compiled-machine `run` with fuel `18 * F` reach the represented endpoint and its encoding. These are at-most fuel budgets in source-transition count, not successful-step counts or input-size bounds. A separate finite work machine starts from the compiled machine's literal canonical `BitString.pair left right` input, produces a valid boundary frame with permitted exterior garbage, and halts accepting at an exact quadratic input-length fuel. Another finite work machine re-frames an already represented tape at its blank-delimited logical output in exactly `2 * n + 4` work steps, or `12 * n + 24` compiled steps, for logical output length `n`. A new axiom-free namespace layer injectively renames all three work machines into pairwise-disjoint state images, proves ordered first-match lookup is preserved, concatenates the three finite rule lists, and transports each existing exact stage-local trace. This is collision-free table composition only: no bridge transition launches the simulator from the framer halt or launches the handoff from a simulator halt. The handoff trace still starts from an encoded internal configuration rather than ordinary `startConfig`, and two-track tags do not provide ordinary raw `machineOutput` semantics. The result does **not** prove target-machine termination, promote a stuck nonhalting stop to a verdict, prove simulated-stage `boundedDecide` or target verdict/output semantics, provide terminal raw output de-tagging, construct composition/precomposition refinement, establish `CNFSAT ∈ P`, prove NP-hardness or NP-completeness, or prove `P = NP`. `PNP.Main.p_eq_np` remains absent and all activation fingerprints remain unset, so the publication gate stays fail-closed. |
...
| **What remains formally?** | The Lean toolchain/root, concrete bitstring and finite-rule machine kernel, boundary geometry, a paired-input executable framer, the local exact-successful-run simulator, an executable internal represented-output handoff, collision-free three-stage state renaming and lookup-isolated rule-table concatenation, the finite charged-pipeline P/NP/reduction interface, the direct raw-machine proof that `CNFSAT ∈ NP`, direct-wire layers, local locked-NAND baselines, the conditional six-field threshold deduction, and a fail-closed explicit-list gain scanner are formalized and axiom-audited. Bridge transitions and end-to-end execution that launch the lifted simulator from the framer and later launch the internal handoff, a `boundedDecide` halt/timeout/verdict bridge, terminal raw output de-tagging, a full pipeline-to-raw-machine refinement with an external input-size polynomial bound, a deterministic decider proving `CNFSAT ∈ P`, concrete NP-hardness/NP-completeness, global route completeness, the global locked-NAND construction, residual-band/`ZeroSlack`, and the root-theorem audit remain. `PNP.Main.ConcretePEqualsNP` exists only as an inactive definition; `PNP.Main.p_eq_np` is absent, and abstract `PNP.PEqualsNP` is not publication-eligible. |
...
canonical `Tape.ofInput` to the frame inside the simulator theorem, prove `boundedDecide` or output preservation, or provide
bridge transitions, composition/precomposition refinement, or an input-size polynomial bound. The
separate namespace layer transports this trace into a disjoint simulation-state image, but does not
launch it from the framer or launch the handoff afterward; the two-track blank tags are not a
canonical `machineOutput` encoding.

The paired-input framer is a separate executable machine. Its compiled theorem starts literally at
`startConfig (compileWorkMachine pairedInputFramer) (BitString.pair left right)`, copies the packed
pair into a fresh marked region, permits the consumed source cells to remain as exterior garbage,
and halts accepting with the head on the first framed data cell. Its exact work budget for `k`
packed cells is `4 * k * k + 9 * k + 7`; the compiled raw budget is the corresponding polynomial
`6 * m * m + 27 * m + 42` in raw paired-input length `m = 2 * k`. This is not a universal framer for
arbitrary, empty, odd, or malformed raw words. Its states are now injectively retagged into an input-stage image and its rules occur in a
lookup-isolated concatenated three-stage table, but no bridge rule changes the renamed accepting
state into the renamed lifted-machine start state. Its local
`boundedDecide` theorem states only `.accept` for the framer at its proved budget; no simulated
target verdict or output theorem follows from framing alone.
...
`encodeWorkConfiguration` of the internal represented configuration. It does not start from
ordinary `startConfig`, receive a bridge transition from either renamed simulator terminal state,
or prove an external input-size polynomial. Its states and rules now occupy a disjoint handoff-stage
image in the concatenated table, but that table supplies no stage launch. Its two-track encoded endpoint is not a terminal raw-output layout:
ordinary raw `machineOutput` can stop at a tag blank between logical data symbols. Terminal output
therefore still needs a separate packer or de-tagger.
...
| Executable canonical paired-input framer | `lean-audit/PNPConcretePipelineInputFramerAxiomAudit.lean` and `audits/lean-concrete-pipeline-input-framer0.test.mjs` | One literal finite work machine starts from compiled canonical paired input, reaches a represented boundary frame with permitted exterior garbage, halts accepting, and has exact work and compiled raw polynomial budgets. | It is restricted to `BitString.pair` inputs and is not composed with `liftMachine`; it proves no simulated-stage termination, verdict/output preservation, handoff, pipeline refinement, class equivalence, or `P = NP`. |
| Executable internal represented-output handoff | `lean-audit/PNPConcretePipelineOutputHandoffAxiomAudit.lean` and `audits/lean-concrete-pipeline-output-handoff0.test.mjs` | One literal finite work machine turns an already represented tape into a represented `Tape.handoffTarget`, preserves the logical blank-delimited output, halts accepting, and has exact `2 * n + 4` work and `12 * n + 24` compiled costs. | Its compiled trace starts from an encoded internal configuration, not ordinary `startConfig`; it is not connected to simulator states and proves no raw-visible `machineOutput`, terminal de-tagging, composition, refinement, external input-size polynomial, class equivalence, or `P = NP`. |
| Collision-free pipeline state namespace | `lean-audit/PNPConcretePipelineStateNamespaceAxiomAudit.lean` and `audits/lean-concrete-pipeline-state-namespace0.test.mjs` | Injective work-machine renaming preserves ordered first-match lookup, halting, step/run/exact-run semantics, start configurations, and bounded verdicts; three pairwise-disjoint stage images give one lookup-isolated concatenated rule table and preserve all three existing exact stage-local traces. | There are no bridge rules, no framer-to-simulator or simulator-to-handoff launch theorem, no complete composed execution, no terminal raw-output packer, no pipeline `RawRefinement`, no external input-size polynomial, and no `P = NP`. |
...
- [Lean executable paired-input framer](./docs/lean_pipeline_input_framer.md): a literal finite machine from canonical paired input to a represented frame, with exact quadratic work and compiled raw budgets.
...
- [Lean collision-free pipeline state namespace](./docs/lean_pipeline_state_namespace.md): injective three-stage renaming, lookup-isolated finite rule-table concatenation, and exact stage-local trace transport without bridge transitions.
