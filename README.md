...
| **What remains formally?** | The Lean toolchain/root, concrete bitstring and finite-rule machine kernel, boundary geometry, a paired-input executable framer, the local exact-successful-run simulator, finite charged-pipeline P/NP/reduction interface, the direct raw-machine proof that `CNFSAT ∈ NP`, direct-wire layers, local locked-NAND baselines, the conditional six-field threshold deduction, and a fail-closed explicit-list gain scanner are formalized and axiom-audited. A state-safe composition that launches the lifted simulator from the framer, a `boundedDecide` halt/timeout/verdict bridge, output de-tagging and terminal handoff, a full pipeline-to-raw-machine refinement with an input-size polynomial bound, a deterministic decider proving `CNFSAT ∈ P`, concrete NP-hardness/NP-completeness, global route completeness, the global locked-NAND construction, residual-band/`ZeroSlack`, and the root-theorem audit remain. `PNP.Main.ConcretePEqualsNP` exists only as an inactive definition; `PNP.Main.p_eq_np` is absent, and abstract `PNP.PEqualsNP` is not publication-eligible. |
