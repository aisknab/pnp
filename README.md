...
| **What is the current verification status?** | Formal reconstruction is in progress. Lean proves `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT` using a literal finite raw-machine verifier with an explicit polynomial bound. It recursively compiles every finite charged function/decision program into one literal raw machine, discharging `Formal.ConcreteComplexityMachineLink`. The concrete Cook–Levin formula is semantically equivalent to raw verifier execution; its canonical encoding has an explicit polynomial size bound and exact answer-independent schedule/cursor. A fixed 19-rule builder stage preserves every input bit while tallying its length, and a new collision-free finite table composes the total raw-input framer, one literal launch, and that tally within `18*n^2 + 63*n + 93` compiled steps. Lean still does **not** emit formula bits or provide a complete polynomial-time raw formula builder or concrete `PolynomialReduction`, prove `CNFSAT ∈ P`, prove NP-hardness/NP-completeness, or prove `P = NP`. `PNP.Main.p_eq_np` remains absent and the publication gate stays false. |
