...
| Concrete publication gate and report | `node scripts/generate-formal-publication.mjs --check` and `npm run report:check` | Status and the concise thirteen-page canonical report match the compiled inventory and the false, fail-closed gate. | `P = NP`, permission to emit a theorem, or validation of the historical claim report. |
...
[TeX](./canonical_proof_report.tex) form a generated, concise thirteen-page reconstruction report. It
