...
| **What is the current verification status?** | Formal reconstruction is in progress. Lean proves `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT` using a literal finite raw-machine verifier with an explicit polynomial bound. The finite pipeline foundation has an axiom-free bridged work machine: canonical paired input is framed, launched into the lifted simulator, and a supplied exact target accept/reject trace is launched into a verdict-indexed represented-output handoff. `PipelineTerminalBridge` now appends two disjoint terminal-packer copies, preserves every successful earlier bridge step, and for each supplied exact accepting or rejecting target run proves one complete four-stage trace, exact verdict, compiled execution, and ordinary raw-output equality. Its local suffix bound is `18*n^2 + 36*n + 12`. Lean therefore does **not** yet prove target termination, a uniform all-input `RawRefinement`, an external-input-size polynomial, `CNFSAT ∈ P`, NP-hardness, NP-completeness, or `P = NP`. `PNP.Main.p_eq_np` remains absent and the publication gate stays false. |
...
accepting or rejecting target execution. No theorem supplies target termination, an all-input
`RawRefinement`, or an external-input-size polynomial.
