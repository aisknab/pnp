...
| **What is the current verification status?** | Formal reconstruction is in progress. Lean proves `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT` using a literal finite raw-machine verifier with an explicit polynomial bound. The finite pipeline foundation now also has one axiom-free bridged work machine: canonical paired input is framed, launched into the lifted simulator in one step, and a supplied exact target accept/reject trace is launched into one of two verdict-indexed represented-output handoff copies. First-match dispatch is isolated, target accept/reject classification is preserved, a supplied stuck nonhalting endpoint is timeout at the exact prefix budget, and compilation from ordinary paired raw input costs exactly six times `inputFramerWorkSteps + 1 + 3 * n + 1 + framedOutputHandoffWorkSteps`. This remains conditional on a supplied exact target run and ends in an internal two-track representation. It does **not** prove target termination, terminal raw `machineOutput`, an external-input-size pipeline polynomial, a complete `RawRefinement`, `CNFSAT ∈ P`, NP-hardness, NP-completeness, or `P = NP`. `PNP.Main.p_eq_np` remains absent and the publication gate stays false. |
...
| **What remains formally?** | The Lean toolchain/root, concrete machine kernel, boundary geometry, paired-input framer, exact-run simulator, represented-output handoff, collision-free namespaces, explicit stage launches, verdict-preserving internal execution, finite charged-pipeline P/NP/reduction interface, direct proof that `CNFSAT ∈ NP`, direct-wire layers, local locked-NAND baselines, conditional threshold deduction, and fail-closed explicit-list gain scanner are formalized and axiom-audited. Terminal raw-output packing, ordinary `machineOutput` equality, complete pipeline `RawRefinement`, an external encoded-input-size polynomial, a deterministic decider proving `CNFSAT ∈ P`, concrete NP-hardness/NP-completeness, global locked-NAND construction and threshold, residual-band minimization, global `ZeroSlack`, assumption elimination, and the root-theorem audit remain. `PNP.Main.ConcretePEqualsNP` is only an inactive definition; `PNP.Main.p_eq_np` is absent. |
