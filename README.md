...
| **What remains formally?** | The Lean toolchain/root, direct-wire NAND semantics, exact-width enumerator, executable truth-table equivalence, exhaustive empty-context reference minimum/residual slack, a concrete serial framed replacement/slack law, and the local locked-NAND candidate/source-accounting/square-baseline layer are formalized and axiom-audited. Global cross-instance baseline distinctness, the locked builder and threshold, arbitrary-support replacement/slack, concrete complexity and SAT definitions, the residual-band minimizer and `ZeroSlack`, polynomial bounds, and the root-theorem axiom audit remain. `PNP.Main.p_eq_np` is absent. |
