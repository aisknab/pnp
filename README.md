...
| **What is the current verification status?** | Formal reconstruction is in progress. Lean proves `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT` using a literal finite raw-machine verifier with an explicit polynomial bound. The finite pipeline foundation has an axiom-free bridged work machine: canonical paired input is framed, launched into the lifted simulator, and a supplied exact target accept/reject trace is launched into a verdict-indexed represented-output handoff. `PipelineTerminalBridge` now appends two disjoint terminal-packer copies and proves the local accepting/rejecting handoff-to-packer launches, terminal halts, compiled raw output equality, and local bound `18*n^2 + 36*n + 12`. The earlier ordinary-input trace has not yet been transported into that extended machine, and it remains conditional on a supplied exact target run. Lean therefore does **not** yet prove target termination, one complete four-stage trace and `RawRefinement`, an external-input-size polynomial, `CNFSAT ∈ P`, NP-hardness, NP-completeness, or `P = NP`. `PNP.Main.p_eq_np` remains absent and the publication gate stays false. |
...
| **What remains formally?** | The Lean toolchain/root, concrete machine kernel, boundary geometry, paired-input framer, exact-run simulator, represented-output handoff, collision-free namespaces, explicit stage launches, verdict-preserving internal execution, terminal raw-output packing, the local handoff-to-packer suffix, finite charged-pipeline P/NP/reduction interface, direct proof that `CNFSAT ∈ NP`, direct-wire layers, local locked-NAND baselines, conditional threshold deduction, and fail-closed explicit-list gain scanner are formalized and axiom-audited. Transport of the earlier ordinary-input trace into the extended terminal-bridge machine, complete four-stage composition and pipeline `RawRefinement`, external encoded-input-size polynomial, deterministic decider proving `CNFSAT ∈ P`, concrete NP-hardness/NP-completeness, global locked-NAND construction and threshold, residual-band minimization, global `ZeroSlack`, assumption elimination, and root-theorem audit remain. `PNP.Main.ConcretePEqualsNP` is only an inactive definition; `PNP.Main.p_eq_np` is absent. |
...
lookup-isolated bridge-first table. `inputLaunch_workStep` changes the renamed accepting state into
the renamed lifted-machine start state in exactly one symbol-preserving step. The local framer
theorem alone still states only `.accept`; target behavior comes from a supplied exact target run.
...
the local bound `18*n^2 + 36*n + 12`. The earlier cumulative trace is still a theorem about
`bridgedMachine`; it has not yet been transported through `terminalBridgeMachine`, so there is no
single four-stage trace or external-input-size polynomial.
