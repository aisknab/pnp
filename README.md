...
The historical [PDF](./canonical_proof_report.pdf) and [TeX](./canonical_proof_report.tex) state the former candidate-proof claim. They are retained for provenance and are not current theorem-status documents.
...
The generator is untrusted. The checker compares canonical bytes rather than digest equality. A reject run emits a replayable first failure and no public theorem conclusion. An accept run is not current theorem evidence; current theorem status is controlled by the formal reconstruction gate.
...
The primary JavaScript utility module is [`pcc-core.mjs`](./pcc-core.mjs). It provides parsing, canonicalization, digest, row-key, and scoped checker utilities. These utilities are infrastructure, not a replacement for a formal proof kernel.
