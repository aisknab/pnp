...
| **What is the current verification status?** | Formal reconstruction is in progress. Lean now has an axiom-free, universally correct finite raw-machine verifier for canonically encoded CNF formula/certificate pairs, with an explicit polynomial bound; this establishes `PNP.Concrete.CNFSAT ∈ NP`. A separate local simulator lifts every supplied exact chain of `n` successful raw transitions from an already represented boundary frame to exactly `3 * n` successful work transitions; ordinary compiled-machine `run` with fuel `18 * n` reaches the encoded endpoint. These factors are in source-transition count, not input length. The result does **not** create an input frame, prove bounded-decision or output semantics, hand off output, compile a complete pipeline, establish `CNFSAT ∈ P`, prove NP-hardness or NP-completeness, or prove `P = NP`. `PNP.Main.p_eq_np` remains absent and all activation fingerprints remain unset, so the publication gate stays fail-closed. |
...
exactly `3 * n` successful work steps to a represented endpoint. The existing compiler theorem says
that ordinary raw `run` with fuel `18 * n` reaches the encoded work endpoint; it does not assert
`18 * n` successful transitions. This does not connect canonical `Tape.ofInput` to the frame, and
the two-track blank tags are not a canonical `machineOutput` encoding.
...
| Local framed raw-machine simulation | `lean-audit/PNPConcretePipelineMachineSimulationAxiomAudit.lean` and `audits/lean-concrete-pipeline-machine-simulation0.test.mjs` | Ordered finite rules preserve raw first-match selection, omit terminal-source entries, shift markers across arbitrary exterior garbage, and lift every supplied exact `n`-step successful raw execution to exactly `3 * n` successful work steps; compiled `run` with fuel `18 * n` reaches the encoding. | It starts from an already represented frame. The fuel factors are in transition count, not input length, and it supplies no frame creator, `boundedDecide` or output theorem, decoder/handoff, composition refinement, input-size polynomial end-to-end bound, class equivalence, or `P = NP`. |
...
- [Lean local framed machine simulation](./docs/lean_pipeline_machine_simulation.md): ordered first-match lifting of an exact successful run to `3 * n` work steps and compiled-run fuel `18 * n` from an already represented start.
