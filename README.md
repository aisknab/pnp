...
| **What remains formally?** | The Lean toolchain/root, direct-wire NAND semantics, exact-width enumerator, executable truth-table equivalence, exhaustive empty-context reference minimum/residual slack, and a concrete serial framed replacement/slack law are formalized and axiom-audited. Arbitrary-support replacement/slack, concrete complexity and SAT definitions, the locked-NAND threshold, residual-band minimizer and `ZeroSlack`, polynomial bounds, and the root-theorem axiom audit remain. `PNP.Main.p_eq_np` is absent. |
