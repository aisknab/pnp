...
| **What is the current verification status?** | Formal reconstruction is in progress. Lean proves `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT` using a literal finite raw-machine verifier with an explicit polynomial bound. It recursively compiles every finite charged function/decision program into one literal raw machine, discharging `Formal.ConcreteComplexityMachineLink`. The concrete Cook–Levin formula is semantically equivalent to raw verifier execution; its canonical encoding has an explicit polynomial size bound and exact answer-independent schedule/cursor. The builder emits the complete first four populated clauses, traverses the remainder of the first scheduled constraint, emits the second constraint's separator and first positive literal through its width-selected successor, and consumes the next seven schedule opportunities: it emits nothing for width-one padding or the first four unary `T` tokens, the second literal's terminating `F`, and the following literal's opening and first unary-index `T` at wider widths. Separately, the global locked-NAND construction now has an exact typed semantic threshold, strict external bytes, and a proved all-bitstring semantic transformation containing the complete candidate and source-derived threshold. A literal 228-state, 2,052-rule source parser is proved correct for every bitstring: it accepts exactly the strict-v0 valid encodings, preserves every valid source byte-for-byte, clears every invalid source to the empty word, and its compiled machine cannot time out within `6 × 4096 × (n+1)^3` raw transitions. The validator is packaged as a polynomial-time language machine, a nonexpanding polynomial-time function, and an exact leaf raw-machine refinement. Lean still does **not** consume the following Cook–Levin padding-or-second-unary-index-`T` slot, finish that formula builder, implement the locked-NAND target emitter, compose the parser and emitter into the required concrete `PolynomialReduction`, prove `CNFSAT ∈ P`, prove NP-hardness/NP-completeness, discharge the abstract locked-NAND threshold language, or prove `P = NP`. `PNP.Main.p_eq_np` remains absent and the publication gate stays false. |
...
| **What remains formally?** | The Lean toolchain/root, concrete machine kernel, all executable pipeline stages, recursive function/decision `RawRefinement`, the finite charged-pipeline P/NP/reduction interface, direct proof that `CNFSAT ∈ NP`, the current Cook–Levin schedule/builder prefix, and the locked-NAND baseline, complete candidate, both final branches, typed threshold, strict codec, normalization semantics, complete target bytes, pure semantic reduction, and total strict-v0 source parser are formalized and axiom-audited. The parser boundary includes all-input exact accept/reject behavior, byte-preserving valid output, empty invalid output, compiled non-timeout at the explicit cubic bound, polynomial-time machine/function witnesses, and its leaf raw refinement. The remaining work includes finishing the Cook–Levin builder and reduction; implementing and bounding the complete locked-NAND emitter; composing the parser and emitter and packaging the required concrete `PolynomialReduction`; discharging the abstract locked-NAND threshold language; proving the required NP-hardness transport and a deterministic `CNFSAT ∈ P` decider; and formalizing residual-band minimization, global `ZeroSlack`, assumption elimination, and the root theorem audit. `PNP.Main.ConcretePEqualsNP` is only an inactive definition; `PNP.Main.p_eq_np` is absent. |
...
- [Lean strict-v0 locked-NAND source parser](./docs/lean_concrete_locked_nand_source_parser.md): literal rule table, all-input exact accept/reject behavior, byte-preserving-or-empty output, compiled cubic non-timeout, polynomial machine/function witnesses, raw refinement, and the remaining emitter/composition boundary.
