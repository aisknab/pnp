...
| **What remains formally?** | The Lean toolchain/root, axiom-free direct-wire NAND semantics, and exact-width syntactic enumerator are formalized. Semantic equivalence, minimum size, compatible replacement and slack, concrete complexity and SAT definitions, the locked-NAND threshold, residual-band minimizer and `ZeroSlack`, polynomial bounds, and the root-theorem axiom audit remain. `PNP.Main.p_eq_np` is absent. |
