...
| **What is the current verification status?** | Formal reconstruction is in progress. Lean now has an axiom-free, universally correct finite raw-machine verifier for canonically encoded CNF formula/certificate pairs, with an explicit polynomial bound; this establishes `PNP.Concrete.CNFSAT ∈ NP`. It does **not** establish `CNFSAT ∈ P`, NP-hardness or NP-completeness, or `P = NP`. The axiom-free definition `PNP.Main.ConcretePEqualsNP` still only names the finite charged-pipeline target: `PNP.Main.p_eq_np` is absent, the general charged-pipeline compiler/refinement is unproved, and all activation fingerprints are intentionally unset. The publication gate therefore remains fail-closed and public theorem emission remains disabled. |
...
| **What remains formally?** | The Lean toolchain/root, concrete bitstring and finite-rule machine kernel, finite charged-pipeline P/NP/reduction interface, the direct raw-machine proof that `CNFSAT ∈ NP`, direct-wire layers, local locked-NAND baselines, the conditional six-field threshold deduction, and a fail-closed explicit-list gain scanner are formalized and axiom-audited. A deterministic polynomial-time decider proving `CNFSAT ∈ P`, concrete NP-hardness/NP-completeness, the general pipeline-to-raw-machine compiler/refinement, global route completeness, carrier layout, cross-instance baseline distinctness, trace equivalence, derived final-output laws, the uniform builder and report threshold, unconditional slack at most four, residual-band/`ZeroSlack`, remaining end-to-end polynomial bounds, and the root-theorem audit remain. `PNP.Main.ConcretePEqualsNP` exists only as an inactive definition; `PNP.Main.p_eq_np` is absent, and abstract `PNP.PEqualsNP` is not publication-eligible. |
