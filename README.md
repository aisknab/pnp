...
| Conditional locked-NAND threshold boundary | `node --test audits/lean-locked-nand-threshold-boundary0.test.mjs` and `lean-audit/PNPLockedNANDThresholdBoundaryAxiomAudit.lean` | From actual typed candidates plus six explicit semantic premises, Lean derives the conditional unsat/sat minimum boundary and conditional residual slack at most four. Later global milestones instantiate five fields. | The remaining `satisfiableFinalConditions` field, the report threshold theorem, an answer-independent polynomial builder, SAT, or `P = NP`. |
...
| Global locked-NAND unsatisfiable final-zero branch | `node --test audits/lean-locked-nand-global-unsatisfiable-final-zero0.test.mjs`, `lean-audit/PNPLockedNANDGlobalUnsatisfiableFinalZeroAxiomAudit.lean`, and `lean-regression/PNPLockedNANDGlobalUnsatisfiableFinalZero.lean` | If the source circuit is unsatisfiable, the complete final output is false for every carrier valuation and the exhaustive reference minimum is exactly `B`. | The satisfiable final-output conditions, completion of the six-field threshold package, the uniform polynomial builder, the locked-NAND threshold, global residual slack at most four, SAT, or `P = NP`. |
