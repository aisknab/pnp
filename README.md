...
canonical `Tape.ofInput` to the frame, prove `boundedDecide` or output preservation, or provide
handoff, composition/precomposition refinement, or an input-size polynomial bound; the two-track
blank tags are not a canonical `machineOutput` encoding.
...
arbitrary, empty, odd, or malformed raw words. Its accepting state is not retagged as a lifted
machine's start state, and its finite rule list is not combined with the simulator. Its local
...
ordinary `startConfig`, connect simulator terminal states to its start state, or prove an external
input-size polynomial. Its two-track encoded endpoint is not a terminal raw-output layout:
