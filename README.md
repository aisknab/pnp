...
| **What is the current verification status?** | Formal reconstruction is in progress. Lean proves `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT` using a literal finite raw-machine verifier with an explicit polynomial bound. The finite pipeline foundation has an axiom-free bridged work machine: canonical paired input is framed, launched into the lifted simulator, and a supplied exact target accept/reject trace is launched into a verdict-indexed represented-output handoff. A separate axiom-free `TerminalOutputPacker` now uniformly turns canonical represented logical output into ordinary blank-delimited raw output, with exact compiled output equality, local bound `18*n^2 + 36*n + 6`, and one-step-short timeout. The packer is not yet launched from the bridge endpoint, and the bridge remains conditional on a supplied exact target run. Lean therefore does **not** yet prove target termination, one complete pipeline machine and `RawRefinement`, an external-input-size polynomial, `CNFSAT ∈ P`, NP-hardness, NP-completeness, or `P = NP`. `PNP.Main.p_eq_np` remains absent and the publication gate stays false. |
...
`encodeWorkConfiguration` of the internal represented configuration. The bridge module now launches
simulator accept and reject sentinels into two disjoint copies and compiles the cumulative trace from
ordinary paired `startConfig`, but it does not prove an external input-size polynomial. Its two-track
encoded endpoint is not a terminal raw-output layout:
...
- [Lean executable pipeline stage bridges](./docs/lean_pipeline_stage_bridges.md): exact framer/simulator/handoff launches, verdict-indexed handoff copies, cumulative work/raw costs, and the remaining terminal-output boundary.
