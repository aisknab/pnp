...
## Final 7072f8d external-review package lock

The final external-review package for the residual-hardened 7072f8d release is anchored by:
...
Reviewers should use the cover note for frozen tags/checksums, then use the hostile checklist as the attack plan. The checklist prioritizes locked NAND threshold correctness, residual-band ZeroSlack correctness, final theorem-boundary discipline, and tamper/replay behavior.

## Submission readiness memo

The current status memo for the residual-hardened release is:
...
The current outreach target list and response log are:

```text
review/reviewer_outreach_targets_7072f8d.md
review/reviewer_response_log_7072f8d.md
```

Use these only after the current-release hygiene check has passed. The first wave should include one complexity theorist, one proof-complexity reviewer, and one proof-engineering reviewer.

## Edward Savage round-1 feedback: locked reduction boundary

The first trusted NDA review feedback is recorded in:

```text
review/ed_savage_locked_nand_feedback_round1.md
```

The corresponding manuscript patch adds the named theorem:

```text
Locked-family SAT-hardness bridge
```

Reviewers should attack whether the locked construction is genuinely a polynomial-time SAT-hard reduction target, rather than a restricted topology that has already removed the hard part.
