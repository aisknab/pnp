...
| Current `main` | moving branch | Read reviewer guides and verify the formal-status/archive boundary with `npm run pnp:verify -- --no-write`. It is not the frozen 1,121-test source release. |
...
[`FORMAL_RECONSTRUCTION.md`](../../docs/FORMAL_RECONSTRUCTION.md).
...
Do not expect the 1,121-test count on current `main`. The active package intentionally runs a small
formal-status, package-boundary, archive-integrity, and replay-guard suite.

## Current-main authority and archive check

Use this to check the current non-claiming boundary, not to reproduce the frozen 7072f8d test count:
...
Expected authority fields include:
...
The current checks verify status, surface, immutable archive pins, and safety guards. They neither
execute the legacy replay nor add evidence to the frozen theorem release retroactively.
...
Confirm the pinned source commit. Current `main` commands such as `formal:status`, `legacy:v0:check`,
and `pnp:verify` do not exist in the older frozen source release.
...
Reproducibility is one audit layer. It must be combined with the mathematical, checker-soundness, parser, and complexity reviews described in [reviewer_guide.md](../../docs/reviewer_guide.md), [proof_pipeline.md](../../docs/proof_pipeline.md), and [trust_model.md](../../docs/trust_model.md).
