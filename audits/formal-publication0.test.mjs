...
test('status retains five blockers, no project axioms, and an absent compatibility root', async () => {
  const status = await status0();
...
test('milestone ledger is evidence-backed and keeps premise/global boundaries explicit', async () => {
  ...
  const terminalPacketBN4ActivationRouteReflection = byId.get(
    'residual-terminal-packet-bn4-activation-route-reflection',
  );
  assert.equal(terminalPacketBN4ActivationRouteReflection.status,
    'formalized-residual-terminal-packet-bn4-activation-route-reflection');
  assert.equal(terminalPacketBN4ActivationRouteReflection.earned, true);
  assert.equal(
    terminalPacketBN4ActivationRouteReflection
      .axiomClosureUsesOnlyLeanStandardAllowlist,
    true,
  );
  assert.equal(
    terminalPacketBN4ActivationRouteReflection.requiredTheorems.length,
    28,
  );
  assert.match(terminalPacketBN4ActivationRouteReflection.scope,
    /BN4 activation predicates on every cut/u);
  assert.match(terminalPacketBN4ActivationRouteReflection.scope,
    /activation first route.*frontier and obligation equality.*activation-atom inequality/u);
  assert.match(terminalPacketBN4ActivationRouteReflection.nonClaim,
    /two remaining routes/u);
  assert.match(terminalPacketBN4ActivationRouteReflection.nonClaim,
    /does not construct those coordinates from terminal data/u);
  const terminalPacketDirectionRouteReflection = byId.get(
    'residual-terminal-packet-direction-route-reflection',
  );
  assert.equal(terminalPacketDirectionRouteReflection.status,
    'formalized-residual-terminal-packet-direction-route-reflection');
  assert.equal(terminalPacketDirectionRouteReflection.earned, true);
  assert.equal(
    terminalPacketDirectionRouteReflection
      .axiomClosureUsesOnlyLeanStandardAllowlist,
    true,
  );
  assert.equal(
    terminalPacketDirectionRouteReflection.requiredTheorems.length,
    28,
  );
  assert.match(terminalPacketDirectionRouteReflection.scope,
    /typed direction values/u);
  assert.match(terminalPacketDirectionRouteReflection.scope,
    /direction first route.*frontier, obligation, and activation equality.*typed-direction inequality/u);
  ...
  const terminalPacketBudgetHBActivityBinding = byId.get(
    'residual-terminal-packet-budget-hb-activity-binding',
  );
  assert.equal(terminalPacketBudgetHBActivityBinding.status,
    'formalized-residual-terminal-packet-budget-hb-activity-binding');
  assert.equal(terminalPacketBudgetHBActivityBinding.earned, true);
  assert.equal(
    terminalPacketBudgetHBActivityBinding
      .axiomClosureUsesOnlyLeanStandardAllowlist,
    true,
  );
  assert.equal(
    terminalPacketBudgetHBActivityBinding.requiredTheorems.length,
    5,
  );
  assert.match(terminalPacketBudgetHBActivityBinding.scope,
    /budget mismatch.*activity.*HB budget node/u);
  assert.match(terminalPacketBudgetHBActivityBinding.scope,
    /excludes the budget route/u);
  ...
  const terminalPacketDescentNoLowerBinding = byId.get(
    'residual-terminal-packet-descent-no-lower-binding',
  );
  assert.equal(terminalPacketDescentNoLowerBinding.status,
    'formalized-residual-terminal-packet-descent-no-lower-binding');
  assert.equal(terminalPacketDescentNoLowerBinding.earned, true);
  assert.equal(
    terminalPacketDescentNoLowerBinding
      .axiomClosureUsesOnlyLeanStandardAllowlist,
    true,
  );
  assert.equal(terminalPacketDescentNoLowerBinding.requiredTheorems.length, 4);
  assert.match(terminalPacketDescentNoLowerBinding.scope,
    /scans every canonical handle.*no fully computed first route/u);
  assert.match(terminalPacketDescentNoLowerBinding.scope,
    /checker is forced to reject/u);
  ...
  const terminalHResolveSupportResolver = byId.get(
    'residual-terminal-hresolve-support-resolver',
  );
  assert.equal(terminalHResolveSupportResolver.status,
    'formalized-residual-terminal-hresolve-support-resolver');
  assert.equal(terminalHResolveSupportResolver.earned, true);
  assert.equal(
    terminalHResolveSupportResolver.axiomClosureUsesOnlyLeanStandardAllowlist,
    true,
  );
  assert.equal(terminalHResolveSupportResolver.requiredTheorems.length, 10);
  assert.match(terminalHResolveSupportResolver.scope,
    /complete duplicate-free family.*canonical terminal support seeds/u);
  assert.match(terminalHResolveSupportResolver.scope,
    /exact route.*semantic-minimum.*gain route.*strict-equivalent-gain/u);
  assert.match(terminalHResolveSupportResolver.nonClaim,
    /exhaustive.*may be exponential/u);
  assert.match(terminalHResolveSupportResolver.nonClaim,
    /HN grammar.*BWL.*polynomial HResolve.*ZeroSlack.*PCCMin/u);
  const terminalBudgetEnvelopeResolver = byId.get(
    'residual-terminal-budget-envelope-resolver',
  );
  assert.equal(terminalBudgetEnvelopeResolver.status,
    'formalized-residual-terminal-budget-envelope-resolver');
  assert.equal(terminalBudgetEnvelopeResolver.earned, true);
  assert.equal(
    terminalBudgetEnvelopeResolver.axiomClosureUsesOnlyLeanStandardAllowlist,
    true,
  );
  assert.equal(terminalBudgetEnvelopeResolver.requiredTheorems.length, 7);
  assert.match(terminalBudgetEnvelopeResolver.scope,
    /canonical terminal support.*gate.*interface.*saturated-record caps/u);
  assert.match(terminalBudgetEnvelopeResolver.scope,
    /semantic minimum.*strict equivalent gain.*NoBudget/u);
