...
const SOURCE =
  'lean/PNP/Concrete/CookLevinBuilderFourthClausePaddingRun.lean';
const AXIOM_AUDIT =
  'lean-audit/PNPConcreteCookLevinBuilderFourthClausePaddingRunAxiomAudit.lean';
const REGRESSION =
  'lean-regression/PNPConcreteCookLevinBuilderFourthClausePaddingRun.lean';
const DOCS = 'docs/lean_cook_levin_builder_fourth_clause_padding_run.md';
const TEST =
...
def fifthClauseSlotStartPolynomial
def fifthClauseSlotStart
theorem fifthClauseSlotStart_eq
...
theorem fifthClauseSlotStart_direct_eq_padding
...
def finalTokenSlot
theorem finalTokenSlot_eq_fifthClauseSlotStart
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderFourthClausePrefix.machine problem) (paddingSuffixMachine problem)')
      || !compact.includes(
      ...
      || !compact.includes('(machine problem).rules.length = 4328 +')) {
    failures.push('composed-table');
  }
  if (!compact.includes(
    'remainingPaddingCount problem = problem.formulaTokensPerClause - 9')
      || !compact.includes(
        'BuilderFourthClausePrefix.finalTokenSlot problem + remainingPaddingCount problem = fifthClauseSlotStart problem')
      || !compact.includes(
        'fifthClauseSlotStart problem = problem.formulaVariableSlotBound + 1 + 4 * problem.formulaTokensPerClause')) {
    failures.push('exact-coordinate');
  }
  if (!compact.includes(
    'problem.formulaTokenSlotDirect (fifthClauseSlotStart problem) = some none')
      || !compact.includes(
        'FormulaTokenCursor.step problem ⟨fifthClauseSlotStart problem⟩ = some (none, ⟨fifthClauseSlotStart problem + 1⟩)')
  ...
  for (const theorem of [
    ...
    'paddingSlot_direct_eq_padding', 'fifthClauseSlotStart_direct_eq_padding',
...
test('remaining fourth-clause padding run is literal, exact, and shortcut-free',
  async () => {
...
test('kernel transcript covers every public padding-run declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderFourthClausePaddingRun.';
    const reusedPrefix =
...
test('root, durable CI, regression, and documentation publish the run',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderFourthClausePaddingRunAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderFourthClausePaddingRun\.lean/u);
    assert.match(regression, /remainingPaddingCount \(inputOnlyProblem \[\]\) = 81/u);
    assert.match(regression, /fifthClauseSlotStart_direct_eq_padding/u);
    assert.match(regression, /malformedCountdownRoot_timeout/u);
    assert.match(docs, /entire remaining\s+fourth-clause padding block/u);
...
test('hostile reuse, bridge, coordinate, outcome, and shortcut mutations fail',
  async () => {
    ...
    const wrongOutcome = source.replace(
      ...
      'some (some CNFToken.sep, ⟨fifthClauseSlotStart problem + 1⟩) := by');
