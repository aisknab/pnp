...
const SOURCE =
  'lean/PNP/Concrete/CookLevinBuilderFourthClausePrefix.lean';
const AXIOM_AUDIT =
...
const DOCS =
  'docs/lean_cook_levin_builder_fourth_clause_prefix.md';
const TEST =
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderFourthClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine')
      || !compact.includes('(machine problem).rules.length = 4276 +')) {
    failures.push('composed-table');
  }
  if (!compact.includes(
    'BuilderFourthClauseSecondLiteralPrefix.rule_source_ne_acceptState problem')
      || !compact.includes(
  ...
  if (!compact.includes('(.add (.constant 618)')
      || !compact.includes('24 * problem.input.length')
      || !compact.includes('12 * problem.FormulaWidth')
      || !compact.includes('12 * (BuilderFourthClauseSeparatorStep.cursorWord problem).length')
  ...
  for (const theorem of [
    ...
    'finalTokenBits_eq_encodedFormula_fourthClause',
...
test('fourth-clause completion is literal, exact, and shortcut-free',
  async () => {
...
test('kernel transcript covers every public clause-prefix declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderFourthClausePrefix.';
    const wrappers = [
...
test('root, durable CI, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderFourthClausePrefixAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderFourthClausePrefix\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 287/u);
    ...
    assert.match(docs, /complete fourth clause/u);
    assert.match(docs, /does not traverse clause-four padding/u);
  });
