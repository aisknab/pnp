...
const SOURCE =
  'lean/PNP/Concrete/CookLevinBuilderFourthClauseSeparatorStep.lean';
const AXIOM_AUDIT =
...
const DOCS =
  'docs/lean_cook_levin_builder_fourth_clause_separator_step.md';
const TEST =
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderThirdClausePaddingRun.machine problem) BuilderSecondClauseSeparatorStep.SeparatorCursor.machine')
      || !compact.includes('(machine problem).rules.length = 3300 +')
      || !compact.includes(
  ...
  if (!compact.includes(
    'BuilderThirdClausePrefix.thirdClauseTokens problem ++ [.sep]')
      || !compact.includes(
        'problem.encodedFormula.take (2 * (problem.FormulaWidth + 28))')) {
  ...
  if (!compact.includes('(.add (.constant 426)')
...
test('fourth-clause separator step is literal, exact, and shortcut-free',
  async () => {
...
test('kernel transcript covers every public separator-step declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderFourthClauseSeparatorStep.';
    const wrappers = [
...
test('root, durable CI, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderFourthClauseSeparatorStepAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderFourthClauseSeparatorStep\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 279/u);
    assert.match(regression, /nextTokenSlot_direct_eq_f/u);
    assert.match(regression, /malformedCursorScratch_timeout/u);
    assert.match(docs, /separator beginning clause four/u);
...
test('hostile request, bridge, cursor, output, and shortcut mutations fail',
  async () => {
    ...
    const wrongToken = source.replace(
      'BuilderThirdClausePrefix.thirdClauseTokens problem ++ [.sep]',
      'BuilderThirdClausePrefix.thirdClauseTokens problem ++ [.t]');
