...
const DOCS =
  'docs/lean_cook_levin_builder_second_clause_first_literal_prefix.md';
const TEST =
...
def secondClauseFirstLiteralTokens
def firstCursorWord
...
function validate0(source) {
  ...
  if (!compact.includes(
    ...
    failures.push('false-token-cursor');
  }
  if (!compact.includes(
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderSecondClauseSeparatorStep.machine problem) FirstLiteralSuffix.machine')
      || !compact.includes('(machine problem).rules.length = 1610 +')) {
  ...
  if (!compact.includes(
    'BuilderSecondClauseSeparatorStep.secondClauseStartTokens problem ++ [.f]')
      ...
      || !compact.includes(
        'problem.encodedFormula.take (2 * (problem.FormulaWidth + 15))')) {
  ...
  if (!compact.includes('firstLiteralSignSlot_direct_eq_f')
      ...
      || !compact.includes(
        '(BuilderFirstClausePaddingRun.secondClauseStart problem + 1) = some (some CNFToken.f)')
      || !compact.includes(
        '(BuilderFirstClausePaddingRun.secondClauseStart problem + 2) = some (some CNFToken.f)')
      || !compact.includes(
  ...
  if (!compact.includes('(.add (.constant 564)')
      || !compact.includes('48 * problem.input.length')
      || !compact.includes('24 * problem.FormulaWidth')
      || !compact.includes(
        '24 * (BuilderSecondClauseSeparatorStep.cursorWord problem).length')
...
test('second-clause first-literal prefix is literal, exact, and shortcut-free',
  async () => {
...
test('kernel transcript covers every public first-literal-prefix declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderSecondClauseFirstLiteralPrefix.';
    const wrappers = [
    ...
    assert.equal(printed.filter((name) => name.startsWith(prefix)).length, 85);
...
test('root, durable CI, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondClauseFirstLiteralPrefixAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondClauseFirstLiteralPrefix\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 101/u);
    ...
    assert.match(docs, /does not\s+complete clause two/u);
  });
...
test('hostile request, bridge, cursor, output, and shortcut mutations fail',
  async () => {
    ...
    assert.ok(validate0(wrongRequest).includes('false-token-cursor'));
    const removedInnerBridge = source.replace(
      ...
      'appender');
    assert.ok(validate0(removedInnerBridge).includes('false-token-cursor'));
    const removedSuffixBridge = source.replace(
      ...
      'FalseTokenCursor.machine');
    assert.ok(validate0(removedSuffixBridge).includes('two-component-suffix'));
    ...
    const wrongFirstToken = source.replace(
      'BuilderSecondClauseSeparatorStep.secondClauseStartTokens problem ++ [.f]',
      'BuilderSecondClauseSeparatorStep.secondClauseStartTokens problem ++ [.t]');
