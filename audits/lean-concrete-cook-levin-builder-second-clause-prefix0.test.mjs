...
const SOURCE =
  'lean/PNP/Concrete/CookLevinBuilderSecondClausePrefix.lean';
const AXIOM_AUDIT =
...
const DOCS =
  'docs/lean_cook_levin_builder_second_clause_prefix.md';
const TEST =
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderSecondClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine')
      || !compact.includes('(machine problem).rules.length = 2098 +')) {
  ...
  if (!compact.includes('(.add (.constant 390)')
      || !compact.includes('24 * problem.input.length')
      || !compact.includes('12 * problem.FormulaWidth')
      || !compact.includes('12 * (BuilderSecondClauseSeparatorStep.cursorWord problem).length')
...
test('second-clause completion is literal, exact, and shortcut-free',
  async () => {
...
test('kernel transcript covers every public clause-prefix declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderSecondClausePrefix.';
    const wrappers = [
...
test('root, durable CI, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondClausePrefixAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondClausePrefix\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 105/u);
    ...
    assert.match(docs, /complete second clause/u);
    assert.match(docs, /does not traverse clause-two padding/u);
  });
