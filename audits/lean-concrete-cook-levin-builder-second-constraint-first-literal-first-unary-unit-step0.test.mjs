...
const DOCS =
  'docs/lean_cook_levin_builder_second_constraint_first_literal_first_unary_unit_step.md';
const TEST =
...
def secondConstraintFirstLiteralFirstUnaryTokens
def cursorWord
def finalOutside
def finalTape
...
theorem prefixFirstUnaryUnit_launch_workStep
theorem workRunExact
theorem specification_firstUnaryUnit_step
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderSecondConstraintFirstLiteralSignStep.machine problem) BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine')
      || !compact.includes('(machine problem).rules.length = 4798 +')
      || !compact.includes(
  ...
  if (!compact.includes(
    'BuilderSecondConstraintFirstLiteralSignStep.secondConstraintFirstLiteralSignTokens problem ++ [.t]')
      || !compact.includes(
        'problem.encodedFormula.take (2 * (problem.FormulaWidth + 39))')) {
    failures.push('exact-output');
  }
  if (!compact.includes('nextTokenSlot_direct_eq_t')
      || !compact.includes('some (some CNFToken.t)')
      || !compact.includes('specification_firstUnaryUnit_step')
      || !compact.includes('specification_next_step')) {
    failures.push('schedule-semantics');
  }
  if (!compact.includes('(.add (.constant 558)')
  ...
  for (const theorem of [
    ...
    'prefixFirstUnaryUnit_launch_workStep', 'workRunExact',
    'finalTokenBits_eq_encodedFormula_secondConstraintFirstLiteralFirstUnary',
...
test('second-constraint first-literal unary unit is literal, exact, and shortcut-free',
  async () => {
...
test('kernel transcript covers every public unary-unit-step declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.';
    const wrappers = [
...
test('root, durable CI, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintFirstLiteralFirstUnaryUnitStepAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintFirstLiteralFirstUnaryUnitStep\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 4511/u);
    assert.match(regression, /nextTokenSlot_direct_eq_t/u);
    assert.match(regression, /malformedCursorScratch_timeout/u);
    assert.match(docs, /first unary unit of the second constraint/u);
    assert.match(docs, /does not emit the following second unary `T`/u);
  });
