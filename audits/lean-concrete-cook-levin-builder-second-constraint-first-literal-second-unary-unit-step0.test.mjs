...
const DOCS =
  'docs/lean_cook_levin_builder_second_constraint_first_literal_second_unary_unit_step.md';
const TEST =
...
def secondConstraintFirstLiteralSecondUnaryTokens
def cursorWord
def finalOutside
def finalTape
...
theorem trueTokenCursor_launch_workStep
theorem suffix_workRunExact
theorem prefix_workRunExact
theorem prefixSecondUnaryUnit_launch_workStep
theorem workRunExact
theorem specification_secondUnaryUnit_step
...
theorem nextTokenSlot_direct_eq_t
...
function validate0(source) {
  ...
  if (!compact.includes(
    ...
    failures.push('literal-true-token-suffix');
  }
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem) BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine')
      || !compact.includes('(machine problem).rules.length = 4920 +')
      || !compact.includes(
  ...
  if (!compact.includes(
    'BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.secondConstraintFirstLiteralFirstUnaryTokens problem ++ [.t]')
      || !compact.includes(
        'problem.encodedFormula.take (2 * (problem.FormulaWidth + 40))')) {
    failures.push('exact-output');
  }
  if (!compact.includes('nextTokenSlot_direct_eq_t')
      || !compact.includes('some (some CNFToken.t)')
      || !compact.includes('specification_secondUnaryUnit_step')
      || !compact.includes('specification_next_step')) {
    failures.push('schedule-semantics');
  }
  if (!compact.includes(
    'finalTokenSlot problem = problem.formulaVariableSlotBound + 1 + problem.formulaClauseSlotsPerConstraint * problem.formulaTokensPerClause + 4')) {
    failures.push('retained-coordinate');
  }
  if (!compact.includes('(.add (.constant 570)')
  ...
  for (const theorem of [
    ...
    'cursor_workRunExact', 'trueTokenCursor_launch_workStep',
    'prefixSecondUnaryUnit_launch_workStep', 'workRunExact',
    'finalTokenBits_eq_encodedFormula_secondConstraintFirstLiteralSecondUnary',
    'nextTokenSlot_direct_eq_t', 'rawTimeBound_le',
...
test('second-constraint first-literal second unary unit is literal, exact, and shortcut-free',
  async () => {
    assert.deepEqual(validate0(await text0(SOURCE)), []);
  });

test('kernel transcript covers every public unary-unit-step declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.';
    const wrappers = [
...
test('root, durable CI, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintFirstLiteralSecondUnaryUnitStepAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintFirstLiteralSecondUnaryUnitStep\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 4512/u);
    assert.match(regression, /nextTokenSlot_direct_eq_t/u);
    assert.match(regression, /malformedCursorScratch_timeout/u);
    assert.match(docs, /second unary unit of the second constraint/u);
    assert.match(docs, /does not emit the following third unary `T`/u);
  });
...
test('hostile request, bridge, cursor, output, and shortcut mutations fail',
  async () => {
    ...
    assert.ok(validate0(wrongSuffix).includes('literal-true-token-suffix'));
    ...
    const wrongBits = source.replace(
      'problem.FormulaWidth + 40',
      'problem.FormulaWidth + 39');
