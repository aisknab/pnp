...
const DOCS =
  'docs/lean_cook_levin_builder_second_constraint_first_literal_sign_step.md';
const TEST =
...
def secondConstraintFirstLiteralSignTokens
def cursorWord
def finalOutside
def finalTape
...
theorem prefixSign_launch_workStep
theorem workRunExact
theorem specification_sign_step
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderSecondConstraintSeparatorStep.machine problem) BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine')
      || !compact.includes('(machine problem).rules.length = 4676 +')
      || !compact.includes(
  ...
  if (!compact.includes(
    'BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens problem ++ [.t]')
      || !compact.includes(
        'problem.encodedFormula.take (2 * (problem.FormulaWidth + 38))')) {
    failures.push('exact-output');
  }
  if (!compact.includes('nextTokenSlot_direct_eq_t')
      || !compact.includes('some (some CNFToken.t)')
      || !compact.includes('specification_sign_step')
      || !compact.includes('specification_next_step')) {
    failures.push('schedule-semantics');
  }
  if (!compact.includes('(.add (.constant 546)')
  ...
  for (const theorem of [
    ...
    'prefixSign_launch_workStep', 'workRunExact',
...
test('second-constraint first-literal sign is literal, exact, and shortcut-free',
  async () => {
    assert.deepEqual(validate0(await text0(SOURCE)), []);
  });

test('kernel transcript covers every public sign-step declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderSecondConstraintFirstLiteralSignStep.';
    const wrappers = [
...
test('root, durable CI, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintFirstLiteralSignStepAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintFirstLiteralSignStep\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 4510/u);
    assert.match(regression, /nextTokenSlot_direct_eq_t/u);
    assert.match(regression, /malformedCursorScratch_timeout/u);
    assert.match(docs, /positive sign beginning the second constraint/u);
    assert.match(docs, /does not emit the following unary `T`/u);
  });
