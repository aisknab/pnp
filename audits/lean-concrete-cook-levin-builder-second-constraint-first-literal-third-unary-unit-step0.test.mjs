...
const DOCS =
  'docs/lean_cook_levin_builder_second_constraint_first_literal_third_unary_unit_step.md';
const TEST =
...
def secondConstraintFirstLiteralThirdUnaryTokens
def cursorWord
def finalOutside
def finalTape
...
theorem prefixThirdUnaryUnit_launch_workStep
theorem workRunExact
theorem specification_thirdUnaryUnit_step
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.machine problem) BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine')
      || !compact.includes('(machine problem).rules.length = 5042 +')
      || !compact.includes(
  ...
  if (!compact.includes(
    'BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.secondConstraintFirstLiteralSecondUnaryTokens problem ++ [.t]')
      || !compact.includes(
        'problem.encodedFormula.take (2 * (problem.FormulaWidth + 41))')) {
  ...
  if (!compact.includes('nextTokenSlot_direct_eq_f')
      ...
      || !compact.includes(
        'scheduledAtLeastOneClauseTokens_starts_sep_t_t_t_t_f')
      || !compact.includes('(hEqualsThree : first.val = 3)')
      || !compact.includes('specification_thirdUnaryUnit_step')
      || !compact.includes('specification_next_step')) {
    failures.push('schedule-semantics');
  }
  if (!compact.includes(
    'finalTokenSlot problem = problem.formulaVariableSlotBound + 1 + problem.formulaClauseSlotsPerConstraint * problem.formulaTokensPerClause + 5')) {
    failures.push('retained-coordinate');
  }
  if (!compact.includes('(.add (.constant 582)')
  ...
  for (const theorem of [
    ...
    'prefixThirdUnaryUnit_launch_workStep', 'workRunExact',
...
test('second-constraint first-literal third unary unit is literal, exact, and shortcut-free',
  async () => {
...
test('kernel transcript covers every public unary-unit-step declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderSecondConstraintFirstLiteralThirdUnaryUnitStep.';
    const wrappers = [
...
test('root, durable CI, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintFirstLiteralThirdUnaryUnitStepAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintFirstLiteralThirdUnaryUnitStep\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 4513/u);
    assert.match(regression, /nextTokenSlot_direct_eq_f/u);
    assert.match(regression, /malformedCursorScratch_timeout/u);
    assert.match(docs, /third unary unit of the second constraint/u);
    assert.match(docs, /does not emit the following terminating `F`/u);
  });
