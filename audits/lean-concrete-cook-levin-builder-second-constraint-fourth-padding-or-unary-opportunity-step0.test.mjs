...
const DOCS =
  'docs/lean_cook_levin_builder_second_constraint_fourth_padding_or_unary_opportunity_step.md';
const TEST =
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderSecondConstraintThirdPaddingOrUnaryOpportunityStep.machine problem) (suffixMachine problem)')
      || !compact.includes('(machine problem).rules.length = 5764 +')
  ...
  if (!compact.includes(
    'BuilderSecondConstraintThirdPaddingOrUnaryOpportunityStep.secondConstraintThirdPaddingOrUnaryTokens problem ++ opportunityOutput problem')
      || !compact.includes(
        'problem.encodedFormula.take (2 * (problem.FormulaWidth + 43 + if problem.dimensions.tapeWidth problem.tableauInputMode = 1 then 0 else 4))')
      || !compact.includes(
  ...
  if (!compact.includes(
    'opportunityOutput problem = if problem.dimensions.tapeWidth problem.tableauInputMode = 1 then [] else [CNFToken.t]')
      ...
      || !compact.includes('followingTokenSlot_direct_eq_padding_or_f')
      || !compact.includes(
        'problem.formulaTokenSlotDirect (finalTokenSlot problem) = some (if problem.dimensions.tapeWidth problem.tableauInputMode = 1 then none else some CNFToken.f)')
      || !compact.includes('specification_following_step')) {
    failures.push('schedule-semantics');
  }
  if (!compact.includes(
    'finalTokenSlot problem = problem.formulaVariableSlotBound + 1 + problem.formulaClauseSlotsPerConstraint * problem.formulaTokensPerClause + 11')
      || !compact.includes('finalOutside_contains_finalTokenSlot')) {
    failures.push('retained-coordinate');
  }
  if (!compact.includes('(.add (.constant 648)')
  ...
  for (const theorem of [
    ...
    'workRunExact',
    'finalTokenBits_eq_encodedFormula_secondConstraintFourthPaddingOrUnary',
...
test('kernel transcript covers all public declarations and boundary lemmas',
  async () => {
    ...
    const modulePrefix =
      'PNP.Concrete.CookLevin.BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.';
...
test('root, durable checks, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintFourthPaddingOrUnaryOpportunityStepAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 4519/u);
    ...
    assert.match(regression, /some \(some CNFToken\.f\)/u);
...
test('hostile reuse, bridge, output, schedule, and shortcut mutations fail',
  async () => {
    ...
    const wrongFollowing = source.replaceAll(
      ...
      'then some CNFToken.f else none');
