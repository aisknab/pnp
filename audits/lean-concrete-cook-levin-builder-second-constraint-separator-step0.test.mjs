...
const SOURCE =
  'lean/PNP/Concrete/CookLevinBuilderSecondConstraintSeparatorStep.lean';
const AXIOM_AUDIT =
...
const DOCS =
  'docs/lean_cook_levin_builder_second_constraint_separator_step.md';
const TEST =
...
def secondConstraintStartTokens
def cursorWord
def finalOutside
def finalTape
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderFirstConstraintPaddingRun.machine problem) BuilderSecondClauseSeparatorStep.SeparatorCursor.machine')
      || !compact.includes('(machine problem).rules.length = 4554 +')
      || !compact.includes(
  ...
  if (!compact.includes(
    'BuilderFourthClausePrefix.fourthClauseTokens problem ++ [.sep]')
      || !compact.includes(
        'problem.encodedFormula.take (2 * (problem.FormulaWidth + 37))')) {
  ...
  if (!compact.includes('(.add (.constant 534)')
  ...
  for (const theorem of [
    ...
    'finalTokenBits_eq_encodedFormula_secondConstraintStart',
...
test('second-constraint separator step is literal, exact, and shortcut-free',
  async () => {
...
test('kernel transcript covers every public separator-step declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderSecondConstraintSeparatorStep.';
    const wrappers = [
...
test('root, durable CI, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintSeparatorStepAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintSeparatorStep\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 4509/u);
    assert.match(regression, /nextTokenSlot_direct_eq_t/u);
    assert.match(regression, /malformedCursorScratch_timeout/u);
    assert.match(docs, /separator beginning the second scheduled constraint/u);
    assert.match(docs, /does not emit the following `T`/u);
  });
...
test('hostile request, bridge, cursor, output, and shortcut mutations fail',
  async () => {
    ...
    const wrongToken = source.replace(
      'BuilderFourthClausePrefix.fourthClauseTokens problem ++ [.sep]',
      'BuilderFourthClausePrefix.fourthClauseTokens problem ++ [.t]');
