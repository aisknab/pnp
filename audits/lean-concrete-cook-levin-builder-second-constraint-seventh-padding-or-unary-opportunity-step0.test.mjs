...
const DOCS =
  'docs/lean_cook_levin_builder_second_constraint_seventh_padding_or_unary_opportunity_step.md';
const TEST =
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderSecondConstraintSixthPaddingOrOpeningUnaryOpportunityStep.machine problem) (suffixMachine problem)')
      || !compact.includes('(machine problem).rules.length = 6124 +')
  ...
  if (!compact.includes(
    'BuilderSecondConstraintSixthPaddingOrOpeningUnaryOpportunityStep.secondConstraintSixthPaddingOrOpeningUnaryTokens problem ++ opportunityOutput problem')
      || !compact.includes(
        'problem.encodedFormula.take (2 * (problem.FormulaWidth + 43 + if problem.dimensions.tapeWidth problem.tableauInputMode = 1 then 0 else 7))')
      || !compact.includes(
  ...
  if (!compact.includes(
    'finalTokenSlot problem = problem.formulaVariableSlotBound + 1 + problem.formulaClauseSlotsPerConstraint * problem.formulaTokensPerClause + 14')
      || !compact.includes('finalOutside_contains_finalTokenSlot')) {
    failures.push('retained-coordinate');
  }
  if (!compact.includes('(.add (.constant 684)')
  ...
  for (const theorem of [
    ...
    'workRunExact',
    'finalTokenBits_eq_encodedFormula_secondConstraintSeventhPaddingOrUnary',
...
test('kernel transcript covers all public declarations and boundary lemmas',
  async () => {
    ...
    const modulePrefix =
      'PNP.Concrete.CookLevin.BuilderSecondConstraintSeventhPaddingOrUnaryOpportunityStep.';
    ...
    assert.deepEqual(
      printed.filter((name) => name.startsWith(modulePrefix))
        .map((name) => name.slice(modulePrefix.length)).sort(),
      declarations.map((entry) => entry.name).sort(),
    );
    assert.equal(printed.filter((name) =>
...
test('root, durable checks, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintSeventhPaddingOrUnaryOpportunityStepAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintSeventhPaddingOrUnaryOpportunityStep\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 4522/u);
    ...
    assert.match(docs, /does not\s+consume the following slot/u);
  });
