...
const DOCS =
  'docs/lean_cook_levin_builder_second_constraint_third_padding_or_unary_opportunity_step.md';
const TEST =
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderSecondConstraintSecondPaddingOrUnaryOpportunityStep.machine problem) (suffixMachine problem)')
      || !compact.includes('(machine problem).rules.length = 5644 +')
  ...
  if (!compact.includes(
    'BuilderSecondConstraintSecondPaddingOrUnaryOpportunityStep.secondConstraintSecondPaddingOrUnaryTokens problem ++ opportunityOutput problem')
      || !compact.includes(
        'problem.encodedFormula.take (2 * (problem.FormulaWidth + 43 + if problem.dimensions.tapeWidth problem.tableauInputMode = 1 then 0 else 3))')
      || !compact.includes(
  ...
  if (!compact.includes(
    'finalTokenSlot problem = problem.formulaVariableSlotBound + 1 + problem.formulaClauseSlotsPerConstraint * problem.formulaTokensPerClause + 10')
      || !compact.includes('finalOutside_contains_finalTokenSlot')) {
    failures.push('retained-coordinate');
  }
  if (!compact.includes('(.add (.constant 636)')
  ...
  for (const theorem of [
    ...
    'workRunExact',
    'finalTokenBits_eq_encodedFormula_secondConstraintThirdPaddingOrUnary',
...
test('kernel transcript covers all public declarations and boundary lemmas',
  async () => {
    ...
    const modulePrefix =
      'PNP.Concrete.CookLevin.BuilderSecondConstraintThirdPaddingOrUnaryOpportunityStep.';
...
test('root, durable checks, regression, and documentation publish the step',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintThirdPaddingOrUnaryOpportunityStepAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderSecondConstraintThirdPaddingOrUnaryOpportunityStep\.lean/u);
    assert.match(regression, /finalTokenSlot \(inputOnlyProblem \[\]\) = 4518/u);
