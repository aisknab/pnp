...
const SOURCE =
  'lean/PNP/Concrete/CookLevinBuilderThirdClausePaddingRun.lean';
const AXIOM_AUDIT =
...
const DOCS = 'docs/lean_cook_levin_builder_third_clause_padding_run.md';
const TEST =
...
def fourthClauseStartPolynomial
def fourthClauseStart
theorem fourthClauseStart_eq
...
theorem fourthClauseStart_direct_eq_sep
...
def finalTokenSlot
theorem finalTokenSlot_eq_fourthClauseStart
...
function validate0(source) {
  ...
  if (!compact.includes(
    'BuilderFirstClausePrefix.WorkChain.machine (BuilderThirdClausePrefix.machine problem) (paddingSuffixMachine problem)')
      || !compact.includes(
      ...
      || !compact.includes('(machine problem).rules.length = 3178 +')) {
    failures.push('composed-table');
  }
  if (!compact.includes(
    'remainingPaddingCount problem = problem.formulaTokensPerClause - 8')
      || !compact.includes(
        'BuilderThirdClausePrefix.finalTokenSlot problem + remainingPaddingCount problem = fourthClauseStart problem')
      || !compact.includes(
        'fourthClauseStart problem = problem.formulaVariableSlotBound + 1 + 3 * problem.formulaTokensPerClause')) {
  ...
  for (const theorem of [
    ...
    'paddingSlot_direct_eq_padding', 'fourthClauseStart_direct_eq_sep',
...
test('remaining third-clause padding run is literal, exact, and shortcut-free',
  async () => {
...
test('kernel transcript covers every public padding-run declaration',
  async () => {
    ...
    const prefix =
      'PNP.Concrete.CookLevin.BuilderThirdClausePaddingRun.';
    const reusedPrefix =
...
test('root, durable CI, regression, and documentation publish the run',
  async () => {
    ...
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderThirdClausePaddingRunAxiomAudit\.lean/u);
    assert.match(workflow,
      /PNPConcreteCookLevinBuilderThirdClausePaddingRun\.lean/u);
    assert.match(regression, /remainingPaddingCount \(inputOnlyProblem \[\]\) = 82/u);
    assert.match(regression, /fourthClauseStart_direct_eq_sep/u);
    assert.match(regression, /malformedCountdownRoot_timeout/u);
    assert.match(docs, /entire remaining\s+third-clause padding block/u);
...
test('hostile reuse, bridge, coordinate, outcome, and shortcut mutations fail',
  async () => {
    ...
    const wrongBits = source.replace('problem.FormulaWidth + 27',
