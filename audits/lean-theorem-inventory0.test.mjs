...
test('compiled Lean inventory is canonical, complete, deterministic, and byte-mirrored', async () => {
  ...
  assert.equal(inventory.declarationCount, 4659);
  assert.equal(inventory.excludedPrivateDeclarationCount, 560);
  assert.equal(inventory.theoremCount, 1932);
  assert.equal(inventory.assumptionFreeTheoremCount, 1831);
  ...
  assert.equal(inventory.milestoneCandidates.length, 58);
...
test('positive Lean probe parser rejects empty, malformed, noisy, failed, or noncanonical output', async () => {
  ...
  assert.equal(valid.inventory.declarationCount, 4659);
  for (const input of [
...
test('publication map cannot set fingerprints or expand the axiom allowlist in release 16', async () => {
  const { inventoryBytes, inventory, map } = await fixture0();
