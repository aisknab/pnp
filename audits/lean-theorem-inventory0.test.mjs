...
test('compiled Lean inventory is canonical, complete, deterministic, and byte-mirrored', async () => {
  ...
  assert.equal(inventory.declarationCount, 4501);
  assert.equal(inventory.excludedPrivateDeclarationCount, 551);
  assert.equal(inventory.theoremCount, 1863);
  assert.equal(inventory.assumptionFreeTheoremCount, 1762);
  assert.equal(inventory.axiomCount, 4);
  assert.equal(inventory.sourceClosureModuleCount, 39);
  ...
  assert.equal(inventory.milestoneCandidates.length, 54);
...
test('positive Lean probe parser rejects empty, malformed, noisy, failed, or noncanonical output', async () => {
  ...
  assert.equal(valid.inventory.declarationCount, 4501);
  for (const input of [
...
test('publication map cannot set fingerprints or expand the axiom allowlist in release 15', async () => {
  const { inventoryBytes, inventory, map } = await fixture0();
