...
test('compiled Lean inventory is canonical, complete, deterministic, and byte-mirrored', async () => {
  ...
  assert.equal(inventory.declarationCount, 4735);
  assert.equal(inventory.excludedPrivateDeclarationCount, 671);
  assert.equal(inventory.theoremCount, 1948);
  assert.equal(inventory.assumptionFreeTheoremCount, 1847);
  assert.equal(inventory.axiomCount, 4);
  assert.equal(inventory.sourceClosureModuleCount, 41);
  ...
  assert.equal(inventory.milestoneCandidates.length, 70);
...
test('positive Lean probe parser rejects empty, malformed, noisy, failed, or noncanonical output', async () => {
  ...
  assert.equal(valid.inventory.declarationCount, 4735);
  for (const input of [
...
test('publication map cannot set fingerprints or expand the axiom allowlist in release 19', async () => {
  const { inventoryBytes, inventory, map } = await fixture0();
