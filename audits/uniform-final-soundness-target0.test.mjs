...
test('uniform final soundness target accepts the current theorem target surface', async () => {
  const out = await CheckUniformFinalSoundnessTarget0({ historicalReplay: true, writeOutput: false });
...
test('uniform target rejects premature theorem discharge', async () => {
  ...
  const out = await CheckUniformFinalSoundnessTarget0({ historicalReplay: true, writeOutput: false, manifestOverride: manifest });
...
test('uniform target rejects cleared finite-to-unbounded gap before proof', async () => {
  ...
  const out = await CheckUniformFinalSoundnessTarget0({ historicalReplay: true, writeOutput: false, gapLedgerOverride: gapLedger });
  assert.equal(out.tag, 'reject');
