...
EnforceHistoricalReplayCli0({ entrypoint: 'bin/write-concrete-materialized-final-integration0.mjs' });

const args = process.argv.slice(2);
const outDir = args.find((arg) => !arg.startsWith('--')) ?? './concrete-materialized-final-integration0';
const full = args.includes('--full');

const result = await writeConcreteMaterializedFinalIntegrationFiles0(outDir, { historicalReplay: true });
