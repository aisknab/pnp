...
EnforceHistoricalReplayCli0({ entrypoint: 'bin/write-concrete-materialized-pcc-pack0.mjs' });

const args = process.argv.slice(2);
const outDir = args.find((arg) => !arg.startsWith('--')) ?? './concrete-materialized-pcc-pack0';
const full = args.includes('--full');

const result = await writeConcreteMaterializedPCCPackFiles0(outDir, { historicalReplay: true });
