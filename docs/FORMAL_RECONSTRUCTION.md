...
the next six schedule opportunities, emitting nothing for width-one padding or the first four unary
`T` tokens, terminating `F` of the second literal, and opening positive `T` of the following literal
at wider widths.
It is not a general dynamic cursor or an arbitrary raw slot decoder: no machine emits the following
padding or first-unary-index-`T` opportunity, traverses the rest of the second constraint, or emits the
