...
many-one reductions. The later `PipelineRefinement` theorem described below now
compiles every finite charged program tree to the raw single-tape model; this
does not supply the missing deterministic SAT decider. A blank-materialization
layer proves that finite tape lists denote the
...
the proof-bearing loop certificate. M243 consumes M231's checked concrete SAT
NP-completeness without an additional trust-model argument; packaging and
checking still do not construct the required loop certificate.
