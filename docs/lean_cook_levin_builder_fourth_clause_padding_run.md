# Concrete Cook–Levin fourth-clause padding run

`lean/PNP/Concrete/CookLevinBuilderFourthClausePaddingRun.lean` composes the
raw-input builder through the complete fourth clause and the entire remaining
fourth-clause padding block in that fixed-width clause rectangle. The endpoint is the
first opportunity in the fifth clause rectangle. That opportunity is
...
`S = V + 1 + 3*C` be the fourth-clause start coordinate. The predecessor
retains `S + 9`, the first padding opportunity after the nine populated tokens
in clause four. This milestone proves the exact traversal count
...
The right side is the first opportunity in the fifth fixed-width clause slot.
...
3. `fifthClauseSlotStartPolynomial` materializes `V + 1 + 4*C`.
...
```text
4328
+ ten inherited unary-evaluator ruleCount terms
+ remaining-padding evaluator ruleCount
+ fifth-clause-target evaluator ruleCount.
```

The constant is the predecessor's `4276`, three bridges, and the reused
...
through its four populated clauses and the next empty clause slot. It proves
that:

- clause four occupies nine populated token opportunities;
- every coordinate from `S + 9` through the end of that rectangle is padding;
- `V + 1 + 4*C` is the first token opportunity in an intentionally empty
  fifth clause rectangle.

Thus `paddingSlot_direct_eq_padding` returns `some none` for every traversed
coordinate and `fifthClauseSlotStart_direct_eq_padding` returns `some none` at
...
```text
BuilderFourthClausePrefix.rawTimeBound(n)
+ 18
+ 6 * Unary.workSteps(remainingPaddingPolynomial, n)
+ 6 * (D * (2*R + 8) + D*D)
+ 6 * Unary.workSteps(fifthClauseSlotStartPolynomial, n).
```
...
output, the clause-four padding block, the empty fifth-clause endpoint,
...
It does not traverse the empty fifth clause rectangle, reach the next
