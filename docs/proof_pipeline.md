...
| Selector universe | Triple and spine payloads can hide large descriptions. The current Lean handles are unique positions in an explicit finite grouped list, which gives exact decoding but no polynomial bound. | Claimed polynomial selector bound and finite payload alphabets. | Actual handle count, bit encoding length, generation time, and completeness proof beyond input-relative indexing. |
