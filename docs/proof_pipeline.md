...
| Selector universe | Triple and spine payloads can hide large descriptions. The current Lean codec gives each unique position in an explicit finite grouped list one unary bitstring and fails closed on malformed input, but its list-relative length can still be exponential in encoded circuit size and it does not encode payload data. | Claimed polynomial selector bound and finite payload alphabets. | Bound the explicit list and unary position by encoded circuit size, encode the payload, prove polynomial generation time, faithfulness, compatibility, and completeness beyond input-relative indexing. |
