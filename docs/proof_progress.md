...
milestone ledger. At
`PNP-FORMAL-RECONSTRUCTION-STATUS-2026-09-19-275`, 251 of 253
...
M275 connects actual restoration cost to full and quotient minima and derives
the exact strict-gain boundary. A checked positive-slack refusal records the
limit of restoration plus physical normalization; the existing complete unary
route recovers the example without extending completeness to arbitrary inputs.
Reference minimization remains exhaustive. No fixed checkpoint or global gate
changes. See the [component boundary](./lean_wire_profile_restoration.md).
...
M233 extends this to arbitrary primitive-record supports and proves retained
ambient-profile locality and whole-support preservation. The retained-profile
premise is necessary; the observer remains supplied and influence still enumerates
all subsets. No fixed checkpoint or global gate changes.

M234 proves actual saturation replay fidelity, generated event validity and
structural metadata cost balance. Physical-gate transparency and obligation
discharge remain separate; checked counterexamples preserve those limits.
...
M236 derives the complete duplicate-free physical-NAND charge partition of
computed saturation, with verified introduction provenance, exact lookup and
whole-support size accounting. Introduction provenance is not the fixed
manuscript-wide ownership map. Materializer grouping, full-minimum growth
and global routing remain open. The observer remains supplied and the
reference constructions remain exhaustive. No fixed checkpoint or global
gate changes.

M237 computes the physical replacement context around every production
saturated support, with input-only boundaries, exact gate partition, whole
Boolean reconstruction, equivalent replacement preservation and the physical
slack inequality. Full-profile compatibility and global ownership remain open.
The observer remains supplied and the reference constructions remain
exhaustive. No fixed checkpoint or global gate changes.

M238 computes whole-circuit physical reference replacements from the actual
production proper-positive search. Successful results preserve ordered Boolean
outputs and strictly decrease physical gate count and slack by the exact local
gain. Search failure excludes proper positive supports, not global minimality.
Full-profile compatibility, materializer ownership and global routes remain
open. The observer remains supplied and the searches remain exhaustive.
No fixed checkpoint or global gate changes.
...
M209 adds the first non-repeatable raw decoder layer above the complete-schedule
controller. One fixed 54-rule deterministic machine compares every supplied
natural coordinate with the exact problem-derived first-body boundary. The raw
verdict agrees with the semantic top-level direct-slot route, every in-range
coordinate compiles within `36 * terminalSlotPolynomial^2`, and one-step-short
fuel or malformed symbols remain timeout. The router does not decode the
...
M211 adds a fixed 99-rule unary quotient/remainder kernel. For every natural
dividend and positive width it proves an exact accepting trace, exact natural
quotient and strict remainder, dividend reconstruction, exact compiled raw
simulation, one-step-short timeout, and a quadratic complete-unary-input bound.
M210 body coordinates instantiate the same decoded pair. The machine remains a
standalone kernel: it is not spliced onto M209's checked raw result and emits no
body token, so the complete raw builder, `RawRefinement`, and packaged
...
M218 composes the M217 fixed physical dispatcher across every canonical
post-header coordinate. Its total recursion agrees with the canonical emitted
prefix at every bounded coordinate and reaches the exact encoded CNF formula.
Each coordinate retains exact work, compiled and one-step-short traces for
arbitrary exterior workspace, the M214 classifier evidence, and the aggregate
compiled work fits one source-size polynomial. Requests remain constructed from
the canonical Lean schedule and successive configurations are not one literal
raw-machine loop or physical tape-to-tape handoff; builder `RawRefinement` and
the packaged `PolynomialReduction` remain open. No fixed checkpoint or global
...
composes the router-to-divider bridge, exact raw divider,
divider-to-comparator bridge and comparator through three exact tape handoffs
over arbitrary protected builder workspace. The final physical state agrees
with typed body-or-`Finish` semantics, and exact work, compiled execution,
one-step-short nonhalting, component decomposition and one source-size polynomial
bound are checked. Body-token and padding request symbols, connection to the
request dispatcher, one literal repeated loop, builder `RawRefinement` and the
packaged `PolynomialReduction` remain open. No fixed checkpoint or global gate
...
M223 mechanically reflects M217's fixed dispatcher and executes it after the
unique full-classifier `Finish` path. Reflection swaps tape sides and left/right
head moves while preserving finite rule queries and states; exact steps and
runs commute with that transformation. Chained after M222, the collision-free
813-rule machine reaches a reflected appender endpoint containing the complete
canonical CNF token encoding. Exact work, compiled execution, one-step-short
nonhalting, and one source-size polynomial bound are checked. Body-token and
...
M228 removes the staged request cell and derives the unique `Finish` request
from the physical classifier terminal. The fixed 823-rule composition marks
every body coordinate at an explicit non-request pending boundary and dispatches
`Finish` to the exact next prefix with exact work, compiled, one-step-short,
and source-size polynomial evidence. Body-token and padding request synthesis,
successive physical configurations, the repeated loop, builder
`RawRefinement`, and the packaged `PolynomialReduction` remain open. No fixed
