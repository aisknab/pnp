...
namespace PNP.Concrete.CookLevinBuilderFourthClauseFirstLiteralPrefixRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderFourthClauseFirstLiteralPrefix
...
example : BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine.rules.length = 113 :=
  BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.rules_length

example : BuilderSecondClauseSecondLiteralPrefix.TrueFalseSuffix.machine.rules.length = 235 :=
  BuilderSecondClauseSecondLiteralPrefix.TrueFalseSuffix.rules_length

example : BuilderSecondClauseSecondLiteralPrefix.SecondLiteralSuffix.machine.rules.length = 357 :=
...
example : finalTokenSlot (inputOnlyProblem []) = 282 := rfl
example : finalTokenSlot (inputOnlyProblem [false]) = 562 := rfl
example : finalTokenSlot (inputOnlyProblem [true]) = 562 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, false, true]) = 1410 := rfl
example : finalTokenSlot
    (inputOnlyProblem [false, false, false, false]) = 1978 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, true, true, true]) = 1978 := rfl
example : finalTokenSlot
    (pairedProblem [true, false, true]) = 20010 := rfl

example (input : BitString) :
    fourthClauseFirstLiteralTokens (inputOnlyProblem input) =
...
example (input : BitString) :
    ∃ rest, encodeCNFTokens (inputOnlyProblem input).formula =
      fourthClauseFirstLiteralTokens (inputOnlyProblem input) ++ rest :=
  fourthClauseFirstLiteralTokens_eq_canonical_formula_prefix
    (inputOnlyProblem input)
...
example (input : BitString) :
    encodeTokenPairs
      ...
      (inputOnlyProblem input).encodedFormula.take
        (2 * ((inputOnlyProblem input).FormulaWidth + 31)) :=
    ...
    finalTape (inputOnlyProblem input) =
      BuilderTokenAppender.workspaceTape input
        ...
        (BuilderFourthClauseSeparatorStep.fourthClauseStartTokens
            (inputOnlyProblem input) ++
          [CNFToken.f, CNFToken.t, CNFToken.f]) := by
  simp [finalTape, fourthClauseFirstLiteralTokens, unaryTokenOutput,
    signTokenOutput, inputOnlyProblem, List.append_assoc]
...
example (input : BitString) :
  ...
  BuilderFourthClauseSeparatorStep.specification_separator_step
    (inputOnlyProblem input)
...
example (input : BitString) :
    (inputOnlyProblem input).formulaTokenSlotDirect
        (BuilderThirdClausePaddingRun.fourthClauseStart
          (inputOnlyProblem input) + 2) = some (some CNFToken.t) :=
  firstLiteralUnaryUnitSlot_direct_eq_t (inputOnlyProblem input)

example (input : BitString) :
    (inputOnlyProblem input).formulaTokenSlotDirect
        (BuilderThirdClausePaddingRun.fourthClauseStart
          (inputOnlyProblem input) + 3) = some (some CNFToken.f) :=
  firstLiteralTerminatorSlot_direct_eq_f (inputOnlyProblem input)
...
example (input : BitString) :
    VerifierTableauProblem.FormulaTokenCursor.step
      ...
      some (some CNFToken.f,
        ⟨BuilderThirdClausePaddingRun.fourthClauseStart
          (inputOnlyProblem input) + 2⟩) :=
  specification_firstLiteral_sign_step (inputOnlyProblem input)
...
example (input : BitString) :
    VerifierTableauProblem.FormulaTokenCursor.step
      ...
      some (some CNFToken.t,
        ⟨BuilderThirdClausePaddingRun.fourthClauseStart
          (inputOnlyProblem input) + 3⟩) :=
  specification_firstLiteral_unaryUnit_step (inputOnlyProblem input)
...
example (input : BitString) :
    VerifierTableauProblem.FormulaTokenCursor.step
      ...
      some (some CNFToken.f,
        ⟨BuilderThirdClausePaddingRun.fourthClauseStart
          (inputOnlyProblem input) + 4⟩) :=
  specification_firstLiteral_terminator_step (inputOnlyProblem input)
...
example (input : BitString) :
    (rawTimeBound inputOnlyVerifier).eval input.length =
      (BuilderFourthClauseSeparatorStep.rawTimeBound
          inputOnlyVerifier).eval input.length + 1422 + 72 * input.length +
        36 * (inputOnlyProblem input).FormulaWidth +
        36 * (BuilderFourthClauseSeparatorStep.cursorWord
...
example (input : BitString) :
    workBoundedDecide (machine (inputOnlyProblem input))
        (BuilderFourthClauseSeparatorStep.workSteps
          (inputOnlyProblem input))
...
end PNP.Concrete.CookLevinBuilderFourthClauseFirstLiteralPrefixRegression
