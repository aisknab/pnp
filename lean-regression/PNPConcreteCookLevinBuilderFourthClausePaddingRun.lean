...
namespace PNP.Concrete.CookLevinBuilderFourthClausePaddingRunRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderFourthClausePaddingRun
...
example (input : BitString) :
    (machine (inputOnlyProblem input)).rules.length =
      4328 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (fifthClauseSlotStartPolynomial inputOnlyVerifier) :=
  rules_length (inputOnlyProblem input)

example : remainingPaddingCount (inputOnlyProblem []) = 81 := rfl
example : remainingPaddingCount (inputOnlyProblem [false]) = 173 := rfl
example : remainingPaddingCount (inputOnlyProblem [true]) = 173 := rfl
example : remainingPaddingCount
    (inputOnlyProblem [true, false, true]) = 453 := rfl
example : remainingPaddingCount
    (pairedProblem [true, false, true]) = 6633 := rfl
example : remainingPaddingCount
    (inputOnlyProblem [false, false, false, false]) = 641 := rfl
example : remainingPaddingCount
    (inputOnlyProblem [true, true, true, true]) = 641 := rfl

example : fifthClauseSlotStart (inputOnlyProblem []) = 368 := rfl
example : fifthClauseSlotStart (inputOnlyProblem [false]) = 740 := rfl
example : fifthClauseSlotStart
    (inputOnlyProblem [true, false, true]) = 1868 := rfl
example : fifthClauseSlotStart
    (pairedProblem [true, false, true]) = 26648 := rfl
example : fifthClauseSlotStart
    (inputOnlyProblem [false, false, false, false]) = 2624 := rfl
example : fifthClauseSlotStart
    (inputOnlyProblem [true, true, true, true]) = 2624 := rfl

example (input : BitString) :
    (rawTimeBound inputOnlyVerifier).eval input.length =
      (BuilderFourthClausePrefix.rawTimeBound inputOnlyVerifier).eval
          input.length + 18 +
        ...
        6 *
          (remainingPaddingCount (inputOnlyProblem input) *
            ...
            remainingPaddingCount (inputOnlyProblem input) *
              remainingPaddingCount (inputOnlyProblem input)) +
        6 * BuilderUnaryPolynomial.workSteps
          (fifthClauseSlotStartPolynomial inputOnlyVerifier) input :=
  rawTimeBound_eval (inputOnlyProblem input)

example (input : BitString) :
    remainingPaddingCount (inputOnlyProblem input) =
      (inputOnlyProblem input).formulaTokensPerClause - 9 :=
  remainingPaddingCount_eq_formulaTokensPerClause_sub_nine
    (inputOnlyProblem input)

example (input : BitString) :
    BuilderFourthClausePrefix.finalTokenSlot (inputOnlyProblem input) +
        remainingPaddingCount (inputOnlyProblem input) =
      fifthClauseSlotStart (inputOnlyProblem input) :=
...
example (input : BitString) (offset : Nat)
    ...
    (inputOnlyProblem input).formulaTokenSlotDirect
        (BuilderFourthClausePrefix.finalTokenSlot
...
example (input : BitString) :
    (inputOnlyProblem input).formulaTokenSlotDirect
        (fifthClauseSlotStart (inputOnlyProblem input)) =
      some none :=
  fifthClauseSlotStart_direct_eq_padding (inputOnlyProblem input)
...
example (input : BitString) :
    specificationRun (inputOnlyProblem input)
        ...
        ⟨BuilderFourthClausePrefix.finalTokenSlot
          (inputOnlyProblem input)⟩ =
      some ([], ⟨fifthClauseSlotStart (inputOnlyProblem input)⟩) :=
...
example (input : BitString) :
    VerifierTableauProblem.FormulaTokenCursor.step
        ...
        ⟨fifthClauseSlotStart (inputOnlyProblem input)⟩ =
      some (none,
        ⟨fifthClauseSlotStart (inputOnlyProblem input) + 1⟩) :=
...
example (input : BitString) :
    workBoundedDecide (machine (inputOnlyProblem input))
        (BuilderFourthClausePrefix.workSteps (inputOnlyProblem input))
...
end PNP.Concrete.CookLevinBuilderFourthClausePaddingRunRegression
