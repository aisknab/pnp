...
namespace PNP.Concrete.CookLevinBuilderFourthClausePrefixRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderFourthClausePrefix
...
example : finalTokenSlot (inputOnlyProblem []) = 287 := rfl
example : finalTokenSlot (inputOnlyProblem [false]) = 567 := rfl
example : finalTokenSlot (inputOnlyProblem [true]) = 567 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, false, true]) = 1415 := rfl
example : finalTokenSlot
    (inputOnlyProblem [false, false, false, false]) = 1983 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, true, true, true]) = 1983 := rfl
example : finalTokenSlot
    (pairedProblem [true, false, true]) = 20015 := rfl

example (input : BitString) :
    (rawTimeBound inputOnlyVerifier).eval input.length =
      (BuilderFourthClauseSecondLiteralPrefix.rawTimeBound
          inputOnlyVerifier).eval input.length + 618 +
        24 * input.length +
        12 * (inputOnlyProblem input).FormulaWidth +
        12 * (BuilderFourthClauseSeparatorStep.cursorWord
          (inputOnlyProblem input)).length :=
  rawTimeBound_eval (inputOnlyProblem input)

example (input : BitString) :
    fourthClauseTokens (inputOnlyProblem input) =
...
example (input : BitString) :
    ∃ rest, encodeCNFTokens (inputOnlyProblem input).formula =
      fourthClauseTokens (inputOnlyProblem input) ++ rest :=
  fourthClauseTokens_eq_canonical_formula_prefix
    (inputOnlyProblem input)

example (input : BitString) :
    encodeTokenPairs (fourthClauseTokens (inputOnlyProblem input)) =
...
example (input : BitString) :
    (inputOnlyProblem input).formulaTokenSlotDirect
        (BuilderFourthClauseSecondLiteralPrefix.finalTokenSlot
          (inputOnlyProblem input)) =
...
example (input : BitString) :
    workBoundedDecide (machine (inputOnlyProblem input))
        (BuilderFourthClauseSecondLiteralPrefix.workSteps (inputOnlyProblem input))
        (rawInputWorkTape input) = .timeout :=
  prefixEndpoint_before_launch_timeout (inputOnlyProblem input)

example (input : BitString) :
    workBoundedDecide (machine (inputOnlyProblem input))
        (BuilderFourthClauseSecondLiteralPrefix.workSteps (inputOnlyProblem input) + 1 +
...
end PNP.Concrete.CookLevinBuilderFourthClausePrefixRegression
