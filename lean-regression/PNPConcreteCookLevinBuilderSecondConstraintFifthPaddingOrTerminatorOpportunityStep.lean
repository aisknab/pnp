...
namespace PNP.Concrete.CookLevinBuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStepRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep
...
example :
    WidthOptionalTerminatorAppender.machine.rules.length =
      93 :=
  WidthOptionalTerminatorAppender.rules_length
...
example : finalTokenSlot (inputOnlyProblem []) = 4520 := rfl
example : finalTokenSlot (inputOnlyProblem [false]) = 22228 := rfl
example : finalTokenSlot (inputOnlyProblem [true]) = 22228 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, false, true]) = 167276 := rfl
example : finalTokenSlot
    (pairedProblem [true, false, true]) = 41459456 := rfl
example : finalTokenSlot
    (inputOnlyProblem [false, false, false, false]) = 344536 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, true, true, true]) = 344536 := rfl

example (input : BitString) :
    secondConstraintFifthPaddingOrTerminatorTokens (inputOnlyProblem input) =
...
example (input : BitString) :
    ∃ rest, encodeCNFTokens (inputOnlyProblem input).formula =
      secondConstraintFifthPaddingOrTerminatorTokens
...
example :
    opportunityOutput (inputOnlyProblem [false]) = [CNFToken.f] := by
  rw [opportunityOutput_eq_nil_or_f]
  rfl

example :
    opportunityOutput (inputOnlyProblem [true]) = [CNFToken.f] := by
  rw [opportunityOutput_eq_nil_or_f]
  rfl

example :
    opportunityOutput (pairedProblem []) = [CNFToken.f] := by
  rw [opportunityOutput_eq_nil_or_f]
  rfl
      ...
      some
        (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState
          { state := BuilderTokenAppender.machine.acceptState
            tape := tape }) :=
  WidthOptionalTerminatorAppender.controller_done_skip_workStep
    tape
...
example (tape : WorkTape) :
    workStep?
      ...
      some
        (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState
          (BuilderTokenAppender.entryConfiguration .f tape)) :=
  WidthOptionalTerminatorAppender.controller_more_launch_workStep
    tape
...
end PNP.Concrete.CookLevinBuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStepRegression
