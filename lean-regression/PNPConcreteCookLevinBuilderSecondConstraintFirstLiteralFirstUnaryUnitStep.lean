...
namespace PNP.Concrete.CookLevinBuilderSecondConstraintFirstLiteralFirstUnaryUnitStepRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep
...
example : finalTokenSlot (inputOnlyProblem []) = 4511 := rfl
example : finalTokenSlot (inputOnlyProblem [false]) = 22219 := rfl
example : finalTokenSlot (inputOnlyProblem [true]) = 22219 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, false, true]) = 167267 := rfl
example : finalTokenSlot
    (pairedProblem [true, false, true]) = 41459447 := rfl
example : finalTokenSlot
    (inputOnlyProblem [false, false, false, false]) = 344527 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, true, true, true]) = 344527 := rfl

example (input : BitString) :
    workSteps (inputOnlyProblem input) =
      BuilderSecondConstraintFirstLiteralSignStep.workSteps
...
example (input : BitString) :
    (rawTimeBound inputOnlyVerifier).eval input.length =
      (BuilderSecondConstraintFirstLiteralSignStep.rawTimeBound inputOnlyVerifier).eval
          input.length + 558 +
...
example (input : BitString) :
    secondConstraintFirstLiteralFirstUnaryTokens (inputOnlyProblem input) =
...
example (input : BitString) :
    ∃ rest, encodeCNFTokens (inputOnlyProblem input).formula =
      secondConstraintFirstLiteralFirstUnaryTokens (inputOnlyProblem input) ++ rest :=
  secondConstraintFirstLiteralFirstUnaryTokens_eq_canonical_formula_prefix
    (inputOnlyProblem input)

example (input : BitString) :
    encodeTokenPairs (secondConstraintFirstLiteralFirstUnaryTokens (inputOnlyProblem input)) =
      (inputOnlyProblem input).encodedFormula.take
        (2 * ((inputOnlyProblem input).FormulaWidth + 39)) :=
...
example (input : BitString) :
    VerifierTableauProblem.FormulaTokenCursor.step
      ...
      some (some CNFToken.t,
        ⟨BuilderSecondConstraintFirstLiteralSignStep.finalTokenSlot
          (inputOnlyProblem input) + 1⟩) :=
  specification_firstUnaryUnit_step (inputOnlyProblem input)
    ...
    workStep? (machine (inputOnlyProblem input))
      ...
      some (renameConfiguration
        ...
        (workStartConfiguration
          ...
          (BuilderSecondConstraintFirstLiteralSignStep.finalTape
            (inputOnlyProblem input)))) :=
  prefixFirstUnaryUnit_launch_workStep (inputOnlyProblem input)
...
end PNP.Concrete.CookLevinBuilderSecondConstraintFirstLiteralFirstUnaryUnitStepRegression
