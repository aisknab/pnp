...
namespace PNP.Concrete.CookLevinBuilderSecondConstraintFirstLiteralSecondUnaryUnitStepRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep
...
example : finalTokenSlot (inputOnlyProblem []) = 4512 := rfl
example : finalTokenSlot (inputOnlyProblem [false]) = 22220 := rfl
example : finalTokenSlot (inputOnlyProblem [true]) = 22220 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, false, true]) = 167268 := rfl
example : finalTokenSlot
    (pairedProblem [true, false, true]) = 41459448 := rfl
example : finalTokenSlot
    (inputOnlyProblem [false, false, false, false]) = 344528 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, true, true, true]) = 344528 := rfl

example (input : BitString) :
    workSteps (inputOnlyProblem input) =
      BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.workSteps
...
example (input : BitString) :
    (rawTimeBound inputOnlyVerifier).eval input.length =
      (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.rawTimeBound inputOnlyVerifier).eval
          input.length + 570 +
...
example (input : BitString) :
    secondConstraintFirstLiteralSecondUnaryTokens (inputOnlyProblem input) =
...
example (input : BitString) :
    ∃ rest, encodeCNFTokens (inputOnlyProblem input).formula =
      secondConstraintFirstLiteralSecondUnaryTokens (inputOnlyProblem input) ++ rest :=
  secondConstraintFirstLiteralSecondUnaryTokens_eq_canonical_formula_prefix
    (inputOnlyProblem input)

example (input : BitString) :
    encodeTokenPairs (secondConstraintFirstLiteralSecondUnaryTokens (inputOnlyProblem input)) =
      (inputOnlyProblem input).encodedFormula.take
        (2 * ((inputOnlyProblem input).FormulaWidth + 40)) :=
...
example (input : BitString) :
    VerifierTableauProblem.FormulaTokenCursor.step
      ...
      some (some CNFToken.t,
        ⟨BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTokenSlot
          (inputOnlyProblem input) + 1⟩) :=
  specification_secondUnaryUnit_step (inputOnlyProblem input)
    ...
    workStep? (machine (inputOnlyProblem input))
      ...
      some (renameConfiguration
        ...
        (workStartConfiguration
          ...
          (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTape
            (inputOnlyProblem input)))) :=
  prefixSecondUnaryUnit_launch_workStep (inputOnlyProblem input)
...
end PNP.Concrete.CookLevinBuilderSecondConstraintFirstLiteralSecondUnaryUnitStepRegression
