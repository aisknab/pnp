...
namespace PNP.Concrete.CookLevinBuilderSecondConstraintFirstLiteralThirdUnaryUnitStepRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderSecondConstraintFirstLiteralThirdUnaryUnitStep
...
example : finalTokenSlot (inputOnlyProblem []) = 4513 := rfl
example : finalTokenSlot (inputOnlyProblem [false]) = 22221 := rfl
example : finalTokenSlot (inputOnlyProblem [true]) = 22221 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, false, true]) = 167269 := rfl
example : finalTokenSlot
    (pairedProblem [true, false, true]) = 41459449 := rfl
example : finalTokenSlot
    (inputOnlyProblem [false, false, false, false]) = 344529 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, true, true, true]) = 344529 := rfl

example (input : BitString) :
    workSteps (inputOnlyProblem input) =
      BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.workSteps
...
example (input : BitString) :
    (rawTimeBound inputOnlyVerifier).eval input.length =
      (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.rawTimeBound inputOnlyVerifier).eval
          input.length + 582 +
...
example (input : BitString) :
    secondConstraintFirstLiteralThirdUnaryTokens (inputOnlyProblem input) =
...
example (input : BitString) :
    ∃ rest, encodeCNFTokens (inputOnlyProblem input).formula =
      secondConstraintFirstLiteralThirdUnaryTokens (inputOnlyProblem input) ++ rest :=
  secondConstraintFirstLiteralThirdUnaryTokens_eq_canonical_formula_prefix
    (inputOnlyProblem input)

example (input : BitString) :
    encodeTokenPairs (secondConstraintFirstLiteralThirdUnaryTokens (inputOnlyProblem input)) =
      (inputOnlyProblem input).encodedFormula.take
        (2 * ((inputOnlyProblem input).FormulaWidth + 41)) :=
...
example (input : BitString) :
    VerifierTableauProblem.FormulaTokenCursor.step
      ...
      some (some CNFToken.t,
        ⟨BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTokenSlot
          (inputOnlyProblem input) + 1⟩) :=
  specification_thirdUnaryUnit_step (inputOnlyProblem input)
    ...
    workStep? (machine (inputOnlyProblem input))
      ...
      some (renameConfiguration
        ...
        (workStartConfiguration
          ...
          (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTape
            (inputOnlyProblem input)))) :=
  prefixThirdUnaryUnit_launch_workStep (inputOnlyProblem input)
...
end PNP.Concrete.CookLevinBuilderSecondConstraintFirstLiteralThirdUnaryUnitStepRegression
