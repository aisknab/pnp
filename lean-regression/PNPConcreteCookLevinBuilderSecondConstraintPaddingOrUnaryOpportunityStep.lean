...
namespace PNP.Concrete.CookLevinBuilderSecondConstraintPaddingOrUnaryOpportunityStepRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderSecondConstraintPaddingOrUnaryOpportunityStep
...
example :
    WidthOptionalAppender.machine.rules.length = 93 :=
  WidthOptionalAppender.rules_length
...
example : finalTokenSlot (inputOnlyProblem []) = 4516 := rfl
example : finalTokenSlot (inputOnlyProblem [false]) = 22224 := rfl
example : finalTokenSlot (inputOnlyProblem [true]) = 22224 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, false, true]) = 167272 := rfl
example : finalTokenSlot
    (pairedProblem [true, false, true]) = 41459452 := rfl
example : finalTokenSlot
    (inputOnlyProblem [false, false, false, false]) = 344532 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, true, true, true]) = 344532 := rfl

example (input : BitString) :
    secondConstraintPaddingOrUnaryTokens (inputOnlyProblem input) =
...
example (input : BitString) :
    ∃ rest, encodeCNFTokens (inputOnlyProblem input).formula =
      secondConstraintPaddingOrUnaryTokens
...
example (tape : WorkTape) :
    workStep? WidthOptionalAppender.machine
      ...
      some
        (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState
          { state := BuilderTokenAppender.machine.acceptState
            tape := tape }) :=
  WidthOptionalAppender.controller_done_skip_workStep tape
...
example (tape : WorkTape) :
  ...
  WidthOptionalAppender.controller_more_launch_workStep tape
...
end PNP.Concrete.CookLevinBuilderSecondConstraintPaddingOrUnaryOpportunityStepRegression
