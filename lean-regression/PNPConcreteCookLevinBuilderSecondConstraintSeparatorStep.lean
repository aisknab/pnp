...
namespace PNP.Concrete.CookLevinBuilderSecondConstraintSeparatorStepRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderSecondConstraintSeparatorStep
...
example : finalTokenSlot (inputOnlyProblem []) = 4509 := rfl
example : finalTokenSlot (inputOnlyProblem [false]) = 22217 := rfl
example : finalTokenSlot (inputOnlyProblem [true]) = 22217 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, false, true]) = 167265 := rfl
example : finalTokenSlot
    (pairedProblem [true, false, true]) = 41459445 := rfl
example : finalTokenSlot
    (inputOnlyProblem [false, false, false, false]) = 344525 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, true, true, true]) = 344525 := rfl

example (input : BitString) :
    workSteps (inputOnlyProblem input) =
      BuilderFirstConstraintPaddingRun.workSteps (inputOnlyProblem input) + 1 +
...
example (input : BitString) :
    (rawTimeBound inputOnlyVerifier).eval input.length =
      (BuilderFirstConstraintPaddingRun.rawTimeBound inputOnlyVerifier).eval
          input.length + 534 +
...
example (input : BitString) :
    secondConstraintStartTokens (inputOnlyProblem input) =
      BuilderFourthClausePrefix.fourthClauseTokens (inputOnlyProblem input) ++
        [CNFToken.sep] := rfl

example (input : BitString) :
    ∃ rest, encodeCNFTokens (inputOnlyProblem input).formula =
      secondConstraintStartTokens (inputOnlyProblem input) ++ rest :=
  secondConstraintStartTokens_eq_canonical_formula_prefix
    (inputOnlyProblem input)

example (input : BitString) :
    encodeTokenPairs (secondConstraintStartTokens (inputOnlyProblem input)) =
      (inputOnlyProblem input).encodedFormula.take
        (2 * ((inputOnlyProblem input).FormulaWidth + 37)) :=
...
example (input : BitString) :
    VerifierTableauProblem.FormulaTokenCursor.step
        (inputOnlyProblem input)
        ⟨BuilderFirstConstraintPaddingRun.secondConstraintStart
          (inputOnlyProblem input)⟩ =
...
example (input : BitString) :
    workBoundedDecide (machine (inputOnlyProblem input))
        (BuilderFirstConstraintPaddingRun.workSteps (inputOnlyProblem input))
...
end PNP.Concrete.CookLevinBuilderSecondConstraintSeparatorStepRegression
