...
namespace PNP.Concrete.CookLevinBuilderThirdClauseFirstLiteralPrefixRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderThirdClauseFirstLiteralPrefix
...
example : BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine.rules.length = 113 :=
  BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.rules_length

example : BuilderSecondClauseFirstLiteralPrefix.FirstLiteralSuffix.machine.rules.length = 235 :=
...
example : finalTokenSlot (inputOnlyProblem []) = 191 := rfl
example : finalTokenSlot (inputOnlyProblem [false]) = 379 := rfl
example : finalTokenSlot (inputOnlyProblem [true]) = 379 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, false, true]) = 947 := rfl
example : finalTokenSlot
    (pairedProblem [true, false, true]) = 13367 := rfl
example : finalTokenSlot
    (inputOnlyProblem [false, false, false, false]) = 1327 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, true, true, true]) = 1327 := rfl

example : workSteps (inputOnlyProblem []) = 470046 := rfl
example : workSteps (inputOnlyProblem [false]) = 1546782 := rfl
example : workSteps (inputOnlyProblem [true]) = 1546782 := rfl
example : workSteps
    (inputOnlyProblem [true, false, true]) = 8379075 := rfl

example : (rawTimeBound inputOnlyVerifier).eval 0 = 2822427 := rfl
example : (rawTimeBound inputOnlyVerifier).eval 1 = 9285492 := rfl
example : (rawTimeBound inputOnlyVerifier).eval 3 = 50286258 := rfl
example : (rawTimeBound inputOnlyVerifier).eval 4 = 95417751 := rfl
example : (rawTimeBound pairedVerifier).eval 3 = 8713542096 := rfl

example (input : BitString) :
    thirdClauseFirstLiteralTokens (inputOnlyProblem input) =
      BuilderThirdClauseSeparatorStep.thirdClauseStartTokens
          (inputOnlyProblem input) ++ [CNFToken.f, CNFToken.f] := by
  simp [thirdClauseFirstLiteralTokens, firstTokenOutput, List.append_assoc]

example (input : BitString) :
    ∃ rest, encodeCNFTokens (inputOnlyProblem input).formula =
      thirdClauseFirstLiteralTokens (inputOnlyProblem input) ++ rest :=
  thirdClauseFirstLiteralTokens_eq_canonical_formula_prefix
...
example (input : BitString) :
    encodeTokenPairs
      ...
      (inputOnlyProblem input).encodedFormula.take
        (2 * ((inputOnlyProblem input).FormulaWidth + 22)) :=
...
example (input : BitString) :
    (rawTimeBound inputOnlyVerifier).eval input.length =
      (BuilderThirdClauseSeparatorStep.rawTimeBound inputOnlyVerifier).eval
          input.length + 732 + 48 * input.length +
        24 * (inputOnlyProblem input).FormulaWidth +
        24 * (BuilderThirdClauseSeparatorStep.cursorWord
...
example (input : BitString) :
    workBoundedDecide (machine (inputOnlyProblem input))
        (BuilderThirdClauseSeparatorStep.workSteps (inputOnlyProblem input))
...
end PNP.Concrete.CookLevinBuilderThirdClauseFirstLiteralPrefixRegression
