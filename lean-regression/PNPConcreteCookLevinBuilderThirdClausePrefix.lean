...
namespace PNP.Concrete.CookLevinBuilderThirdClausePrefixRegression

open PipelineTape PipelineStateNamespace
open CookLevin
open CookLevin.BuilderThirdClausePrefix
...
example : finalTokenSlot (inputOnlyProblem []) = 196 := rfl
example : finalTokenSlot (inputOnlyProblem [false]) = 384 := rfl
example : finalTokenSlot (inputOnlyProblem [true]) = 384 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, false, true]) = 952 := rfl
example : finalTokenSlot
    (inputOnlyProblem [false, false, false, false]) = 1332 := rfl
example : finalTokenSlot
    (inputOnlyProblem [true, true, true, true]) = 1332 := rfl
example : finalTokenSlot
    (pairedProblem [true, false, true]) = 13372 := rfl

example (input : BitString) :
    (rawTimeBound inputOnlyVerifier).eval input.length =
      (BuilderThirdClauseSecondLiteralPrefix.rawTimeBound
          inputOnlyVerifier).eval input.length + 498 +
        24 * input.length +
        12 * (inputOnlyProblem input).FormulaWidth +
        12 * (BuilderThirdClauseSeparatorStep.cursorWord
          (inputOnlyProblem input)).length :=
  rawTimeBound_eval (inputOnlyProblem input)

example (input : BitString) :
    thirdClauseTokens (inputOnlyProblem input) =
...
example (input : BitString) :
    ∃ rest, encodeCNFTokens (inputOnlyProblem input).formula =
      thirdClauseTokens (inputOnlyProblem input) ++ rest :=
  thirdClauseTokens_eq_canonical_formula_prefix
    (inputOnlyProblem input)

example (input : BitString) :
    encodeTokenPairs (thirdClauseTokens (inputOnlyProblem input)) =
...
example (input : BitString) :
    (inputOnlyProblem input).formulaTokenSlotDirect
        (BuilderThirdClauseSecondLiteralPrefix.finalTokenSlot
          (inputOnlyProblem input)) =
...
example (input : BitString) :
    workBoundedDecide (machine (inputOnlyProblem input))
        (BuilderThirdClauseSecondLiteralPrefix.workSteps (inputOnlyProblem input))
        (rawInputWorkTape input) = .timeout :=
  prefixEndpoint_before_launch_timeout (inputOnlyProblem input)

example (input : BitString) :
    workBoundedDecide (machine (inputOnlyProblem input))
        (BuilderThirdClauseSecondLiteralPrefix.workSteps (inputOnlyProblem input) + 1 +
...
end PNP.Concrete.CookLevinBuilderThirdClausePrefixRegression
