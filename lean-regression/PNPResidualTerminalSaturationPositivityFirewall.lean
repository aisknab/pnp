...
/-! ## Total saturation-positivity firewall regressions -/

def saturationFirewallKeepsAll : TerminalProfileProjection 1 :=
  { keep := fun _coordinate => true }

def saturationFirewallLossProblem : TerminalBCELAnchorProblem
    bcelAnchorCandidate bcelAnchorReadySaturationSystem :=
  { support := bcelAnchorReadySupport
    projection := saturationFirewallKeepsAll
    observe := bcelAnchorReadyObserver }

theorem saturationFirewallLossDefectZero :
    saturationFirewallLossProblem.familyDefect
      saturationFirewallLossProblem.anchorRecords = 0 := by
  decide

example :
    ∃ loss : TerminalProjectionPositivityLoss saturationFirewallLossProblem,
      classifyTerminalSaturationPositivity saturationFirewallLossProblem =
        .projectionPositivityLost loss :=
  classifyTerminalSaturationPositivity_loss_of_zero
    saturationFirewallLossProblem saturationFirewallLossDefectZero

example :
    terminalQuotientProfileMinimum
        saturationFirewallLossProblem.wholeCorners.system
        saturationFirewallLossProblem.wholeCorners.projection
        saturationFirewallLossProblem.wholeCorners.left =
      terminalFullProfileMinimum
        saturationFirewallLossProblem.wholeCorners.system
        saturationFirewallLossProblem.wholeCorners.left := by
  obtain ⟨loss, _classified⟩ :=
    classifyTerminalSaturationPositivity_loss_of_zero
      saturationFirewallLossProblem saturationFirewallLossDefectZero
  exact loss.minima_eq

def saturationFirewallReadyOutcome : Nat :=
  match classifyTerminalSaturationPositivity bcelAnchorReadyProblem with
  | .projectionPositivityLost _loss => 0
  | .bcel _positive outcome =>
      match outcome with
      | .insufficient _failure => 1
      | .algebraFailure _nucleus _first _failure => 2
      | .cutDefectFailure _nucleus _first _failure => 3
      | .cutRouteFailure _nucleus _first _failure => 4
      | .ready _result => 5

example : saturationFirewallReadyOutcome = 5 := by decide

def saturationFirewallSingletonOutcome : Nat :=
  match classifyTerminalSaturationPositivity bcelAnchorSingletonProblem with
  | .projectionPositivityLost _loss => 0
  | .bcel _positive outcome =>
      match outcome with
      | .insufficient _failure => 1
...
example : saturationFirewallSingletonOutcome = 1 := by decide

def saturationFirewallSharedOutcome : Nat :=
  match classifyTerminalSaturationPositivity bcelAnchorSharedProblem with
  | .projectionPositivityLost _loss => 0
  | .bcel _positive outcome =>
      match outcome with
      | .insufficient _failure => 1
...
example : saturationFirewallSharedOutcome = 2 := by decide

def saturationFirewallRouteOutcome : Nat :=
  match classifyTerminalSaturationPositivity bcelAnchorRouteProblem with
  | .projectionPositivityLost _loss => 0
  | .bcel _positive outcome =>
      match outcome with
      | .insufficient _failure => 1
...
example : saturationFirewallRouteOutcome = 4 := by decide

example
    (comparison : TerminalQuotientComparison
      bcelAnchorReadyProblem.wholeCorners.system
      bcelAnchorReadyProblem.wholeCorners.projection
      bcelAnchorReadyProblem.wholeCorners.left)
  ...
  terminalSaturationPositivity_no_checkedFullLiftAtMinimum
    bcelAnchorReadyProblem bcelAnchorReadyWholePositive
    comparison atMinimum
