...
Input-dependent traversal of the intentionally empty fifth fixed-width
Cook--Levin clause rectangle.

The predecessor retains the first opportunity in that empty rectangle. This
module evaluates its exact width `formulaTokensPerClause`, consumes that unary
counter with one literal loop, and then materializes the first coordinate of
the also-empty sixth fixed-width clause slot. No formula token is emitted by
this milestone.
-/
...
namespace BuilderFifthClausePaddingRun

open PipelineTape PipelineStateNamespace PipelineStageBridges

/-! ### Exact external coordinates -/

/-- Number of padding opportunities in one complete fixed-width clause
rectangle. -/
...
/-- Absolute token coordinate of the first opportunity in the sixth
fixed-width clause slot. -/
def sixthClauseSlotStartPolynomial {language : Language}
    (verifier : PolynomialTimeVerifier language) : NatPolynomial :=
  .add (BuilderFourthClausePaddingRun.fifthClauseSlotStartPolynomial verifier)
    (formulaClauseTokenPolynomial verifier)

def sixthClauseSlotStart {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  (sixthClauseSlotStartPolynomial problem.verifier).eval problem.input.length

theorem sixthClauseSlotStart_eq {language : Language}
    (problem : VerifierTableauProblem language) :
    sixthClauseSlotStart problem =
      problem.formulaVariableSlotBound + 1 +
        5 * problem.formulaTokensPerClause := by
  unfold sixthClauseSlotStart sixthClauseSlotStartPolynomial
  simp only [NatPolynomial.eval_add]
  change BuilderFourthClausePaddingRun.fifthClauseSlotStart problem +
      (formulaClauseTokenPolynomial problem.verifier).eval
        problem.input.length = _
  rw [BuilderFourthClausePaddingRun.fifthClauseSlotStart_eq]
  have hClause :
      (formulaClauseTokenPolynomial problem.verifier).eval
...
theorem predecessorSlot_add_paddingCount
    {language : Language} (problem : VerifierTableauProblem language) :
    BuilderFourthClausePaddingRun.finalTokenSlot problem +
        paddingCount problem = sixthClauseSlotStart problem := by
  rw [BuilderFourthClausePaddingRun.finalTokenSlot_eq_fifthClauseSlotStart,
    paddingCount_eq_formulaTokensPerClause,
    sixthClauseSlotStart_eq]
  omega
...
def targetEvaluator {language : Language}
    (problem : VerifierTableauProblem language) : WorkMachine :=
  BuilderUnaryPolynomial.machine
    (sixthClauseSlotStartPolynomial problem.verifier)
...
clause and across the intentionally empty fifth clause rectangle. -/
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderFourthClausePaddingRun.machine problem) := by
  exact BuilderFourthClausePaddingRun.rule_source_ne_acceptState problem
...
private theorem targetEvaluator_noRuleAtAccept {language : Language}
  ...
  intro rule hRule
  exact Nat.ne_of_lt (BuilderUnaryPolynomial.rule_source_lt_acceptState
    (sixthClauseSlotStartPolynomial problem.verifier) rule hRule)
...
theorem rules_length {language : Language}
    ...
    (machine problem).rules.length =
      4380 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (paddingPolynomial problem.verifier) +
        BuilderUnaryPolynomial.ruleCount
          (sixthClauseSlotStartPolynomial problem.verifier) := by
  have hPredecessor := BuilderFourthClausePaddingRun.rules_length problem
  ...
  have hTargetMachine :
      (BuilderUnaryPolynomial.machine
        (sixthClauseSlotStartPolynomial problem.verifier)).rules.length =
      BuilderUnaryPolynomial.ruleCount
        (sixthClauseSlotStartPolynomial problem.verifier) := by
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderFourthClausePaddingRun.machine problem)
    (paddingSuffixMachine problem)
    (BuilderFourthClausePaddingRun.rules_pairwise_query_distinct problem)
...
def countOutside {language : Language}
  ...
  BuilderUnaryPolynomial.finalOutsideLeft
    ...
    (BuilderFourthClausePaddingRun.finalOutside problem)
...
def countdownFinalOutside {language : Language}
  ...
  (countWord problem).take (countRootPrefixLength problem) ++
    ...
    (BuilderFourthClausePaddingRun.finalOutside problem).drop
...
def finalOutside {language : Language}
    (problem : VerifierTableauProblem language) : List WorkSymbol :=
  BuilderUnaryPolynomial.finalOutsideLeft
    (sixthClauseSlotStartPolynomial problem.verifier) problem.input
    (countdownFinalOutside problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderFourthClausePaddingRun.workSteps problem + 1 +
...
theorem countdown_workRunExact {language : Language}
  ...
  let outside := BuilderFourthClausePaddingRun.finalOutside problem
...
private theorem paddingSuffix_workRunExact {language : Language}
    ...
    workRunExact? (paddingSuffixMachine problem) (suffixWorkSteps problem)
        (workStartConfiguration (paddingSuffixMachine problem)
          (BuilderFourthClausePaddingRun.finalTape problem)) =
      some
  ...
  simpa [paddingSuffixMachine, countdownTargetMachine, targetEvaluator,
    ...
    BuilderFourthClausePaddingRun.finalOutside,
    BuilderUnaryPolynomial.initialConfiguration,
    BuilderFirstClausePrefix.WorkChain.machine, workStartConfiguration,
    renameConfiguration, Nat.add_assoc] using hCombined
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderFourthClausePaddingRun.workSteps problem)
      ...
      some (renameConfiguration
        ...
        (BuilderFourthClausePaddingRun.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderFourthClausePaddingRun.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderFourthClausePaddingRun.machine problem)
      (paddingSuffixMachine problem))
    (BuilderFourthClausePaddingRun.workSteps problem)
    (workStartConfiguration (BuilderFourthClausePaddingRun.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderFourthClausePaddingRun.finalConfiguration problem)
    (BuilderFourthClausePaddingRun.workRunExact problem)
...
theorem launch_workStep {language : Language}
    ...
    workStep? (machine problem)
      ...
      some (renameConfiguration
        ...
        (workStartConfiguration (paddingSuffixMachine problem)
          (BuilderFourthClausePaddingRun.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderFourthClausePaddingRun.machine problem)
    (paddingSuffixMachine problem)
    (BuilderFourthClausePaddingRun.finalTape problem)
  simpa [machine, BuilderFourthClausePaddingRun.finalConfiguration,
...
materializes the sixth-slot boundary coordinate. -/
theorem workRunExact {language : Language}
  ...
  let predecessorInitial := workStartConfiguration
    (BuilderFourthClausePaddingRun.machine problem)
    (rawInputWorkTape problem.input)
  let predecessorFinal := BuilderFourthClausePaddingRun.finalConfiguration
    problem
  let suffixFinal : WorkConfiguration :=
    { state := (paddingSuffixMachine problem).acceptState,
      tape := finalTape problem }
...
private theorem formulaClauseSchedule_starts_fourShapeClauses_then_two_empty
    {language : Language} (problem : VerifierTableauProblem language) :
    let time : Fin problem.dimensions.timeCount :=
      ⟨0, problem.dimensions.timeCount_positive⟩
    let position : Fin
  ...
  have hVariableBound :=
    BuilderFirstClausePaddingRun.formulaVariableSlotBound_at_least_three
      problem
  have hSlots : 6 ≤ problem.formulaClauseSlotsPerConstraint := by
  ...
  have hPadding : problem.formulaClauseSlotsPerConstraint - 4 =
      Nat.succ (Nat.succ
        (problem.formulaClauseSlotsPerConstraint - 6)) := by
    omega
  let rest : List (Option (BoundedClause problem.FormulaWidth)) :=
    List.replicate (problem.formulaClauseSlotsPerConstraint - 6) none ++
  ...
  rw [hPadding, List.replicate_succ, List.replicate_succ]
...
private theorem formulaClauseTokens_first_four_rectangles_then_two_emptySlots
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
  ...
  rcases formulaClauseSchedule_starts_fourShapeClauses_then_two_empty problem with
    ⟨left, right, thirdLeft, thirdRight, fourthLeft, fourthRight,
...
opportunity in the intentionally empty fifth fixed-width clause rectangle. -/
theorem paddingSlot_direct_eq_padding {language : Language}
    ...
    problem.formulaTokenSlotDirect
        (BuilderFourthClausePaddingRun.finalTokenSlot problem + offset) =
      some none := by
  rw [problem.formulaTokenSlotDirect_eq]
  rcases formulaClauseTokens_first_four_rectangles_then_two_emptySlots problem with
    ⟨rest, hClauses⟩
  ...
  have hCount :=
    paddingCount_eq_formulaTokensPerClause problem
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause + offset -
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + offset - 9 =
...
opportunity in the intentionally empty sixth fixed-width clause slot. -/
theorem sixthClauseSlotStart_direct_eq_padding {language : Language}
    (problem : VerifierTableauProblem language) :
    problem.formulaTokenSlotDirect (sixthClauseSlotStart problem) =
      some none := by
  rw [problem.formulaTokenSlotDirect_eq]
  rcases formulaClauseTokens_first_four_rectangles_then_two_emptySlots problem with
    ⟨rest, hClauses⟩
  unfold VerifierTableauProblem.formulaTokenSchedule
  rw [hClauses, sixthClauseSlotStart_eq]
  have hHeader :
      (FormulaSchedule.pad (problem.formulaVariableSlotBound + 1)
        (encodeUnaryTokens problem.FormulaWidth)).length =
          problem.formulaVariableSlotBound + 1 := by
  ...
  rw [show problem.formulaVariableSlotBound + 1 +
      ...
      (problem.formulaVariableSlotBound + 1) =
        5 * problem.formulaTokensPerClause by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 5 * problem.formulaTokensPerClause -
      11 = (problem.formulaTokensPerClause - 11) +
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 4 * problem.formulaTokensPerClause - 7 =
      (problem.formulaTokensPerClause - 7) +
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 3 * problem.formulaTokensPerClause - 8 =
      (problem.formulaTokensPerClause - 8) +
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause - 9 =
  ...
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause -
      problem.formulaTokensPerClause = 0 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_pos (by omega)]
  rw [List.getElem?_replicate, if_pos (by omega)]
...
private theorem specificationRun_padding_from_offset
  ...
  | succ count ih =>
      ...
      have hStep : VerifierTableauProblem.FormulaTokenCursor.step problem
          ⟨BuilderFourthClausePaddingRun.finalTokenSlot problem + offset⟩ =
        some (none,
          ⟨BuilderFourthClausePaddingRun.finalTokenSlot problem + offset + 1⟩) := by
      ...
      rw [show BuilderFourthClausePaddingRun.finalTokenSlot problem +
          offset + 1 =
        BuilderFourthClausePaddingRun.finalTokenSlot problem +
          (offset + 1) by omega]
      simp only
...
entire intentionally empty fifth-clause rectangle and stops exactly at the
sixth fixed-width clause slot. -/
theorem specification_padding_run {language : Language}
    (problem : VerifierTableauProblem language) :
    specificationRun problem (paddingCount problem)
        ⟨BuilderFourthClausePaddingRun.finalTokenSlot problem⟩ =
      some ([], ⟨sixthClauseSlotStart problem⟩) := by
...
padding opportunity in the intentionally empty sixth fixed-width clause slot. -/
theorem specification_target_step {language : Language}
  ...
  unfold VerifierTableauProblem.FormulaTokenCursor.step
  rw [sixthClauseSlotStart_direct_eq_padding]
...
theorem finalTokenBits_eq_encodedFormula_fourthClause
  ...
  BuilderFourthClausePaddingRun.finalTokenBits_eq_encodedFormula_fourthClause
    problem
...
theorem finalTokenSlot_eq_sixthClauseSlotStart {language : Language}
    (problem : VerifierTableauProblem language) :
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
      5 * problem.formulaTokensPerClause := by
  exact sixthClauseSlotStart_eq problem
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  rcases BuilderUnaryPolynomial.root_register_length
      (sixthClauseSlotStartPolynomial problem.verifier) problem.input.length with
  ...
  unfold finalOutside BuilderUnaryPolynomial.finalOutsideLeft
    BuilderUnaryPolynomial.overlayScratch finalTokenSlot sixthClauseSlotStart
  rw [hScratch]
...
def rawTimeBound {language : Language}
    (verifier : PolynomialTimeVerifier language) : NatPolynomial :=
  let count := paddingPolynomial verifier
  let target := sixthClauseSlotStartPolynomial verifier
  .add (BuilderFourthClausePaddingRun.rawTimeBound verifier)
    (.add (.constant 18)
      (.add
        (scalePolynomial 6
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderFourthClausePaddingRun.rawTimeBound problem.verifier).eval
        ...
        6 * BuilderUnaryPolynomial.workSteps
          (sixthClauseSlotStartPolynomial problem.verifier) problem.input := by
  rw [rawTimeBound]
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPredecessor := BuilderFourthClausePaddingRun.rawTimeBound_le problem
...
/-- The fifth-clause-slot endpoint is globally nonhalting until the outer
bridge launches the full-rectangle padding evaluator. -/
...
end BuilderFifthClausePaddingRun
