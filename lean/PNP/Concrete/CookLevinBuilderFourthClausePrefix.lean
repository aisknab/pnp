...
Completion of the fourth canonical Cook--Levin clause.

The machine in this file composes the complete fourth-clause second literal with
one state-selected clause terminator appender and the existing unary cursor
advance. Every raw input therefore completes clause four and leaves the retained
coordinate on that clause's first in-range padding opportunity. This is one
fixed populated transition, not a padding loop, a general schedule decoder,
or a complete formula builder.
-/
...
namespace BuilderFourthClausePrefix

open PipelineTape PipelineStateNamespace PipelineStageBridges

/-! ### Selected terminator appender followed by one cursor advance -/

namespace FinishTokenCursor
...
/-- One selected terminator append followed by the existing unary cursor
...
end FinishTokenCursor

private theorem predecessor_noRuleAtAccept {language : Language}
    (problem : VerifierTableauProblem language) :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderFourthClauseSecondLiteralPrefix.machine problem) := by
  exact BuilderFourthClauseSecondLiteralPrefix.rule_source_ne_acceptState problem

private theorem suffix_noRuleAtAccept :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      FinishTokenCursor.machine := by
  exact FinishTokenCursor.rule_source_ne_acceptState

/-- One literal finite work machine from raw input through the complete
fourth canonical clause. -/
def machine {language : Language}
  ...
  BuilderFirstClausePrefix.WorkChain.machine
    (BuilderFourthClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine
      ...
      4276 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderThirdClausePaddingRun.fourthClauseStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderFourthClauseSecondLiteralPrefix.rules_length problem
  have hSuffix := FinishTokenCursor.rules_length
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderFourthClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine
    (BuilderFourthClauseSecondLiteralPrefix.rules_pairwise_query_distinct problem)
    FinishTokenCursor.rules_pairwise_query_distinct
...
theorem machine_acceptState_ne_rejectState {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.machine_acceptState_ne_rejectState
    (BuilderFourthClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine
    FinishTokenCursor.machine_acceptState_ne_rejectState
...
def fourthClauseTokens {language : Language}
...
private theorem predecessor_finalOutside_eq_cursor {language : Language}
  ...
  simp only [BuilderFourthClauseSecondLiteralPrefix.finalOutside,
    List.length_append, List.length_singleton]
  rw [show
      (BuilderFourthClauseSecondLiteralPrefix.fourthCursorWord problem).length +
          1 + 1 =
        (BuilderFourthClauseSecondLiteralPrefix.fourthCursorWord problem).length +
          2 by omega]
...
def appenderWorkSteps {language : Language}
  ...
  BuilderTokenAppender.workSteps problem.input
    (BuilderFourthClauseSecondLiteralPrefix.fourthClauseSecondLiteralTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderFourthClauseSecondLiteralPrefix.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (fourthClauseTokens problem)

def appenderFinalConfiguration {language : Language}
    (problem : VerifierTableauProblem language) : WorkConfiguration :=
  { state := FinishTokenCursor.appender.acceptState
...
theorem appender_workRunExact {language : Language}
    (problem : VerifierTableauProblem language) :
    workRunExact? FinishTokenCursor.appender (appenderWorkSteps problem)
        (workStartConfiguration FinishTokenCursor.appender
          (BuilderFourthClauseSecondLiteralPrefix.finalTape problem)) =
      some (appenderFinalConfiguration problem) := by
  have hRunEq : ∀ steps config,
      workRunExact? FinishTokenCursor.appender steps config =
    ...
    | succ steps ih =>
        ...
        change
          (match workStep? BuilderTokenAppender.machine config with
           | none => none
           | some next => workRunExact? FinishTokenCursor.appender steps next) =
          (match workStep? BuilderTokenAppender.machine config with
           | none => none
  ...
  have hExact := BuilderTokenAppender.appendToken_workRunExact
    problem.input (BuilderFourthClauseSecondLiteralPrefix.finalOutside problem)
    (BuilderFourthClauseSecondLiteralPrefix.fourthClauseSecondLiteralTokens problem) .finish
  rw [hRunEq]
  simpa [FinishTokenCursor.appender, appenderWorkSteps,
    BuilderFourthClauseSecondLiteralPrefix.finalTape,
    BuilderTokenAppender.entryConfiguration,
    BuilderTokenAppender.finalConfiguration,
    appenderFinalConfiguration, fourthClauseTokens,
  ...
  have hWord : ∀ symbol ∈ cursorWord problem,
    ...
    have hBase :
        symbol ∈ BuilderFourthClauseSeparatorStep.cursorWord problem ∨
          symbol = BuilderUnaryPolynomial.unitSymbol := by
      simpa [cursorWord,
        BuilderFourthClauseSecondLiteralPrefix.fourthCursorWord,
        BuilderFourthClauseSecondLiteralPrefix.thirdCursorWord,
        BuilderFourthClauseSecondLiteralPrefix.secondCursorWord,
        BuilderFourthClauseSecondLiteralPrefix.firstCursorWord,
        BuilderFourthClauseFirstLiteralPrefix.thirdCursorWord,
        BuilderFourthClauseFirstLiteralPrefix.secondCursorWord,
        BuilderFourthClauseFirstLiteralPrefix.firstCursorWord] using hMem
    rcases hBase with hBase | hUnit
...
theorem finishTokenCursor_launch_workStep {language : Language}
  ...
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    FinishTokenCursor.appender
    BuilderDynamicTokenCursorStep.CursorAdvance.machine
    (appenderFinalConfiguration problem).tape
  simpa [FinishTokenCursor.machine, appenderFinalConfiguration,
    workStartConfiguration] using hLaunch

theorem suffix_workRunExact {language : Language}
    (problem : VerifierTableauProblem language) :
    workRunExact? FinishTokenCursor.machine (suffixWorkSteps problem)
        (workStartConfiguration FinishTokenCursor.machine
          (BuilderFourthClauseSecondLiteralPrefix.finalTape problem)) =
      some
        { state := FinishTokenCursor.machine.acceptState
          tape := finalTape problem } := by
  let appenderInitial := workStartConfiguration FinishTokenCursor.appender
    (BuilderFourthClauseSecondLiteralPrefix.finalTape problem)
  let appenderFinal := appenderFinalConfiguration problem
  let cursorFinal := cursorFinalConfiguration problem
  have hAppender : workRunExact? FinishTokenCursor.appender
  ...
  simpa [FinishTokenCursor.machine, suffixWorkSteps, appenderInitial,
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderFourthClauseSecondLiteralPrefix.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderFourthClauseSecondLiteralPrefix.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderFourthClauseSecondLiteralPrefix.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderFourthClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine)
    (BuilderFourthClauseSecondLiteralPrefix.workSteps problem)
    (workStartConfiguration (BuilderFourthClauseSecondLiteralPrefix.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderFourthClauseSecondLiteralPrefix.finalConfiguration problem)
    (BuilderFourthClauseSecondLiteralPrefix.workRunExact problem)
  simpa [machine, BuilderFirstClausePrefix.WorkChain.machine,
    workStartConfiguration, renameConfiguration] using hTransport

theorem prefixFinish_launch_workStep {language : Language}
    (problem : VerifierTableauProblem language) :
    workStep? (machine problem)
      ...
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.secondState
        (workStartConfiguration FinishTokenCursor.machine
          (BuilderFourthClauseSecondLiteralPrefix.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderFourthClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine
    (BuilderFourthClauseSecondLiteralPrefix.finalTape problem)
  simpa [machine, BuilderFourthClauseSecondLiteralPrefix.finalConfiguration,
    workStartConfiguration] using hLaunch

set_option maxRecDepth 1000000 in
/-- Every raw input follows the complete fourth-clause second-literal trace,
appends the clause terminator, and advances to the first padding coordinate. -/
theorem workRunExact {language : Language}
  ...
  let prefixInitial := workStartConfiguration
    (BuilderFourthClauseSecondLiteralPrefix.machine problem)
    (rawInputWorkTape problem.input)
  let prefixFinal := BuilderFourthClauseSecondLiteralPrefix.finalConfiguration problem
  ...
  have hPrefix : workRunExact?
    ...
    simpa [prefixInitial, prefixFinal] using
      BuilderFourthClauseSecondLiteralPrefix.workRunExact problem
  have hSuffix : workRunExact? FinishTokenCursor.machine
      (suffixWorkSteps problem)
      { state := FinishTokenCursor.machine.startState
        tape := prefixFinal.tape } = some suffixFinal := by
    simpa [prefixFinal, suffixFinal,
      BuilderFourthClauseSecondLiteralPrefix.finalConfiguration,
...
/-- The populated fourth-clause terminator executed by the suffix agrees with
...
theorem specification_terminator_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderFourthClauseSecondLiteralPrefix.finalTokenSlot problem⟩ =
      some (some CNFToken.finish,
        ⟨BuilderFourthClauseSecondLiteralPrefix.finalTokenSlot problem + 1⟩) :=
  BuilderFourthClauseSecondLiteralPrefix.specification_next_step problem
...
private theorem formulaClauseSchedule_starts_four_shape_clauses
    {language : Language} (problem : VerifierTableauProblem language) :
    let time : Fin problem.dimensions.timeCount :=
      ⟨0, problem.dimensions.timeCount_positive⟩
    let position : Fin
...
private theorem formulaClauseTokens_first_three_rectangles_then_completeFourthClause
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
      problem.formulaClauseSchedule.flatMap problem.scheduledClauseTokens =
        ...
        [some CNFToken.sep,
         ...
         some CNFToken.f, some CNFToken.t, some CNFToken.t,
         some CNFToken.f, some CNFToken.finish] ++
        List.replicate (problem.formulaTokensPerClause - 9) none ++ rest := by
  rcases formulaClauseSchedule_starts_four_shape_clauses problem with
    ⟨left, right, thirdLeft, thirdRight, fourthLeft, fourthRight,
...
complete fourth clause. -/
theorem fourthClauseTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      fourthClauseTokens problem ++ rest := by
  rcases formulaClauseTokens_first_three_rectangles_then_completeFourthClause
      problem with
...
through the complete fourth clause. -/
theorem finalTokenBits_eq_encodedFormula_fourthClause
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (fourthClauseTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 36)) := by
  rcases fourthClauseTokens_eq_canonical_formula_prefix problem with
    ⟨rest, hTokens⟩
  have hLength :
      (encodeTokenPairs (fourthClauseTokens problem)).length =
        2 * (problem.FormulaWidth + 36) := by
    rw [encodeTokenPairs_length]
    simp [fourthClauseTokens,
      BuilderFourthClauseSecondLiteralPrefix.fourthClauseSecondLiteralTokens,
      BuilderFourthClauseSecondLiteralPrefix.secondUnaryTokenOutput,
      BuilderFourthClauseSecondLiteralPrefix.firstUnaryTokenOutput,
      BuilderFourthClauseSecondLiteralPrefix.signTokenOutput,
  ...
  have hShape : problem.encodedFormula =
      encodeTokenPairs (fourthClauseTokens problem) ++ suffix := by
...
/-- Retained token coordinate immediately after the fourth clause. -/
def finalTokenSlot {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderThirdClausePaddingRun.fourthClauseStart problem + 9

theorem finalTokenSlot_eq_fourthClauseStart_add_nine
    {language : Language} (problem : VerifierTableauProblem language) :
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
        3 * problem.formulaTokensPerClause + 9 := by
  rw [finalTokenSlot,
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  rw [hPrefix]
  rw [show BuilderThirdClausePaddingRun.fourthClauseStart problem + 9 =
...
/-- The populated coordinate consumed by this milestone is the fourth
clause terminator. -/
theorem clauseTerminatorSlot_direct_eq_finish {language : Language}
    (problem : VerifierTableauProblem language) :
    problem.formulaTokenSlotDirect
        (BuilderFourthClauseSecondLiteralPrefix.finalTokenSlot problem) =
      some (some CNFToken.finish) :=
  BuilderFourthClauseSecondLiteralPrefix.nextTokenSlot_direct_eq_finish problem

/-- The retained coordinate is the first in-range padding opportunity after
the complete fourth clause. -/
theorem nextTokenSlot_direct_eq_padding {language : Language}
  ...
  rw [show problem.formulaVariableSlotBound + 1 +
      3 * problem.formulaTokensPerClause + 9 -
        (problem.formulaVariableSlotBound + 1) =
      3 * problem.formulaTokensPerClause + 9 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 3 * problem.formulaTokensPerClause + 9 - 11 =
      (problem.formulaTokensPerClause - 11) +
        2 * problem.formulaTokensPerClause + 9 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 11) +
      2 * problem.formulaTokensPerClause + 9 -
      (problem.formulaTokensPerClause - 11) =
        2 * problem.formulaTokensPerClause + 9 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause + 9 - 7 =
      (problem.formulaTokensPerClause - 7) +
        problem.formulaTokensPerClause + 9 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + 9 - 8 =
      (problem.formulaTokensPerClause - 8) + 9 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 8) + 9 -
      (problem.formulaTokensPerClause - 8) = 9 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 9 - 9 = 0 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_pos (by omega)]
...
theorem specification_next_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
      ...
      some (none, ⟨finalTokenSlot problem + 1⟩) := by
  unfold VerifierTableauProblem.FormulaTokenCursor.step
  rw [nextTokenSlot_direct_eq_padding]
...
terminator append, and the complete bidirectional cursor scan. -/
def rawTimeBound {language : Language}
    (verifier : PolynomialTimeVerifier language) : NatPolynomial :=
  .add (BuilderFourthClauseSecondLiteralPrefix.rawTimeBound verifier)
    (.add (.constant 618)
...
private theorem predecessorTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderFourthClauseSecondLiteralPrefix.fourthClauseSecondLiteralTokens
      problem).length = problem.FormulaWidth + 35 := by
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderFourthClauseSecondLiteralPrefix.rawTimeBound problem.verifier).eval
          problem.input.length + 618 +
        24 * problem.input.length + 12 * problem.FormulaWidth +
        12 * (BuilderFourthClauseSeparatorStep.cursorWord problem).length := by
  rw [rawTimeBound]
...
private theorem appenderWorkSteps_le {language : Language}
    ...
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 78 := by
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix := BuilderFourthClauseSecondLiteralPrefix.rawTimeBound_le problem
  have hAppender := appenderWorkSteps_le problem
  have hCursorLength :
      (cursorWord problem).length =
        (BuilderFourthClauseSeparatorStep.cursorWord problem).length + 8 := by
    simp [cursorWord,
      BuilderFourthClauseSecondLiteralPrefix.fourthCursorWord,
      BuilderFourthClauseSecondLiteralPrefix.thirdCursorWord,
      BuilderFourthClauseSecondLiteralPrefix.secondCursorWord,
...
private theorem machine_isHalted_predecessor_false {language : Language}
  ...
  have hHalted :=
    BuilderFirstClausePrefix.WorkChain.machine_isHalted_first_false
      (BuilderFourthClauseSecondLiteralPrefix.machine problem)
      FinishTokenCursor.machine config
  simpa [machine] using hHalted

/-- The complete fourth-clause second-literal endpoint is still globally nonhalting
until the outer bridge launches the selected terminator appender. -/
...
private theorem appenderEndpoint_workRunExact {language : Language}
  ...
  have hPrefix : workRunExact? (machine problem)
      (BuilderFourthClauseSecondLiteralPrefix.workSteps problem) initial =
        some prefixFinal := by
  ...
  have hLaunch : workRunExact? (machine problem) 1 prefixFinal =
    ...
    simpa [prefixFinal, suffixInitial] using
      prefixFinish_launch_workStep problem
  have hInner := PipelineStageBridges.workRunExact?_transport
    FinishTokenCursor.appender FinishTokenCursor.machine
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      FinishTokenCursor.appender
      BuilderDynamicTokenCursorStep.CursorAdvance.machine)
  ...
  have hGlobal := PipelineStageBridges.workRunExact?_transport
    FinishTokenCursor.machine (machine problem)
    BuilderFirstClausePrefix.WorkChain.secondState
...
private theorem machine_isHalted_appender_false {language : Language}
  ...
  have hGlobal :=
    BuilderFirstClausePrefix.WorkChain.machine_isHalted_second_false_of_local
      (BuilderFourthClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine
      (renameConfiguration BuilderFirstClausePrefix.WorkChain.firstState
        config) (by simpa [FinishTokenCursor.machine] using hInner)
  simpa [machine] using hGlobal

/-- The selected terminator appender endpoint remains a timeout until the
...
private theorem selectedAppender_stuck_timeout {language : Language}
  ...
  have hSelectedHalted : FinishTokenCursor.appender.isHalted bad = false := by
    change BuilderTokenAppender.machine.isHalted bad = false
    exact hLocalHalted
  have hSelectedStep : workStep? FinishTokenCursor.appender bad = none := by
    change workStep? BuilderTokenAppender.machine bad = none
    exact hLocalStep
  have hAccept := state_ne_accept_of_not_halted
    FinishTokenCursor.appender bad hSelectedHalted
  have hInnerStep : workStep? FinishTokenCursor.machine innerConfig = none := by
    have hStep := first_workStep_none_of_local FinishTokenCursor.appender
        BuilderDynamicTokenCursorStep.CursorAdvance.machine bad hAccept
        hSelectedHalted hSelectedStep
    simpa [FinishTokenCursor.machine, innerConfig] using hStep
  have hInnerHalted : FinishTokenCursor.machine.isHalted innerConfig = false := by
    have hHalted :=
      BuilderFirstClausePrefix.WorkChain.machine_isHalted_first_false
        FinishTokenCursor.appender
        BuilderDynamicTokenCursorStep.CursorAdvance.machine bad
    simpa [FinishTokenCursor.machine, innerConfig] using hHalted
...
/-- A malformed tally-phase symbol in the selected terminator appender is
...
/-- A malformed output-phase symbol in the selected terminator appender also
...
private theorem globalCursorMalformed_workStep {language : Language}
  ...
  have hGlobal := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    ...
    (by simpa [FinishTokenCursor.machine] using hInner)
...
private theorem globalCursorDead_workStep {language : Language}
  ...
  have hGlobal := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    ...
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState
      (cursorDeadConfiguration tape)) (by
        simpa [FinishTokenCursor.machine] using hInner)
  simpa [machine] using hGlobal
