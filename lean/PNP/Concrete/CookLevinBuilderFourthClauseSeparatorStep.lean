...
One populated token-cursor transition at the beginning of the fourth
canonical Cook--Levin clause.

The machine in this file composes the complete third-clause padding run with
the already-audited selected separator appender and unary cursor advance.
Every raw input therefore emits the separator beginning clause four and leaves
...
namespace BuilderFourthClauseSeparatorStep
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderThirdClausePaddingRun.machine problem) := by
  exact BuilderThirdClausePaddingRun.rule_source_ne_acceptState problem
...
separator transition at the beginning of clause four. -/
def machine {language : Language}
  ...
  BuilderFirstClausePrefix.WorkChain.machine
    (BuilderThirdClausePaddingRun.machine problem)
      ...
      3300 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderThirdClausePaddingRun.fourthClauseStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderThirdClausePaddingRun.rules_length problem
  have hSuffix :=
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderThirdClausePaddingRun.machine problem)
    BuilderSecondClauseSeparatorStep.SeparatorCursor.machine
    (BuilderThirdClausePaddingRun.rules_pairwise_query_distinct problem)
...
def fourthClauseStartTokens {language : Language}
    (problem : VerifierTableauProblem language) : List CNFToken :=
  BuilderThirdClausePrefix.thirdClauseTokens problem ++ [.sep]
...
def finalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input (finalOutside problem)
    (fourthClauseStartTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderThirdClausePaddingRun.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (fourthClauseStartTokens problem)
...
theorem appender_workRunExact {language : Language}
    ...
    workRunExact? BuilderSecondClauseSeparatorStep.SeparatorCursor.appender (appenderWorkSteps problem)
        (workStartConfiguration BuilderSecondClauseSeparatorStep.SeparatorCursor.appender
          (BuilderThirdClausePaddingRun.finalTape problem)) =
  ...
  have hExact := BuilderTokenAppender.appendToken_workRunExact
    problem.input (BuilderThirdClausePaddingRun.finalOutside problem)
    (BuilderThirdClausePrefix.thirdClauseTokens problem) .sep
  rw [hRunEq]
  simpa [BuilderSecondClauseSeparatorStep.SeparatorCursor.appender, appenderWorkSteps,
    ...
    appenderFinalConfiguration, fourthClauseStartTokens,
...
theorem cursor_workRunExact {language : Language}
  ...
  have hExact :=
    BuilderDynamicTokenCursorStep.CursorAdvance.advance_workRunExact
      ...
      (fourthClauseStartTokens problem) hWord
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderThirdClausePaddingRun.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderThirdClausePaddingRun.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderThirdClausePaddingRun.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderThirdClausePaddingRun.machine problem)
        BuilderSecondClauseSeparatorStep.SeparatorCursor.machine)
    (BuilderThirdClausePaddingRun.workSteps problem)
    (workStartConfiguration (BuilderThirdClausePaddingRun.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderThirdClausePaddingRun.finalConfiguration problem)
    (BuilderThirdClausePaddingRun.workRunExact problem)
...
theorem prefixSeparator_launch_workStep {language : Language}
    ...
    workStep? (machine problem)
      ...
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.secondState
        (workStartConfiguration BuilderSecondClauseSeparatorStep.SeparatorCursor.machine
          (BuilderThirdClausePaddingRun.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderThirdClausePaddingRun.machine problem)
    BuilderSecondClauseSeparatorStep.SeparatorCursor.machine
    (BuilderThirdClausePaddingRun.finalTape problem)
  simpa [machine, BuilderThirdClausePaddingRun.finalConfiguration,
    workStartConfiguration] using hLaunch

set_option maxRecDepth 1000000 in
/-- Every raw input follows the complete third-clause padding trace, appends
the clause-four separator, and advances the retained unary coordinate once. -/
theorem workRunExact {language : Language}
  ...
  let prefixInitial := workStartConfiguration
    (BuilderThirdClausePaddingRun.machine problem)
    (rawInputWorkTape problem.input)
  let prefixFinal := BuilderThirdClausePaddingRun.finalConfiguration problem
...
theorem specification_separator_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderThirdClausePaddingRun.fourthClauseStart problem⟩ =
      some (some CNFToken.sep,
        ⟨BuilderThirdClausePaddingRun.fourthClauseStart problem + 1⟩) :=
  BuilderThirdClausePaddingRun.specification_target_step problem
...
private theorem formulaClauseTokens_first_three_rectangles_then_fourthSepF
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
  ...
  let fourthTail : List (Option CNFToken) :=
    (encodeUnaryTokens fourthLeft.val ++
        CNFToken.f :: encodeUnaryTokens fourthRight.val ++
...
separator beginning the fourth clause. -/
theorem fourthClauseStartTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      fourthClauseStartTokens problem ++ rest := by
  rcases formulaClauseTokens_first_three_rectangles_then_fourthSepF problem with
  ...
  have hEmit :
      FormulaSchedule.emit
        ...
        [CNFToken.sep,
         ...
         CNFToken.finish, CNFToken.sep, CNFToken.f] ++
  ...
  unfold fourthClauseStartTokens
...
through the fourth-clause separator. -/
theorem finalTokenBits_eq_encodedFormula_fourthClauseStart
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (fourthClauseStartTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 28)) := by
  rcases fourthClauseStartTokens_eq_canonical_formula_prefix problem with
    ⟨rest, hTokens⟩
  have hLength :
      (encodeTokenPairs (fourthClauseStartTokens problem)).length =
        2 * (problem.FormulaWidth + 28) := by
    rw [encodeTokenPairs_length, fourthClauseStartTokens,
      List.length_append,
      BuilderThirdClausePrefix.thirdClauseTokens]
  ...
  have hShape : problem.encodedFormula =
      encodeTokenPairs (fourthClauseStartTokens problem) ++ suffix := by
...
theorem finalTokenSlot_eq_fourthClauseStart_add_one
    {language : Language} (problem : VerifierTableauProblem language) :
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
        3 * problem.formulaTokensPerClause + 1 := by
  rw [finalTokenSlot,
...
sign token beginning the first literal of clause four. -/
theorem nextTokenSlot_direct_eq_f {language : Language}
  ...
  rw [show problem.formulaVariableSlotBound + 1 +
      3 * problem.formulaTokensPerClause + 1 -
      (problem.formulaVariableSlotBound + 1) =
        3 * problem.formulaTokensPerClause + 1 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 3 * problem.formulaTokensPerClause + 1 - 11 =
      (problem.formulaTokensPerClause - 11) +
        2 * problem.formulaTokensPerClause + 1 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 11) +
      2 * problem.formulaTokensPerClause + 1 -
      (problem.formulaTokensPerClause - 11) =
        2 * problem.formulaTokensPerClause + 1 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause + 1 - 7 =
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + 1 - 8 =
      (problem.formulaTokensPerClause - 8) + 1 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 8) + 1 -
      (problem.formulaTokensPerClause - 8) = 1 by omega]
  simp
...
def rawTimeBound {language : Language}
  ...
  .add (BuilderThirdClausePaddingRun.rawTimeBound verifier)
    (.add (.constant 426)
...
private theorem thirdClauseTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderThirdClausePrefix.thirdClauseTokens problem).length =
      problem.FormulaWidth + 27 := by
  simp [BuilderThirdClausePrefix.thirdClauseTokens,
    BuilderThirdClauseSecondLiteralPrefix.thirdClauseSecondLiteralTokens,
    BuilderThirdClauseSecondLiteralPrefix.secondUnaryTokenOutput,
    BuilderThirdClauseSecondLiteralPrefix.firstUnaryTokenOutput,
    BuilderThirdClauseSecondLiteralPrefix.signTokenOutput,
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderThirdClausePaddingRun.rawTimeBound problem.verifier).eval
          problem.input.length + 426 +
...
private theorem appenderWorkSteps_le {language : Language}
    ...
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 62 := by
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix := BuilderThirdClausePaddingRun.rawTimeBound_le problem
...
private theorem appenderEndpoint_workRunExact {language : Language}
  ...
  have hPrefix : workRunExact? (machine problem)
      (BuilderThirdClausePaddingRun.workSteps problem) initial =
        some prefixFinal := by
  ...
  have hGlobal := PipelineStageBridges.workRunExact?_transport
    ...
    (BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
      (BuilderThirdClausePaddingRun.machine problem) BuilderSecondClauseSeparatorStep.SeparatorCursor.machine)
    (appenderWorkSteps problem)
    (workStartConfiguration BuilderSecondClauseSeparatorStep.SeparatorCursor.machine
      (BuilderThirdClausePaddingRun.finalTape problem))
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.firstState
  ...
  have h01 := PipelineMachineSimulation.workRunExact?_compose
    (machine problem) (BuilderThirdClausePaddingRun.workSteps problem) 1
...
end BuilderFourthClauseSeparatorStep
