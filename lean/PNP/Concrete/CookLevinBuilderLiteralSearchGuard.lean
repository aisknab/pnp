...
A restoring count guard for the runtime literal-list search. One fixed graph
copies the actual remaining count from the loop frame, checks zero, and erases
only that temporary copy on both branches. The original frame and tape interior
are preserved; zero and positive counts have distinct terminal states.

The complete list locator must derive its count from the actual input list.
-/

import PNP.Concrete.CookLevinBuilderLiteralSearchSelect

namespace PNP.Concrete.CookLevin.BuilderLiteralSearchGuard
...
def testNode : Node :=
  {name := 1, program := BuilderUnaryTagMatch.machine 0,
   onAccept := .node eraseZeroNode.reference, onReject := .node erasePositiveNode.reference}
def copyNode : Node :=
  {name := 0, program := RegisterCopy.machine 1, onAccept := .node testNode.reference, onReject := .dead}
def graph : Graph := {nodes := [copyNode, testNode, eraseZeroNode, erasePositiveNode], entry := copyNode.reference}
...
private theorem eraseZero_mem : eraseZeroNode ∈ graph.nodes := List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _))
private theorem erasePositive_mem : erasePositiveNode ∈ graph.nodes := List.Mem.tail _ (List.Mem.tail _ (List.Mem.tail _ (List.Mem.head _)))
...
theorem graph_wellFormed : graph.WellFormed := by
  ...
  · intro node hMem
    ...
    · exact ⟨⟨eraseZeroNode, eraseZero_mem, rfl, rfl⟩, ⟨erasePositiveNode, erasePositive_mem, rfl, rfl⟩⟩
...
def workSteps (count position : Nat) : Nat :=
  RegisterCopy.steps [position] count + 1 +
    (BuilderUnaryTagMatch.workSteps 0 count + 1 + (count + 2 + 1))
def endpoint (count : Nat) : Endpoint := if count = 0 then .accept else .reject
...
private theorem execution_path (older : List Nat) (ordinal count position : Nat)
  ...
  have hEraseZero : LocalAcceptRun eraseZeroNode (count + 2) copied final := hErase
  have hErasePositive : LocalAcceptRun erasePositiveNode (count + 2) copied final := hErase
  by_cases hZero : count = 0
  · have hTag := BuilderUnaryTagMatch.accept_workRunExact 0 values inside (outside.drop (count + 1))
    have hTest : LocalAcceptRun testNode (BuilderUnaryTagMatch.workSteps 0 count) copied copied := by
      simpa only [LocalAcceptRun, testNode, workStartConfiguration, copied, hZero] using hTag
    have hEnd := AcceptPath.step eraseZeroNode .accept (count + 2) 0 copied final final
      eraseZero_mem hEraseZero (.terminal .accept final)
    have hMiddle := AcceptPath.step testNode .accept (BuilderUnaryTagMatch.workSteps 0 count)
    ...
    have h := AcceptPath.step copyNode .accept (RegisterCopy.steps [position] count)
      (BuilderUnaryTagMatch.workSteps 0 count + 1 + (count + 2 + 1 + 0))
      _ copied final copy_mem hCopyLocal hMiddle
    simpa only [endpoint, if_pos hZero, workSteps, Nat.add_zero] using h
  · have hTag := BuilderUnaryTagMatch.reject_workRunExact 0 count values inside (outside.drop (count + 1)) hZero
    have hTest : LocalRejectRun testNode (BuilderUnaryTagMatch.workSteps 0 count) copied copied := hTag
    have hEnd := AcceptPath.step erasePositiveNode .reject (count + 2) 0 copied final final
      erasePositive_mem hErasePositive (.terminal .reject final)
    have hMiddle := AcceptPath.stepReject testNode .reject (BuilderUnaryTagMatch.workSteps 0 count)
      (count + 2 + 1 + 0) copied copied final test_mem hTest hEnd
    have h := AcceptPath.step copyNode .reject (RegisterCopy.steps [position] count)
      (BuilderUnaryTagMatch.workSteps 0 count + 1 + (count + 2 + 1 + 0))
      _ copied final copy_mem hCopyLocal hMiddle
    simpa only [endpoint, if_neg hZero, workSteps, Nat.add_zero] using h
...
theorem final_accept_iff (older : List Nat) (ordinal count position : Nat)
    (inside outside : List WorkSymbol) :
    (finalConfiguration older ordinal count position inside outside).state = machine.acceptState ↔ count = 0 := by
  by_cases hZero : count = 0
  · simp only [finalConfiguration, endpoint, if_pos hZero, endpointConfiguration, WorkMachineProgramGraph.endpointState, hZero] <;> decide
  · simp only [finalConfiguration, endpoint, if_neg hZero, endpointConfiguration, WorkMachineProgramGraph.endpointState, hZero] <;> decide

theorem final_reject_iff (older : List Nat) (ordinal count position : Nat)
    (inside outside : List WorkSymbol) :
    (finalConfiguration older ordinal count position inside outside).state = machine.rejectState ↔ 0 < count := by
  by_cases hZero : count = 0
  · subst count
    change (0 : Nat) = 1 ↔ 0 < 0
    decide
  · have hPositive : 0 < count := by omega
    simp only [finalConfiguration, endpoint, if_neg hZero, endpointConfiguration, WorkMachineProgramGraph.endpointState, hPositive] <;> decide
...
theorem workSteps_eq (count position : Nat) :
    workSteps count position = RegisterCopy.steps [position] count + count + 8 := by
  simp only [workSteps, BuilderUnaryTagMatch.workSteps, Nat.zero_min, Nat.mul_zero, Nat.zero_add]
  omega
...
def workBound (bound : Nat) : Nat := BuilderLiteralSearchAdvance.copyBound bound + bound + 8
def spanBound (bound : Nat) : Nat := 2 * bound + 1
def rawTimePolynomial (bound : NatPolynomial) : NatPolynomial :=
  .mul (.constant 6) (.add (.add (BuilderLiteralSearchAdvance.copyBoundPolynomial bound) bound) (.constant 8))
...
end PNP.Concrete.CookLevin.BuilderLiteralSearchGuard
