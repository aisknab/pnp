...
Input-dependent traversal of the complete padding rectangle after the second
canonical Cook--Levin clause.

The predecessor retains the first padding opportunity at the second-clause
start plus seven. This module evaluates the exact remaining padding count
`formulaTokensPerClause - 7`, consumes that unary counter with one literal
loop, and then materializes the first coordinate of the third clause. No
...
namespace BuilderSecondClausePaddingRun

open PipelineTape PipelineStateNamespace PipelineStageBridges

/-! ### Exact external coordinates -/

/-- Number of padding opportunities from the retained second-clause endpoint
...
theorem remainingPaddingCount_eq_formulaTokensPerClause_sub_seven
    ...
    remainingPaddingCount problem = problem.formulaTokensPerClause - 7 := by
...
/-- Absolute token coordinate of the first opportunity in the third clause
rectangle. -/
def thirdClauseStartPolynomial {language : Language}
    (verifier : PolynomialTimeVerifier language) : NatPolynomial :=
  .add (BuilderFirstClausePaddingRun.secondClauseStartPolynomial verifier)
    (formulaClauseTokenPolynomial verifier)

def thirdClauseStart {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  (thirdClauseStartPolynomial problem.verifier).eval problem.input.length

theorem thirdClauseStart_eq {language : Language}
    (problem : VerifierTableauProblem language) :
    thirdClauseStart problem =
      problem.formulaVariableSlotBound + 1 +
        2 * problem.formulaTokensPerClause := by
  unfold thirdClauseStart thirdClauseStartPolynomial
  simp only [NatPolynomial.eval_add]
  change BuilderFirstClausePaddingRun.secondClauseStart problem +
      (formulaClauseTokenPolynomial problem.verifier).eval
        problem.input.length = _
  rw [BuilderFirstClausePaddingRun.secondClauseStart_eq]
  have hClause :
      (formulaClauseTokenPolynomial problem.verifier).eval
...
theorem predecessorSlot_add_remainingPaddingCount
    {language : Language} (problem : VerifierTableauProblem language) :
    BuilderSecondClausePrefix.finalTokenSlot problem +
        remainingPaddingCount problem = thirdClauseStart problem := by
  rw [BuilderSecondClausePrefix.finalTokenSlot_eq_secondClauseStart_add_seven,
    remainingPaddingCount_eq_formulaTokensPerClause_sub_seven,
    thirdClauseStart_eq]
  have hWidth : 7 ≤ problem.formulaTokensPerClause := by
    have hBound :=
...
def targetEvaluator {language : Language}
    (problem : VerifierTableauProblem language) : WorkMachine :=
  BuilderUnaryPolynomial.machine
    (thirdClauseStartPolynomial problem.verifier)
...
/-- One literal finite rule table from raw input through the complete second
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondClausePrefix.machine problem) := by
  exact BuilderSecondClausePrefix.rule_source_ne_acceptState problem
...
private theorem targetEvaluator_noRuleAtAccept {language : Language}
  ...
  intro rule hRule
  exact Nat.ne_of_lt (BuilderUnaryPolynomial.rule_source_lt_acceptState
    (thirdClauseStartPolynomial problem.verifier) rule hRule)
...
theorem rules_length {language : Language}
    ...
    (machine problem).rules.length =
      2150 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (remainingPaddingPolynomial problem.verifier) +
        BuilderUnaryPolynomial.ruleCount
          (thirdClauseStartPolynomial problem.verifier) := by
  have hPredecessor := BuilderSecondClausePrefix.rules_length problem
  ...
  have hTargetMachine :
      (BuilderUnaryPolynomial.machine
        (thirdClauseStartPolynomial problem.verifier)).rules.length =
      BuilderUnaryPolynomial.ruleCount
        (thirdClauseStartPolynomial problem.verifier) := by
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondClausePrefix.machine problem)
    (paddingSuffixMachine problem)
    (BuilderSecondClausePrefix.rules_pairwise_query_distinct problem)
...
def countOutside {language : Language}
  ...
  BuilderUnaryPolynomial.finalOutsideLeft
    ...
    (BuilderSecondClausePrefix.finalOutside problem)
...
def countTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input (countOutside problem)
    (BuilderSecondClausePrefix.secondClauseTokens problem)
...
def countdownFinalOutside {language : Language}
  ...
  (countWord problem).take (countRootPrefixLength problem) ++
    ...
    (BuilderSecondClausePrefix.finalOutside problem).drop
...
def finalOutside {language : Language}
    (problem : VerifierTableauProblem language) : List WorkSymbol :=
  BuilderUnaryPolynomial.finalOutsideLeft
    (thirdClauseStartPolynomial problem.verifier) problem.input
    (countdownFinalOutside problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondClausePrefix.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (BuilderSecondClausePrefix.secondClauseTokens problem)
    ...
    workRunExact? (countEvaluator problem)
      ...
      some
        (BuilderUnaryPolynomial.finalConfiguration
          (remainingPaddingPolynomial problem.verifier) problem.input
          (BuilderSecondClausePrefix.finalOutside problem)
          (BuilderSecondClausePrefix.secondClauseTokens problem)) := by
...
theorem countdown_workRunExact {language : Language}
  ...
  let outside := BuilderSecondClausePrefix.finalOutside problem
  ...
  | succ remaining =>
      ...
      have hLoop := BuilderFirstClausePaddingRun.PaddingCountdown.loop_workRunExact problem.input
        ...
        (BuilderSecondClausePrefix.secondClauseTokens problem) hPrefixSymbols
...
private theorem paddingSuffix_workRunExact {language : Language}
    ...
    workRunExact? (paddingSuffixMachine problem) (suffixWorkSteps problem)
        (workStartConfiguration (paddingSuffixMachine problem)
          (BuilderSecondClausePrefix.finalTape problem)) =
      some
  ...
  simpa [paddingSuffixMachine, countdownTargetMachine, targetEvaluator,
    ...
    BuilderSecondClausePrefix.finalOutside,
    BuilderUnaryPolynomial.initialConfiguration,
    BuilderFirstClausePrefix.WorkChain.machine, workStartConfiguration,
    renameConfiguration, Nat.add_assoc] using hCombined
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondClausePrefix.workSteps problem)
      ...
      some (renameConfiguration
        ...
        (BuilderSecondClausePrefix.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderSecondClausePrefix.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderSecondClausePrefix.machine problem)
      (paddingSuffixMachine problem))
    (BuilderSecondClausePrefix.workSteps problem)
    (workStartConfiguration (BuilderSecondClausePrefix.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderSecondClausePrefix.finalConfiguration problem)
    (BuilderSecondClausePrefix.workRunExact problem)
...
theorem launch_workStep {language : Language}
    ...
    workStep? (machine problem)
      ...
      some (renameConfiguration
        ...
        (workStartConfiguration (paddingSuffixMachine problem)
          (BuilderSecondClausePrefix.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderSecondClausePrefix.machine problem)
    (paddingSuffixMachine problem)
    (BuilderSecondClausePrefix.finalTape problem)
  simpa [machine, BuilderSecondClausePrefix.finalConfiguration,
...
theorem workRunExact {language : Language}
  ...
  let predecessorInitial := workStartConfiguration
    (BuilderSecondClausePrefix.machine problem)
    (rawInputWorkTape problem.input)
  let predecessorFinal := BuilderSecondClausePrefix.finalConfiguration
    problem
  let suffixFinal : WorkConfiguration :=
    { state := (paddingSuffixMachine problem).acceptState,
      tape := finalTape problem }
...
private theorem formulaClauseSchedule_starts_threeShapeClauses
    {language : Language} (problem : VerifierTableauProblem language) :
    let time : Fin problem.dimensions.timeCount :=
      ⟨0, problem.dimensions.timeCount_positive⟩
    let position : Fin
        (problem.dimensions.tapeWidth problem.tableauInputMode) :=
      ⟨0, problem.dimensions.tapeWidth_positive problem.tableauInputMode⟩
    ∃ left right : Fin problem.FormulaWidth, ∃ third rest,
...
private theorem formulaClauseTokens_first_two_rectangles_then_thirdSep
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
  ...
  rcases formulaClauseSchedule_starts_threeShapeClauses problem with
    ⟨left, right, third, clauses, hClauses, hLeft, hRight⟩
  rw [hClauses]
  simp only [List.flatMap_cons]
  ...
  let thirdTail : List (Option CNFToken) :=
    (encodeLiteralListTokens third.emit ++ [CNFToken.finish]).map some ++
      List.replicate (problem.formulaTokensPerClause -
        (CNFToken.sep ::
          (encodeLiteralListTokens third.emit ++ [CNFToken.finish])).length)
...
opportunity in the second fixed-width clause rectangle. -/
theorem paddingSlot_direct_eq_padding {language : Language}
    ...
    problem.formulaTokenSlotDirect
        (BuilderSecondClausePrefix.finalTokenSlot problem + offset) =
      some none := by
  rw [problem.formulaTokenSlotDirect_eq]
  rcases formulaClauseTokens_first_two_rectangles_then_thirdSep problem with
    ⟨rest, hClauses⟩
  unfold VerifierTableauProblem.formulaTokenSchedule
  rw [hClauses,
    BuilderSecondClausePrefix.finalTokenSlot_eq_secondClauseStart_add_seven]
  have hHeader :
  ...
  have hCount :=
    remainingPaddingCount_eq_formulaTokensPerClause_sub_seven problem
  ...
  rw [show problem.formulaVariableSlotBound + 1 +
      problem.formulaTokensPerClause + 7 + offset -
      (problem.formulaVariableSlotBound + 1) =
  ...
  rw [show problem.formulaTokensPerClause + 7 + offset -
      11 = (problem.formulaTokensPerClause - 11) + 7 + offset by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 11) + 7 + offset -
      (problem.formulaTokensPerClause - 11) = 7 + offset by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 7 + offset - 7 = offset by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_pos (by omega)]
  rw [List.getElem?_replicate, if_pos (by omega)]

/-- The exact target materialized after the countdown is the populated
separator opportunity beginning the third clause rectangle. -/
theorem thirdClauseStart_direct_eq_sep {language : Language}
    (problem : VerifierTableauProblem language) :
    problem.formulaTokenSlotDirect (thirdClauseStart problem) =
      some (some CNFToken.sep) := by
  rw [problem.formulaTokenSlotDirect_eq]
  rcases formulaClauseTokens_first_two_rectangles_then_thirdSep problem with
    ⟨rest, hClauses⟩
  unfold VerifierTableauProblem.formulaTokenSchedule
  rw [hClauses, thirdClauseStart_eq]
  have hHeader :
      (FormulaSchedule.pad (problem.formulaVariableSlotBound + 1)
        (encodeUnaryTokens problem.FormulaWidth)).length =
          problem.formulaVariableSlotBound + 1 := by
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause -
  ...
  rw [show problem.formulaTokensPerClause - 7 =
      problem.formulaTokensPerClause - 7 by rfl]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause - 7 -
      (problem.formulaTokensPerClause - 7) = 0 by omega]
  rfl
...
private theorem specificationRun_padding_from_offset
  ...
  | succ count ih =>
      ...
      have hStep : VerifierTableauProblem.FormulaTokenCursor.step problem
          ⟨BuilderSecondClausePrefix.finalTokenSlot problem + offset⟩ =
        some (none,
          ⟨BuilderSecondClausePrefix.finalTokenSlot problem + offset + 1⟩) := by
      ...
      rw [show BuilderSecondClausePrefix.finalTokenSlot problem +
          offset + 1 =
        BuilderSecondClausePrefix.finalTokenSlot problem +
          (offset + 1) by omega]
      simp only
...
entire remaining second-clause padding block and stops exactly at clause three. -/
theorem specification_padding_run {language : Language}
    (problem : VerifierTableauProblem language) :
    specificationRun problem (remainingPaddingCount problem)
        ⟨BuilderSecondClausePrefix.finalTokenSlot problem⟩ =
      some ([], ⟨thirdClauseStart problem⟩) := by
...
not yet emit, the separator beginning the third clause. -/
theorem specification_target_step {language : Language}
    (problem : VerifierTableauProblem language) :
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨thirdClauseStart problem⟩ =
      some (some CNFToken.sep, ⟨thirdClauseStart problem + 1⟩) := by
  unfold VerifierTableauProblem.FormulaTokenCursor.step
  rw [thirdClauseStart_direct_eq_sep]

theorem finalTokenBits_eq_encodedFormula_secondClause
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (BuilderSecondClausePrefix.secondClauseTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 19)) :=
  BuilderSecondClausePrefix.finalTokenBits_eq_encodedFormula_secondClause
    problem
...
theorem finalTokenSlot_eq_thirdClauseStart {language : Language}
    (problem : VerifierTableauProblem language) :
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
      2 * problem.formulaTokensPerClause := by
  exact thirdClauseStart_eq problem
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  rcases BuilderUnaryPolynomial.root_register_length
      (thirdClauseStartPolynomial problem.verifier) problem.input.length with
  ...
  unfold finalOutside BuilderUnaryPolynomial.finalOutsideLeft
    BuilderUnaryPolynomial.overlayScratch finalTokenSlot thirdClauseStart
  rw [hScratch]
...
def rawTimeBound {language : Language}
    (verifier : PolynomialTimeVerifier language) : NatPolynomial :=
  let count := remainingPaddingPolynomial verifier
  let target := thirdClauseStartPolynomial verifier
  .add (BuilderSecondClausePrefix.rawTimeBound verifier)
    (.add (.constant 18)
      (.add
        (scalePolynomial 6
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderSecondClausePrefix.rawTimeBound problem.verifier).eval
        ...
        6 * BuilderUnaryPolynomial.workSteps
          (thirdClauseStartPolynomial problem.verifier) problem.input := by
  rw [rawTimeBound]
  simp only [NatPolynomial.eval_add, NatPolynomial.eval_constant,
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPredecessor := BuilderSecondClausePrefix.rawTimeBound_le problem
...
end BuilderSecondClausePaddingRun
