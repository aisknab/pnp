...
Completion of the second canonical Cook--Levin clause.

The machine in this file composes the complete second negative literal with
one state-selected clause terminator appender and the existing unary cursor
advance. Every raw input therefore completes clause two and leaves the retained
...
namespace BuilderSecondClausePrefix
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondClauseSecondLiteralPrefix.machine problem) := by
  exact BuilderSecondClauseSecondLiteralPrefix.rule_source_ne_acceptState problem
...
def machine {language : Language}
  ...
  BuilderFirstClausePrefix.WorkChain.machine
    (BuilderSecondClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine
      ...
      2098 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderFirstClausePaddingRun.secondClauseStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderSecondClauseSecondLiteralPrefix.rules_length problem
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine
    (BuilderSecondClauseSecondLiteralPrefix.rules_pairwise_query_distinct problem)
...
def secondClauseTokens {language : Language}
...
private theorem predecessor_finalOutside_eq_cursor {language : Language}
  ...
  simp only [BuilderSecondClauseSecondLiteralPrefix.finalOutside,
    List.length_append, List.length_singleton]
  rw [show
      (BuilderSecondClauseSecondLiteralPrefix.thirdCursorWord problem).length +
          1 + 1 =
        (BuilderSecondClauseSecondLiteralPrefix.thirdCursorWord problem).length +
          2 by omega]
...
def appenderWorkSteps {language : Language}
  ...
  BuilderTokenAppender.workSteps problem.input
    (BuilderSecondClauseSecondLiteralPrefix.secondClauseSecondLiteralTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondClauseSecondLiteralPrefix.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (secondClauseTokens problem)
...
theorem appender_workRunExact {language : Language}
    ...
    workRunExact? FinishTokenCursor.appender (appenderWorkSteps problem)
        (workStartConfiguration FinishTokenCursor.appender
          (BuilderSecondClauseSecondLiteralPrefix.finalTape problem)) =
  ...
  have hExact := BuilderTokenAppender.appendToken_workRunExact
    problem.input (BuilderSecondClauseSecondLiteralPrefix.finalOutside problem)
    (BuilderSecondClauseSecondLiteralPrefix.secondClauseSecondLiteralTokens problem) .finish
  rw [hRunEq]
  simpa [FinishTokenCursor.appender, appenderWorkSteps,
    ...
    appenderFinalConfiguration, secondClauseTokens,
...
theorem cursor_workRunExact {language : Language}
  ...
  have hWord : ∀ symbol ∈ cursorWord problem,
    ...
    intro symbol hMem
    have hBase :
        symbol ∈ BuilderSecondClauseSeparatorStep.cursorWord problem ∨
          symbol = BuilderUnaryPolynomial.unitSymbol := by
      simpa [cursorWord,
        BuilderSecondClauseSecondLiteralPrefix.thirdCursorWord,
        BuilderSecondClauseSecondLiteralPrefix.secondCursorWord,
        BuilderSecondClauseSecondLiteralPrefix.firstCursorWord,
        BuilderSecondClauseFirstLiteralPrefix.secondCursorWord,
        BuilderSecondClauseFirstLiteralPrefix.firstCursorWord] using hMem
  ...
  have hExact :=
    BuilderDynamicTokenCursorStep.CursorAdvance.advance_workRunExact
      ...
      (secondClauseTokens problem) hWord
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondClauseSecondLiteralPrefix.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderSecondClauseSecondLiteralPrefix.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderSecondClauseSecondLiteralPrefix.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderSecondClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine)
    (BuilderSecondClauseSecondLiteralPrefix.workSteps problem)
    (workStartConfiguration (BuilderSecondClauseSecondLiteralPrefix.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderSecondClauseSecondLiteralPrefix.finalConfiguration problem)
    (BuilderSecondClauseSecondLiteralPrefix.workRunExact problem)
...
theorem prefixFinish_launch_workStep {language : Language}
    ...
    workStep? (machine problem)
      ...
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.secondState
        (workStartConfiguration FinishTokenCursor.machine
          (BuilderSecondClauseSecondLiteralPrefix.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderSecondClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine
    (BuilderSecondClauseSecondLiteralPrefix.finalTape problem)
  simpa [machine, BuilderSecondClauseSecondLiteralPrefix.finalConfiguration,
    workStartConfiguration] using hLaunch

set_option maxRecDepth 1000000 in
/-- Every raw input follows the complete second-literal trace, appends the
clause terminator, and advances to the first padding coordinate. -/
theorem workRunExact {language : Language}
  ...
  let prefixInitial := workStartConfiguration
    (BuilderSecondClauseSecondLiteralPrefix.machine problem)
    (rawInputWorkTape problem.input)
  let prefixFinal := BuilderSecondClauseSecondLiteralPrefix.finalConfiguration problem
...
theorem specification_terminator_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondClauseSecondLiteralPrefix.finalTokenSlot problem⟩ =
      some (some CNFToken.finish,
        ⟨BuilderSecondClauseSecondLiteralPrefix.finalTokenSlot problem + 1⟩) :=
  BuilderSecondClauseSecondLiteralPrefix.specification_next_step problem
...
private theorem formulaClauseTokens_firstRectangle_then_completeSecondClause
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
      problem.formulaClauseSchedule.flatMap problem.scheduledClauseTokens =
        ...
        [some CNFToken.sep,
         ...
         some CNFToken.finish, none] ++ rest := by
  ...
  have hPadding :
      problem.formulaTokensPerClause - 7 =
        (problem.formulaTokensPerClause - 8) + 1 := by
    omega
  ...
  simp [pairTail, BoundedClause.emit, excludeBoundedPairClause, falseLiteral,
    ...
    encodeUnaryTokens, hLeft, hRight, hPadding, List.replicate_succ,
    List.append_assoc]

/-- The finite output is exactly the canonical token prefix through the
complete second clause. -/
theorem secondClauseTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      secondClauseTokens problem ++ rest := by
  rcases formulaClauseTokens_firstRectangle_then_completeSecondClause problem with
...
through the complete second clause. -/
theorem finalTokenBits_eq_encodedFormula_secondClause
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (secondClauseTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 19)) := by
  rcases secondClauseTokens_eq_canonical_formula_prefix problem with
    ⟨rest, hTokens⟩
  have hLength :
      (encodeTokenPairs (secondClauseTokens problem)).length =
        2 * (problem.FormulaWidth + 19) := by
    rw [encodeTokenPairs_length]
    simp [secondClauseTokens,
      BuilderSecondClauseSecondLiteralPrefix.secondClauseSecondLiteralTokens,
      BuilderSecondClauseSecondLiteralPrefix.unaryTokenOutput,
      BuilderSecondClauseSecondLiteralPrefix.signTokenOutput,
      BuilderSecondClauseFirstLiteralPrefix.secondClauseFirstLiteralTokens,
  ...
  have hShape : problem.encodedFormula =
      encodeTokenPairs (secondClauseTokens problem) ++ suffix := by
...
/-- Retained token coordinate immediately after the second clause. -/
def finalTokenSlot {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderFirstClausePaddingRun.secondClauseStart problem + 7

theorem finalTokenSlot_eq_secondClauseStart_add_seven
    {language : Language} (problem : VerifierTableauProblem language) :
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  unfold finalOutside cursorWord finalTokenSlot
    ...
    BuilderSecondClauseSecondLiteralPrefix.firstCursorWord
    BuilderSecondClauseFirstLiteralPrefix.secondCursorWord
    BuilderSecondClauseFirstLiteralPrefix.firstCursorWord
    BuilderSecondClauseSeparatorStep.cursorWord
  rw [hPrefix]
  rw [show BuilderFirstClausePaddingRun.secondClauseStart problem + 7 =
      ((((((BuilderFirstClausePaddingRun.secondClauseStart problem + 1) + 1) +
        1) + 1) + 1) + 1) + 1 by omega]
...
/-- The populated coordinate consumed by this milestone is the second
clause terminator. -/
theorem clauseTerminatorSlot_direct_eq_finish {language : Language}
    ...
    problem.formulaTokenSlotDirect
        (BuilderSecondClauseSecondLiteralPrefix.finalTokenSlot problem) =
      some (some CNFToken.finish) :=
  BuilderSecondClauseSecondLiteralPrefix.nextTokenSlot_direct_eq_finish problem

/-- The retained coordinate is the first in-range padding opportunity after
the complete second clause. -/
theorem nextTokenSlot_direct_eq_padding {language : Language}
  ...
  rw [show problem.formulaVariableSlotBound + 1 +
      ...
      problem.formulaTokensPerClause + 7 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + 7 - 11 =
      (problem.formulaTokensPerClause - 11) + 7 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 11) + 7 -
      (problem.formulaTokensPerClause - 11) = 7 by omega]
  simp
...
def rawTimeBound {language : Language}
  ...
  .add (BuilderSecondClauseSecondLiteralPrefix.rawTimeBound verifier)
    (.add (.constant 390)
...
private theorem predecessorTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderSecondClauseSecondLiteralPrefix.secondClauseSecondLiteralTokens
      problem).length = problem.FormulaWidth + 18 := by
  simp [BuilderSecondClauseSecondLiteralPrefix.secondClauseSecondLiteralTokens,
    BuilderSecondClauseSecondLiteralPrefix.unaryTokenOutput,
    BuilderSecondClauseSecondLiteralPrefix.signTokenOutput,
    BuilderSecondClauseFirstLiteralPrefix.secondClauseFirstLiteralTokens,
    BuilderSecondClauseFirstLiteralPrefix.firstTokenOutput,
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderSecondClauseSecondLiteralPrefix.rawTimeBound problem.verifier).eval
          problem.input.length + 390 +
        24 * problem.input.length + 12 * problem.FormulaWidth +
        12 * (BuilderSecondClauseSeparatorStep.cursorWord problem).length := by
  rw [rawTimeBound]
...
private theorem appenderWorkSteps_le {language : Language}
    ...
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 44 := by
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix := BuilderSecondClauseSecondLiteralPrefix.rawTimeBound_le problem
  have hAppender := appenderWorkSteps_le problem
  have hCursorLength :
      (cursorWord problem).length =
        (BuilderSecondClauseSeparatorStep.cursorWord problem).length + 6 := by
    simp [cursorWord,
      BuilderSecondClauseSecondLiteralPrefix.thirdCursorWord,
      BuilderSecondClauseSecondLiteralPrefix.secondCursorWord,
      BuilderSecondClauseSecondLiteralPrefix.firstCursorWord,
...
private theorem appenderEndpoint_workRunExact {language : Language}
  ...
  have hPrefix : workRunExact? (machine problem)
      (BuilderSecondClauseSecondLiteralPrefix.workSteps problem) initial =
        some prefixFinal := by
  ...
  have h01 := PipelineMachineSimulation.workRunExact?_compose
    (machine problem) (BuilderSecondClauseSecondLiteralPrefix.workSteps problem) 1
