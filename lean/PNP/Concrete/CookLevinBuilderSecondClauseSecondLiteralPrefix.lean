...
The second complete negative literal of the second canonical Cook--Levin
clause.

The machine in this file composes the complete clause-two first-literal
...
clause terminator.  This is a fixed three-token prefix, not a general schedule
decoder, a complete clause-two emitter, or a complete formula builder.
-/
...
namespace BuilderSecondClauseSecondLiteralPrefix

open PipelineTape PipelineStateNamespace PipelineStageBridges

/-! ### Selected token appenders followed by cursor advances -/

namespace TrueTokenCursor
...
/-- One selected true-token append followed by the existing unary cursor
...
namespace TrueFalseSuffix

/-- The unary-one token followed by its false terminator.  Only the final
false-token cursor supplies global halts. -/
def machine : WorkMachine :=
...
private theorem first_noRuleAtAccept :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      TrueTokenCursor.machine := by
  exact TrueTokenCursor.rule_source_ne_acceptState
...
theorem rules_length : machine.rules.length = 235 := by
  have hTrue := TrueTokenCursor.rules_length
  have hFalse :=
...
namespace SecondLiteralSuffix

/-- The negative sign followed by the complete unary-one/terminator tail.
Only the final false-token cursor supplies global halts. -/
def machine : WorkMachine :=
...
private theorem second_noRuleAtAccept :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      TrueFalseSuffix.machine := by
  exact TrueFalseSuffix.rule_source_ne_acceptState

theorem rules_length : machine.rules.length = 357 := by
  have hFirst :=
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.rules_length
  have hTail := TrueFalseSuffix.rules_length
  unfold machine BuilderFirstClausePrefix.WorkChain.machine
    BuilderFirstClausePrefix.WorkChain.rules
    BuilderFirstClausePrefix.WorkChain.bridgeRules
  simp only [List.length_append, List.length_map]
...
theorem rules_pairwise_query_distinct :
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    ...
    TrueFalseSuffix.machine
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.rules_pairwise_query_distinct
    TrueFalseSuffix.rules_pairwise_query_distinct first_noRuleAtAccept

theorem machine_acceptState_ne_rejectState :
    machine.acceptState ≠ machine.rejectState := by
  exact BuilderFirstClausePrefix.WorkChain.machine_acceptState_ne_rejectState
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
    TrueFalseSuffix.machine TrueFalseSuffix.machine_acceptState_ne_rejectState
...
theorem rule_source_ne_acceptState (rule : WorkRule)
  ...
  exact BuilderFirstClausePrefix.WorkChain.noRuleAtAccept
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
    TrueFalseSuffix.machine second_noRuleAtAccept rule hRule

end SecondLiteralSuffix

private theorem predecessor_noRuleAtAccept {language : Language}
    (problem : VerifierTableauProblem language) :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondClauseFirstLiteralPrefix.machine problem) := by
  exact BuilderSecondClauseFirstLiteralPrefix.rule_source_ne_acceptState problem

private theorem suffix_noRuleAtAccept :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      SecondLiteralSuffix.machine := by
  exact SecondLiteralSuffix.rule_source_ne_acceptState
      ...
      1976 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderFirstClausePaddingRun.secondClauseStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderSecondClauseFirstLiteralPrefix.rules_length problem
  have hSuffix := SecondLiteralSuffix.rules_length
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondClauseFirstLiteralPrefix.machine problem)
    SecondLiteralSuffix.machine
    (BuilderSecondClauseFirstLiteralPrefix.rules_pairwise_query_distinct problem)
    SecondLiteralSuffix.rules_pairwise_query_distinct
...
theorem machine_acceptState_ne_rejectState {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.machine_acceptState_ne_rejectState
    (BuilderSecondClauseFirstLiteralPrefix.machine problem)
    SecondLiteralSuffix.machine
    SecondLiteralSuffix.machine_acceptState_ne_rejectState
...
def secondClauseSecondLiteralTokens {language : Language}
    (problem : VerifierTableauProblem language) : List CNFToken :=
...
def firstCursorWord {language : Language}
  ...
  BuilderSecondClauseFirstLiteralPrefix.secondCursorWord problem ++
...
private def cursorOutsideTail {language : Language}
    (problem : VerifierTableauProblem language) : List WorkSymbol :=
  (BuilderSecondClauseFirstLiteralPrefix.finalOutside problem).drop
    ((firstCursorWord problem).length + 1)

private theorem predecessor_finalOutside_eq_firstCursor {language : Language}
    (problem : VerifierTableauProblem language) :
    BuilderSecondClauseFirstLiteralPrefix.finalOutside problem =
  ...
  simp only [BuilderSecondClauseFirstLiteralPrefix.finalOutside,
    List.length_append, List.length_singleton]
  rw [show
      (BuilderSecondClauseFirstLiteralPrefix.secondCursorWord problem).length +
          1 + 1 =
        (BuilderSecondClauseFirstLiteralPrefix.secondCursorWord problem).length +
          2 by omega]
...
def finalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input (finalOutside problem)
    (secondClauseSecondLiteralTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondClauseFirstLiteralPrefix.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (secondClauseSecondLiteralTokens problem)
...
theorem firstAppender_workRunExact {language : Language}
    ...
    workRunExact?
        ...
        (workStartConfiguration
          BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.appender
          (BuilderSecondClauseFirstLiteralPrefix.finalTape problem)) =
...
private theorem predecessorCursorWord_symbol {language : Language}
    (problem : VerifierTableauProblem language) :
    ∀ symbol ∈ BuilderSecondClauseFirstLiteralPrefix.secondCursorWord problem,
      symbol = BuilderUnaryPolynomial.unitSymbol ∨
        symbol = BuilderUnaryPolynomial.separatorSymbol := by
  intro symbol hMem
  unfold BuilderSecondClauseFirstLiteralPrefix.secondCursorWord
    BuilderSecondClauseFirstLiteralPrefix.firstCursorWord at hMem
  simp only [List.mem_append, List.mem_singleton] at hMem
  rcases hMem with (hBase | hFirstUnit) | hSecondUnit
...
theorem thirdAppender_workRunExact {language : Language}
  ...
  simpa [selectedAppender,
    ...
    thirdAppenderFinalConfiguration, secondClauseSecondLiteralTokens] using
      selectedAppender_workRunExact problem.input
        (thirdCursorWord problem ++ BuilderUnaryPolynomial.scratchEndSymbol ::
          (cursorOutsideTail problem).drop 2)
...
theorem trueFalseSuffix_launch_workStep {language : Language}
  ...
  simpa [TrueFalseSuffix.machine, unaryTokenCursorFinalConfiguration,
    workStartConfiguration] using hLaunch
...
theorem secondLiteralSuffix_launch_workStep {language : Language}
  ...
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
    TrueFalseSuffix.machine (signTokenCursorFinalConfiguration problem).tape
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondClauseFirstLiteralPrefix.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderSecondClauseFirstLiteralPrefix.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderSecondClauseFirstLiteralPrefix.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    ...
    (workStartConfiguration
      (BuilderSecondClauseFirstLiteralPrefix.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderSecondClauseFirstLiteralPrefix.finalConfiguration problem)
    (BuilderSecondClauseFirstLiteralPrefix.workRunExact problem)
...
theorem prefixSecondLiteral_launch_workStep {language : Language}
    ...
    workStep? (machine problem)
      ...
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.secondState
        (workStartConfiguration SecondLiteralSuffix.machine
          (BuilderSecondClauseFirstLiteralPrefix.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderSecondClauseFirstLiteralPrefix.machine problem)
    SecondLiteralSuffix.machine
    (BuilderSecondClauseFirstLiteralPrefix.finalTape problem)
...
private theorem formulaClauseTokens_firstRectangle_then_secondLiteral
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
  ...
  simp [pairTail, BoundedClause.emit, excludeBoundedPairClause, falseLiteral,
    ...
    encodeUnaryTokens, hLeft, hRight, List.append_assoc]

/-- The finite output is exactly the canonical token prefix through the
second negative literal of clause two. -/
theorem secondClauseSecondLiteralTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      secondClauseSecondLiteralTokens problem ++ rest := by
  rcases formulaClauseTokens_firstRectangle_then_secondLiteral problem with
  ...
  unfold secondClauseSecondLiteralTokens unaryTokenOutput signTokenOutput
    BuilderSecondClauseFirstLiteralPrefix.secondClauseFirstLiteralTokens
...
through negative variable one in clause two. -/
theorem finalTokenBits_eq_encodedFormula_secondClauseSecondLiteral
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (secondClauseSecondLiteralTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 18)) := by
  rcases secondClauseSecondLiteralTokens_eq_canonical_formula_prefix problem with
    ⟨rest, hTokens⟩
  have hLength :
      (encodeTokenPairs (secondClauseSecondLiteralTokens problem)).length =
        2 * (problem.FormulaWidth + 18) := by
  ...
  have hShape : problem.encodedFormula =
      encodeTokenPairs (secondClauseSecondLiteralTokens problem) ++ suffix := by
...
def finalTokenSlot {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderFirstClausePaddingRun.secondClauseStart problem + 6
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  rw [hPrefix]
  rw [show BuilderFirstClausePaddingRun.secondClauseStart problem + 6 =
...
private theorem directSlot_at_secondClauseOffset
    {language : Language} (problem : VerifierTableauProblem language)
    ...
    (hLookup :
      ([some CNFToken.sep,
        some CNFToken.f, some CNFToken.f,
        some CNFToken.f, some CNFToken.t, some CNFToken.f,
        some CNFToken.finish] : List (Option CNFToken))[offset]? =
          some (some token)) :
    problem.formulaTokenSlotDirect
        (BuilderFirstClausePaddingRun.secondClauseStart problem + offset) =
      some (some token) := by
  ...
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + offset - 11 =
      (problem.formulaTokensPerClause - 11) + offset by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 11) + offset -
      (problem.formulaTokensPerClause - 11) = offset by omega]
  rw [List.getElem?_append]
...
theorem secondLiteralSignSlot_direct_eq_f {language : Language}
  ...
  exact directSlot_at_secondClauseOffset problem 3 CNFToken.f
    (by decide) (by decide)

/-- Variable one contributes one unary `T` unit. -/
theorem secondLiteralUnaryUnitSlot_direct_eq_t {language : Language}
    (problem : VerifierTableauProblem language) :
    problem.formulaTokenSlotDirect
        (BuilderFirstClausePaddingRun.secondClauseStart problem + 4) =
      some (some CNFToken.t) := by
  exact directSlot_at_secondClauseOffset problem 4 CNFToken.t
    (by decide) (by decide)
...
theorem secondLiteralTerminatorSlot_direct_eq_f {language : Language}
    (problem : VerifierTableauProblem language) :
    problem.formulaTokenSlotDirect
        (BuilderFirstClausePaddingRun.secondClauseStart problem + 5) =
      some (some CNFToken.f) := by
  exact directSlot_at_secondClauseOffset problem 5 CNFToken.f
    (by decide) (by decide)
...
theorem nextTokenSlot_direct_eq_finish {language : Language}
  ...
  simpa [finalTokenSlot] using
    directSlot_at_secondClauseOffset problem 6 CNFToken.finish
      (by decide) (by decide)
...
theorem specification_secondLiteral_sign_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
      ...
      some (some CNFToken.f,
        ⟨BuilderFirstClausePaddingRun.secondClauseStart problem + 4⟩) := by
...
theorem specification_secondLiteral_unaryUnit_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
      ...
      some (some CNFToken.t,
        ⟨BuilderFirstClausePaddingRun.secondClauseStart problem + 5⟩) := by
  unfold VerifierTableauProblem.FormulaTokenCursor.step
  rw [secondLiteralUnaryUnitSlot_direct_eq_t]
...
theorem specification_secondLiteral_terminator_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderFirstClausePaddingRun.secondClauseStart problem + 5⟩ =
      some (some CNFToken.f,
        ⟨BuilderFirstClausePaddingRun.secondClauseStart problem + 6⟩) := by
  unfold VerifierTableauProblem.FormulaTokenCursor.step
...
def rawTimeBound {language : Language}
  ...
  .add (BuilderSecondClauseFirstLiteralPrefix.rawTimeBound verifier)
    (.add (.constant 1026)
...
private theorem predecessorTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderSecondClauseFirstLiteralPrefix.secondClauseFirstLiteralTokens
      problem).length = problem.FormulaWidth + 15 := by
  rw [BuilderSecondClauseFirstLiteralPrefix.secondClauseFirstLiteralTokens,
    BuilderSecondClauseFirstLiteralPrefix.firstTokenOutput,
    BuilderSecondClauseSeparatorStep.secondClauseStartTokens]
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderSecondClauseFirstLiteralPrefix.rawTimeBound
          problem.verifier).eval problem.input.length + 1026 +
        72 * problem.input.length + 36 * problem.FormulaWidth +
        36 * (BuilderSecondClauseSeparatorStep.cursorWord problem).length := by
  rw [rawTimeBound]
...
private theorem firstAppenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    firstAppenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 38 := by
...
private theorem secondAppenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    secondAppenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 40 := by
  have hSource := BuilderTokenAppender.sourceCellCount_le problem.input
...
private theorem thirdAppenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    thirdAppenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 42 := by
  have hSource := BuilderTokenAppender.sourceCellCount_le problem.input
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix :=
    BuilderSecondClauseFirstLiteralPrefix.rawTimeBound_le problem
  ...
  have hFirstCursorLength :
      (firstCursorWord problem).length =
        (BuilderSecondClauseSeparatorStep.cursorWord problem).length + 3 := by
    simp [firstCursorWord,
      BuilderSecondClauseFirstLiteralPrefix.secondCursorWord,
      BuilderSecondClauseFirstLiteralPrefix.firstCursorWord]
  have hSecondCursorLength :
      (secondCursorWord problem).length =
        (BuilderSecondClauseSeparatorStep.cursorWord problem).length + 4 := by
    simp [secondCursorWord, hFirstCursorLength]
  have hThirdCursorLength :
      (thirdCursorWord problem).length =
        (BuilderSecondClauseSeparatorStep.cursorWord problem).length + 5 := by
...
private theorem signAppender_stuck_timeout {language : Language}
  ...
  have hSuffixStep : workStep? SecondLiteralSuffix.machine suffix = none := by
    have hStep := first_workStep_none_of_local
      BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
      TrueFalseSuffix.machine component
      (state_ne_accept_of_not_halted _ component hComponentHalted)
      hComponentHalted hComponentStep
    simpa [SecondLiteralSuffix.machine, suffix] using hStep
  have hSuffixHalted : SecondLiteralSuffix.machine.isHalted suffix = false := by
    have hHalted :=
      BuilderFirstClausePrefix.WorkChain.machine_isHalted_first_false
        BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
        TrueFalseSuffix.machine component
    simpa [SecondLiteralSuffix.machine, suffix] using hHalted
  have hGlobalStep : workStep? (machine problem)
      (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState
        suffix) = none := by
    have hStep := second_workStep_none_of_local
      (BuilderSecondClauseFirstLiteralPrefix.machine problem)
      SecondLiteralSuffix.machine suffix hSuffixHalted hSuffixStep
  ...
  have hGlobalHalted : (machine problem).isHalted
    ...
    have hHalted :=
      BuilderFirstClausePrefix.WorkChain.machine_isHalted_second_false_of_local
        (BuilderSecondClauseFirstLiteralPrefix.machine problem)
        SecondLiteralSuffix.machine suffix hSuffixHalted
...
private theorem unaryAppender_stuck_timeout {language : Language}
  ...
  have hAppenderStep : workStep? TrueTokenCursor.appender bad = none := by
    change workStep? BuilderTokenAppender.machine bad = none
    exact hLocalStep
  have hComponentStep : workStep? TrueTokenCursor.machine component = none := by
    have hStep := first_workStep_none_of_local TrueTokenCursor.appender
      BuilderDynamicTokenCursorStep.CursorAdvance.machine bad
      (state_ne_accept_of_not_halted _ bad hAppenderHalted)
      hAppenderHalted hAppenderStep
    simpa [TrueTokenCursor.machine, component] using hStep
  have hComponentHalted : TrueTokenCursor.machine.isHalted component = false := by
    have hHalted :=
      BuilderFirstClausePrefix.WorkChain.machine_isHalted_first_false
        TrueTokenCursor.appender
        BuilderDynamicTokenCursorStep.CursorAdvance.machine bad
    simpa [TrueTokenCursor.machine, component] using hHalted
  have hTailStep : workStep? TrueFalseSuffix.machine tail = none := by
    have hStep := first_workStep_none_of_local TrueTokenCursor.machine
      BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine component
      (state_ne_accept_of_not_halted _ component hComponentHalted)
      hComponentHalted hComponentStep
    simpa [TrueFalseSuffix.machine, tail] using hStep
  have hTailHalted : TrueFalseSuffix.machine.isHalted tail = false := by
    have hHalted :=
      BuilderFirstClausePrefix.WorkChain.machine_isHalted_first_false
        TrueTokenCursor.machine
        BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
        component
    simpa [TrueFalseSuffix.machine, tail] using hHalted
  have hSuffixStep : workStep? SecondLiteralSuffix.machine suffix = none := by
    have hStep := second_workStep_none_of_local
      BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
      TrueFalseSuffix.machine tail hTailHalted hTailStep
    simpa [SecondLiteralSuffix.machine, suffix] using hStep
  have hSuffixHalted : SecondLiteralSuffix.machine.isHalted suffix = false := by
    have hHalted :=
      BuilderFirstClausePrefix.WorkChain.machine_isHalted_second_false_of_local
        BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
        TrueFalseSuffix.machine tail hTailHalted
    simpa [SecondLiteralSuffix.machine, suffix] using hHalted
  have hGlobalStep : workStep? (machine problem)
      (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState
        suffix) = none := by
...
private theorem terminatorAppender_stuck_timeout {language : Language}
  ...
  have hComponentHalted :
    ...
    simpa [BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine,
      component] using hHalted
  have hTailStep : workStep? TrueFalseSuffix.machine tail = none := by
    have hStep := second_workStep_none_of_local TrueTokenCursor.machine
      BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine component
      hComponentHalted hComponentStep
    simpa [TrueFalseSuffix.machine, tail] using hStep
  have hTailHalted : TrueFalseSuffix.machine.isHalted tail = false := by
...
private theorem signCursorGlobal_workStep_of_some {language : Language}
  ...
  have hGlobal := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    ...
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.firstState
      (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState
        next))
    (by simpa [SecondLiteralSuffix.machine] using hSuffix)
...
private theorem unaryCursorGlobal_workStep_of_some {language : Language}
  ...
  have hTail := BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
    ...
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState config)
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState next)
    (by simpa [TrueTokenCursor.machine] using hComponent)
  have hSuffix := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    ...
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.firstState
      (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState
        next))
    (by simpa [TrueFalseSuffix.machine] using hTail)
  have hGlobal := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    (BuilderSecondClauseFirstLiteralPrefix.machine problem)
    SecondLiteralSuffix.machine
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState
...
private theorem unaryAppenderGlobal_workStep_of_some {language : Language}
    (problem : VerifierTableauProblem language)
    (config next : WorkConfiguration)
    (hStep : workStep? TrueTokenCursor.appender config = some next) :
    workStep? (machine problem)
...
private theorem prefixGlobal_launch_workStep {language : Language}
  ...
  simpa [prefixGlobalEndpoint, signAppenderGlobalConfiguration,
    ...
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine,
    BuilderFirstClausePrefix.WorkChain.machine, workStartConfiguration,
    renameConfiguration] using prefixSecondLiteral_launch_workStep problem
...
private theorem signAppenderGlobal_launch_workStep {language : Language}
  ...
  have hSuffix := BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
    TrueFalseSuffix.machine _ _ hComponent
  have hGlobal := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    (BuilderSecondClauseFirstLiteralPrefix.machine problem)
...
private theorem signCursorGlobal_launch_workStep {language : Language}
  ...
  have hSuffix := secondLiteralSuffix_launch_workStep problem
  have hGlobal := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    (BuilderSecondClauseFirstLiteralPrefix.machine problem)
    SecondLiteralSuffix.machine _ _ hSuffix
...
private theorem unaryAppenderGlobal_launch_workStep {language : Language}
  ...
  have hSuffix := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
    TrueFalseSuffix.machine _ _ (by
  ...
  simpa [machine, unaryAppenderGlobalEndpoint,
    ...
    SecondLiteralSuffix.machine, TrueFalseSuffix.machine,
    TrueTokenCursor.machine, BuilderFirstClausePrefix.WorkChain.machine,
...
private theorem unaryCursorGlobal_launch_workStep {language : Language}
  ...
  have hSuffix := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
    TrueFalseSuffix.machine _ _ hTail
  have hGlobal := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    (BuilderSecondClauseFirstLiteralPrefix.machine problem)
    SecondLiteralSuffix.machine _ _ (by
      simpa [SecondLiteralSuffix.machine] using hSuffix)
  simpa [machine, unaryCursorGlobalEndpoint,
    ...
    TrueFalseSuffix.machine, TrueTokenCursor.machine,
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine,
    BuilderFirstClausePrefix.WorkChain.machine, workStartConfiguration,
    renameConfiguration] using hGlobal
...
private theorem terminatorAppenderGlobal_launch_workStep
  ...
  simpa [machine, terminatorAppenderGlobalEndpoint,
    ...
    terminatorCursorGlobalConfiguration, SecondLiteralSuffix.machine,
    TrueFalseSuffix.machine,
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine,
    BuilderFirstClausePrefix.WorkChain.machine, workStartConfiguration,
    renameConfiguration] using hGlobal
...
private theorem unaryAppenderGlobal_workRunExact {language : Language}
  ...
  exact workRunExact_transport_of_lift TrueTokenCursor.appender
...
end BuilderSecondClauseSecondLiteralPrefix
