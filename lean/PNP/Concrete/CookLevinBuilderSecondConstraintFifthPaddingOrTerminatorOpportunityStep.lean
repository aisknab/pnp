...
The fifth width-dependent padding-or-terminator opportunity after the first
...
audited token-appender rule table at its `F` state and emits the second
literal's terminator.  Both branches then materialize the following formula
...
emit the following padding or opening `T`, complete the following literal,
traverse the second constraint, implement a general schedule cursor, complete
the formula builder, or establish P = NP.
-/
...
namespace BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep
...
def opportunitySlotPolynomial {language : Language}
  ...
  .add
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.opportunitySlotPolynomial
      verifier)
    (.constant 1)
...
theorem opportunitySlot_eq_secondConstraintStart_add_twelve
  ...
  change
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.opportunitySlot
        problem +
      1 = _
  rw [
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.opportunitySlot_eq_secondConstraintStart_add_eleven]

/-! ### Width-selected optional terminator appender -/

namespace WidthOptionalTerminatorAppender

open BuilderFirstClausePrefix

/-- The token appender's rule table is token-generic.  Selecting the `F`
seek state as the entry point reuses those rules while fixing this bounded
controller's wider-width output to the literal terminator. -/
private abbrev fAppenderMachine : WorkMachine :=
  { BuilderTokenAppender.machine with
...
def doneBridge : List WorkRule :=
  launchRules
    ...
    (WorkChain.secondState
      fAppenderMachine.acceptState)

/-- The ordinary `WorkChain` bridge handles the wider `F` branch.  This
additional bridge handles the width-one padding branch by entering the
appender's halt state without emitting a token. -/
...
private theorem appender_rule_source_ne_accept (rule : WorkRule)
    (hMem : rule ∈ fAppenderMachine.rules) :
    rule.sourceState ≠ fAppenderMachine.acceptState := by
  decide +revert
...
theorem rule_source_ne_acceptState (rule : WorkRule)
  ...
  have hBase : WorkChain.NoRuleAtAccept
    ...
    WorkChain.noRuleAtAccept
      BuilderCompleteHeader.HeaderController.machine
      fAppenderMachine appender_rule_source_ne_accept
  ...
  | some rule =>
      ...
      have hGlobalHalted :=
        WorkChain.machine_isHalted_first_false
          BuilderCompleteHeader.HeaderController.machine
          fAppenderMachine config
      have hGlobalFind := WorkChain.findWorkRule_first_of_some
        BuilderCompleteHeader.HeaderController.machine
        fAppenderMachine config.state config.tape.head rule
        hMore hFind
...
theorem appender_workStep_of_some
  ...
  have hStep' : workStep? fAppenderMachine config = some next := by
    change workStep? BuilderTokenAppender.machine config = some next
    exact hStep
  exact WorkChain.second_workStep_of_some
    BuilderCompleteHeader.HeaderController.machine
    fAppenderMachine config next hStep'
...
theorem controller_more_launch_workStep (tape : WorkTape) :
    workStep? machine
      ...
      some
        (renameConfiguration WorkChain.secondState
          (BuilderTokenAppender.entryConfiguration .f tape)) := by
  have hDone :
  ...
  simpa [BuilderCompleteHeader.HeaderController.machine,
    ...
    WorkChain.launch_workStep
      BuilderCompleteHeader.HeaderController.machine
      fAppenderMachine tape

theorem controller_done_skip_workStep (tape : WorkTape) :
    workStep? machine
        (renameConfiguration WorkChain.firstState
      ...
      some
        (renameConfiguration WorkChain.secondState
          { state := fAppenderMachine.acceptState
            tape := tape }) := by
  ...
  have hBridge := findWorkRule_launchRules
    (WorkChain.firstState
      BuilderCompleteHeader.HeaderController.doneExitState)
    (WorkChain.secondState
      fAppenderMachine.acceptState) tape.head
  have hFind : findWorkRule machine.rules start.state start.tape.head =
      some (launchRule
        (WorkChain.firstState
          BuilderCompleteHeader.HeaderController.doneExitState)
        (WorkChain.secondState
          fAppenderMachine.acceptState) tape.head) := by
  ...
  have hStep := workStep?_eq_apply_of_find machine start
    (launchRule
      ...
      (WorkChain.secondState
        fAppenderMachine.acceptState) tape.head)
    hHalted hFind
  simpa [start, target, renameConfiguration, launchRule, applyWorkRule,
...
theorem workRunExact (input : BitString)
    ...
    let emitted : List CNFToken :=
      if remaining = 0 then [] else [.f]
    let appendSteps :=
      if remaining = 0 then 0
      else BuilderTokenAppender.workSteps input output
  ...
  · subst remaining
    ...
    have hSkip : workRunExact? machine 1
      ...
      simpa [controllerFinal,
        BuilderCompleteHeader.HeaderController.finalConfiguration,
        branchFinal, BuilderTokenAppender.finalConfiguration] using
    ...
    have h01 := PipelineMachineSimulation.workRunExact?_compose machine
      ...
      1 _ _ _ hController hSkip
    simpa [controllerInitial, controllerFinal, branchFinal,
      BuilderTokenAppender.finalConfiguration, Nat.add_assoc] using h01
  · cases remaining with
    ...
    | succ rest =>
      ...
      let appenderFinal :=
        BuilderTokenAppender.finalConfiguration input
          (BuilderCompleteHeader.HeaderController.outsideAfter
            wordPrefix (rest + 1) tail) (output ++ [.f])
...
end WidthOptionalTerminatorAppender
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.machine problem) := by
  exact
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.rule_source_ne_acceptState
      problem
...
private theorem branch_noRuleAtAccept :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      WidthOptionalTerminatorAppender.machine :=
  WidthOptionalTerminatorAppender.rule_source_ne_acceptState
...
theorem rules_length {language : Language}
  ...
  have hPredecessor :=
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.rules_length problem
  have hWidth := BuilderUnaryPolynomial.rules_length (widthPolynomial problem)
  have hTarget := BuilderUnaryPolynomial.rules_length
    (opportunitySlotPolynomial problem.verifier)
  have hBranch := WidthOptionalTerminatorAppender.rules_length
  ...
  simp only [List.length_append, List.length_map, widthEvaluator,
    targetEvaluator, WidthOptionalTerminatorAppender.machine,
    BuilderUnaryPolynomial.machine]
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  have hWidthBranch :=
    BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
      ...
      WidthOptionalTerminatorAppender.rules_pairwise_query_distinct
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.machine problem)
    (suffixMachine problem)
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.rules_pairwise_query_distinct
      problem)
...
def opportunityOutput {language : Language}
  ...
  if width problem = 1 then [] else [.f]

def secondConstraintFifthPaddingOrTerminatorTokens {language : Language}
    (problem : VerifierTableauProblem language) : List CNFToken :=
  BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.secondConstraintFourthPaddingOrUnaryTokens
      problem ++
...
def widthOutside {language : Language}
  ...
  BuilderUnaryPolynomial.finalOutsideLeft
    ...
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalOutside problem)
...
def controllerFinalOutside {language : Language}
  ...
  (widthWord problem).take (widthRootPrefixLength problem) ++
    ...
    [BuilderUnaryPolynomial.scratchEndSymbol,
     BuilderUnaryPolynomial.scratchEndSymbol] ++
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalOutside problem).drop
...
def branchFinalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input
    ...
    (secondConstraintFifthPaddingOrTerminatorTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.workSteps problem + 1 +
...
theorem widthEvaluator_workRunExact {language : Language}
    ...
    workRunExact? (widthEvaluator problem) (widthWorkSteps problem)
        (BuilderUnaryPolynomial.initialConfiguration
          (widthPolynomial problem) problem.input
          (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalOutside
            problem)
          (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.secondConstraintFourthPaddingOrUnaryTokens
            problem)) =
      some
...
theorem optionalAppender_workRunExact {language : Language}
    (problem : VerifierTableauProblem language) :
    workRunExact? WidthOptionalTerminatorAppender.machine (branchWorkSteps problem)
        (workStartConfiguration WidthOptionalTerminatorAppender.machine
          (widthTape problem)) =
      some
        { state := WidthOptionalTerminatorAppender.machine.acceptState
  ...
  | succ remaining =>
      ...
      have hRun := WidthOptionalTerminatorAppender.workRunExact problem.input
      ...
      | zero =>
          ...
          simpa [hBranchWorkSteps, controllerWorkSteps,
            ...
            secondConstraintFifthPaddingOrTerminatorTokens, hOpportunityOutput,
      ...
      | succ rest =>
          ...
          have hOpportunityOutput :
              opportunityOutput problem = [CNFToken.f] := by
    ...
    workRunExact? (targetEvaluator problem) (targetWorkSteps problem)
      ...
      some
        (BuilderUnaryPolynomial.finalConfiguration
          (opportunitySlotPolynomial problem.verifier) problem.input
          (controllerFinalOutside problem)
          (secondConstraintFifthPaddingOrTerminatorTokens problem)) := by
  exact BuilderUnaryPolynomial.workRunExact
    (opportunitySlotPolynomial problem.verifier) problem.input
    (controllerFinalOutside problem)
    (secondConstraintFifthPaddingOrTerminatorTokens problem)
...
private theorem widthBranch_workRunExact {language : Language}
    ...
    workRunExact? (widthBranchMachine problem)
        ...
        (workStartConfiguration (widthBranchMachine problem)
          (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalTape
            problem)) =
      some
  ...
  let widthFinal := BuilderUnaryPolynomial.finalConfiguration
    ...
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.secondConstraintFourthPaddingOrUnaryTokens
      problem)
  let branchInitial := workStartConfiguration WidthOptionalTerminatorAppender.machine
    (widthTape problem)
  ...
  have hBranch : workRunExact? WidthOptionalTerminatorAppender.machine
  ...
  have hCombined := BuilderFirstClausePrefix.WorkChain.workRunExact
    (widthEvaluator problem) WidthOptionalTerminatorAppender.machine
...
theorem suffix_workRunExact {language : Language}
  ...
  let widthBranchInitial := workStartConfiguration
    ...
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalTape problem)
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.workSteps problem)
      ...
      some
        (renameConfiguration
          ...
          (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalConfiguration
            problem)) := by
  ...
  have hRun := hTransport
    ...
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalConfiguration
      problem)
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.workRunExact problem)
  exact hRun
...
theorem workRunExact {language : Language}
  ...
  let prefixInitial := workStartConfiguration
    ...
    (rawInputWorkTape problem.input)
  let prefixFinal :=
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalConfiguration problem
...
theorem opportunityOutput_eq_nil_or_f {language : Language}
    (problem : VerifierTableauProblem language) :
    opportunityOutput problem =
      if problem.dimensions.tapeWidth problem.tableauInputMode = 1
        then [] else [CNFToken.f] := by
...
theorem opportunityOutput_eq_nil_iff {language : Language}
  ...
  rw [opportunityOutput_eq_nil_or_f]
  by_cases hWidth :
      problem.dimensions.tapeWidth problem.tableauInputMode = 1
  · simp [hWidth]
  · simp [hWidth]

theorem opportunityOutput_eq_singleton_f_iff {language : Language}
    (problem : VerifierTableauProblem language) :
    opportunityOutput problem = [CNFToken.f] ↔
      problem.dimensions.tapeWidth problem.tableauInputMode ≠ 1 := by
  rw [opportunityOutput_eq_nil_or_f]
  by_cases hWidth :
      problem.dimensions.tapeWidth problem.tableauInputMode = 1
...
theorem specification_opportunity_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalTokenSlot
          problem⟩ =
      some ((if problem.dimensions.tapeWidth problem.tableauInputMode = 1
        ...
        ⟨BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalTokenSlot
          problem + 1⟩) := by
  exact
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.specification_following_step
      problem

/-- The finite output is exactly the canonical token prefix after consuming
the fifth width-selected padding-or-terminator opportunity. -/
theorem secondConstraintFifthPaddingOrTerminatorTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      secondConstraintFifthPaddingOrTerminatorTokens problem ++ rest := by
  rcases
      BuilderSecondConstraintFirstLiteralTerminatorStep.encodeCNFTokens_eq_terminator_then_successor_and_four_optional_unary_and_optional_terminator
        problem with
    ⟨rest, hTokens⟩
  refine ⟨rest, ?_⟩
  unfold secondConstraintFifthPaddingOrTerminatorTokens
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.secondConstraintFourthPaddingOrUnaryTokens
  ...
  rw [
    ...
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.opportunityOutput_eq_nil_or_t,
    opportunityOutput_eq_nil_or_f]
  exact hTokens
...
theorem finalTokenSlot_eq_predecessor_add_one {language : Language}
  (problem : VerifierTableauProblem language) :
    finalTokenSlot problem =
      BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalTokenSlot problem +
        1 := by
  rw [finalTokenSlot, opportunitySlot_eq_secondConstraintStart_add_twelve,
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalTokenSlot_eq_secondConstraintStart_add_eleven]

theorem finalTokenSlot_eq_secondConstraintStart_add_twelve
    {language : Language} (problem : VerifierTableauProblem language) :
    finalTokenSlot problem =
      problem.formulaVariableSlotBound + 1 +
        problem.formulaClauseSlotsPerConstraint *
          problem.formulaTokensPerClause + 12 := by
  exact opportunitySlot_eq_secondConstraintStart_add_twelve problem

/-- The retained coordinate identifies the opportunity after the processed
slot: another padding opportunity at width one, otherwise the opening unary
`T` of the following literal. -/
theorem followingTokenSlot_direct_eq_padding_or_t {language : Language}
  ...
  have hFollowing :=
    BuilderSecondConstraintFirstLiteralTerminatorStep.sixthFollowingTokenSlot_direct_eq_padding_or_t
      problem
  rw [finalTokenSlot_eq_predecessor_add_one,
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.finalTokenSlot_eq_predecessor_add_one,
...
def rawTimeBound {language : Language}
  ...
  .add
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.rawTimeBound verifier)
    (.add (.constant 660)
...
private theorem predecessorTokens_length {language : Language}
    ...
    (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.secondConstraintFourthPaddingOrUnaryTokens
      problem).length =
        problem.FormulaWidth + 43 +
          (if problem.dimensions.tapeWidth problem.tableauInputMode = 1
            then 0 else 4) := by
  rw [BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.secondConstraintFourthPaddingOrUnaryTokens,
    List.length_append]
  ...
  have hThirdTokens :
    ...
    have hOpportunity :
      ...
      · simp [hWidth]
      · simp [hWidth]
    rw [hSecondOpportunity, hOpportunity]
    by_cases hWidth :
        problem.dimensions.tapeWidth problem.tableauInputMode = 1
    · simp [hWidth]
    · simp [hWidth]
  have hFourthOpportunity :
      (BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.opportunityOutput
        problem).length =
          if problem.dimensions.tapeWidth problem.tableauInputMode = 1
            then 0 else 1 := by
    rw [
    ...
    · simp [hWidth]
    · simp [hWidth]
  rw [hThirdTokens, hFourthOpportunity]
  by_cases hWidth :
      problem.dimensions.tapeWidth problem.tableauInputMode = 1
  · simp [hWidth]
  · simp [hWidth]
...
after the fifth width-selected padding-or-terminator opportunity. -/
theorem finalTokenBits_eq_encodedFormula_secondConstraintFifthPaddingOrTerminator
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (secondConstraintFifthPaddingOrTerminatorTokens problem) =
  ...
  have hLength :
      (encodeTokenPairs
        (secondConstraintFifthPaddingOrTerminatorTokens problem)).length =
        2 * (problem.FormulaWidth + 43 +
          if problem.dimensions.tapeWidth problem.tableauInputMode = 1
            then 0 else 5) := by
    rw [encodeTokenPairs_length,
      secondConstraintFifthPaddingOrTerminatorTokens, List.length_append,
...
private theorem appenderWorkSteps_le {language : Language}
    ...
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 102 := by
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      ...
      660 + 24 * problem.input.length + 12 * problem.FormulaWidth +
  ...
  unfold width widthPolynomial widthRootPrefixLength widthWorkSteps
    targetWorkSteps
  simp only
    [BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.widthPolynomial]
  omega
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix :=
    BuilderSecondConstraintFourthPaddingOrUnaryOpportunityStep.rawTimeBound_le problem
...
outer bridge launches the width-dependent padding-or-terminator suffix. -/
...
end BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep
