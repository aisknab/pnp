...
sign step with the already-audited selected true-token appender and unary
cursor advance.  Every raw input therefore emits the first unary unit of that
literal's index and leaves the retained coordinate on the second unary unit.
...
namespace BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondConstraintFirstLiteralSignStep.machine problem) := by
  exact BuilderSecondConstraintFirstLiteralSignStep.rule_source_ne_acceptState problem
...
/-- One literal finite work machine from raw input through the first unary unit
...
def machine {language : Language}
  ...
  BuilderFirstClausePrefix.WorkChain.machine
    (BuilderSecondConstraintFirstLiteralSignStep.machine problem)
      ...
      4798 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderFirstConstraintPaddingRun.secondConstraintStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderSecondConstraintFirstLiteralSignStep.rules_length problem
  have hSuffix :=
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondConstraintFirstLiteralSignStep.machine problem)
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
    (BuilderSecondConstraintFirstLiteralSignStep.rules_pairwise_query_distinct problem)
...
def secondConstraintFirstLiteralFirstUnaryTokens {language : Language}
    (problem : VerifierTableauProblem language) : List CNFToken :=
  BuilderSecondConstraintFirstLiteralSignStep.secondConstraintFirstLiteralSignTokens problem ++
    [.t]

def cursorWord {language : Language}
    (problem : VerifierTableauProblem language) : List WorkSymbol :=
  BuilderSecondConstraintFirstLiteralSignStep.cursorWord problem ++
    [BuilderUnaryPolynomial.unitSymbol]

private def cursorOutsideTail {language : Language}
    (problem : VerifierTableauProblem language) : List WorkSymbol :=
  (BuilderSecondConstraintFirstLiteralSignStep.finalOutside problem).drop
    ((cursorWord problem).length + 1)

private theorem predecessor_finalOutside_eq_cursor {language : Language}
    (problem : VerifierTableauProblem language) :
    BuilderSecondConstraintFirstLiteralSignStep.finalOutside problem =
  ...
  simp only [BuilderSecondConstraintFirstLiteralSignStep.finalOutside,
    List.length_append, List.length_singleton]
  rw [show
      (BuilderSecondConstraintFirstLiteralSignStep.cursorWord problem).length + 1 + 1 =
        (BuilderSecondConstraintFirstLiteralSignStep.cursorWord problem).length + 2 by
          omega]
...
def finalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input (finalOutside problem)
    (secondConstraintFirstLiteralFirstUnaryTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondConstraintFirstLiteralSignStep.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (secondConstraintFirstLiteralFirstUnaryTokens problem)
...
theorem appender_workRunExact {language : Language}
    ...
    workRunExact?
        ...
        (workStartConfiguration
          BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender
          (BuilderSecondConstraintFirstLiteralSignStep.finalTape problem)) =
  ...
  have hExact := BuilderTokenAppender.appendToken_workRunExact
    problem.input (BuilderSecondConstraintFirstLiteralSignStep.finalOutside problem)
    (BuilderSecondConstraintFirstLiteralSignStep.secondConstraintFirstLiteralSignTokens problem)
    .t
  rw [hRunEq]
  simpa [BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender,
    appenderWorkSteps, BuilderSecondConstraintFirstLiteralSignStep.finalTape,
    BuilderTokenAppender.entryConfiguration,
    BuilderTokenAppender.finalConfiguration,
    appenderFinalConfiguration, secondConstraintFirstLiteralFirstUnaryTokens,
...
theorem cursor_workRunExact {language : Language}
  ...
  have hWord : ∀ symbol ∈ cursorWord problem,
    ...
    · simp only [BuilderSecondConstraintFirstLiteralSignStep.cursorWord,
      ...
      rcases hBase with hScratch | hPreviousUnit
  ...
  have hExact :=
    BuilderDynamicTokenCursorStep.CursorAdvance.advance_workRunExact
      ...
      (secondConstraintFirstLiteralFirstUnaryTokens problem) hWord
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondConstraintFirstLiteralSignStep.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderSecondConstraintFirstLiteralSignStep.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderSecondConstraintFirstLiteralSignStep.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderSecondConstraintFirstLiteralSignStep.machine problem)
        BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine)
    (BuilderSecondConstraintFirstLiteralSignStep.workSteps problem)
    (workStartConfiguration (BuilderSecondConstraintFirstLiteralSignStep.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderSecondConstraintFirstLiteralSignStep.finalConfiguration problem)
    (BuilderSecondConstraintFirstLiteralSignStep.workRunExact problem)
  simpa [machine, BuilderFirstClausePrefix.WorkChain.machine,
    workStartConfiguration, renameConfiguration] using hTransport

theorem prefixFirstUnaryUnit_launch_workStep {language : Language}
    (problem : VerifierTableauProblem language) :
    workStep? (machine problem)
      ...
      some (renameConfiguration
        ...
        (workStartConfiguration
          BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
          (BuilderSecondConstraintFirstLiteralSignStep.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderSecondConstraintFirstLiteralSignStep.machine problem)
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
    (BuilderSecondConstraintFirstLiteralSignStep.finalTape problem)
  simpa [machine, BuilderSecondConstraintFirstLiteralSignStep.finalConfiguration,
    workStartConfiguration] using hLaunch

set_option maxRecDepth 1000000 in
/-- Every raw input follows the complete second-constraint first-literal sign
trace, appends the first unary unit, and advances the retained coordinate
once. -/
theorem workRunExact {language : Language}
  ...
  have hSuffix : workRunExact?
    ...
    simpa [prefixFinal, suffixFinal,
      BuilderSecondConstraintFirstLiteralSignStep.finalConfiguration,
...
theorem specification_firstUnaryUnit_step {language : Language}
    (problem : VerifierTableauProblem language) :
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondConstraintFirstLiteralSignStep.finalTokenSlot problem⟩ =
      some (some CNFToken.t,
        ⟨BuilderSecondConstraintFirstLiteralSignStep.finalTokenSlot problem + 1⟩) :=
  BuilderSecondConstraintFirstLiteralSignStep.specification_next_step problem
...
private theorem scheduledAtLeastOneClauseTokens_starts_sep_t_t_t
    {language : Language} (problem : VerifierTableauProblem language)
    ...
    ∃ rest,
      problem.scheduledClauseTokens
        ...
        some CNFToken.sep :: some CNFToken.t :: some CNFToken.t ::
          some CNFToken.t :: rest := by
  let value := first.val - 2
  have hValue : first.val = value + 2 := by
    omega
  let rest :=
  ...
  simp [VerifierTableauProblem.scheduledClauseTokens,
    ...
    List.append_assoc]
  rw [show encodeUnaryTokens (value + 2) =
      CNFToken.t :: CNFToken.t :: encodeUnaryTokens value by rfl]
  rfl

private theorem formulaClauseTokens_firstConstraint_padding_then_sep_t_t_t
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
  ...
  rcases scheduledAtLeastOneClauseTokens_starts_sep_t_t_t problem
...
theorem secondConstraintFirstLiteralFirstUnaryTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      secondConstraintFirstLiteralFirstUnaryTokens problem ++ rest := by
  rcases formulaClauseTokens_firstConstraint_padding_then_sep_t_t_t problem with
  ...
  unfold secondConstraintFirstLiteralFirstUnaryTokens
...
through the second-constraint first-literal first unary unit. -/
theorem finalTokenBits_eq_encodedFormula_secondConstraintFirstLiteralFirstUnary
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (secondConstraintFirstLiteralFirstUnaryTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 39)) := by
  rcases
      secondConstraintFirstLiteralFirstUnaryTokens_eq_canonical_formula_prefix
        problem with
    ⟨rest, hTokens⟩
  have hLength :
      (encodeTokenPairs
        (secondConstraintFirstLiteralFirstUnaryTokens problem)).length =
        2 * (problem.FormulaWidth + 39) := by
    rw [encodeTokenPairs_length, secondConstraintFirstLiteralFirstUnaryTokens,
      BuilderSecondConstraintFirstLiteralSignStep.secondConstraintFirstLiteralSignTokens,
      BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens,
      List.length_append,
      BuilderFourthClausePrefix.fourthClauseTokens]
...
theorem finalTokenSlot_eq_secondConstraintStart_add_three
    {language : Language} (problem : VerifierTableauProblem language) :
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
        problem.formulaClauseSlotsPerConstraint *
          problem.formulaTokensPerClause + 3 := by
  rw [finalTokenSlot,
    BuilderSecondConstraintFirstLiteralSignStep.finalTokenSlot_eq_secondConstraintStart_add_two]
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  rw [BuilderSecondConstraintFirstLiteralSignStep.finalTokenSlot,
...
/-- The coordinate retained after the first unary-unit transition is the
second unary unit of an index that is at least three. -/
theorem nextTokenSlot_direct_eq_t {language : Language}
  ...
  rw [show problem.formulaVariableSlotBound + 1 +
      ...
      (problem.formulaVariableSlotBound + 1) =
        4 * problem.formulaTokensPerClause + emptyCount + 3 by
          rw [hDecompose]
          omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 4 * problem.formulaTokensPerClause + emptyCount + 3 - 11 =
      (problem.formulaTokensPerClause - 11) +
        3 * problem.formulaTokensPerClause + emptyCount + 3 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 11) +
      3 * problem.formulaTokensPerClause + emptyCount + 3 -
      (problem.formulaTokensPerClause - 11) =
        3 * problem.formulaTokensPerClause + emptyCount + 3 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 3 * problem.formulaTokensPerClause + emptyCount + 3 - 7 =
      (problem.formulaTokensPerClause - 7) +
        2 * problem.formulaTokensPerClause + emptyCount + 3 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 7) +
      2 * problem.formulaTokensPerClause + emptyCount + 3 -
      (problem.formulaTokensPerClause - 7) =
        2 * problem.formulaTokensPerClause + emptyCount + 3 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause + emptyCount + 3 - 8 =
      (problem.formulaTokensPerClause - 8) +
        problem.formulaTokensPerClause + emptyCount + 3 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 8) +
      problem.formulaTokensPerClause + emptyCount + 3 -
      (problem.formulaTokensPerClause - 8) =
        problem.formulaTokensPerClause + emptyCount + 3 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + emptyCount + 3 - 9 =
      (problem.formulaTokensPerClause - 9) + emptyCount + 3 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 9) + emptyCount + 3 -
      (problem.formulaTokensPerClause - 9) = emptyCount + 3 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [show (problem.formulaClauseSlotsPerConstraint - 4) *
      problem.formulaTokensPerClause = emptyCount by rfl]
  rw [if_neg (by omega)]
  rw [show emptyCount + 3 - emptyCount = 3 by omega]
  simp
...
def rawTimeBound {language : Language}
  ...
  .add (BuilderSecondConstraintFirstLiteralSignStep.rawTimeBound verifier)
    (.add (.constant 558)
...
private theorem predecessorTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderSecondConstraintFirstLiteralSignStep.secondConstraintFirstLiteralSignTokens
      problem).length = problem.FormulaWidth + 38 := by
  simp [BuilderSecondConstraintFirstLiteralSignStep.secondConstraintFirstLiteralSignTokens,
    BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens,
    BuilderFourthClausePrefix.fourthClauseTokens,
    BuilderFourthClauseSecondLiteralPrefix.fourthClauseSecondLiteralTokens,
    BuilderFourthClauseSecondLiteralPrefix.secondUnaryTokenOutput,
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderSecondConstraintFirstLiteralSignStep.rawTimeBound problem.verifier).eval
          problem.input.length + 558 +
  ...
  simp only [cursorWord, BuilderSecondConstraintFirstLiteralSignStep.cursorWord,
    BuilderSecondConstraintSeparatorStep.cursorWord,
    List.length_append, List.length_singleton]
  omega
...
private theorem appenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 84 := by
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix := BuilderSecondConstraintFirstLiteralSignStep.rawTimeBound_le problem
...
/-- The complete second-constraint separator endpoint is still globally
...
private theorem appenderEndpoint_workRunExact {language : Language}
  ...
  have hPrefix : workRunExact? (machine problem)
      (BuilderSecondConstraintFirstLiteralSignStep.workSteps problem) initial =
        some prefixFinal := by
  ...
  have hLaunch : workRunExact? (machine problem) 1 prefixFinal =
    ...
    simpa [prefixFinal, suffixInitial] using
      prefixFirstUnaryUnit_launch_workStep problem
  ...
  have hGlobal := PipelineStageBridges.workRunExact?_transport
    ...
    (BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
      (BuilderSecondConstraintFirstLiteralSignStep.machine problem) BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine)
    (appenderWorkSteps problem)
    (workStartConfiguration BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
      (BuilderSecondConstraintFirstLiteralSignStep.finalTape problem))
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.firstState
  ...
  have h01 := PipelineMachineSimulation.workRunExact?_compose
    (machine problem) (BuilderSecondConstraintFirstLiteralSignStep.workSteps problem) 1
...
end BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep
