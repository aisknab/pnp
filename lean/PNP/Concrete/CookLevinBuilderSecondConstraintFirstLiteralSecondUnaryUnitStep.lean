...
One populated unary-unit transition at the first literal of the second
canonical Cook--Levin constraint.

The machine in this file composes the complete second-constraint first-literal
first-unary-unit step with the already-audited selected true-token appender and
unary cursor advance.  Every raw input therefore emits the second unary unit
of that literal's index and leaves the retained coordinate on the third unary
unit.
This is one fixed populated transition, not a complete literal, a general
schedule decoder, a complete cursor loop, or a complete formula builder.
-/
...
namespace BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep

open PipelineTape PipelineStateNamespace PipelineStageBridges

/-! ### Reused selected true-token/cursor suffix -/

private theorem predecessor_noRuleAtAccept {language : Language}
    (problem : VerifierTableauProblem language) :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem) := by
  exact BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.rule_source_ne_acceptState problem

private theorem suffix_noRuleAtAccept :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine := by
  exact
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.rule_source_ne_acceptState

/-- One literal finite work machine from raw input through the second unary unit
of the first literal of constraint two. -/
def machine {language : Language}
  ...
  BuilderFirstClausePrefix.WorkChain.machine
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem)
      ...
      4920 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderFirstConstraintPaddingRun.secondConstraintStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.rules_length problem
  have hSuffix :=
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem)
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.rules_pairwise_query_distinct problem)
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.rules_pairwise_query_distinct
...
theorem machine_acceptState_ne_rejectState {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.machine_acceptState_ne_rejectState
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem)
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine_acceptState_ne_rejectState
...
def secondConstraintFirstLiteralSecondUnaryTokens {language : Language}
    (problem : VerifierTableauProblem language) : List CNFToken :=
  BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.secondConstraintFirstLiteralFirstUnaryTokens problem ++
    [.t]

def cursorWord {language : Language}
    (problem : VerifierTableauProblem language) : List WorkSymbol :=
  BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.cursorWord problem ++
    [BuilderUnaryPolynomial.unitSymbol]

private def cursorOutsideTail {language : Language}
    (problem : VerifierTableauProblem language) : List WorkSymbol :=
  (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalOutside problem).drop
    ((cursorWord problem).length + 1)

private theorem predecessor_finalOutside_eq_cursor {language : Language}
    (problem : VerifierTableauProblem language) :
    BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalOutside problem =
  ...
  simp only [BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalOutside,
    List.length_append, List.length_singleton]
  rw [show
      (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.cursorWord problem).length + 1 + 1 =
        (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.cursorWord problem).length + 2 by
          omega]
...
def finalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input (finalOutside problem)
    (secondConstraintFirstLiteralSecondUnaryTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (secondConstraintFirstLiteralSecondUnaryTokens problem)

def appenderFinalConfiguration {language : Language}
    (problem : VerifierTableauProblem language) : WorkConfiguration :=
  { state :=
      BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender.acceptState
...
theorem appender_workRunExact {language : Language}
    ...
    workRunExact?
        ...
        (workStartConfiguration
          BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender
          (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTape problem)) =
  ...
  have hExact := BuilderTokenAppender.appendToken_workRunExact
    problem.input (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalOutside problem)
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.secondConstraintFirstLiteralFirstUnaryTokens problem)
    .t
  rw [hRunEq]
  simpa [BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender,
    appenderWorkSteps, BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTape,
    BuilderTokenAppender.entryConfiguration,
    BuilderTokenAppender.finalConfiguration,
    appenderFinalConfiguration, secondConstraintFirstLiteralSecondUnaryTokens,
...
theorem cursor_workRunExact {language : Language}
  ...
  have hExact :=
    BuilderDynamicTokenCursorStep.CursorAdvance.advance_workRunExact
      ...
      (secondConstraintFirstLiteralSecondUnaryTokens problem) hWord
  rw [predecessor_finalOutside_eq_cursor]
  simpa [cursorWorkSteps, cursorFinalConfiguration, finalTape, finalOutside]
    using hExact

theorem trueTokenCursor_launch_workStep {language : Language}
    (problem : VerifierTableauProblem language) :
  ...
  simpa [BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine,
...
theorem suffix_workRunExact {language : Language}
    ...
    workRunExact?
      ...
      some
        { state :=
            BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine.acceptState
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem)
        BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine)
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.workSteps problem)
    (workStartConfiguration (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalConfiguration problem)
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.workRunExact problem)
  simpa [machine, BuilderFirstClausePrefix.WorkChain.machine,
    workStartConfiguration, renameConfiguration] using hTransport

theorem prefixSecondUnaryUnit_launch_workStep {language : Language}
    (problem : VerifierTableauProblem language) :
    workStep? (machine problem)
      ...
      some (renameConfiguration
        ...
        (workStartConfiguration
          BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
          (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem)
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTape problem)
  simpa [machine, BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalConfiguration,
    workStartConfiguration] using hLaunch

set_option maxRecDepth 1000000 in
/-- Every raw input follows the complete first-unary-unit trace, appends the
second unary unit, and advances the retained coordinate
once. -/
theorem workRunExact {language : Language}
  ...
  have hSuffix : workRunExact?
      ...
      (suffixWorkSteps problem)
      { state :=
          BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine.startState
        tape := prefixFinal.tape } = some suffixFinal := by
    simpa [prefixFinal, suffixFinal,
      BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalConfiguration,
...
theorem specification_secondUnaryUnit_step {language : Language}
    (problem : VerifierTableauProblem language) :
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTokenSlot problem⟩ =
      some (some CNFToken.t,
        ⟨BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTokenSlot problem + 1⟩) :=
  BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.specification_next_step problem
...
private theorem finiteIndices_drop_one_positive (width : Nat)
    (index : Fin width) (hMem : index ∈ (finiteIndices width).drop 1) :
    0 < index.val := by
  cases width with
  | zero => exact Fin.elim0 index
  | succ width =>
      simp only [finiteIndices] at hMem
      rcases List.mem_map.mp hMem with ⟨previous, _hPrevious, hIndex⟩
      subst index
      exact Nat.zero_lt_succ _

private theorem scheduledShapeConstraints_second_firstVariable_at_least_three
    {language : Language} (problem : VerifierTableauProblem language)
  ...
  | nil =>
      ...
      have hTime := problem.dimensions.timeCount_positive
      have hTape :=
        problem.dimensions.tapeWidth_positive problem.tableauInputMode
      rw [← hSecond.1]
      have hBase : 1 ≤ problem.dimensions.timeCount *
          problem.dimensions.tapeWidth problem.tableauInputMode :=
        Nat.succ_le_iff.mpr (Nat.mul_pos hTime hTape)
      exact Nat.lt_of_lt_of_le (by decide : 2 < 3)
        (Nat.mul_le_mul_right 3 hBase)
  | cons position remaining =>
      have hPositionMem :
          position ∈
            (finiteIndices
              (problem.dimensions.tapeWidth problem.tableauInputMode)).drop 1 := by
        rw [hPositions]
        simp
      have hPositionPositive := finiteIndices_drop_one_positive
        (problem.dimensions.tapeWidth problem.tableauInputMode)
        position hPositionMem
      simp [VerifierTableauProblem.symbolShapeAt,
        ...
        tapeSymbols] at hSecond
      rw [← hSecond.1]
      have hBase : 1 ≤ position.val := Nat.succ_le_iff.mpr hPositionPositive
      exact Nat.lt_of_lt_of_le (by decide : 2 < 3)
        (Nat.mul_le_mul_right 3 hBase)
...
private theorem formulaConstraintSchedule_starts_twoExactlyOne_nonempty
  ...
  have hAtLeastThree :=
    scheduledShapeConstraints_second_firstVariable_at_least_three problem
      nextVariable nextVariables rest hRest
...
private theorem formulaClauseSchedule_starts_firstConstraint_then_nextClause
  ...
  rcases formulaConstraintSchedule_starts_twoExactlyOne_nonempty problem with
    ⟨nextVariable, nextVariables, constraints, hConstraints, hAtLeastThree⟩
...
private theorem scheduledAtLeastOneClauseTokens_starts_sep_t_t_t_t
    {language : Language} (problem : VerifierTableauProblem language)
    (first : Fin problem.FormulaWidth)
    (variables : List (Fin problem.FormulaWidth))
    (hAtLeastThree : 2 < first.val) :
    ∃ rest,
      problem.scheduledClauseTokens
          (some (atLeastOneBoundedClause (first :: variables))) =
        some CNFToken.sep :: some CNFToken.t :: some CNFToken.t ::
          some CNFToken.t :: some CNFToken.t :: rest := by
  let value := first.val - 3
  have hValue : first.val = value + 3 := by
    omega
  let rest :=
    (encodeUnaryTokens value ++
  ...
  simp [VerifierTableauProblem.scheduledClauseTokens,
    ...
    encodeLiteralTokens, BoundedClause.emit, atLeastOneBoundedClause,
    trueLiteral, BoundedLiteral.emit, hValue, rest,
    List.append_assoc]
  rw [show encodeUnaryTokens (value + 3) =
      CNFToken.t :: CNFToken.t :: CNFToken.t ::
        encodeUnaryTokens value by rfl]
  rfl

private theorem formulaClauseTokens_firstConstraint_padding_then_sep_t_t_t_t
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
  ...
  rcases formulaClauseSchedule_starts_firstConstraint_then_nextClause problem with
    ⟨left, right, thirdLeft, thirdRight, fourthLeft, fourthRight,
      ...
      hNextAtLeastThree, hLeft, hRight, hThirdLeft, hThirdRight,
      hFourthLeft, hFourthRight⟩
  rcases scheduledAtLeastOneClauseTokens_starts_sep_t_t_t_t problem
      nextVariable nextVariables hNextAtLeastThree with
...
/-- The finite output is exactly the canonical token prefix through the second
unary unit of the second constraint's first literal. -/
theorem secondConstraintFirstLiteralSecondUnaryTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      secondConstraintFirstLiteralSecondUnaryTokens problem ++ rest := by
  rcases formulaClauseTokens_firstConstraint_padding_then_sep_t_t_t_t problem with
  ...
  unfold secondConstraintFirstLiteralSecondUnaryTokens
...
through the second-constraint first-literal second unary unit. -/
theorem finalTokenBits_eq_encodedFormula_secondConstraintFirstLiteralSecondUnary
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (secondConstraintFirstLiteralSecondUnaryTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 40)) := by
  rcases
      secondConstraintFirstLiteralSecondUnaryTokens_eq_canonical_formula_prefix
        problem with
    ⟨rest, hTokens⟩
  have hLength :
      (encodeTokenPairs
        (secondConstraintFirstLiteralSecondUnaryTokens problem)).length =
        2 * (problem.FormulaWidth + 40) := by
    rw [encodeTokenPairs_length, secondConstraintFirstLiteralSecondUnaryTokens,
      BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.secondConstraintFirstLiteralFirstUnaryTokens,
      BuilderSecondConstraintFirstLiteralSignStep.secondConstraintFirstLiteralSignTokens,
      BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens,
      List.length_append,
...
theorem finalTokenSlot_eq_secondConstraintStart_add_four
    {language : Language} (problem : VerifierTableauProblem language) :
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
        problem.formulaClauseSlotsPerConstraint *
          problem.formulaTokensPerClause + 4 := by
  rw [finalTokenSlot,
    BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTokenSlot_eq_secondConstraintStart_add_three]
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  rw [BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTokenSlot,
...
/-- The coordinate retained after the second unary-unit transition is the
third unary unit of an index that is at least three. -/
theorem nextTokenSlot_direct_eq_t {language : Language}
  ...
  rw [show problem.formulaVariableSlotBound + 1 +
      problem.formulaClauseSlotsPerConstraint *
          problem.formulaTokensPerClause + 4 -
      (problem.formulaVariableSlotBound + 1) =
        4 * problem.formulaTokensPerClause + emptyCount + 4 by
          rw [hDecompose]
          omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 4 * problem.formulaTokensPerClause + emptyCount + 4 - 11 =
      (problem.formulaTokensPerClause - 11) +
        3 * problem.formulaTokensPerClause + emptyCount + 4 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 11) +
      3 * problem.formulaTokensPerClause + emptyCount + 4 -
      (problem.formulaTokensPerClause - 11) =
        3 * problem.formulaTokensPerClause + emptyCount + 4 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 3 * problem.formulaTokensPerClause + emptyCount + 4 - 7 =
      (problem.formulaTokensPerClause - 7) +
        2 * problem.formulaTokensPerClause + emptyCount + 4 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 7) +
      2 * problem.formulaTokensPerClause + emptyCount + 4 -
      (problem.formulaTokensPerClause - 7) =
        2 * problem.formulaTokensPerClause + emptyCount + 4 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause + emptyCount + 4 - 8 =
      (problem.formulaTokensPerClause - 8) +
        problem.formulaTokensPerClause + emptyCount + 4 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 8) +
      problem.formulaTokensPerClause + emptyCount + 4 -
      (problem.formulaTokensPerClause - 8) =
        problem.formulaTokensPerClause + emptyCount + 4 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + emptyCount + 4 - 9 =
      (problem.formulaTokensPerClause - 9) + emptyCount + 4 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 9) + emptyCount + 4 -
      (problem.formulaTokensPerClause - 9) = emptyCount + 4 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [show (problem.formulaClauseSlotsPerConstraint - 4) *
      problem.formulaTokensPerClause = emptyCount by rfl]
  rw [if_neg (by omega)]
  rw [show emptyCount + 4 - emptyCount = 4 by omega]
  simp
...
/-- External raw-transition bound for both new bridges, the selected true-token
...
def rawTimeBound {language : Language}
  ...
  .add (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.rawTimeBound verifier)
    (.add (.constant 570)
...
private theorem predecessorTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.secondConstraintFirstLiteralFirstUnaryTokens
      problem).length = problem.FormulaWidth + 39 := by
  simp [BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.secondConstraintFirstLiteralFirstUnaryTokens,
    BuilderSecondConstraintFirstLiteralSignStep.secondConstraintFirstLiteralSignTokens,
    BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens,
    BuilderFourthClausePrefix.fourthClauseTokens,
    BuilderFourthClauseSecondLiteralPrefix.fourthClauseSecondLiteralTokens,
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.rawTimeBound problem.verifier).eval
          problem.input.length + 570 +
  ...
  simp only [cursorWord, BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.cursorWord,
    BuilderSecondConstraintFirstLiteralSignStep.cursorWord,
    BuilderSecondConstraintSeparatorStep.cursorWord,
    List.length_append, List.length_singleton]
  omega
...
private theorem appenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 86 := by
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix := BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.rawTimeBound_le problem
...
private theorem machine_isHalted_predecessor_false {language : Language}
  ...
  have hHalted :=
    BuilderFirstClausePrefix.WorkChain.machine_isHalted_first_false
      (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem)
      BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine config
  simpa [machine] using hHalted

/-- The complete first-unary-unit endpoint is still globally
nonhalting until the outer bridge launches the selected true-token appender. -/
...
private theorem appenderEndpoint_workRunExact {language : Language}
  ...
  have hPrefix : workRunExact? (machine problem)
      (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.workSteps problem) initial =
        some prefixFinal := by
  ...
  have hLaunch : workRunExact? (machine problem) 1 prefixFinal =
    ...
    simpa [prefixFinal, suffixInitial] using
      prefixSecondUnaryUnit_launch_workStep problem
  have hInner := PipelineStageBridges.workRunExact?_transport
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender
      BuilderDynamicTokenCursorStep.CursorAdvance.machine)
  ...
  have hGlobal := PipelineStageBridges.workRunExact?_transport
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine (machine problem)
    BuilderFirstClausePrefix.WorkChain.secondState
    (BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
      (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem) BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine)
    (appenderWorkSteps problem)
    (workStartConfiguration BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
      (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.finalTape problem))
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.firstState
  ...
  have h01 := PipelineMachineSimulation.workRunExact?_compose
    (machine problem) (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.workSteps problem) 1
...
private theorem machine_isHalted_appender_false {language : Language}
  ...
  have hGlobal :=
    BuilderFirstClausePrefix.WorkChain.machine_isHalted_second_false_of_local
      (BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.machine problem) BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
      (renameConfiguration BuilderFirstClausePrefix.WorkChain.firstState
        config) (by simpa [BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine] using hInner)
  simpa [machine] using hGlobal

/-- The selected true-token appender endpoint remains a timeout until the
...
private theorem selectedAppender_stuck_timeout {language : Language}
  ...
  have hSelectedHalted : BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender.isHalted bad = false := by
    change BuilderTokenAppender.machine.isHalted bad = false
    exact hLocalHalted
  have hSelectedStep : workStep? BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender bad = none := by
    change workStep? BuilderTokenAppender.machine bad = none
    exact hLocalStep
  have hAccept := state_ne_accept_of_not_halted
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender bad hSelectedHalted
  have hInnerStep : workStep? BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine innerConfig = none := by
    ...
    simpa [BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine, innerConfig] using hStep
  have hInnerHalted : BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine.isHalted innerConfig = false := by
    have hHalted :=
      BuilderFirstClausePrefix.WorkChain.machine_isHalted_first_false
        BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender
        BuilderDynamicTokenCursorStep.CursorAdvance.machine bad
    simpa [BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine, innerConfig] using hHalted
...
/-- A malformed tally-phase symbol in the selected true-token appender is
...
/-- A malformed output-phase symbol in the selected true-token appender also
...
private theorem globalCursorMalformed_workStep {language : Language}
  ...
  have hGlobal := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    ...
    (by simpa [BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine] using hInner)
...
private theorem globalCursorDead_workStep {language : Language}
  ...
  have hGlobal := BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
    ...
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.secondState
      (cursorDeadConfiguration tape)) (by
        simpa [BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine] using hInner)
  simpa [machine] using hGlobal
...
end BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep
