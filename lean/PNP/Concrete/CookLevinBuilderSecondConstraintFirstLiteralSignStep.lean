...
One populated token-cursor transition at the first literal of the second
canonical Cook--Levin constraint.

The machine in this file composes the complete second-constraint separator
step with the already-audited selected true-token appender and unary cursor
advance.  Every raw input therefore emits the positive sign beginning the
constraint's first literal and leaves the retained coordinate on the first
unary-index token.  This is one fixed populated transition, not a complete
literal, a general schedule decoder, a complete cursor loop, or a complete
formula builder.
-/
...
namespace BuilderSecondConstraintFirstLiteralSignStep
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondConstraintSeparatorStep.machine problem) := by
  exact BuilderSecondConstraintSeparatorStep.rule_source_ne_acceptState problem
...
/-- One literal finite work machine from raw input through the positive sign
beginning the first literal of constraint two. -/
def machine {language : Language}
  ...
  BuilderFirstClausePrefix.WorkChain.machine
    (BuilderSecondConstraintSeparatorStep.machine problem)
      ...
      4676 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderFirstConstraintPaddingRun.secondConstraintStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderSecondConstraintSeparatorStep.rules_length problem
  have hSuffix :=
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondConstraintSeparatorStep.machine problem)
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
    (BuilderSecondConstraintSeparatorStep.rules_pairwise_query_distinct problem)
...
private theorem predecessor_finalOutside_eq_cursor {language : Language}
  ...
  simp only [BuilderSecondConstraintSeparatorStep.finalOutside,
    List.length_append, List.length_singleton]
  rw [show
      (BuilderSecondConstraintSeparatorStep.cursorWord problem).length + 1 + 1 =
        (BuilderSecondConstraintSeparatorStep.cursorWord problem).length + 2 by
          omega]
...
def finalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input (finalOutside problem)
    (secondConstraintFirstLiteralSignTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondConstraintSeparatorStep.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (secondConstraintFirstLiteralSignTokens problem)
...
theorem appender_workRunExact {language : Language}
    ...
    workRunExact?
        ...
        (workStartConfiguration
          BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender
          (BuilderSecondConstraintSeparatorStep.finalTape problem)) =
  ...
  have hExact := BuilderTokenAppender.appendToken_workRunExact
    problem.input (BuilderSecondConstraintSeparatorStep.finalOutside problem)
    (BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens problem)
    .t
  rw [hRunEq]
  simpa [BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender,
    appenderWorkSteps, BuilderSecondConstraintSeparatorStep.finalTape,
    BuilderTokenAppender.entryConfiguration,
    BuilderTokenAppender.finalConfiguration,
    appenderFinalConfiguration, secondConstraintFirstLiteralSignTokens,
    ...
    · exact BuilderUnaryPolynomial.scratchWord_symbol
        (BuilderFirstConstraintPaddingRun.secondConstraintStartPolynomial
          problem.verifier) problem.input.length symbol (by
            simpa [BuilderSecondConstraintSeparatorStep.cursorWord] using hBase)
    · exact Or.inl hUnit
  have hExact :=
    BuilderDynamicTokenCursorStep.CursorAdvance.advance_workRunExact
      ...
      (secondConstraintFirstLiteralSignTokens problem) hWord
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondConstraintSeparatorStep.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderSecondConstraintSeparatorStep.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderSecondConstraintSeparatorStep.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderSecondConstraintSeparatorStep.machine problem)
        BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine)
    (BuilderSecondConstraintSeparatorStep.workSteps problem)
    (workStartConfiguration (BuilderSecondConstraintSeparatorStep.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderSecondConstraintSeparatorStep.finalConfiguration problem)
    (BuilderSecondConstraintSeparatorStep.workRunExact problem)
  simpa [machine, BuilderFirstClausePrefix.WorkChain.machine,
    workStartConfiguration, renameConfiguration] using hTransport

theorem prefixSign_launch_workStep {language : Language}
    (problem : VerifierTableauProblem language) :
    workStep? (machine problem)
      ...
      some (renameConfiguration
        ...
        (workStartConfiguration
          BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
          (BuilderSecondConstraintSeparatorStep.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderSecondConstraintSeparatorStep.machine problem)
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
    (BuilderSecondConstraintSeparatorStep.finalTape problem)
  simpa [machine, BuilderSecondConstraintSeparatorStep.finalConfiguration,
    workStartConfiguration] using hLaunch

set_option maxRecDepth 1000000 in
/-- Every raw input follows the complete second-constraint separator trace,
appends the first positive sign, and advances the retained unary coordinate
once. -/
...
theorem specification_sign_step {language : Language}
    (problem : VerifierTableauProblem language) :
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondConstraintSeparatorStep.finalTokenSlot problem⟩ =
      some (some CNFToken.t,
        ⟨BuilderSecondConstraintSeparatorStep.finalTokenSlot problem + 1⟩) :=
  BuilderSecondConstraintSeparatorStep.specification_next_step problem
...
private theorem scheduledShapeConstraints_second_firstVariable_positive
    {language : Language} (problem : VerifierTableauProblem language)
  ...
  | nil =>
      ...
      have hTape :=
        problem.dimensions.tapeWidth_positive problem.tableauInputMode
      rw [← hSecond.1]
      exact Nat.mul_pos (Nat.mul_pos hTime hTape) (by decide)
  | cons position remaining =>
      ...
      simp [VerifierTableauProblem.symbolShapeAt,
        ...
        tapeSymbols] at hSecond
      rw [← hSecond.1]
      exact Nat.mul_pos hPositionPositive (by decide)
...
private theorem formulaConstraintSchedule_starts_twoExactlyOne_nonempty
  ...
  have hPositive :=
    scheduledShapeConstraints_second_firstVariable_positive problem
      nextVariable nextVariables rest hRest
...
private theorem formulaClauseSchedule_starts_firstConstraint_then_nextClause
  ...
  rcases formulaConstraintSchedule_starts_twoExactlyOne_nonempty problem with
    ⟨nextVariable, nextVariables, constraints, hConstraints, hPositive⟩
...
private theorem scheduledAtLeastOneClauseTokens_starts_sep_t_t
    {language : Language} (problem : VerifierTableauProblem language)
    (first : Fin problem.FormulaWidth)
    (variables : List (Fin problem.FormulaWidth))
    (hPositive : 0 < first.val) :
    ∃ rest,
      problem.scheduledClauseTokens
          (some (atLeastOneBoundedClause (first :: variables))) =
        some CNFToken.sep :: some CNFToken.t :: some CNFToken.t :: rest := by
  have hNonzero : first.val ≠ 0 := Nat.ne_of_gt hPositive
  rcases Nat.exists_eq_succ_of_ne_zero hNonzero with ⟨value, hValue⟩
  let rest :=
  ...
  simp [VerifierTableauProblem.scheduledClauseTokens,
    ...
    List.append_assoc]
  rw [show encodeUnaryTokens (value + 1) =
      CNFToken.t :: encodeUnaryTokens value by rfl]
  rfl

private theorem formulaClauseTokens_firstConstraint_padding_then_sep_t_t
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
  ...
  rcases formulaClauseSchedule_starts_firstConstraint_then_nextClause problem with
    ⟨left, right, thirdLeft, thirdRight, fourthLeft, fourthRight,
      ...
      hNextPositive, hLeft, hRight, hThirdLeft, hThirdRight,
      hFourthLeft, hFourthRight⟩
  rcases scheduledAtLeastOneClauseTokens_starts_sep_t_t problem
      nextVariable nextVariables hNextPositive with
...
positive sign beginning the second constraint's first literal. -/
theorem secondConstraintFirstLiteralSignTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      secondConstraintFirstLiteralSignTokens problem ++ rest := by
  rcases formulaClauseTokens_firstConstraint_padding_then_sep_t_t problem with
  ...
  unfold secondConstraintFirstLiteralSignTokens
...
through the second-constraint first-literal sign. -/
theorem finalTokenBits_eq_encodedFormula_secondConstraintFirstLiteralSign
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (secondConstraintFirstLiteralSignTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 38)) := by
  rcases
  ...
  have hLength :
      (encodeTokenPairs
        (secondConstraintFirstLiteralSignTokens problem)).length =
        2 * (problem.FormulaWidth + 38) := by
    rw [encodeTokenPairs_length, secondConstraintFirstLiteralSignTokens,
      BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens,
      List.length_append,
      BuilderFourthClausePrefix.fourthClauseTokens]
...
theorem finalTokenSlot_eq_secondConstraintStart_add_two
    {language : Language} (problem : VerifierTableauProblem language) :
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
        problem.formulaClauseSlotsPerConstraint *
          problem.formulaTokensPerClause + 2 := by
  rw [finalTokenSlot,
    BuilderSecondConstraintSeparatorStep.finalTokenSlot_eq_secondConstraintStart_add_one]
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  rw [BuilderSecondConstraintSeparatorStep.finalTokenSlot,
    hPrefix, List.replicate_succ', List.replicate_succ']
  simp [BuilderFirstConstraintPaddingRun.secondConstraintStart,
    List.append_assoc]

/-- The coordinate retained after the sign transition is the first unary unit
of the positive literal's nonzero variable index. -/
theorem nextTokenSlot_direct_eq_t {language : Language}
  ...
  rw [show problem.formulaVariableSlotBound + 1 +
      ...
      (problem.formulaVariableSlotBound + 1) =
        4 * problem.formulaTokensPerClause + emptyCount + 2 by
          rw [hDecompose]
          omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 4 * problem.formulaTokensPerClause + emptyCount + 2 - 11 =
      (problem.formulaTokensPerClause - 11) +
        3 * problem.formulaTokensPerClause + emptyCount + 2 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 11) +
      3 * problem.formulaTokensPerClause + emptyCount + 2 -
      (problem.formulaTokensPerClause - 11) =
        3 * problem.formulaTokensPerClause + emptyCount + 2 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 3 * problem.formulaTokensPerClause + emptyCount + 2 - 7 =
      (problem.formulaTokensPerClause - 7) +
        2 * problem.formulaTokensPerClause + emptyCount + 2 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 7) +
      2 * problem.formulaTokensPerClause + emptyCount + 2 -
      (problem.formulaTokensPerClause - 7) =
        2 * problem.formulaTokensPerClause + emptyCount + 2 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause + emptyCount + 2 - 8 =
      (problem.formulaTokensPerClause - 8) +
        problem.formulaTokensPerClause + emptyCount + 2 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 8) +
      problem.formulaTokensPerClause + emptyCount + 2 -
      (problem.formulaTokensPerClause - 8) =
        problem.formulaTokensPerClause + emptyCount + 2 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + emptyCount + 2 - 9 =
      (problem.formulaTokensPerClause - 9) + emptyCount + 2 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 9) + emptyCount + 2 -
      (problem.formulaTokensPerClause - 9) = emptyCount + 2 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [show (problem.formulaClauseSlotsPerConstraint - 4) *
      problem.formulaTokensPerClause = emptyCount by rfl]
  rw [if_neg (by omega)]
  rw [show emptyCount + 2 - emptyCount = 2 by omega]
  simp
...
def rawTimeBound {language : Language}
  ...
  .add (BuilderSecondConstraintSeparatorStep.rawTimeBound verifier)
    (.add (.constant 546)
...
private theorem predecessorTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens
      problem).length = problem.FormulaWidth + 37 := by
  simp [BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens,
    BuilderFourthClausePrefix.fourthClauseTokens,
    BuilderFourthClauseSecondLiteralPrefix.fourthClauseSecondLiteralTokens,
    BuilderFourthClauseSecondLiteralPrefix.secondUnaryTokenOutput,
    BuilderFourthClauseSecondLiteralPrefix.firstUnaryTokenOutput,
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderSecondConstraintSeparatorStep.rawTimeBound problem.verifier).eval
          problem.input.length + 546 +
  ...
  simp only [cursorWord, BuilderSecondConstraintSeparatorStep.cursorWord,
    List.length_append, List.length_singleton]
  omega
...
private theorem appenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 82 := by
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix := BuilderSecondConstraintSeparatorStep.rawTimeBound_le problem
...
private theorem appenderEndpoint_workRunExact {language : Language}
  ...
  have hPrefix : workRunExact? (machine problem)
      (BuilderSecondConstraintSeparatorStep.workSteps problem) initial =
        some prefixFinal := by
  ...
  have hLaunch : workRunExact? (machine problem) 1 prefixFinal =
    ...
    simpa [prefixFinal, suffixInitial] using
      prefixSign_launch_workStep problem
  ...
  have hGlobal := PipelineStageBridges.workRunExact?_transport
    ...
    (BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
      (BuilderSecondConstraintSeparatorStep.machine problem) BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine)
    (appenderWorkSteps problem)
    (workStartConfiguration BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
      (BuilderSecondConstraintSeparatorStep.finalTape problem))
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.firstState
  ...
  have h01 := PipelineMachineSimulation.workRunExact?_compose
    (machine problem) (BuilderSecondConstraintSeparatorStep.workSteps problem) 1
...
end BuilderSecondConstraintFirstLiteralSignStep
