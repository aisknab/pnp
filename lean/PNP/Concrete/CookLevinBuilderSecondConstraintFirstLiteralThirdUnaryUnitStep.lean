...
second-unary-unit step with the already-audited selected true-token appender and
unary cursor advance.  Every raw input therefore emits the third unary unit
of that literal's index and leaves the retained coordinate on its terminating
false token.
...
namespace BuilderSecondConstraintFirstLiteralThirdUnaryUnitStep
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.machine problem) := by
  exact BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.rule_source_ne_acceptState problem
...
/-- One literal finite work machine from raw input through the third unary unit
...
def machine {language : Language}
  ...
  BuilderFirstClausePrefix.WorkChain.machine
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.machine problem)
      ...
      5042 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderFirstConstraintPaddingRun.secondConstraintStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.rules_length problem
  have hSuffix :=
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.machine problem)
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.rules_pairwise_query_distinct problem)
...
def secondConstraintFirstLiteralThirdUnaryTokens {language : Language}
    (problem : VerifierTableauProblem language) : List CNFToken :=
  BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.secondConstraintFirstLiteralSecondUnaryTokens problem ++
    [.t]

def cursorWord {language : Language}
    (problem : VerifierTableauProblem language) : List WorkSymbol :=
  BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.cursorWord problem ++
    [BuilderUnaryPolynomial.unitSymbol]

private def cursorOutsideTail {language : Language}
    (problem : VerifierTableauProblem language) : List WorkSymbol :=
  (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalOutside problem).drop
    ((cursorWord problem).length + 1)

private theorem predecessor_finalOutside_eq_cursor {language : Language}
    (problem : VerifierTableauProblem language) :
    BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalOutside problem =
  ...
  simp only [BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalOutside,
    List.length_append, List.length_singleton]
  rw [show
      (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.cursorWord problem).length + 1 + 1 =
        (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.cursorWord problem).length + 2 by
          omega]
...
def finalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input (finalOutside problem)
    (secondConstraintFirstLiteralThirdUnaryTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (secondConstraintFirstLiteralThirdUnaryTokens problem)
...
theorem appender_workRunExact {language : Language}
    ...
    workRunExact?
        ...
        (workStartConfiguration
          BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender
          (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTape problem)) =
  ...
  have hExact := BuilderTokenAppender.appendToken_workRunExact
    problem.input (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalOutside problem)
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.secondConstraintFirstLiteralSecondUnaryTokens problem)
    .t
  rw [hRunEq]
  simpa [BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.appender,
    appenderWorkSteps, BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTape,
    BuilderTokenAppender.entryConfiguration,
    BuilderTokenAppender.finalConfiguration,
    appenderFinalConfiguration, secondConstraintFirstLiteralThirdUnaryTokens,
...
theorem cursor_workRunExact {language : Language}
  ...
  have hExact :=
    BuilderDynamicTokenCursorStep.CursorAdvance.advance_workRunExact
      ...
      (secondConstraintFirstLiteralThirdUnaryTokens problem) hWord
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.machine problem)
        BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine)
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.workSteps problem)
    (workStartConfiguration (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalConfiguration problem)
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.workRunExact problem)
  simpa [machine, BuilderFirstClausePrefix.WorkChain.machine,
    workStartConfiguration, renameConfiguration] using hTransport

theorem prefixThirdUnaryUnit_launch_workStep {language : Language}
    (problem : VerifierTableauProblem language) :
    workStep? (machine problem)
      ...
      some (renameConfiguration
        ...
        (workStartConfiguration
          BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
          (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.machine problem)
    BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTape problem)
  simpa [machine, BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalConfiguration,
    workStartConfiguration] using hLaunch

set_option maxRecDepth 1000000 in
/-- Every raw input follows the complete second-unary-unit trace, appends the
third unary unit, and advances the retained coordinate
once. -/
theorem workRunExact {language : Language}
  ...
  have hSuffix : workRunExact?
    ...
    simpa [prefixFinal, suffixFinal,
      BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalConfiguration,
...
/-- The populated third-unary opportunity executed by the suffix agrees with the direct
formula-token cursor specification. -/
theorem specification_thirdUnaryUnit_step {language : Language}
    (problem : VerifierTableauProblem language) :
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTokenSlot problem⟩ =
      some (some CNFToken.t,
        ⟨BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTokenSlot problem + 1⟩) :=
  BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.specification_next_step problem
...
private theorem scheduledShapeConstraints_second_firstVariable_eq_three
    {language : Language} (problem : VerifierTableauProblem language)
...
private theorem formulaConstraintSchedule_starts_twoExactlyOne_nonempty
  ...
  have hEqualsThree :=
    scheduledShapeConstraints_second_firstVariable_eq_three problem
      nextVariable nextVariables rest hRest
...
private theorem scheduledAtLeastOneClauseTokens_starts_sep_t_t_t_t_f
    {language : Language} (problem : VerifierTableauProblem language)
    ...
    ∃ rest,
      problem.scheduledClauseTokens
        ...
        some CNFToken.sep :: some CNFToken.t :: some CNFToken.t ::
          some CNFToken.t :: some CNFToken.t :: some CNFToken.f :: rest := by
  let rest :=
...
private theorem formulaClauseTokens_firstConstraint_padding_then_sep_t_t_t_t_f
  ...
  rcases formulaClauseSchedule_starts_firstConstraint_then_nextClause problem with
    ⟨left, right, thirdLeft, thirdRight, fourthLeft, fourthRight,
      ...
      hNextEqualsThree, hLeft, hRight, hThirdLeft, hThirdRight,
      hFourthLeft, hFourthRight⟩
  rcases scheduledAtLeastOneClauseTokens_starts_sep_t_t_t_t_f problem
...
/-- The finite output is exactly the canonical token prefix through the third
unary unit of the second constraint's first literal. -/
theorem secondConstraintFirstLiteralThirdUnaryTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      secondConstraintFirstLiteralThirdUnaryTokens problem ++ rest := by
  ...
  unfold secondConstraintFirstLiteralThirdUnaryTokens
...
through the second-constraint first-literal third unary unit. -/
theorem finalTokenBits_eq_encodedFormula_secondConstraintFirstLiteralThirdUnary
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (secondConstraintFirstLiteralThirdUnaryTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 41)) := by
  rcases
  ...
  have hLength :
      (encodeTokenPairs
        (secondConstraintFirstLiteralThirdUnaryTokens problem)).length =
        2 * (problem.FormulaWidth + 41) := by
    rw [encodeTokenPairs_length, secondConstraintFirstLiteralThirdUnaryTokens,
      BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.secondConstraintFirstLiteralSecondUnaryTokens,
      BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.secondConstraintFirstLiteralFirstUnaryTokens,
      BuilderSecondConstraintFirstLiteralSignStep.secondConstraintFirstLiteralSignTokens,
      BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens,
...
theorem finalTokenSlot_eq_secondConstraintStart_add_five
    {language : Language} (problem : VerifierTableauProblem language) :
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
        problem.formulaClauseSlotsPerConstraint *
          problem.formulaTokensPerClause + 5 := by
  rw [finalTokenSlot,
    BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTokenSlot_eq_secondConstraintStart_add_four]
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  rw [BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTokenSlot,
...
/-- The coordinate retained after the third unary-unit transition is the
terminating false token of variable index three. -/
theorem nextTokenSlot_direct_eq_f {language : Language}
  ...
  rw [show problem.formulaVariableSlotBound + 1 +
      problem.formulaClauseSlotsPerConstraint *
          problem.formulaTokensPerClause + 5 -
      (problem.formulaVariableSlotBound + 1) =
        4 * problem.formulaTokensPerClause + emptyCount + 5 by
          rw [hDecompose]
          omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 4 * problem.formulaTokensPerClause + emptyCount + 5 - 11 =
      (problem.formulaTokensPerClause - 11) +
        3 * problem.formulaTokensPerClause + emptyCount + 5 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 11) +
      3 * problem.formulaTokensPerClause + emptyCount + 5 -
      (problem.formulaTokensPerClause - 11) =
        3 * problem.formulaTokensPerClause + emptyCount + 5 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 3 * problem.formulaTokensPerClause + emptyCount + 5 - 7 =
      (problem.formulaTokensPerClause - 7) +
        2 * problem.formulaTokensPerClause + emptyCount + 5 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 7) +
      2 * problem.formulaTokensPerClause + emptyCount + 5 -
      (problem.formulaTokensPerClause - 7) =
        2 * problem.formulaTokensPerClause + emptyCount + 5 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause + emptyCount + 5 - 8 =
      (problem.formulaTokensPerClause - 8) +
        problem.formulaTokensPerClause + emptyCount + 5 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 8) +
      problem.formulaTokensPerClause + emptyCount + 5 -
      (problem.formulaTokensPerClause - 8) =
        problem.formulaTokensPerClause + emptyCount + 5 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + emptyCount + 5 - 9 =
      (problem.formulaTokensPerClause - 9) + emptyCount + 5 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 9) + emptyCount + 5 -
      (problem.formulaTokensPerClause - 9) = emptyCount + 5 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [show (problem.formulaClauseSlotsPerConstraint - 4) *
      problem.formulaTokensPerClause = emptyCount by rfl]
  rw [if_neg (by omega)]
  rw [show emptyCount + 5 - emptyCount = 5 by omega]
  simp
...
def rawTimeBound {language : Language}
  ...
  .add (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.rawTimeBound verifier)
    (.add (.constant 582)
...
private theorem predecessorTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.secondConstraintFirstLiteralSecondUnaryTokens
      problem).length = problem.FormulaWidth + 40 := by
  simp [BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.secondConstraintFirstLiteralSecondUnaryTokens,
    BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.secondConstraintFirstLiteralFirstUnaryTokens,
    BuilderSecondConstraintFirstLiteralSignStep.secondConstraintFirstLiteralSignTokens,
    BuilderSecondConstraintSeparatorStep.secondConstraintStartTokens,
    BuilderFourthClausePrefix.fourthClauseTokens,
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.rawTimeBound problem.verifier).eval
          problem.input.length + 582 +
  ...
  simp only [cursorWord, BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.cursorWord,
    BuilderSecondConstraintFirstLiteralFirstUnaryUnitStep.cursorWord,
    BuilderSecondConstraintFirstLiteralSignStep.cursorWord,
    BuilderSecondConstraintSeparatorStep.cursorWord,
    List.length_append, List.length_singleton]
...
private theorem appenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 88 := by
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix := BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.rawTimeBound_le problem
...
private theorem appenderEndpoint_workRunExact {language : Language}
  ...
  have hPrefix : workRunExact? (machine problem)
      (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.workSteps problem) initial =
        some prefixFinal := by
  ...
  have hLaunch : workRunExact? (machine problem) 1 prefixFinal =
    ...
    simpa [prefixFinal, suffixInitial] using
      prefixThirdUnaryUnit_launch_workStep problem
  ...
  have hGlobal := PipelineStageBridges.workRunExact?_transport
    ...
    (BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
      (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.machine problem) BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine)
    (appenderWorkSteps problem)
    (workStartConfiguration BuilderSecondClauseSecondLiteralPrefix.TrueTokenCursor.machine
      (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.finalTape problem))
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.firstState
  ...
  have h01 := PipelineMachineSimulation.workRunExact?_compose
    (machine problem) (BuilderSecondConstraintFirstLiteralSecondUnaryUnitStep.workSteps problem) 1
...
end BuilderSecondConstraintFirstLiteralThirdUnaryUnitStep
