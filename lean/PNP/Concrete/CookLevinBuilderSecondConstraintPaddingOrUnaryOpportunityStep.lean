...
One width-dependent padding-or-unary opportunity after the first literal of
the second canonical Cook--Levin constraint.
...
first unary unit.  Both branches then materialize the following formula
coordinate.  No host-side schedule lookup or caller-supplied branch
certificate is used.

This processes exactly one schedule opportunity.  It does not process the
following padding or unary token, complete the second literal, traverse the
second constraint, implement a general schedule cursor, complete the formula
builder, or establish P = NP.
-/
...
namespace BuilderSecondConstraintPaddingOrUnaryOpportunityStep
...
theorem opportunitySlot_eq_secondConstraintStart_add_eight
  ...
  change
    BuilderSecondConstraintFirstLiteralSuccessorTokenStep.successorTokenSlot
        problem +
      1 = _
  rw [
    BuilderSecondConstraintFirstLiteralSuccessorTokenStep.successorTokenSlot_eq_secondConstraintStart_add_seven]

/-! ### One controller with a skip exit and one literal-appender entry -/
...
theorem controller_done_skip_workStep (tape : WorkTape) :
  ...
  have hBridge := findWorkRule_launchRules
    (WorkChain.firstState
      BuilderCompleteHeader.HeaderController.doneExitState)
    (WorkChain.secondState
      BuilderTokenAppender.machine.acceptState) tape.head
  have hFind : findWorkRule machine.rules start.state start.tape.head =
      some (launchRule
        (WorkChain.firstState
          BuilderCompleteHeader.HeaderController.doneExitState)
        (WorkChain.secondState
          BuilderTokenAppender.machine.acceptState) tape.head) := by
...
def widthBranchMachine {language : Language}
  ...
  BuilderFirstClausePrefix.WorkChain.machine
    (widthEvaluator problem) WidthOptionalAppender.machine
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.machine problem) := by
  exact
    BuilderSecondConstraintFirstLiteralSuccessorTokenStep.rule_source_ne_acceptState
      problem
...
private theorem branch_noRuleAtAccept :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      WidthOptionalAppender.machine :=
  WidthOptionalAppender.rule_source_ne_acceptState
...
theorem rules_length {language : Language}
  ...
  have hPredecessor :=
    BuilderSecondConstraintFirstLiteralSuccessorTokenStep.rules_length problem
  have hWidth := BuilderUnaryPolynomial.rules_length (widthPolynomial problem)
  have hTarget := BuilderUnaryPolynomial.rules_length
    (opportunitySlotPolynomial problem.verifier)
  have hBranch := WidthOptionalAppender.rules_length
  ...
  simp only [List.length_append, List.length_map, widthEvaluator,
    targetEvaluator, WidthOptionalAppender.machine,
    BuilderUnaryPolynomial.machine]
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  have hWidthBranch :=
    BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
      ...
      WidthOptionalAppender.rules_pairwise_query_distinct
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.machine problem)
    (suffixMachine problem)
    (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.rules_pairwise_query_distinct
      problem)
...
def secondConstraintPaddingOrUnaryTokens {language : Language}
    (problem : VerifierTableauProblem language) : List CNFToken :=
...
def widthOutside {language : Language}
  ...
  BuilderUnaryPolynomial.finalOutsideLeft
    ...
    (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalOutside problem)
...
def controllerFinalOutside {language : Language}
  ...
  (widthWord problem).take (widthRootPrefixLength problem) ++
    ...
    [BuilderUnaryPolynomial.scratchEndSymbol,
     BuilderUnaryPolynomial.scratchEndSymbol] ++
    (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalOutside problem).drop
...
def branchFinalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input
    ...
    (secondConstraintPaddingOrUnaryTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondConstraintFirstLiteralSuccessorTokenStep.workSteps problem + 1 +
...
theorem widthEvaluator_workRunExact {language : Language}
    ...
    workRunExact? (widthEvaluator problem) (widthWorkSteps problem)
        (BuilderUnaryPolynomial.initialConfiguration
          (widthPolynomial problem) problem.input
          (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalOutside
            problem)
          (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.secondConstraintFirstLiteralSuccessorTokens
            problem)) =
      some
...
theorem optionalAppender_workRunExact {language : Language}
    (problem : VerifierTableauProblem language) :
    workRunExact? WidthOptionalAppender.machine (branchWorkSteps problem)
        (workStartConfiguration WidthOptionalAppender.machine
          (widthTape problem)) =
      some
        { state := WidthOptionalAppender.machine.acceptState
  ...
  | succ remaining =>
      ...
      have hRun := WidthOptionalAppender.workRunExact problem.input
      ...
      | zero =>
          ...
          simpa [hBranchWorkSteps, controllerWorkSteps,
            ...
            secondConstraintPaddingOrUnaryTokens, hOpportunityOutput,
    ...
    workRunExact? (targetEvaluator problem) (targetWorkSteps problem)
      ...
      some
        (BuilderUnaryPolynomial.finalConfiguration
          (opportunitySlotPolynomial problem.verifier) problem.input
          (controllerFinalOutside problem)
          (secondConstraintPaddingOrUnaryTokens problem)) := by
  exact BuilderUnaryPolynomial.workRunExact
    (opportunitySlotPolynomial problem.verifier) problem.input
    (controllerFinalOutside problem)
    (secondConstraintPaddingOrUnaryTokens problem)
...
private theorem widthBranch_workRunExact {language : Language}
    ...
    workRunExact? (widthBranchMachine problem)
        ...
        (workStartConfiguration (widthBranchMachine problem)
          (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalTape
            problem)) =
      some
  ...
  let widthFinal := BuilderUnaryPolynomial.finalConfiguration
    ...
    (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.secondConstraintFirstLiteralSuccessorTokens
      problem)
  let branchInitial := workStartConfiguration WidthOptionalAppender.machine
    (widthTape problem)
  ...
  have hBranch : workRunExact? WidthOptionalAppender.machine
...
theorem suffix_workRunExact {language : Language}
  ...
  let widthBranchInitial := workStartConfiguration
    ...
    (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalTape problem)
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.workSteps problem)
      ...
      some
        (renameConfiguration
          ...
          (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalConfiguration
            problem)) := by
  ...
  have hRun := hTransport
    ...
    (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalConfiguration
      problem)
    (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.workRunExact problem)
  exact hRun
...
theorem workRunExact {language : Language}
  ...
  let prefixInitial := workStartConfiguration
    ...
    (rawInputWorkTape problem.input)
  let prefixFinal :=
    BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalConfiguration problem
...
theorem specification_opportunity_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalTokenSlot
          problem⟩ =
      some ((if problem.dimensions.tapeWidth problem.tableauInputMode = 1
        ...
        ⟨BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalTokenSlot
          problem + 1⟩) := by
  exact
    BuilderSecondConstraintFirstLiteralSuccessorTokenStep.specification_following_step
      problem

/-- The finite output is exactly the canonical token prefix after consuming
the width-selected padding-or-unary opportunity. -/
theorem secondConstraintPaddingOrUnaryTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      secondConstraintPaddingOrUnaryTokens problem ++ rest := by
  rcases
      BuilderSecondConstraintFirstLiteralTerminatorStep.encodeCNFTokens_eq_terminator_then_successor_and_optional_unary
        problem with
    ⟨rest, hTokens⟩
  refine ⟨rest, ?_⟩
  unfold secondConstraintPaddingOrUnaryTokens
    BuilderSecondConstraintFirstLiteralSuccessorTokenStep.secondConstraintFirstLiteralSuccessorTokens
  rw [
...
theorem finalTokenSlot_eq_predecessor_add_one {language : Language}
    ...
    finalTokenSlot problem =
      BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalTokenSlot problem +
        1 := by
  rw [finalTokenSlot, opportunitySlot_eq_secondConstraintStart_add_eight,
    BuilderSecondConstraintFirstLiteralSuccessorTokenStep.finalTokenSlot_eq_secondConstraintStart_add_seven]

theorem finalTokenSlot_eq_secondConstraintStart_add_eight
    {language : Language} (problem : VerifierTableauProblem language) :
    finalTokenSlot problem =
      problem.formulaVariableSlotBound + 1 +
        problem.formulaClauseSlotsPerConstraint *
          problem.formulaTokensPerClause + 8 := by
  exact opportunitySlot_eq_secondConstraintStart_add_eight problem

/-- The retained coordinate identifies the opportunity after the processed
slot: another padding opportunity at width one, otherwise the second unary
`T` of the second literal. -/
theorem followingTokenSlot_direct_eq_padding_or_t {language : Language}
  ...
  have hFollowing :=
    BuilderSecondConstraintFirstLiteralTerminatorStep.secondFollowingTokenSlot_direct_eq_padding_or_t
      problem
...
def rawTimeBound {language : Language}
  ...
  .add
    (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.rawTimeBound verifier)
    (.add (.constant 612)
...
after the width-selected padding-or-unary opportunity. -/
theorem finalTokenBits_eq_encodedFormula_secondConstraintPaddingOrUnary
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (secondConstraintPaddingOrUnaryTokens problem) =
  ...
  have hLength :
      (encodeTokenPairs
        (secondConstraintPaddingOrUnaryTokens problem)).length =
        2 * (problem.FormulaWidth + 43 +
          if problem.dimensions.tapeWidth problem.tableauInputMode = 1
            then 0 else 1) := by
    rw [encodeTokenPairs_length,
      secondConstraintPaddingOrUnaryTokens, List.length_append,
...
private theorem appenderWorkSteps_le {language : Language}
    ...
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 94 := by
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderSecondConstraintFirstLiteralSuccessorTokenStep.rawTimeBound
        problem.verifier).eval problem.input.length +
      612 + 24 * problem.input.length + 12 * problem.FormulaWidth +
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix :=
    BuilderSecondConstraintFirstLiteralSuccessorTokenStep.rawTimeBound_le problem
...
end BuilderSecondConstraintPaddingOrUnaryOpportunityStep
