...
The sixth width-dependent padding-or-token opportunity after the first
...
already-audited token appender at its `T` state and emits the opening token
of the following positive literal.  Both branches then materialize the
following formula coordinate.  No host-side schedule lookup or caller-supplied
branch certificate is used.

This processes exactly one additional schedule opportunity.  It does not
process the following padding or unary-index `T`, complete the following
...
namespace BuilderSecondConstraintSixthPaddingOrOpeningUnaryOpportunityStep
...
theorem opportunitySlot_eq_secondConstraintStart_add_thirteen
  ...
  change
    BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.opportunitySlot
        problem +
      1 = _
  rw [
    BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.opportunitySlot_eq_secondConstraintStart_add_twelve]
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.machine problem) := by
  exact
    BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.rule_source_ne_acceptState
      problem
...
theorem rules_length {language : Language}
  ...
  have hPredecessor :=
    BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.rules_length problem
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.machine problem)
    (suffixMachine problem)
    (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.rules_pairwise_query_distinct
      problem)
...
def secondConstraintSixthPaddingOrOpeningUnaryTokens {language : Language}
    (problem : VerifierTableauProblem language) : List CNFToken :=
...
def widthOutside {language : Language}
  ...
  BuilderUnaryPolynomial.finalOutsideLeft
    ...
    (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalOutside problem)
...
def controllerFinalOutside {language : Language}
  ...
  (widthWord problem).take (widthRootPrefixLength problem) ++
    ...
    [BuilderUnaryPolynomial.scratchEndSymbol,
     BuilderUnaryPolynomial.scratchEndSymbol] ++
    (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalOutside problem).drop
...
def branchFinalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input
    ...
    (secondConstraintSixthPaddingOrOpeningUnaryTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.workSteps problem + 1 +
...
theorem widthEvaluator_workRunExact {language : Language}
    ...
    workRunExact? (widthEvaluator problem) (widthWorkSteps problem)
        (BuilderUnaryPolynomial.initialConfiguration
          (widthPolynomial problem) problem.input
          (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalOutside
            problem)
          (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.secondConstraintFifthPaddingOrTerminatorTokens
            problem)) =
      some
  ...
  | succ remaining =>
      ...
      | zero =>
          ...
          simpa [hBranchWorkSteps, controllerWorkSteps,
            ...
            secondConstraintSixthPaddingOrOpeningUnaryTokens, hOpportunityOutput,
    ...
    workRunExact? (targetEvaluator problem) (targetWorkSteps problem)
      ...
      some
        (BuilderUnaryPolynomial.finalConfiguration
          (opportunitySlotPolynomial problem.verifier) problem.input
          (controllerFinalOutside problem)
          (secondConstraintSixthPaddingOrOpeningUnaryTokens problem)) := by
  exact BuilderUnaryPolynomial.workRunExact
    (opportunitySlotPolynomial problem.verifier) problem.input
    (controllerFinalOutside problem)
    (secondConstraintSixthPaddingOrOpeningUnaryTokens problem)
...
private theorem widthBranch_workRunExact {language : Language}
    ...
    workRunExact? (widthBranchMachine problem)
        ...
        (workStartConfiguration (widthBranchMachine problem)
          (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalTape
            problem)) =
      some
...
theorem suffix_workRunExact {language : Language}
  ...
  let widthBranchInitial := workStartConfiguration
    ...
    (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalTape problem)
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.workSteps problem)
      ...
      some
        (renameConfiguration
          ...
          (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalConfiguration
            problem)) := by
  ...
  have hRun := hTransport
    ...
    (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalConfiguration
      problem)
    (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.workRunExact problem)
  exact hRun
...
theorem workRunExact {language : Language}
  ...
  let prefixInitial := workStartConfiguration
    ...
    (rawInputWorkTape problem.input)
  let prefixFinal :=
    BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalConfiguration problem
...
theorem specification_opportunity_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalTokenSlot
          problem⟩ =
      some ((if problem.dimensions.tapeWidth problem.tableauInputMode = 1
        ...
        ⟨BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalTokenSlot
          problem + 1⟩) := by
  exact
    BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.specification_following_step
      problem

/-- The finite output is exactly the canonical token prefix after consuming
the sixth width-selected padding-or-opening-token opportunity. -/
theorem secondConstraintSixthPaddingOrOpeningUnaryTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      secondConstraintSixthPaddingOrOpeningUnaryTokens problem ++ rest := by
  rcases
      BuilderSecondConstraintFirstLiteralTerminatorStep.encodeCNFTokens_eq_terminator_then_successor_and_four_optional_unary_and_optional_terminator_and_optional_opening_unary
        problem with
    ⟨rest, hTokens⟩
  refine ⟨rest, ?_⟩
  unfold secondConstraintSixthPaddingOrOpeningUnaryTokens
    BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.secondConstraintFifthPaddingOrTerminatorTokens
...
theorem finalTokenSlot_eq_predecessor_add_one {language : Language}
  (problem : VerifierTableauProblem language) :
    finalTokenSlot problem =
      BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalTokenSlot problem +
        1 := by
  rw [finalTokenSlot, opportunitySlot_eq_secondConstraintStart_add_thirteen,
    BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.finalTokenSlot_eq_secondConstraintStart_add_twelve]

theorem finalTokenSlot_eq_secondConstraintStart_add_thirteen
    {language : Language} (problem : VerifierTableauProblem language) :
    finalTokenSlot problem =
      problem.formulaVariableSlotBound + 1 +
        problem.formulaClauseSlotsPerConstraint *
          problem.formulaTokensPerClause + 13 := by
  exact opportunitySlot_eq_secondConstraintStart_add_thirteen problem

/-- The retained coordinate identifies the opportunity after the processed
slot: another padding opportunity at width one, otherwise the first
...
theorem followingTokenSlot_direct_eq_padding_or_t {language : Language}
  ...
  have hFollowing :=
    BuilderSecondConstraintFirstLiteralTerminatorStep.seventhFollowingTokenSlot_direct_eq_padding_or_t
      problem
...
def rawTimeBound {language : Language}
  ...
  .add
    (BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.rawTimeBound verifier)
    (.add (.constant 672)
...
after the sixth width-selected padding-or-opening-token opportunity. -/
theorem finalTokenBits_eq_encodedFormula_secondConstraintSixthPaddingOrOpeningUnary
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (secondConstraintSixthPaddingOrOpeningUnaryTokens problem) =
  ...
  have hLength :
      (encodeTokenPairs
        (secondConstraintSixthPaddingOrOpeningUnaryTokens problem)).length =
        2 * (problem.FormulaWidth + 43 +
          if problem.dimensions.tapeWidth problem.tableauInputMode = 1
            then 0 else 6) := by
    rw [encodeTokenPairs_length,
      secondConstraintSixthPaddingOrOpeningUnaryTokens, List.length_append,
...
private theorem appenderWorkSteps_le {language : Language}
    ...
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 104 := by
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      ...
      672 + 24 * problem.input.length + 12 * problem.FormulaWidth +
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix :=
    BuilderSecondConstraintFifthPaddingOrTerminatorOpportunityStep.rawTimeBound_le problem
...
end BuilderSecondConstraintSixthPaddingOrOpeningUnaryOpportunityStep
