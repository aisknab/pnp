...
Completion of the third canonical Cook--Levin clause.

The machine in this file composes the complete third-clause second literal with
one state-selected clause terminator appender and the existing unary cursor
advance. Every raw input therefore completes clause three and leaves the retained
...
namespace BuilderThirdClausePrefix
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderThirdClauseSecondLiteralPrefix.machine problem) := by
  exact BuilderThirdClauseSecondLiteralPrefix.rule_source_ne_acceptState problem
      ...
      3126 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderSecondClausePaddingRun.thirdClauseStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderThirdClauseSecondLiteralPrefix.rules_length problem
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderThirdClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine
    (BuilderThirdClauseSecondLiteralPrefix.rules_pairwise_query_distinct problem)
...
def thirdClauseTokens {language : Language}
...
private theorem predecessor_finalOutside_eq_cursor {language : Language}
  ...
  simp only [BuilderThirdClauseSecondLiteralPrefix.finalOutside,
    List.length_append, List.length_singleton]
  rw [show
      (BuilderThirdClauseSecondLiteralPrefix.fourthCursorWord problem).length +
          1 + 1 =
        (BuilderThirdClauseSecondLiteralPrefix.fourthCursorWord problem).length +
          2 by omega]
...
def appenderWorkSteps {language : Language}
  ...
  BuilderTokenAppender.workSteps problem.input
    (BuilderThirdClauseSecondLiteralPrefix.thirdClauseSecondLiteralTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderThirdClauseSecondLiteralPrefix.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (thirdClauseTokens problem)
...
theorem appender_workRunExact {language : Language}
    ...
    workRunExact? FinishTokenCursor.appender (appenderWorkSteps problem)
        (workStartConfiguration FinishTokenCursor.appender
          (BuilderThirdClauseSecondLiteralPrefix.finalTape problem)) =
  ...
  have hExact := BuilderTokenAppender.appendToken_workRunExact
    problem.input (BuilderThirdClauseSecondLiteralPrefix.finalOutside problem)
  ...
  simpa [FinishTokenCursor.appender, appenderWorkSteps,
    BuilderThirdClauseSecondLiteralPrefix.finalTape,
    BuilderTokenAppender.entryConfiguration,
    BuilderTokenAppender.finalConfiguration,
    appenderFinalConfiguration, thirdClauseTokens,
...
theorem cursor_workRunExact {language : Language}
  ...
  have hWord : ∀ symbol ∈ cursorWord problem,
    ...
    intro symbol hMem
    have hBase :
        symbol ∈ BuilderThirdClauseSeparatorStep.cursorWord problem ∨
          symbol = BuilderUnaryPolynomial.unitSymbol := by
      simpa [cursorWord,
        BuilderThirdClauseSecondLiteralPrefix.fourthCursorWord,
        BuilderThirdClauseSecondLiteralPrefix.thirdCursorWord,
        BuilderThirdClauseSecondLiteralPrefix.secondCursorWord,
        BuilderThirdClauseSecondLiteralPrefix.firstCursorWord,
        BuilderThirdClauseFirstLiteralPrefix.secondCursorWord,
        BuilderThirdClauseFirstLiteralPrefix.firstCursorWord] using hMem
  ...
  have hExact :=
    BuilderDynamicTokenCursorStep.CursorAdvance.advance_workRunExact
      ...
      (thirdClauseTokens problem) hWord
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderThirdClauseSecondLiteralPrefix.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderThirdClauseSecondLiteralPrefix.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderThirdClauseSecondLiteralPrefix.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderThirdClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine)
    (BuilderThirdClauseSecondLiteralPrefix.workSteps problem)
    (workStartConfiguration (BuilderThirdClauseSecondLiteralPrefix.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderThirdClauseSecondLiteralPrefix.finalConfiguration problem)
    (BuilderThirdClauseSecondLiteralPrefix.workRunExact problem)
...
theorem prefixFinish_launch_workStep {language : Language}
    ...
    workStep? (machine problem)
      ...
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.secondState
        (workStartConfiguration FinishTokenCursor.machine
          (BuilderThirdClauseSecondLiteralPrefix.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderThirdClauseSecondLiteralPrefix.machine problem) FinishTokenCursor.machine
    (BuilderThirdClauseSecondLiteralPrefix.finalTape problem)
  simpa [machine, BuilderThirdClauseSecondLiteralPrefix.finalConfiguration,
    workStartConfiguration] using hLaunch

set_option maxRecDepth 1000000 in
/-- Every raw input follows the complete third-clause second-literal trace,
...
theorem workRunExact {language : Language}
  ...
  let prefixInitial := workStartConfiguration
    ...
    (rawInputWorkTape problem.input)
  let prefixFinal := BuilderThirdClauseSecondLiteralPrefix.finalConfiguration problem
  ...
  have hSuffix : workRunExact? FinishTokenCursor.machine
    ...
    simpa [prefixFinal, suffixFinal,
      BuilderThirdClauseSecondLiteralPrefix.finalConfiguration,
...
/-- The populated third-clause terminator executed by the suffix agrees with
...
theorem specification_terminator_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderThirdClauseSecondLiteralPrefix.finalTokenSlot problem⟩ =
      some (some CNFToken.finish,
        ⟨BuilderThirdClauseSecondLiteralPrefix.finalTokenSlot problem + 1⟩) :=
  BuilderThirdClauseSecondLiteralPrefix.specification_next_step problem
...
private theorem formulaClauseTokens_first_two_rectangles_then_completeThirdClause
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
      problem.formulaClauseSchedule.flatMap problem.scheduledClauseTokens =
        ...
        [some CNFToken.sep,
         ...
         some CNFToken.f, some CNFToken.finish] ++
        List.replicate (problem.formulaTokensPerClause - 8) none ++ rest := by
...
complete third clause. -/
theorem thirdClauseTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      thirdClauseTokens problem ++ rest := by
  rcases formulaClauseTokens_first_two_rectangles_then_completeThirdClause
      problem with
...
through the complete third clause. -/
theorem finalTokenBits_eq_encodedFormula_thirdClause
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (thirdClauseTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 27)) := by
  rcases thirdClauseTokens_eq_canonical_formula_prefix problem with
    ⟨rest, hTokens⟩
  have hLength :
      (encodeTokenPairs (thirdClauseTokens problem)).length =
        2 * (problem.FormulaWidth + 27) := by
    rw [encodeTokenPairs_length]
    simp [thirdClauseTokens,
      BuilderThirdClauseSecondLiteralPrefix.thirdClauseSecondLiteralTokens,
      BuilderThirdClauseSecondLiteralPrefix.secondUnaryTokenOutput,
      BuilderThirdClauseSecondLiteralPrefix.firstUnaryTokenOutput,
      BuilderThirdClauseSecondLiteralPrefix.signTokenOutput,
  ...
  have hShape : problem.encodedFormula =
      encodeTokenPairs (thirdClauseTokens problem) ++ suffix := by
...
/-- Retained token coordinate immediately after the third clause. -/
...
theorem finalTokenSlot_eq_thirdClauseStart_add_eight
    ...
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
        2 * problem.formulaTokensPerClause + 8 := by
  rw [finalTokenSlot,
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  unfold finalOutside cursorWord finalTokenSlot
    ...
    BuilderThirdClauseSecondLiteralPrefix.firstCursorWord
    BuilderThirdClauseFirstLiteralPrefix.secondCursorWord
    BuilderThirdClauseFirstLiteralPrefix.firstCursorWord
    BuilderThirdClauseSeparatorStep.cursorWord
  rw [hPrefix]
  rw [show BuilderSecondClausePaddingRun.thirdClauseStart problem + 8 =
      (((((((BuilderSecondClausePaddingRun.thirdClauseStart problem + 1) + 1) +
...
/-- The populated coordinate consumed by this milestone is the third
clause terminator. -/
theorem clauseTerminatorSlot_direct_eq_finish {language : Language}
    ...
    problem.formulaTokenSlotDirect
        (BuilderThirdClauseSecondLiteralPrefix.finalTokenSlot problem) =
      some (some CNFToken.finish) :=
  BuilderThirdClauseSecondLiteralPrefix.nextTokenSlot_direct_eq_finish problem

/-- The retained coordinate is the first in-range padding opportunity after
the complete third clause. -/
theorem nextTokenSlot_direct_eq_padding {language : Language}
  ...
  rw [show problem.formulaVariableSlotBound + 1 +
      2 * problem.formulaTokensPerClause + 8 -
        (problem.formulaVariableSlotBound + 1) =
      2 * problem.formulaTokensPerClause + 8 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause + 8 - 11 =
      (problem.formulaTokensPerClause - 11) +
        problem.formulaTokensPerClause + 8 by omega]
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + 8 - 7 =
      (problem.formulaTokensPerClause - 7) + 8 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 7) + 8 -
      (problem.formulaTokensPerClause - 7) = 8 by omega]
  rw [List.getElem?_append]
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show 8 - 8 = 0 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_pos (by omega)]
...
def rawTimeBound {language : Language}
  ...
  .add (BuilderThirdClauseSecondLiteralPrefix.rawTimeBound verifier)
    (.add (.constant 498)
...
private theorem predecessorTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderThirdClauseSecondLiteralPrefix.thirdClauseSecondLiteralTokens
      problem).length = problem.FormulaWidth + 26 := by
  simp [BuilderThirdClauseSecondLiteralPrefix.thirdClauseSecondLiteralTokens,
    BuilderThirdClauseSecondLiteralPrefix.secondUnaryTokenOutput,
    BuilderThirdClauseSecondLiteralPrefix.firstUnaryTokenOutput,
    BuilderThirdClauseSecondLiteralPrefix.signTokenOutput,
    BuilderThirdClauseFirstLiteralPrefix.thirdClauseFirstLiteralTokens,
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderThirdClauseSecondLiteralPrefix.rawTimeBound problem.verifier).eval
          problem.input.length + 498 +
        24 * problem.input.length + 12 * problem.FormulaWidth +
        12 * (BuilderThirdClauseSeparatorStep.cursorWord problem).length := by
  rw [rawTimeBound]
...
private theorem appenderWorkSteps_le {language : Language}
    ...
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 60 := by
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix := BuilderThirdClauseSecondLiteralPrefix.rawTimeBound_le problem
  have hAppender := appenderWorkSteps_le problem
  have hCursorLength :
      (cursorWord problem).length =
        (BuilderThirdClauseSeparatorStep.cursorWord problem).length + 7 := by
    simp [cursorWord,
      BuilderThirdClauseSecondLiteralPrefix.fourthCursorWord,
      BuilderThirdClauseSecondLiteralPrefix.thirdCursorWord,
      BuilderThirdClauseSecondLiteralPrefix.secondCursorWord,
...
/-- The complete third-clause second-literal endpoint is still globally nonhalting
...
private theorem appenderEndpoint_workRunExact {language : Language}
  ...
  have hPrefix : workRunExact? (machine problem)
      (BuilderThirdClauseSecondLiteralPrefix.workSteps problem) initial =
        some prefixFinal := by
  ...
  have h01 := PipelineMachineSimulation.workRunExact?_compose
    (machine problem) (BuilderThirdClauseSecondLiteralPrefix.workSteps problem) 1
