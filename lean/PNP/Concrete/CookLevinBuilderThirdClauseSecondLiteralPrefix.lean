...
The second complete negative literal of the third canonical Cook--Levin
clause.

The machine in this file composes the complete clause-three first-literal
...
decoder, a complete clause-three emitter, or a complete formula builder.
-/
...
namespace BuilderThirdClauseSecondLiteralPrefix
...
namespace TrueTrueFalseSuffix

/-- Two unary-one tokens followed by the false literal terminator.  Only the
final false-token cursor supplies global halts. -/
def machine : WorkMachine :=
  BuilderFirstClausePrefix.WorkChain.machine TrueTokenCursor.machine
...
theorem machine_acceptState_ne_rejectState :
  ...
  exact BuilderFirstClausePrefix.WorkChain.machine_acceptState_ne_rejectState
    TrueTokenCursor.machine TrueFalseSuffix.machine
    TrueFalseSuffix.machine_acceptState_ne_rejectState
...
end TrueTrueFalseSuffix

namespace SecondLiteralSuffix

/-- The negative sign followed by the complete unary-two/terminator tail.
...
private theorem second_noRuleAtAccept :
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      TrueTrueFalseSuffix.machine := by
  exact TrueTrueFalseSuffix.rule_source_ne_acceptState

theorem rules_length : machine.rules.length = 479 := by
  have hFirst :=
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.rules_length
  have hTail := TrueTrueFalseSuffix.rules_length
  unfold machine BuilderFirstClausePrefix.WorkChain.machine
    BuilderFirstClausePrefix.WorkChain.rules
    BuilderFirstClausePrefix.WorkChain.bridgeRules
  simp only [List.length_append, List.length_map]
...
theorem rules_pairwise_query_distinct :
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    ...
    TrueTrueFalseSuffix.machine
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.rules_pairwise_query_distinct
    TrueTrueFalseSuffix.rules_pairwise_query_distinct first_noRuleAtAccept
...
theorem machine_acceptState_ne_rejectState :
  ...
  exact BuilderFirstClausePrefix.WorkChain.machine_acceptState_ne_rejectState
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
    TrueTrueFalseSuffix.machine
    TrueTrueFalseSuffix.machine_acceptState_ne_rejectState
...
theorem rule_source_ne_acceptState (rule : WorkRule)
  ...
  exact BuilderFirstClausePrefix.WorkChain.noRuleAtAccept
    BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.machine
    TrueTrueFalseSuffix.machine second_noRuleAtAccept rule hRule
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderThirdClauseFirstLiteralPrefix.machine problem) := by
  exact BuilderThirdClauseFirstLiteralPrefix.rule_source_ne_acceptState problem
      ...
      3004 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderSecondClausePaddingRun.thirdClauseStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderThirdClauseFirstLiteralPrefix.rules_length problem
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderThirdClauseFirstLiteralPrefix.machine problem)
    SecondLiteralSuffix.machine
    (BuilderThirdClauseFirstLiteralPrefix.rules_pairwise_query_distinct problem)
...
def thirdClauseSecondLiteralTokens {language : Language}
    (problem : VerifierTableauProblem language) : List CNFToken :=
...
def firstCursorWord {language : Language}
  ...
  BuilderThirdClauseFirstLiteralPrefix.secondCursorWord problem ++
...
private def cursorOutsideTail {language : Language}
    (problem : VerifierTableauProblem language) : List WorkSymbol :=
  (BuilderThirdClauseFirstLiteralPrefix.finalOutside problem).drop
    ((firstCursorWord problem).length + 1)

private theorem predecessor_finalOutside_eq_firstCursor {language : Language}
    (problem : VerifierTableauProblem language) :
    BuilderThirdClauseFirstLiteralPrefix.finalOutside problem =
  ...
  simp only [BuilderThirdClauseFirstLiteralPrefix.finalOutside,
    List.length_append, List.length_singleton]
  rw [show
      (BuilderThirdClauseFirstLiteralPrefix.secondCursorWord problem).length +
          1 + 1 =
        (BuilderThirdClauseFirstLiteralPrefix.secondCursorWord problem).length +
          2 by omega]
...
def finalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input (finalOutside problem)
    (thirdClauseSecondLiteralTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderThirdClauseFirstLiteralPrefix.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (thirdClauseSecondLiteralTokens problem)
...
theorem firstAppender_workRunExact {language : Language}
    ...
    workRunExact?
        ...
        (workStartConfiguration
          BuilderSecondClauseFirstLiteralPrefix.FalseTokenCursor.appender
          (BuilderThirdClauseFirstLiteralPrefix.finalTape problem)) =
...
private theorem predecessorCursorWord_symbol {language : Language}
    (problem : VerifierTableauProblem language) :
    ∀ symbol ∈ BuilderThirdClauseFirstLiteralPrefix.secondCursorWord problem,
      symbol = BuilderUnaryPolynomial.unitSymbol ∨
        symbol = BuilderUnaryPolynomial.separatorSymbol := by
  intro symbol hMem
  unfold BuilderThirdClauseFirstLiteralPrefix.secondCursorWord
    BuilderThirdClauseFirstLiteralPrefix.firstCursorWord at hMem
  simp only [List.mem_append, List.mem_singleton] at hMem
...
theorem fourthAppender_workRunExact {language : Language}
  ...
  simpa [selectedAppender,
    ...
    fourthAppenderFinalConfiguration, thirdClauseSecondLiteralTokens] using
      selectedAppender_workRunExact problem.input
        (fourthCursorWord problem ++ BuilderUnaryPolynomial.scratchEndSymbol ::
          (cursorOutsideTail problem).drop 3)
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderThirdClauseFirstLiteralPrefix.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderThirdClauseFirstLiteralPrefix.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderThirdClauseFirstLiteralPrefix.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    ...
    (workStartConfiguration
      (BuilderThirdClauseFirstLiteralPrefix.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderThirdClauseFirstLiteralPrefix.finalConfiguration problem)
    (BuilderThirdClauseFirstLiteralPrefix.workRunExact problem)
...
theorem prefixSecondLiteral_launch_workStep {language : Language}
    ...
    workStep? (machine problem)
      ...
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.secondState
        (workStartConfiguration SecondLiteralSuffix.machine
          (BuilderThirdClauseFirstLiteralPrefix.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderThirdClauseFirstLiteralPrefix.machine problem)
    SecondLiteralSuffix.machine
    (BuilderThirdClauseFirstLiteralPrefix.finalTape problem)
...
private theorem formulaClauseTokens_first_two_rectangles_then_thirdSecondLiteral
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
...
second negative literal of clause three. -/
theorem thirdClauseSecondLiteralTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      thirdClauseSecondLiteralTokens problem ++ rest := by
  rcases formulaClauseTokens_first_two_rectangles_then_thirdSecondLiteral
      problem with
  ...
  unfold thirdClauseSecondLiteralTokens secondUnaryTokenOutput
...
through negative variable two in clause three. -/
theorem finalTokenBits_eq_encodedFormula_thirdClauseSecondLiteral
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (thirdClauseSecondLiteralTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 26)) := by
  rcases thirdClauseSecondLiteralTokens_eq_canonical_formula_prefix problem with
    ⟨rest, hTokens⟩
  have hLength :
      (encodeTokenPairs (thirdClauseSecondLiteralTokens problem)).length =
        2 * (problem.FormulaWidth + 26) := by
  ...
  have hShape : problem.encodedFormula =
      encodeTokenPairs (thirdClauseSecondLiteralTokens problem) ++ suffix := by
...
theorem finalTokenSlot_eq_thirdClauseStart_add_seven
    ...
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
        2 * problem.formulaTokensPerClause + 7 := by
  rw [finalTokenSlot,
...
theorem finalOutside_contains_finalTokenSlot {language : Language}
  ...
  rw [hPrefix]
  rw [show BuilderSecondClausePaddingRun.thirdClauseStart problem + 7 =
...
private theorem directSlot_at_thirdClauseOffset
    {language : Language} (problem : VerifierTableauProblem language)
    ...
    problem.formulaTokenSlotDirect
        (BuilderSecondClausePaddingRun.thirdClauseStart problem + offset) =
      some (some token) := by
  ...
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause + offset - 11 =
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + offset - 7 =
      (problem.formulaTokensPerClause - 7) + offset by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 7) + offset -
      (problem.formulaTokensPerClause - 7) = offset by omega]
  rw [List.getElem?_append]
...
theorem secondLiteralSignSlot_direct_eq_f {language : Language}
  ...
  exact directSlot_at_thirdClauseOffset problem 3 CNFToken.f
    (by decide) (by decide)
...
theorem secondLiteralFirstUnaryUnitSlot_direct_eq_t {language : Language}
    (problem : VerifierTableauProblem language) :
    problem.formulaTokenSlotDirect
        (BuilderSecondClausePaddingRun.thirdClauseStart problem + 4) =
      some (some CNFToken.t) := by
  exact directSlot_at_thirdClauseOffset problem 4 CNFToken.t
    (by decide) (by decide)
...
theorem secondLiteralSecondUnaryUnitSlot_direct_eq_t {language : Language}
    (problem : VerifierTableauProblem language) :
    problem.formulaTokenSlotDirect
        (BuilderSecondClausePaddingRun.thirdClauseStart problem + 5) =
      some (some CNFToken.t) := by
  exact directSlot_at_thirdClauseOffset problem 5 CNFToken.t
    (by decide) (by decide)
...
theorem secondLiteralTerminatorSlot_direct_eq_f {language : Language}
    (problem : VerifierTableauProblem language) :
    problem.formulaTokenSlotDirect
        (BuilderSecondClausePaddingRun.thirdClauseStart problem + 6) =
      some (some CNFToken.f) := by
  exact directSlot_at_thirdClauseOffset problem 6 CNFToken.f
    (by decide) (by decide)
...
theorem nextTokenSlot_direct_eq_finish {language : Language}
  ...
  simpa [finalTokenSlot] using
    directSlot_at_thirdClauseOffset problem 7 CNFToken.finish
      (by decide) (by decide)
...
theorem specification_secondLiteral_sign_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
      ...
      some (some CNFToken.f,
        ⟨BuilderSecondClausePaddingRun.thirdClauseStart problem + 4⟩) := by
...
theorem specification_secondLiteral_unaryUnit_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
      ...
      some (some CNFToken.t,
        ⟨BuilderSecondClausePaddingRun.thirdClauseStart problem + 5⟩) := by
  unfold VerifierTableauProblem.FormulaTokenCursor.step
...
theorem specification_secondLiteral_secondUnaryUnit_step
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondClausePaddingRun.thirdClauseStart problem + 5⟩ =
      some (some CNFToken.t,
        ⟨BuilderSecondClausePaddingRun.thirdClauseStart problem + 6⟩) := by
  unfold VerifierTableauProblem.FormulaTokenCursor.step
...
theorem specification_secondLiteral_terminator_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondClausePaddingRun.thirdClauseStart problem + 6⟩ =
      some (some CNFToken.f,
        ⟨BuilderSecondClausePaddingRun.thirdClauseStart problem + 7⟩) := by
  unfold VerifierTableauProblem.FormulaTokenCursor.step
...
def rawTimeBound {language : Language}
  ...
  .add (BuilderThirdClauseFirstLiteralPrefix.rawTimeBound verifier)
    (.add (.constant 1752)
...
private theorem predecessorTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderThirdClauseFirstLiteralPrefix.thirdClauseFirstLiteralTokens
      problem).length = problem.FormulaWidth + 22 := by
  simp [BuilderThirdClauseFirstLiteralPrefix.thirdClauseFirstLiteralTokens,
    BuilderThirdClauseFirstLiteralPrefix.firstTokenOutput,
    BuilderThirdClauseSeparatorStep.thirdClauseStartTokens,
    BuilderSecondClausePrefix.secondClauseTokens,
    BuilderSecondClauseSecondLiteralPrefix.secondClauseSecondLiteralTokens,
...
private theorem signTokenOutput_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (signTokenOutput problem).length = problem.FormulaWidth + 23 := by
...
private theorem firstUnaryTokenOutput_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (firstUnaryTokenOutput problem).length = problem.FormulaWidth + 24 := by
...
private theorem secondUnaryTokenOutput_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (secondUnaryTokenOutput problem).length = problem.FormulaWidth + 25 := by
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderThirdClauseFirstLiteralPrefix.rawTimeBound
          problem.verifier).eval problem.input.length + 1752 +
        96 * problem.input.length + 48 * problem.FormulaWidth +
        48 * (BuilderThirdClauseSeparatorStep.cursorWord problem).length := by
  rw [rawTimeBound]
...
private theorem firstAppenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    firstAppenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 52 := by
...
private theorem secondAppenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    secondAppenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 54 := by
  have hSource := BuilderTokenAppender.sourceCellCount_le problem.input
...
private theorem thirdAppenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    thirdAppenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 56 := by
  have hSource := BuilderTokenAppender.sourceCellCount_le problem.input
...
private theorem fourthAppenderWorkSteps_le {language : Language}
    (problem : VerifierTableauProblem language) :
    fourthAppenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 58 := by
  have hSource := BuilderTokenAppender.sourceCellCount_le problem.input
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix :=
    BuilderThirdClauseFirstLiteralPrefix.rawTimeBound_le problem
...
end BuilderThirdClauseSecondLiteralPrefix
