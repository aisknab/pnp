...
One populated token-cursor transition at the beginning of the third
canonical Cook--Levin clause.

The machine in this file composes the complete second-clause padding run with
the already-audited selected separator appender and unary cursor advance.
Every raw input therefore emits the separator beginning clause three and leaves
...
namespace BuilderThirdClauseSeparatorStep
...
private theorem predecessor_noRuleAtAccept {language : Language}
    ...
    BuilderFirstClausePrefix.WorkChain.NoRuleAtAccept
      (BuilderSecondClausePaddingRun.machine problem) := by
  exact BuilderSecondClausePaddingRun.rule_source_ne_acceptState problem
...
separator transition at the beginning of clause three. -/
      ...
      2272 +
        ...
        BuilderUnaryPolynomial.ruleCount
          (BuilderSecondClausePaddingRun.thirdClauseStartPolynomial
            problem.verifier) := by
  have hPrefix := BuilderSecondClausePaddingRun.rules_length problem
  have hSuffix :=
...
theorem rules_pairwise_query_distinct {language : Language}
  ...
  exact BuilderFirstClausePrefix.WorkChain.rules_pairwise_query_distinct
    (BuilderSecondClausePaddingRun.machine problem)
    BuilderSecondClauseSeparatorStep.SeparatorCursor.machine
    (BuilderSecondClausePaddingRun.rules_pairwise_query_distinct problem)
...
def thirdClauseStartTokens {language : Language}
...
def finalTape {language : Language}
  ...
  BuilderTokenAppender.workspaceTape problem.input (finalOutside problem)
    (thirdClauseStartTokens problem)
...
def workSteps {language : Language}
    (problem : VerifierTableauProblem language) : Nat :=
  BuilderSecondClausePaddingRun.workSteps problem + 1 +
...
theorem finalTape_represents {language : Language}
  ...
  exact BuilderTokenAppender.workspaceTape_represents problem.input
    (finalOutside problem) (thirdClauseStartTokens problem)
...
theorem appender_workRunExact {language : Language}
    ...
    workRunExact? BuilderSecondClauseSeparatorStep.SeparatorCursor.appender (appenderWorkSteps problem)
        (workStartConfiguration BuilderSecondClauseSeparatorStep.SeparatorCursor.appender
          (BuilderSecondClausePaddingRun.finalTape problem)) =
  ...
  have hExact := BuilderTokenAppender.appendToken_workRunExact
    problem.input (BuilderSecondClausePaddingRun.finalOutside problem)
    (BuilderSecondClausePrefix.secondClauseTokens problem) .sep
  rw [hRunEq]
  simpa [BuilderSecondClauseSeparatorStep.SeparatorCursor.appender, appenderWorkSteps,
    ...
    appenderFinalConfiguration, thirdClauseStartTokens,
...
theorem cursor_workRunExact {language : Language}
  ...
  have hExact :=
    BuilderDynamicTokenCursorStep.CursorAdvance.advance_workRunExact
      ...
      (thirdClauseStartTokens problem) hWord
...
theorem prefix_workRunExact {language : Language}
    ...
    workRunExact? (machine problem)
        (BuilderSecondClausePaddingRun.workSteps problem)
        (workStartConfiguration (machine problem)
          (rawInputWorkTape problem.input)) =
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.firstState
        (BuilderSecondClausePaddingRun.finalConfiguration problem)) := by
  have hTransport := PipelineStageBridges.workRunExact?_transport
    (BuilderSecondClausePaddingRun.machine problem) (machine problem)
    BuilderFirstClausePrefix.WorkChain.firstState
    (BuilderFirstClausePrefix.WorkChain.first_workStep_of_some
      (BuilderSecondClausePaddingRun.machine problem)
        BuilderSecondClauseSeparatorStep.SeparatorCursor.machine)
    (BuilderSecondClausePaddingRun.workSteps problem)
    (workStartConfiguration (BuilderSecondClausePaddingRun.machine problem)
      (rawInputWorkTape problem.input))
    (BuilderSecondClausePaddingRun.finalConfiguration problem)
    (BuilderSecondClausePaddingRun.workRunExact problem)
...
theorem prefixSeparator_launch_workStep {language : Language}
    ...
    workStep? (machine problem)
      ...
      some (renameConfiguration
        BuilderFirstClausePrefix.WorkChain.secondState
        (workStartConfiguration BuilderSecondClauseSeparatorStep.SeparatorCursor.machine
          (BuilderSecondClausePaddingRun.finalTape problem))) := by
  have hLaunch := BuilderFirstClausePrefix.WorkChain.launch_workStep
    (BuilderSecondClausePaddingRun.machine problem)
    BuilderSecondClauseSeparatorStep.SeparatorCursor.machine
    (BuilderSecondClausePaddingRun.finalTape problem)
  simpa [machine, BuilderSecondClausePaddingRun.finalConfiguration,
    workStartConfiguration] using hLaunch

set_option maxRecDepth 1000000 in
/-- Every raw input follows the complete second-clause padding trace, appends
the clause-three separator, and advances the retained unary coordinate once. -/
theorem workRunExact {language : Language}
  ...
  let prefixInitial := workStartConfiguration
    (BuilderSecondClausePaddingRun.machine problem)
    (rawInputWorkTape problem.input)
  let prefixFinal := BuilderSecondClausePaddingRun.finalConfiguration problem
...
theorem specification_separator_step {language : Language}
    ...
    VerifierTableauProblem.FormulaTokenCursor.step problem
        ⟨BuilderSecondClausePaddingRun.thirdClauseStart problem⟩ =
      some (some CNFToken.sep,
        ⟨BuilderSecondClausePaddingRun.thirdClauseStart problem + 1⟩) :=
  BuilderSecondClausePaddingRun.specification_target_step problem
...
private theorem formulaClauseTokens_first_two_rectangles_then_thirdSepF
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest,
  ...
  rcases formulaClauseSchedule_starts_three_shape_clauses problem with
    ⟨left, right, thirdLeft, thirdRight, clauses, hClauses, hLeft, hRight⟩
  rw [hClauses]
  simp only [List.flatMap_cons]
  ...
  let thirdTail : List (Option CNFToken) :=
    (encodeUnaryTokens thirdLeft.val ++
        CNFToken.f :: encodeUnaryTokens thirdRight.val ++
...
separator beginning the third clause. -/
theorem thirdClauseStartTokens_eq_canonical_formula_prefix
    {language : Language} (problem : VerifierTableauProblem language) :
    ∃ rest, encodeCNFTokens problem.formula =
      thirdClauseStartTokens problem ++ rest := by
  rcases formulaClauseTokens_first_two_rectangles_then_thirdSepF problem with
...
through the third-clause separator. -/
theorem finalTokenBits_eq_encodedFormula_thirdClauseStart
    {language : Language} (problem : VerifierTableauProblem language) :
    encodeTokenPairs (thirdClauseStartTokens problem) =
      problem.encodedFormula.take (2 * (problem.FormulaWidth + 20)) := by
  rcases thirdClauseStartTokens_eq_canonical_formula_prefix problem with
    ⟨rest, hTokens⟩
  have hLength :
      (encodeTokenPairs (thirdClauseStartTokens problem)).length =
        2 * (problem.FormulaWidth + 20) := by
    rw [encodeTokenPairs_length, thirdClauseStartTokens,
      List.length_append,
      BuilderSecondClausePrefix.secondClauseTokens]
  ...
  have hShape : problem.encodedFormula =
      encodeTokenPairs (thirdClauseStartTokens problem) ++ suffix := by
...
theorem finalTokenSlot_eq_thirdClauseStart_add_one
    ...
    finalTokenSlot problem = problem.formulaVariableSlotBound + 1 +
        2 * problem.formulaTokensPerClause + 1 := by
  rw [finalTokenSlot,
...
sign token beginning the first literal of clause three. -/
theorem nextTokenSlot_direct_eq_f {language : Language}
  ...
  rw [if_neg (by omega)]
  rw [show 2 * problem.formulaTokensPerClause + 1 - 11 =
  ...
  simp only [List.length_cons, List.length_nil]
  rw [if_neg (by omega)]
  rw [show problem.formulaTokensPerClause + 1 - 7 =
      (problem.formulaTokensPerClause - 7) + 1 by omega]
  rw [List.getElem?_append, List.length_replicate]
  rw [if_neg (by omega)]
  rw [show (problem.formulaTokensPerClause - 7) + 1 -
      (problem.formulaTokensPerClause - 7) = 1 by omega]
  simp
...
def rawTimeBound {language : Language}
  ...
  .add (BuilderSecondClausePaddingRun.rawTimeBound verifier)
    (.add (.constant 330)
...
private theorem secondClauseTokens_length {language : Language}
    (problem : VerifierTableauProblem language) :
    (BuilderSecondClausePrefix.secondClauseTokens problem).length =
      problem.FormulaWidth + 19 := by
  simp [BuilderSecondClausePrefix.secondClauseTokens,
    BuilderSecondClauseSecondLiteralPrefix.secondClauseSecondLiteralTokens,
    BuilderSecondClauseSecondLiteralPrefix.unaryTokenOutput,
    BuilderSecondClauseSecondLiteralPrefix.signTokenOutput,
    BuilderSecondClauseFirstLiteralPrefix.secondClauseFirstLiteralTokens,
...
theorem rawTimeBound_eval {language : Language}
    ...
    (rawTimeBound problem.verifier).eval problem.input.length =
      (BuilderSecondClausePaddingRun.rawTimeBound problem.verifier).eval
          problem.input.length + 330 +
...
private theorem appenderWorkSteps_le {language : Language}
    ...
    appenderWorkSteps problem ≤
      4 * problem.input.length + 2 * problem.FormulaWidth + 46 := by
...
theorem rawTimeBound_le {language : Language}
  ...
  have hPrefix := BuilderSecondClausePaddingRun.rawTimeBound_le problem
...
private theorem appenderEndpoint_workRunExact {language : Language}
  ...
  have hPrefix : workRunExact? (machine problem)
      (BuilderSecondClausePaddingRun.workSteps problem) initial =
        some prefixFinal := by
  ...
  have hGlobal := PipelineStageBridges.workRunExact?_transport
    ...
    (BuilderFirstClausePrefix.WorkChain.second_workStep_of_some
      (BuilderSecondClausePaddingRun.machine problem) BuilderSecondClauseSeparatorStep.SeparatorCursor.machine)
    (appenderWorkSteps problem)
    (workStartConfiguration BuilderSecondClauseSeparatorStep.SeparatorCursor.machine
      (BuilderSecondClausePaddingRun.finalTape problem))
    (renameConfiguration BuilderFirstClausePrefix.WorkChain.firstState
  ...
  have h01 := PipelineMachineSimulation.workRunExact?_compose
    (machine problem) (BuilderSecondClausePaddingRun.workSteps problem) 1
...
end BuilderThirdClauseSeparatorStep
