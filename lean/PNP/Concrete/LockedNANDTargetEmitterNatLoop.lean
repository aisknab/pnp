...
A literal finite natural-token loop for the grammar-only locked-NAND target
emitter.

The loop starts on the first retained packed source cell.  Immediately behind
the source-left boundary it receives a unary scratch counter.  It consumes the
counter by changing one `zeroBlank` unit at a time to a context-local mark,
appends one literal `.unit` token for each unit, then appends one literal
...
namespace PNP.Concrete.LockedNAND.TargetEmitterNatLoop
...
def sourceTargetBoundary : WorkSymbol := TargetEmitter.sourceTargetBoundary
...
def packedAction (target : Nat) (move : HeadMove)
    (symbol : WorkSymbol) :
    Nat × WorkSymbol × HeadMove :=
...
def appendPrograms (token : Token) (done : Nat) : List StateProgram :=
  [{ state := TargetEmitter.seekSourceState token,
      action := TargetEmitter.seekSourceAction token },
...
def controllerPrograms : List StateProgram :=
  [{ state := startState,
      action := packedAction inspectBoundaryState .left },
   ...
   { state := natEndDoneState,
      action := packedAction cleanupBoundaryState .left },
...
theorem append_exact (kind : AppendKind)
    (source target controllerOutside outsideRight : List WorkSymbol)
    (sourcePacked :
      ∀ symbol, symbol ∈ source → TargetEmitter.PackedSymbol symbol)
    (targetPacked :
...
private theorem ready_step (used : Nat)
    ...
    (ordinary : TargetEmitter.PackedSymbol sourceHead) :
    workStep? machine
...
theorem unit_launch_exact (used remaining : Nat)
    ...
    (sourceHeadPacked : TargetEmitter.PackedSymbol sourceHead) :
...
/-! ### Exact canonical workspace endpoints -/

def entryTape (raw : RawCircuit) (count : Nat)
...
def entryConfiguration (raw : RawCircuit) (count : Nat)
    (targetPrefix : List Token)
    (outsideLeft outsideRight : List WorkSymbol) : WorkConfiguration :=
  { state := startState
    tape := entryTape raw count targetPrefix outsideLeft outsideRight }

def finalTape (raw : RawCircuit) (count : Nat)
    (targetPrefix : List Token)
    (outsideLeft outsideRight : List WorkSymbol) : WorkTape :=
  match SourceParser.circuitCells raw with
  | [] =>
      { left := sourceLeftBoundary ::
        ...
        right :=
          SourceParser.packedTokenCells
            (targetPrefix ++ naturalTokens count) ++
              (WorkSymbol.blank :: outsideRight) }
  | first :: rest =>
      { left := sourceLeftBoundary ::
          (initialCounterWord count ++ outsideLeft)
...
def finalConfiguration (raw : RawCircuit) (count : Nat)
    (targetPrefix : List Token)
    (outsideLeft outsideRight : List WorkSymbol) : WorkConfiguration :=
  { state := acceptState
    tape := finalTape raw count targetPrefix outsideLeft outsideRight }

theorem finalConfiguration_halted (raw : RawCircuit) (count : Nat)
    (targetPrefix : List Token)
    (outsideLeft outsideRight : List WorkSymbol) :
    machine.isHalted
      (finalConfiguration raw count targetPrefix
        outsideLeft outsideRight) = true := by
  rfl

def workSteps (raw : RawCircuit) (targetPrefix : List Token)
    (count : Nat) : Nat :=
  loopWorkSteps (SourceParser.circuitCells raw)
    (SourceParser.packedTokenCells targetPrefix) 0 count

/-- Exact dynamic natural emission for every raw canonical source and every
unary counter value.  Neither the theorem statement nor the start workspace
contains the emitted token list as machine advice. -/
theorem exact (raw : RawCircuit) (count : Nat)
    (targetPrefix : List Token)
    (outsideLeft outsideRight : List WorkSymbol) :
    workRunExact? machine (workSteps raw targetPrefix count)
        (entryConfiguration raw count targetPrefix
          outsideLeft outsideRight) =
      some (finalConfiguration raw count targetPrefix
        outsideLeft outsideRight) := by
  let source := SourceParser.circuitCells raw
  let target := SourceParser.packedTokenCells targetPrefix
  have sourceNonempty : source ≠ [] :=
    SourceParser.circuitCells_ne_empty raw
  ...
  | cons sourceHead sourceTail =>
      have sourcePacked :
          ∀ symbol, symbol ∈ sourceHead :: sourceTail →
            TargetEmitter.PackedSymbol symbol := by
        intro symbol found
        exact TargetEmitter.circuitCells_packed raw symbol <| by
          simpa [source, sourceEq] using found
      have targetPacked :
          ∀ symbol, symbol ∈ target →
            TargetEmitter.PackedSymbol symbol :=
        TargetEmitter.packedTokenCells_packed targetPrefix
      have complete := loop_exact 0 count sourceHead sourceTail target
        outsideLeft outsideRight sourcePacked targetPacked
      have stepsEq :
          workSteps raw targetPrefix count =
            loopWorkSteps (sourceHead :: sourceTail) target 0 count := by
        simp [workSteps, source, target, sourceEq]
      have entryEq :
          entryConfiguration raw count targetPrefix
              outsideLeft outsideRight =
            loopConfiguration 0 count (sourceHead :: sourceTail) target
              outsideLeft outsideRight := by
        simp [entryConfiguration, entryTape, loopConfiguration,
          phaseConfiguration, readyState, loopCounterWord,
          initialCounterWord, TargetEmitter.configAtWord,
          source, target, sourceEq, List.append_assoc]
      have finalEq :
          finalConfiguration raw count targetPrefix
              outsideLeft outsideRight =
            cleanedConfiguration count (sourceHead :: sourceTail)
              (target ++
                SourceParser.packedTokenCells (naturalTokens count))
              outsideLeft outsideRight := by
        simp [finalConfiguration, finalTape, cleanedConfiguration,
          phaseConfiguration, initialCounterWord,
          TargetEmitter.configAtWord, source, target, sourceEq,
          packedTokenCells_append, List.append_assoc]
      rw [stepsEq, entryEq, finalEq]
      simpa using complete

theorem workSteps_evaluated (raw : RawCircuit)
    (targetPrefix : List Token) (count : Nat) :
    workSteps raw targetPrefix count =
      count *
          (4 * (encodeCircuitTokens raw).length +
            4 * targetPrefix.length + 10) +
        6 * triangular count +
        (4 * (encodeCircuitTokens raw).length +
          4 * targetPrefix.length + 8 * count + 14) := by
  ...
  rw [← SourceParser.packedTokenCells_encodeCircuitTokens raw,
    ...
    SourceParser.packedTokenCells_length]
  have doubleDouble : ∀ value : Nat, 2 * (2 * value) = 4 * value := by
    intro value
    omega
  rw [doubleDouble, doubleDouble]
...
def polynomialWorkBound (raw : RawCircuit)
  ...
  count *
    ...
    6 * (count * count) +
    (4 * (encodeCircuitTokens raw).length +
      4 * targetPrefix.length + 8 * count + 14)

theorem workSteps_le_polynomialWorkBound (raw : RawCircuit)
    (targetPrefix : List Token) (count : Nat) :
    workSteps raw targetPrefix count ≤
      polynomialWorkBound raw targetPrefix count := by
...
end PNP.Concrete.LockedNAND.TargetEmitterNatLoop
