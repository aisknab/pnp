...
export async function CheckActivatedPNPStatus0(options = {}) {
  ...
  if (out.tag !== 'accept') {
    return {
      ...
      activatedPNPStatusAccepted: false,
      activationSuperseded: false,
    };
  }
  return {
    ...out,
    checker: CHECKER,
    claimStatus: 'legacy-activated-status-path-superseded',
    compatibilityAlias: true,
    activatedPNPStatusAccepted: false,
...
function parseArgs0(argv) {
  const out = { json: false, writeOutput: true };
  for (const arg of argv) {
    if (arg === '--json') out.json = true;
    else if (arg === '--no-write') out.writeOutput = false;
...
async function main0() {
  ...
  try { options = parseArgs0(process.argv.slice(2)); }
    ...
    console.error(JSON.stringify({ tag: 'reject', checker: CHECKER, coord: 'ActivatedPNPStatus.CliBadArgument', witness: { reason: error.message }, publicTheoremEmissionAllowed: false }, null, 2));
    process.exit(2);
  }
  const verdict = await CheckActivatedPNPStatus0(options);
