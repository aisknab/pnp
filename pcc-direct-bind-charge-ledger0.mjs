...
const CHECKER = 'CheckCHGDirectBindingSeed0';
const VERSION = 0;
const MANIFEST_PATH = 'report-bindings/direct-bindings/CHG_DIRECT_BINDING_SEED.json';
...
const DEFAULT_OUTPUT_PATH = 'artifacts/direct-bind-charge-ledger/latest-verdict.json';
const EXPECTED_COORDINATE = 'PNP-DIRECT-BIND-CHG-CHARGE-LEDGER-SEED-2026-06-27-01';
const EXPECTED_BLOCKERS = ['Release.UnrestrictedFinalSoundness', 'ExternalReview.Acceptance'];
const EXPECTED_COMPONENT_IDS = ['CHG.GlobalChargeLedgerSurface', 'CHG.MaterializerOwnershipSurface', 'CHG.ExactSizeEquationSurface', 'CHG.ChargeSoundnessActivationBoundary'];

export async function CheckCHGDirectBindingSeed0(options = {}) {
  const root = path.resolve(options.root ?? process.cwd());
  const outputPath = options.outputPath ?? DEFAULT_OUTPUT_PATH;
  const writeOutput = options.writeOutput ?? true;
  try {
    const manifestRead = await readJson0(root, options.manifestPath ?? MANIFEST_PATH, options.manifestOverride, 'CHG direct-binding seed manifest');
...
function validateManifest0(manifest) {
  ...
  if (!plain0(bound) || bound.inventoryEntryId !== 'TL-002-CHG' || bound.coverageEntryId !== 'COV-002-CHG' || bound.closureEntryId !== 'DCC-002-CHG' || bound.sourceLabel !== 'CHG' || bound.expectedCoverageClass !== 'obligation-ledger-surface' || bound.expectedClosureStatus !== 'direct-binding-needed' || bound.expectedNextDirectBindingSurface !== 'pcc-direct-bind-charge-ledger0.mjs') return reject0('CHGDirectBindingSeed.BoundEntry', ['boundInventoryEntry'], 'bound inventory entry mismatch');
...
function printHelp0() { console.log(`Usage: node pcc-direct-bind-charge-ledger0.mjs [options]\n\nOptions:\n  --json             Emit verdict JSON.\n  --no-write         Do not write artifacts/direct-bind-charge-ledger/latest-verdict.json.\n  --root <path>      Repository root. Defaults to cwd.\n  --manifest <path>  Manifest JSON path relative to root.\n  --output <path>    Verdict output path relative to root.\n`); }
async function main0() { let options; try { options = parseArgs0(process.argv.slice(2)); } catch (error) { const verdict = reject0('Cli.BadArgument', [], 'bad CHG direct-binding seed CLI argument', normalizeError0(error)); console.error(JSON.stringify(verdict, null, 2)); process.exit(2); } const verdict = await CheckCHGDirectBindingSeed0(options); const rendered = JSON.stringify(verdict, null, 2); if (options.json || verdict.tag === 'accept') console.log(rendered); else console.error(rendered); process.exit(verdict.tag === 'accept' ? 0 : 1); }
