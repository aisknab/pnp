...
const VERSION = 0;
const COORDINATE = 'PNP-FORMAL-RECONSTRUCTION-STATUS-2026-07-12-23';
...
const NON_CLAIMS = Object.freeze([
  ...
  'PipelineInputFramer is one literal finite machine for canonical BitString.pair inputs. It reaches an accepting represented frame with permitted exterior garbage at an exact quadratic raw-input-length budget. PipelineStageBridges launches that endpoint into the lifted target in one exact symbol-preserving step. The framer still does not accept arbitrary empty, odd, malformed, or unpaired raw words, prove target termination, connect the composed handoff to the separate terminal packer, provide a pipeline RawRefinement, establish CNF-SAT in P or NP-completeness, or establish P = NP.',
  'PipelineOutputHandoff is one literal finite machine for an already represented internal tape. For logical output length n it reaches an accepting representation of Tape.handoffTarget in exactly 2 * n + 4 work steps and 12 * n + 24 compiled steps. PipelineStageBridges launches target accept and reject sentinels into two disjoint verdict-indexed copies and preserves bounded accept/reject classification for supplied exact target runs. TerminalOutputPacker separately turns a canonical represented word into blank-delimited raw output, but no theorem launches the bridge endpoint into that packer or supplies a complete external-input-size pipeline polynomial or RawRefinement. CNF-SAT in P, NP-completeness, and P = NP remain unproved.',
  'PipelineMachineSimulation extracts from every at-most raw run with fuel F an exact successful prefix of length k at most F reaching the same endpoint, and each supplied exact n-step run costs exactly 3 * n work transitions. PipelineStageBridges composes such supplied exact runs with the framer and internal handoff, while TerminalOutputPacker separately proves terminal output normalization. They are not yet one composed machine; target termination and an external-input-size polynomial, FunctionProgram.RawRefinement, DecisionProgram.RawRefinement, CNF-SAT in P, NP-completeness, and P = NP remain missing. A stuck nonhalting exact endpoint is timeout at the proved prefix budget, not rejection.',
  'PipelineStateNamespace remains the injective renaming and lookup-isolation prerequisite. PipelineStageBridges adds literal launch tables, verdict-indexed handoff copies, exact cumulative work traces, and six-for-one compiled raw traces from canonical paired input. TerminalOutputPacker separately proves raw-visible output equality, but it has not been allocated and launched as a fourth stage in the composed rule table; the external-input-size polynomial and pipeline RawRefinement remain missing.',
  'PipelineStageBridges proves exact framer-to-simulator and accept/reject-to-handoff launches, collision-free first-match dispatch, exact cumulative work cost, six-for-one compiled raw cost, and accept/reject/timeout classification for supplied exact target traces. TerminalOutputPacker separately proves exact terminal packing and ordinary raw output equality with a local 18*n^2 + 36*n + 6 bound. No theorem yet composes that stage with the bridges, proves target termination, supplies a complete pipeline RawRefinement or external-input-size polynomial, establishes CNF-SAT in P or NP-completeness, or proves P = NP.',
  'TerminalOutputPacker is a literal finite work machine that uniformly packs empty, one-bit, odd, even, all-zero, all-one, and mixed logical outputs in the presence of arbitrary exterior garbage. It proves exact work execution, a designated halt, ordinary blank-delimited raw output equality, exact six-for-one compilation, the local raw bound 18*n^2 + 36*n + 6, and one-step-short timeout, all with empty axiom closure. It starts from an encoded internal configuration and is not yet launched from PipelineStageBridges, so it does not establish a complete pipeline RawRefinement, an external encoded-input-size polynomial, CNF-SAT in P, NP-completeness, or P = NP.',
...
const EXACT_FIELDS = Object.freeze({
  ...
  publicSurfaceBaselineCoordinate: 'PUBLIC-SURFACE-BASELINE-2026-07-12-TERMINAL-OUTPUT-PACKER-23',
...
function publicationExpected0(publication, inventory, publicationMap, publicationMapSha256, sourceClosureSha256) {
  ...
  return {
    ...
    canonicalReportCoordinate: 'PNP-CANONICAL-FORMAL-RECONSTRUCTION-REPORT-2026-07-12-23',
