...
export async function CheckFormalReconstructionStatus0(options = {}) {
  ...
  try {
    ...
    const checks = [
      validateStatus0(statusRead.value, STATUS_PATH),
      validateStatus0(siteRead.value, SITE_PATH),
      validateMirror0(statusRead.value, siteRead.value),
      validateLegacyTombstone0(legacyRead.value),
      validateWithdrawalManifest0(withdrawalRead.value),
    ];
    for (const check of checks) if (check.tag === 'reject') return write0(root, outputPath, writeOutput, check);

    const evidence = await digestEvidence0({ root, paths: [STATUS_PATH, SITE_PATH, LEGACY_STATUS_PATH, WITHDRAWAL_MANIFEST_PATH, 'FORMAL_RECONSTRUCTION.md'] });
...
export function EvaluateFormalReleaseGateExample0(input) {
  ...
  const requiredTrue = ['closedLeanRootTheorem', 'concreteMachineSemantics', 'noProjectSpecificAxioms', 'noSorryOrAdmit', 'formalPolynomialRuntimeProof', 'paperTheoremInventoryMatch', 'generatedSiteStatus'];
  const missing = requiredTrue.filter((key) => input[key] !== true);
  const forbidden = ['usesJsonBooleanActivation', 'usesJavaScriptCheckerAcceptanceAsTheoremEvidence', 'requiresHumanReview'].filter((key) => input[key] !== false);
  return { tag: 'accept', formalReleaseGatePassed: missing.length === 0 && forbidden.length === 0, publicTheoremEmissionAllowed: missing.length === 0 && forbidden.length === 0, missing, forbidden };
}
...
function validateStatus0(status, label) {
  ...
  const exact = {
    kind: 'PNPFormalReconstructionStatus0', version: VERSION, project: 'PNP', coordinate: COORD,
    status: 'formal-reconstruction-in-progress', claimStatus: 'target-theorem-not-formally-established', targetTheorem: 'P = NP',
    publicTheoremEmissionAllowed: false, publicTheoremStatement: null, publicTheoremConclusion: null,
    publicTheoremUnderCheckerTrustModel: false, finalTheoremReady: false, internalFinalTheoremReady: false,
    unrestrictedFinalSoundnessDischarged: false, uniformFinalSoundnessProved: false, satInPConclusionAccepted: false,
    pEqualsNPConclusionAccepted: false, rootLeanTheoremPresent: false, closedLeanTheoremPresent: false,
    formalReleaseGatePassed: false, projectSpecificAxiomsRemaining: true, legacyCheckerAcceptanceIsTheoremEvidence: false,
    humanReviewRequiredForMathematicalValidity: false, historicalActivationCoordinate: HISTORICAL_ACTIVATION_COORD,
    historicalActivationSuperseded: true, historicalReportRetained: true, currentStatusPayload: STATUS_PATH,
    siteStatusPayload: SITE_PATH, legacyActivatedStatusPayload: LEGACY_STATUS_PATH,
  };
  for (const [key, expected] of Object.entries(exact)) if (status[key] !== expected) return reject0('FormalReconstructionStatus.Field', [label, key], 'status field mismatch', { expected, actual: status[key] });
  ...
  for (const key of ['requiresClosedLeanRootTheorem', 'requiresConcreteMachineSemantics', 'requiresNoProjectSpecificAxioms', 'requiresNoSorryOrAdmit', 'requiresFormalPolynomialRuntimeProof', 'requiresPaperTheoremInventoryMatch', 'requiresGeneratedSiteStatus']) if (gate[key] !== true) return reject0('FormalReconstructionStatus.ReleaseGateField', [label, 'releaseGate', key], 'release gate required field must be true', { actual: gate[key] });
  for (const key of ['allowsJsonBooleanActivation', 'allowsJavaScriptCheckerAcceptanceAsTheoremEvidence', 'requiresHumanReview', 'passed']) if (gate[key] !== false) return reject0('FormalReconstructionStatus.ReleaseGateField', [label, 'releaseGate', key], 'release gate forbidden or incomplete field must be false', { actual: gate[key] });
...
function validateLegacyTombstone0(status) {
  ...
  const exact = { kind: 'PNPActivationSupersession0', version: VERSION, project: 'PNP', coordinate: 'PNP-ACTIVATED-STATUS-2026-07-05-01', status: 'superseded', claimStatus: 'historical-activation-withdrawn', publicTheoremEmissionAllowed: false, finalTheoremReady: false, superseded: true, supersededByCoordinate: COORD, supersededByPath: STATUS_PATH, siteStatusPath: SITE_PATH, historicalRecordRetainedInGit: true };
...
function validateWithdrawalManifest0(manifest) {
  ...
  const exact = { kind: 'PNPPublicTheoremActivationWithdrawal0', version: VERSION, coordinate: 'PNP-PUBLIC-THEOREM-ACTIVATION-WITHDRAWAL-2026-07-09-01', status: 'public-theorem-activation-withdrawn', publicTheoremActivationAccepted: false, publicTheoremActivationWithdrawn: true, publicTheoremEmissionAllowed: false, finalTheoremReady: false, formalReleaseGatePassed: false, supersedesCoordinate: HISTORICAL_ACTIVATION_COORD, supersededByStatusCoordinate: COORD };
...
function validateMirror0(a, b) {
  const left = stableStringify0(a); const right = stableStringify0(b);
...
async function readJson0({ root, filePath, override, label }) { if (override !== undefined) { const bytes = Buffer.from(`${JSON.stringify(override, null, 2)}\n`, 'utf8'); return { tag: 'accept', value: override, bytes }; } try { const bytes = await readFile(path.join(root, filePath)); return { tag: 'accept', value: JSON.parse(bytes.toString('utf8')), bytes }; } catch (error) { return reject0('FormalReconstructionStatus.ReadOrParseFailed', [filePath], `could not read or parse ${label}`, normalizeError0(error)); } }
async function digestEvidence0({ root, paths }) { const evidence = []; for (const rel of paths) { try { const abs = path.join(root, rel); const st = await stat(abs); if (!st.isFile()) return reject0('FormalReconstructionStatus.EvidenceNotFile', ['evidence', rel], 'evidence path is not a file'); const bytes = await readFile(abs); evidence.push({ path: rel, sha256: sha256Hex0(bytes), bytes: bytes.length }); } catch (error) { return reject0('FormalReconstructionStatus.EvidenceMissing', ['evidence', rel], 'evidence file missing', normalizeError0(error)); } } return { tag: 'accept', evidence }; }
async function write0(root, outputPath, writeOutput, verdict) { const rendered = { ...verdict, outputPath: writeOutput ? outputPath : null }; if (writeOutput) { const target = path.join(root, outputPath); await mkdir(path.dirname(target), { recursive: true }); await writeFile(target, `${JSON.stringify(rendered, null, 2)}\n`, 'utf8'); } return rendered; }
function reject0(coord, pathArray, reason, witness = {}) { return { tag: 'reject', kind: 'reject', checker: CHECKER, version: VERSION, coord, path: pathArray, witness: { reason, ...witness }, publicTheoremEmissionAllowed: false, finalTheoremReady: false, formalReleaseGatePassed: false }; }
...
async function main0() { let options; try { options = parseArgs0(process.argv.slice(2)); } catch (error) { const verdict = reject0('FormalReconstructionStatus.CliBadArgument', [], 'bad CLI argument', normalizeError0(error)); console.error(JSON.stringify(verdict, null, 2)); process.exit(2); } const verdict = await CheckFormalReconstructionStatus0(options); const rendered = JSON.stringify(verdict, null, 2); if (options.json || verdict.tag === 'accept') console.log(rendered); else console.error(rendered); process.exit(verdict.tag === 'accept' ? 0 : 1); }
