...
const VERSION = 0;
const COORDINATE = 'PNP-FORMAL-RECONSTRUCTION-STATUS-2026-07-16-45';
...
const NON_CLAIMS = Object.freeze([
  ...
  'CookLevin.BuilderTokenAppender is a standalone fixed 59-rule state-selected work machine. For every raw source word, arbitrary exterior-left garbage, canonical prior token list, and each of F, T, separator, and finish, it appends exactly the selected two-bit token and restores the input focus. Its distinguished start requests the first T header token, reaches that supplied tally endpoint within 24*n + 48 compiled raw steps, and those bits are proved equal to direct formula coordinates zero and one and to encodedFormula.take 2 for both Cook-Levin input modes. Malformed tally/output symbols and one-step-short fuel remain timeout. The input prefix is not composed with this table; no remaining header, dynamic cursor interpreter, complete formula builder, RawRefinement, PolynomialReduction, CNFSAT NP-completeness or in-P theorem, or P = NP theorem is supplied.',
...
const EXACT_FIELDS = Object.freeze({
  ...
  publicSurfaceBaselineCoordinate: 'PUBLIC-SURFACE-BASELINE-2026-07-16-COOK-LEVIN-BUILDER-TOKEN-APPENDER-44',
...
function publicationExpected0(publication, inventory, publicationMap, publicationMapSha256, sourceClosureSha256) {
  ...
  return {
    ...
    canonicalReportCoordinate: 'PNP-CANONICAL-FORMAL-RECONSTRUCTION-REPORT-2026-07-16-45',
