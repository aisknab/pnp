...
const NON_CLAIMS = Object.freeze([
  ...
  'PipelineTerminalBridge is one literal extended finite work machine containing the earlier bridge rules and two disjoint terminal-packer copies. It preserves every successful prior step and exact trace, and for each supplied accepting or rejecting target execution proves one exact trace from ordinary paired startConfig through terminal packing, the corresponding bounded verdict, six-for-one compiled execution, ordinary machineOutput equality, and stuck-endpoint timeout. PipelinePairedCompiler supplies target termination from PolynomialTimeMachine.haltsWithin and an external polynomial for canonical pairs; malformed-input behavior and a complete RawRefinement remain absent.',
  'PipelinePairedCompiler compiles the complete four-stage work table to one finite raw Machine. For every proof-bearing PolynomialTimeMachine target and every left/right bitstring pair, pairedPipeline_boundedDecide_eq, pairedPipeline_machineOutput_eq, pairedPipeline_ne_timeout, and pairedPipeline_accepts_iff are uniform all-pair theorems with empty axiom closure. The output polynomial is m + p(m) + 1; the runtime polynomial explicitly adds the quadratic framer, two six-step launches, 18*p(m), and output-bound substitutions into the handoff and terminal-packer polynomials. This milestone is canonical-pair-only: malformed and arbitrary bitstrings remain unspecified, general RawRefinement is absent, Formal.ConcreteComplexityMachineLink remains open, and there is no CNF-SAT in P, NP-completeness, or P = NP claim.',
