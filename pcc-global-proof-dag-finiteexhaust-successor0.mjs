...
export const GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'GlobalProofDAGSemanticFiniteExhaustReleasePolicy0',
  version: CHECKER_VERSION,
  semanticKBundleInputMustRemainDevelopmentPurpose: true,
  predecessorGlobalGateMustRemainDevelopmentOnly: true,
  predecessorGlobalGateCannotImplyFiniteExhaustReadiness: true,
...
export const GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'GlobalProofDAGSemanticFiniteExhaustCheckerBinding0',
  version: CHECKER_VERSION,
  predecessorGlobalChecker: 'CheckGlobalProofDAGTraceIndSuccessor0',
  semanticKBundleChecker: 'CheckKBundleFiniteExhaustSuccessor0',
  semanticKBundleFinalChecker:
    'CheckKBundleFiniteExhaustFinalTheoremReadiness0',
});

export function makeGlobalProofDAGFiniteExhaustSuccessor0({
  KBundle = makeKBundleFiniteExhaustSuccessor0(),
  LegacyGlobalProofDAG = makeSyntheticGlobalProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeGlobalProofDAGFiniteExhaustSuccessor0 Purpose must be one of ${GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'GlobalProofDAGSemanticFiniteExhaustSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KBundle,
    LegacyGlobalProofDAG,
    Binding: { ...GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_BINDING0 },
    Policy: { ...GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_POLICY0 },
  };
}

/**
 * Development-facing global gate after FiniteExhaust. The TraceInd predecessor
 * global gate is rerun over the filtered seven-rule bundle, then the expanded
 * KBundle is checked and rebound into a new semantic overlay. Legacy final
 * nodes remain quarantined.
 */
export async function CheckGlobalProofDAGFiniteExhaustSuccessor0(input) {
  return checkGlobalInternal0(input, {
    checker: 'CheckGlobalProofDAGFiniteExhaustSuccessor0',
    requiredPurpose: null,
  });
}

/**
 * Explicit final-theorem global gate. It rejects until both the FiniteExhaust
 * KBundle and every global theorem coordinate have semantic derivations.
 */
export async function CheckGlobalProofDAGFiniteExhaustFinalTheoremReadiness0(
  input,
) {
  return checkGlobalInternal0(input, {
    checker: 'CheckGlobalProofDAGFiniteExhaustFinalTheoremReadiness0',
...
async function checkGlobalInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'FiniteExhaust semantic global-DAG final readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'GlobalProofDAGFiniteExhaustPurpose0NF',
    purpose,
  }));
  ...
  const predecessorBundle = makeKBundleTraceIndSuccessor0({
  ...
  const predecessorGlobalInput = makeGlobalProofDAGTraceIndSuccessor0({
  ...
  const predecessorCall = await callChecker0(
    'CheckGlobalProofDAGTraceIndSuccessor0',
    () => CheckGlobalProofDAGTraceIndSuccessor0(predecessorGlobalInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckGlobalProofDAGTraceIndSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['PredecessorGlobal'],
      witness: {
        reason: 'TraceInd predecessor global gate rejected the seven-rule semantic base',
  ...
  const developmentCall = await callChecker0(
    'CheckKBundleFiniteExhaustSuccessor0',
    () => CheckKBundleFiniteExhaustSuccessor0(developmentBundleInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKBundleFiniteExhaustSuccessor0',
  ...
  if (developmentRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KBundle'],
      witness: {
        reason: 'FiniteExhaust development semantic KBundle rejected',
  ...
  const finalCall = await callChecker0(
    'CheckKBundleFiniteExhaustFinalTheoremReadiness0',
    () => CheckKBundleFiniteExhaustFinalTheoremReadiness0(finalBundleInput),
  );
  if (!finalCall.ok) {
    ledger.push(makeLedgerEntry0(
      'CheckKBundleFiniteExhaustFinalTheoremReadiness0',
  ...
  if (purpose === 'final-theorem' && !gate.finalTheoremReady) {
    return makeRejectRecord0({
      ...
      witness: {
        reason: 'FiniteExhaust successor global proof DAG is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'GlobalProofDAGSemanticFiniteExhaustSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
...
function validateAndBuildSemanticOverlay0({ dag, bundleNF }) {
  ...
  if (!Array.isArray(nodes)) {
    return validationReject0(
      ...
      'legacy global DAG must expose Nodes for FiniteExhaust semantic overlay',
      { actual: typeof nodes },
    );
  }
  ...
  if (!sameCanonical0(supportedRules, FINITEEXHAUST_RULES0)) {
    return validationReject0(
      ['KBundle', 'NF', 'semanticKImplSupportedRules'],
      'FiniteExhaust global overlay requires the exact expanded semantic rule set',
      { expected: FINITEEXHAUST_RULES0, actual: supportedRules },
    );
  }
  ...
  for (const rule of [...supportedRules, ...missingRules]) {
    if (!nodeById.has(`K.${rule}`)) {
      return validationReject0(
        ...
        'FiniteExhaust semantic overlay requires a legacy kernel coordinate for every required primitive rule',
        { rule },
      );
    }
  }
  ...
  for (const id of requiredFinalNodeIds) {
    ...
    if (node === undefined || node.nodeKind !== 'final') {
      return validationReject0(
        ...
        'FiniteExhaust semantic overlay requires every legacy final coordinate as a quarantined final node',
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGSemanticFiniteExhaustOverlay0NF',
...
function makeComputedGlobalGate0({
  ...
  const nodes = [
    ...
    {
      id: 'Gate.KBundle.FiniteExhaustDevelopmentAcceptance',
      premises: ['Gate.PredecessorGlobal.DevelopmentAcceptance'],
      ready: true,
      digest: bundleDevelopmentDigest,
    },
    {
      id: 'Gate.KBundle.FiniteExhaustFinalReadiness',
      premises: ['Gate.KBundle.FiniteExhaustDevelopmentAcceptance'],
  ...
  const blockers = [
    {
      coordinate: 'KBundle.FiniteExhaustFinalReadiness',
      ready: kBundleFinalReady,
      reason: kBundleFinalReady
        ? null
        : 'FiniteExhaust successor KBundle semantic readiness remains blocked',
      digest: bundleFinalDigest,
    },
    {
  ...
  return Object.freeze({
    kind: 'GlobalProofDAGComputedSemanticFiniteExhaustGate0',
    version: CHECKER_VERSION,
    semanticKBundleComputedReadinessDigest:
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0(
      [],
      'FiniteExhaust semantic global-DAG input must be an object',
      { actual: typeof input },
    );
  }

  if (input.kind !== 'GlobalProofDAGSemanticFiniteExhaustSuccessor0') {
    return validationReject0(
      ['kind'],
      'FiniteExhaust semantic global-DAG kind must be GlobalProofDAGSemanticFiniteExhaustSuccessor0',
      { actual: input.kind },
    );
  }
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `FiniteExhaust semantic global-DAG version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'FiniteExhaust semantic global-DAG Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_PURPOSES0],
      },
    );
  }
  ...
  for (const field of ['KBundle', 'LegacyGlobalProofDAG', 'Binding', 'Policy']) {
    if (!Object.prototype.hasOwnProperty.call(input, field)) {
      return validationReject0(
        [field],
        'FiniteExhaust semantic global-DAG input is missing a required field',
        { field },
      );
    }
  }
  ...
  if (!isPlainObject0(input.KBundle)) {
    return validationReject0(
      ['KBundle'],
      'FiniteExhaust semantic global-DAG KBundle must be an object',
  ...
  if (!isPlainObject0(input.LegacyGlobalProofDAG)) {
    return validationReject0(
      ['LegacyGlobalProofDAG'],
      'FiniteExhaust semantic global-DAG must include a legacy global DAG object',
      { actual: typeof input.LegacyGlobalProofDAG },
    );
  }

  if (!isPlainObject0(input.Binding)
      || !sameCanonical0(input.Binding, GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['Binding'],
      'FiniteExhaust semantic global-DAG checker binding must match the executable checker boundary',
      {
        expected: GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_BINDING0,
  ...
  if (!isPlainObject0(input.Policy)
      || !sameCanonical0(input.Policy, GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'FiniteExhaust semantic global-DAG release policy must match the fail-closed policy',
      {
        expected: GLOBAL_DAG_FINITEEXHAUST_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGSemanticFiniteExhaustSuccessorShape0NF',
...
function validatePredecessorGlobalBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'TraceInd predecessor global gate did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'TraceInd predecessor global semantic overlay mismatch',
      {
  ...
  if (!Array.isArray(nf.activeFinalNodeIds) || nf.activeFinalNodeIds.length !== 0) {
    return validationReject0(
      ...
      'TraceInd predecessor global gate must expose no active final node',
      { actual: nf.activeFinalNodeIds },
    );
  }

  return validationAccept0({
    kind: 'GlobalProofDAGFiniteExhaustPredecessorBoundary0NF',
    ...expected,
...
function validateDevelopmentBundleBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ['KBundle', 'NF', field],
        'FiniteExhaust semantic KBundle did not preserve its development-only boundary',
        { field, expected: value, actual: nf[field] },
      );
    }
  }

  if (!sameCanonical0(nf.semanticKImplSupportedRules, FINITEEXHAUST_RULES0)) {
    return validationReject0(
      ['KBundle', 'NF', 'semanticKImplSupportedRules'],
      'FiniteExhaust semantic KBundle supported-rule set mismatch',
      { expected: FINITEEXHAUST_RULES0, actual: nf.semanticKImplSupportedRules },
    );
  }

  return validationAccept0({
    kind: 'GlobalProofDAGFiniteExhaustKBundleDevelopmentBoundary0NF',
    ...expected,
    semanticKImplSupportedRules: [...FINITEEXHAUST_RULES0],
  });
}
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'GlobalProofDAGFiniteExhaustProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  ...
  return validationAcceptWith0(
    { kind: 'GlobalProofDAGFiniteExhaustProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
