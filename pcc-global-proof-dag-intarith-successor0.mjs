...
const PREDECESSOR_NODE_IDS0 = Object.freeze(
  SEMANTIC_KERNEL_SUPPORTED_RULES_MINCOUNTEREXAMPLE0.map(
    (rule) => `K.${rule}`,
  ),
);

export const GLOBAL_DAG_INTARITH_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const GLOBAL_DAG_INTARITH_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'GlobalProofDAGSemanticIntArithReleasePolicy0',
  version: VERSION,
  semanticKBundleInputMustRemainDevelopmentPurpose: true,
  predecessorGlobalGateMustRemainDevelopmentOnly: true,
  predecessorGlobalGateCannotImplyIntArithReadiness: true,
...
export const GLOBAL_DAG_INTARITH_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'GlobalProofDAGSemanticIntArithCheckerBinding0',
  version: VERSION,
  predecessorGlobalChecker:
    'CheckGlobalProofDAGMinCounterexampleSuccessor0',
  semanticKBundleChecker: 'CheckKBundleIntArithSuccessor0',
  semanticKBundleFinalChecker:
    'CheckKBundleIntArithFinalTheoremReadiness0',
});

export function makeGlobalProofDAGIntArithSuccessor0({
  KBundle = makeKBundleIntArithSuccessor0(),
  LegacyGlobalProofDAG = makeSyntheticGlobalProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!GLOBAL_DAG_INTARITH_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeGlobalProofDAGIntArithSuccessor0 Purpose must be one of ${GLOBAL_DAG_INTARITH_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }
  return {
    kind: 'GlobalProofDAGSemanticIntArithSuccessor0',
    version: VERSION,
    Purpose,
    KBundle,
    LegacyGlobalProofDAG,
    Binding: { ...GLOBAL_DAG_INTARITH_SUCCESSOR_BINDING0 },
    Policy: { ...GLOBAL_DAG_INTARITH_SUCCESSOR_POLICY0 },
  };
}

export async function CheckGlobalProofDAGIntArithSuccessor0(input) {
  return checkGlobal0(input, 'CheckGlobalProofDAGIntArithSuccessor0', null);
}

export async function CheckGlobalProofDAGIntArithFinalTheoremReadiness0(
  input,
) {
  return checkGlobal0(
    input,
...
async function checkGlobal0(input, checker, requiredPurpose) {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    return reject0(checker, `${checker}.purpose`, ['Purpose'], {
      reason: 'IntArith semantic global-DAG final readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  const predecessorBundle = makeKBundleMinCounterexampleSuccessor0({
  ...
  const predecessorInput = makeGlobalProofDAGMinCounterexampleSuccessor0({
  ...
  const predecessorCall = await call0(
    'CheckGlobalProofDAGMinCounterexampleSuccessor0',
    () => CheckGlobalProofDAGMinCounterexampleSuccessor0(predecessorInput),
  );
  push0(
    ledger,
    'CheckGlobalProofDAGMinCounterexampleSuccessor0',
  ...
  if (predecessorCall.record.tag !== 'accept') {
    return reject0(checker, `${checker}.predecessorGlobal`, ['PredecessorGlobal'], {
      reason: 'MinCounterexample predecessor global gate rejected the twelve-rule semantic base',
  ...
  if (developmentCall.record.tag !== 'accept') {
    return reject0(checker, `${checker}.semanticKBundle`, ['KBundle'], {
      reason: 'IntArith development semantic KBundle rejected',
  ...
  if (purpose === 'final-theorem' && !gate.finalTheoremReady) {
    return reject0(
      ...
      ['ComputedGlobalGate'],
      {
        reason: 'IntArith successor global proof DAG is not ready for final-theorem use',
  ...
  return accept0(checker, {
    kind: 'GlobalProofDAGSemanticIntArithSuccessor0NF',
    checker,
    version: VERSION,
    purpose,
...
function buildOverlay0(dag, bundleNF) {
  ...
  if (!Array.isArray(nodes)) {
    return invalid0(
      ...
      'legacy global DAG must expose Nodes for IntArith semantic overlay',
      { actual: typeof nodes },
    );
  }
  ...
  const missingRules = Array.isArray(bundleNF.semanticKImplMissingRules)
    ...
    : [];
  if (!same0(supportedRules, SEMANTIC_KERNEL_SUPPORTED_RULES_INTARITH0)) {
    return invalid0(
      ['KBundle', 'NF', 'semanticKImplSupportedRules'],
      'IntArith global overlay requires the exact expanded semantic rule set',
      {
        expected: [...SEMANTIC_KERNEL_SUPPORTED_RULES_INTARITH0],
  ...
  for (const rule of [...supportedRules, ...missingRules]) {
    if (!byId.has(`K.${rule}`)) {
      return invalid0(
        ...
        'IntArith semantic overlay requires a legacy kernel coordinate for every required primitive rule',
        { rule },
      );
    }
  }
  ...
  for (const id of requiredFinalNodeIds) {
    if (byId.get(id)?.nodeKind !== 'final') {
      return invalid0(
        ...
        'IntArith semantic overlay requires every legacy final coordinate as a quarantined final node',
  ...
  return valid0({
    ...
    structuralOnlyNodeCount: nodes.length - semanticKernelNodeIds.length,
...
function makeGate0(
  ...
  const blockers = [
    Object.freeze({
      coordinate: 'KBundle.IntArithFinalReadiness',
      ready: kBundleFinalReady,
      reason: kBundleFinalReady
        ? null
        : 'IntArith successor KBundle semantic readiness remains blocked',
      digest: finalDigest,
    }),
    Object.freeze({
  ...
  return Object.freeze({
    kind: 'GlobalProofDAGComputedSemanticIntArithGate0',
    version: VERSION,
    semanticKBundleComputedReadinessDigest:
      developmentNF.computedReadinessDigest ?? null,
...
function validateShape0(input) {
  if (!plain0(input)) {
    return invalid0([], 'IntArith semantic global-DAG input must be an object', {
      actual: typeof input,
    });
  }
  if (input.kind !== 'GlobalProofDAGSemanticIntArithSuccessor0') {
    return invalid0(
      ['kind'],
      'IntArith semantic global-DAG kind must be GlobalProofDAGSemanticIntArithSuccessor0',
      { actual: input.kind },
    );
  }
  if (input.version !== VERSION) {
    return invalid0(['version'], `IntArith semantic global-DAG version must be ${VERSION}`, {
      actual: input.version,
    });
  }
  if (!GLOBAL_DAG_INTARITH_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return invalid0(['Purpose'], 'IntArith semantic global-DAG Purpose is unsupported', {
      actual: input.Purpose,
      supportedPurposes: [...GLOBAL_DAG_INTARITH_SUCCESSOR_PURPOSES0],
    });
  }
  if (!plain0(input.KBundle)) {
    return invalid0(['KBundle'], 'IntArith semantic global-DAG KBundle must be an object', {
  ...
  if (input.KBundle.Purpose !== 'development') {
    return invalid0(
      ['KBundle', 'Purpose'],
      'IntArith semantic global-DAG input KBundle must remain development-purpose; final readiness is recomputed internally',
  ...
  if (!plain0(input.LegacyGlobalProofDAG)) {
    return invalid0(
      ['LegacyGlobalProofDAG'],
      'IntArith semantic global-DAG must include a legacy global DAG object',
      { actual: typeof input.LegacyGlobalProofDAG },
    );
  }
  if (!same0(input.Binding, GLOBAL_DAG_INTARITH_SUCCESSOR_BINDING0)) {
    return invalid0(
      ['Binding'],
      'IntArith semantic global-DAG checker binding must match the executable checker boundary',
      { expected: GLOBAL_DAG_INTARITH_SUCCESSOR_BINDING0, actual: input.Binding },
    );
  }
  if (!same0(input.Policy, GLOBAL_DAG_INTARITH_SUCCESSOR_POLICY0)) {
    return invalid0(
      ['Policy'],
      'IntArith semantic global-DAG release policy must match the fail-closed policy',
      { expected: GLOBAL_DAG_INTARITH_SUCCESSOR_POLICY0, actual: input.Policy },
    );
  }
  ...
  return valid0({
    kind: 'GlobalProofDAGSemanticIntArithSuccessorShape0NF',
...
function validatePredecessor0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return invalid0(
        ...
        'MinCounterexample predecessor global gate did not preserve its development-only boundary',
  ...
  if (!Array.isArray(nf.activeFinalNodeIds)
    ...
    return invalid0(
      ...
      'MinCounterexample predecessor global gate must expose no active final node',
  ...
  if (!same0(
    ...
    return invalid0(
      ...
      'MinCounterexample predecessor global semantic-kernel node set mismatch',
      {
...
function validateDevelopmentBundle0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return invalid0(
        ['KBundle', 'NF', field],
        'IntArith semantic KBundle did not preserve its development-only boundary',
        { field, expected: value, actual: nf[field] },
      );
    }
  }
  if (!same0(
    ...
    return invalid0(
      ['KBundle', 'NF', 'semanticKImplSupportedRules'],
      'IntArith semantic KBundle supported-rule set mismatch',
      {
        expected: [...SEMANTIC_KERNEL_SUPPORTED_RULES_INTARITH0],
