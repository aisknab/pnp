...
const PREDECESSOR_NODE_IDS0 = Object.freeze(
  SEMANTIC_KERNEL_SUPPORTED_RULES_RANKIND0.map((rule) => `K.${rule}`),
);

export const GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'GlobalProofDAGSemanticMinCounterexampleReleasePolicy0',
  version: CHECKER_VERSION,
  semanticKBundleInputMustRemainDevelopmentPurpose: true,
  predecessorGlobalGateMustRemainDevelopmentOnly: true,
  predecessorGlobalGateCannotImplyMinCounterexampleReadiness: true,
...
export const GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'GlobalProofDAGSemanticMinCounterexampleCheckerBinding0',
  version: CHECKER_VERSION,
  predecessorGlobalChecker: 'CheckGlobalProofDAGRankIndSuccessor0',
  semanticKBundleChecker: 'CheckKBundleMinCounterexampleSuccessor0',
  semanticKBundleFinalChecker:
    'CheckKBundleMinCounterexampleFinalTheoremReadiness0',
});

export function makeGlobalProofDAGMinCounterexampleSuccessor0({
  KBundle = makeKBundleMinCounterexampleSuccessor0(),
  LegacyGlobalProofDAG = makeSyntheticGlobalProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeGlobalProofDAGMinCounterexampleSuccessor0 Purpose must be one of ${GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'GlobalProofDAGSemanticMinCounterexampleSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KBundle,
    LegacyGlobalProofDAG,
    Binding: { ...GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_BINDING0 },
    Policy: { ...GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_POLICY0 },
  };
}

export async function CheckGlobalProofDAGMinCounterexampleSuccessor0(input) {
  return checkGlobalInternal0(input, {
    checker: 'CheckGlobalProofDAGMinCounterexampleSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckGlobalProofDAGMinCounterexampleFinalTheoremReadiness0(
  input,
) {
  return checkGlobalInternal0(input, {
    checker: 'CheckGlobalProofDAGMinCounterexampleFinalTheoremReadiness0',
...
async function checkGlobalInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'MinCounterexample semantic global-DAG final readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'GlobalProofDAGMinCounterexamplePurpose0NF',
    purpose,
  }));
  ...
  const predecessorNodes = fullNodes.nodes.filter(
    (node) => SEMANTIC_KERNEL_SUPPORTED_RULES_RANKIND0.includes(
      node?.RuleName,
    ),
  );
  const predecessorBundle = makeKBundleRankIndSuccessor0({
  ...
  const predecessorGlobalInput = makeGlobalProofDAGRankIndSuccessor0({
  ...
  const predecessorCall = await callChecker0(
    'CheckGlobalProofDAGRankIndSuccessor0',
    () => CheckGlobalProofDAGRankIndSuccessor0(predecessorGlobalInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckGlobalProofDAGRankIndSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['PredecessorGlobal'],
      witness: {
        reason: 'RankInd predecessor global gate rejected the eleven-rule semantic base',
  ...
  const developmentCall = await callChecker0(
    'CheckKBundleMinCounterexampleSuccessor0',
    () => CheckKBundleMinCounterexampleSuccessor0(developmentBundleInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKBundleMinCounterexampleSuccessor0',
  ...
  if (developmentRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KBundle'],
      witness: {
        reason: 'MinCounterexample development semantic KBundle rejected',
  ...
  if (purpose === 'final-theorem' && !gate.finalTheoremReady) {
    return makeRejectRecord0({
      ...
      witness: {
        reason: 'MinCounterexample successor global proof DAG is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'GlobalProofDAGSemanticMinCounterexampleSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
...
function validateAndBuildSemanticOverlay0({ dag, bundleNF }) {
  ...
  if (!Array.isArray(nodes)) {
    return validationReject0(
      ...
      'legacy global DAG must expose Nodes for MinCounterexample semantic overlay',
      { actual: typeof nodes },
    );
  }
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'MinCounterexample global overlay requires the exact expanded semantic rule set',
      {
        expected: [
          ...SEMANTIC_KERNEL_SUPPORTED_RULES_MINCOUNTEREXAMPLE0,
        ],
  ...
  for (const rule of [...supportedRules, ...missingRules]) {
    if (!nodeById.has(`K.${rule}`)) {
      return validationReject0(
        ...
        'MinCounterexample semantic overlay requires a legacy kernel coordinate for every required primitive rule',
        { rule },
      );
    }
  }
  ...
  for (const id of requiredFinalNodeIds) {
    ...
    if (node === undefined || node.nodeKind !== 'final') {
      return validationReject0(
        ...
        'MinCounterexample semantic overlay requires every legacy final coordinate as a quarantined final node',
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGSemanticMinCounterexampleOverlay0NF',
...
function makeComputedGlobalGate0({
  ...
  const nodes = [
    ...
    {
      id: 'Gate.KBundle.MinCounterexampleDevelopmentAcceptance',
      premises: ['Gate.PredecessorGlobal.DevelopmentAcceptance'],
      ready: true,
      digest: bundleDevelopmentDigest,
    },
    {
      id: 'Gate.KBundle.MinCounterexampleFinalReadiness',
      premises: ['Gate.KBundle.MinCounterexampleDevelopmentAcceptance'],
  ...
  const blockers = [
    {
      coordinate: 'KBundle.MinCounterexampleFinalReadiness',
      ready: kBundleFinalReady,
      reason: kBundleFinalReady
        ? null
        : 'MinCounterexample successor KBundle semantic readiness remains blocked',
      digest: bundleFinalDigest,
    },
    {
  ...
  return Object.freeze({
    kind: 'GlobalProofDAGComputedSemanticMinCounterexampleGate0',
    version: CHECKER_VERSION,
    semanticKBundleComputedReadinessDigest:
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0(
      [],
      'MinCounterexample semantic global-DAG input must be an object',
      { actual: typeof input },
    );
  }

  if (input.kind !== 'GlobalProofDAGSemanticMinCounterexampleSuccessor0') {
    return validationReject0(
      ['kind'],
      'MinCounterexample semantic global-DAG kind must be GlobalProofDAGSemanticMinCounterexampleSuccessor0',
      { actual: input.kind },
    );
  }
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `MinCounterexample semantic global-DAG version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_PURPOSES0.includes(
    input.Purpose,
  )) {
    return validationReject0(
      ['Purpose'],
      'MinCounterexample semantic global-DAG Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [
          ...GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_PURPOSES0,
        ],
      },
    );
  }
  ...
  for (const field of ['KBundle', 'LegacyGlobalProofDAG', 'Binding', 'Policy']) {
    if (!Object.prototype.hasOwnProperty.call(input, field)) {
      return validationReject0(
        [field],
        'MinCounterexample semantic global-DAG input is missing a required field',
        { field },
      );
    }
  }
  ...
  if (!isPlainObject0(input.KBundle)) {
    return validationReject0(
      ['KBundle'],
      'MinCounterexample semantic global-DAG KBundle must be an object',
  ...
  if (input.KBundle.Purpose !== 'development') {
    return validationReject0(
      ['KBundle', 'Purpose'],
      'MinCounterexample semantic global-DAG input KBundle must remain development-purpose; final readiness is recomputed internally',
  ...
  if (!isPlainObject0(input.LegacyGlobalProofDAG)) {
    return validationReject0(
      ['LegacyGlobalProofDAG'],
      'MinCounterexample semantic global-DAG must include a legacy global DAG object',
  ...
  if (!isPlainObject0(input.Binding)
      || !sameCanonical0(
        input.Binding,
        GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_BINDING0,
      )) {
    return validationReject0(
      ['Binding'],
      'MinCounterexample semantic global-DAG checker binding must match the executable checker boundary',
      {
        expected: GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_BINDING0,
  ...
  if (!isPlainObject0(input.Policy)
      || !sameCanonical0(
        input.Policy,
        GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_POLICY0,
      )) {
    return validationReject0(
      ['Policy'],
      'MinCounterexample semantic global-DAG release policy must match the fail-closed policy',
      {
        expected: GLOBAL_DAG_MINCOUNTEREXAMPLE_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGSemanticMinCounterexampleSuccessorShape0NF',
...
function validatePredecessorGlobalBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'RankInd predecessor global gate did not preserve its development-only boundary',
  ...
  if (!Array.isArray(nf.activeFinalNodeIds)
    ...
    return validationReject0(
      ...
      'RankInd predecessor global gate must expose no active final node',
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'RankInd predecessor global semantic-kernel node set mismatch',
      {
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGMinCounterexamplePredecessorBoundary0NF',
    ...expected,
    activeFinalNodeIds: [],
...
function validateDevelopmentBundleBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ['KBundle', 'NF', field],
        'MinCounterexample semantic KBundle did not preserve its development-only boundary',
        { field, expected: value, actual: nf[field] },
      );
    }
  }
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ['KBundle', 'NF', 'semanticKImplSupportedRules'],
      'MinCounterexample semantic KBundle supported-rule set mismatch',
      {
        expected: [
          ...SEMANTIC_KERNEL_SUPPORTED_RULES_MINCOUNTEREXAMPLE0,
        ],
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGMinCounterexampleKBundleBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      {
        kind: 'GlobalProofDAGMinCounterexampleProofInput0NF',
        form: 'array',
