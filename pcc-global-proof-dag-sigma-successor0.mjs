...
export const GLOBAL_DAG_SIGMA_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const GLOBAL_DAG_SIGMA_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'GlobalProofDAGSemanticSigmaReleasePolicy0',
  version: CHECKER_VERSION,
  semanticKBundleInputMustRemainDevelopmentPurpose: true,
  predecessorGlobalGateMustRemainDevelopmentOnly: true,
  predecessorGlobalGateCannotImplySigmaReadiness: true,
  legacyGlobalDAGAcceptanceIsStructuralOnly: true,
  sigmaNodesBindSemanticDerivationDigests: true,
...
export const GLOBAL_DAG_SIGMA_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'GlobalProofDAGSemanticSigmaCheckerBinding0',
  version: CHECKER_VERSION,
  predecessorGlobalChecker: 'CheckGlobalProofDAGK0ConformanceSuccessor0',
  semanticKBundleChecker: 'CheckKBundleSigmaSuccessor0',
  semanticKBundleFinalChecker: 'CheckKBundleSigmaFinalTheoremReadiness0',
});

export function makeGlobalProofDAGSigmaSuccessor0({
  KBundle = makeKBundleSigmaSuccessor0(),
  LegacyGlobalProofDAG = makeSyntheticGlobalProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!GLOBAL_DAG_SIGMA_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
  ...
  return {
    ...
    Binding: { ...GLOBAL_DAG_SIGMA_SUCCESSOR_BINDING0 },
    Policy: { ...GLOBAL_DAG_SIGMA_SUCCESSOR_POLICY0 },
  };
}

export async function CheckGlobalProofDAGSigmaSuccessor0(input) {
  return checkGlobalInternal0(input, {
    checker: 'CheckGlobalProofDAGSigmaSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckGlobalProofDAGSigmaFinalTheoremReadiness0(input) {
  return checkGlobalInternal0(input, {
    checker: 'CheckGlobalProofDAGSigmaFinalTheoremReadiness0',
...
async function checkGlobalInternal0(input, { checker, requiredPurpose }) {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'semantic Sigma global-DAG final readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'GlobalProofDAGSigmaPurpose0NF',
    purpose,
  }));
  ...
  const predecessorGlobalInput = makeGlobalProofDAGK0ConformanceSuccessor0({
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['PredecessorGlobal'],
      witness: {
        reason: 'semantic K0 conformance predecessor global gate rejected before Sigma upgrade',
  ...
  if (developmentRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KBundle'],
      witness: {
        reason: 'semantic Sigma development KBundle rejected',
  ...
  if (purpose === 'final-theorem' && !gate.finalTheoremReady) {
    return makeRejectRecord0({
      ...
      witness: {
        reason: 'semantic Sigma global proof DAG is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'GlobalProofDAGSemanticSigmaSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticSigmaTheorems: developmentNF.semanticSigmaTheorems ?? [],
      semanticSigmaDerivationDigests:
        developmentNF.semanticSigmaDerivationDigests ?? [],
...
function validateAndBuildSemanticOverlay0({ dag, bundleNF }) {
  ...
  if (!Array.isArray(nodes)) {
    return validationReject0(
      ...
      'legacy global DAG must expose Nodes for semantic Sigma overlay',
      { actual: typeof nodes },
    );
  }
  ...
  if (missingRules.length !== 0) {
    return validationReject0(
      ...
      'semantic Sigma global overlay requires an empty primitive missing-rule set',
      { actual: missingRules },
    );
  }
  if (bundleNF.semanticK0ConformanceReady !== true) {
    return validationReject0(
      ...
      'semantic Sigma global overlay requires accepted K0 semantic conformance',
      { actual: bundleNF.semanticK0ConformanceReady },
    );
  }
  if (bundleNF.semanticSigmaReady !== true) {
    return validationReject0(
      ['KBundle', 'NF', 'semanticSigmaReady'],
      'semantic Sigma global overlay requires accepted Sigma derivations',
      { actual: bundleNF.semanticSigmaReady },
    );
  }
  ...
  for (const rule of supportedRules) {
    if (!nodeById.has(`K.${rule}`)) {
      return validationReject0(
        ...
        'semantic Sigma overlay requires a legacy kernel coordinate for every primitive rule',
        { rule },
      );
    }
  }
  ...
  const derivationByTheorem = new Map(
    (bundleNF.semanticSigmaDerivationDigests ?? []).map((entry) => [
      entry.theorem,
      entry,
    ]),
  );
  const semanticSigmaBindings = [];
  for (const theorem of ['V53', 'V54']) {
    const nodeId = `Sigma.${theorem}`;
    const node = nodeById.get(nodeId);
    if (node === undefined || node.nodeKind !== 'sigma') {
      return validationReject0(
        ['LegacyGlobalProofDAG', 'Nodes', 'sigma', theorem],
        'semantic Sigma overlay requires every legacy Sigma coordinate as a sigma node',
        { theorem, actualKind: node?.nodeKind ?? null },
      );
    }
    if (!sameCanonical0(node.premises, ['K.FiniteRel', 'K.IntArith'])) {
      return validationReject0(
    ...
    if (node.conclusion?.tag !== 'SigmaTheoremAccepted0'
        || node.conclusion?.theorem !== theorem) {
      return validationReject0(
        ['LegacyGlobalProofDAG', 'Nodes', 'sigma', theorem, 'conclusion'],
        'semantic Sigma node conclusion must match its theorem coordinate',
        { theorem, actual: node.conclusion },
      );
    }
    const derivation = derivationByTheorem.get(theorem);
    if (derivation === undefined) {
      return validationReject0(
        ['KBundle', 'NF', 'semanticSigmaDerivationDigests', theorem],
        'semantic Sigma overlay is missing a checked derivation digest',
        { theorem },
      );
    }
    semanticSigmaBindings.push(Object.freeze({
      theorem,
      nodeId,
      ...
      registryEntryDigest: derivation.registryEntryDigest,
    }));
  }
  ...
  for (const id of requiredFinalNodeIds) {
    ...
    if (node === undefined || node.nodeKind !== 'final') {
      return validationReject0(
        ...
        'semantic Sigma overlay requires every legacy final coordinate as a quarantined final node',
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGSemanticSigmaOverlay0NF',
...
function makeComputedGlobalGate0({
  ...
  const blockers = [
    Object.freeze({
      ...
      reason: kBundleFinalReady
        ? null
        : 'semantic Sigma KBundle readiness remains blocked by reflection soundness',
    ...
    Object.freeze({
      ...
      reason: overlay.globalSemanticNodeDerivationsReady === true
        ? null
        : 'legacy reflection, row, package, and final nodes have not been migrated to semantic derivations',
  ...
  return Object.freeze({
    ...
    primitiveSemanticRuleCoverageComplete: true,
    semanticK0ConformanceReady: true,
    semanticSigmaReady: overlay.semanticSigmaReady === true,
    semanticReflectionReady: false,
...
function validateShape0(input) {
  ...
  if (input.kind !== 'GlobalProofDAGSemanticSigmaSuccessor0') {
    return validationReject0(
      ['kind'],
      'semantic Sigma global-DAG kind must be GlobalProofDAGSemanticSigmaSuccessor0',
      { actual: input.kind },
    );
  }
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `semantic Sigma global-DAG version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }
  if (!GLOBAL_DAG_SIGMA_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
  ...
  for (const field of ['KBundle', 'LegacyGlobalProofDAG', 'Binding', 'Policy']) {
    if (!Object.prototype.hasOwnProperty.call(input, field)) {
      return validationReject0(
        [field],
        'semantic Sigma global-DAG input is missing a required field',
        { field },
      );
    }
  }
  ...
  if (!sameCanonical0(input.Binding, GLOBAL_DAG_SIGMA_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['Binding'],
      'semantic Sigma global-DAG checker binding must match the executable checker boundary',
      { expected: GLOBAL_DAG_SIGMA_SUCCESSOR_BINDING0, actual: input.Binding },
    );
  }
  if (!sameCanonical0(input.Policy, GLOBAL_DAG_SIGMA_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'semantic Sigma global-DAG release policy must match the fail-closed policy',
      { expected: GLOBAL_DAG_SIGMA_SUCCESSOR_POLICY0, actual: input.Policy },
    );
  }
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGSemanticSigmaSuccessorShape0NF',
...
function validatePredecessorGlobalBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'semantic K0 conformance predecessor global gate did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(nf.semanticOverlay?.blockedKernelNodeIds, [])) {
    return validationReject0(
      ...
      'semantic K0 conformance predecessor global must expose an empty primitive blocked-kernel set',
      { expected: [], actual: nf.semanticOverlay?.blockedKernelNodeIds },
    );
  }
  if (nf.semanticOverlay?.semanticSigmaReady !== false) {
    return validationReject0(
      ['PredecessorGlobal', 'NF', 'semanticOverlay', 'semanticSigmaReady'],
      'semantic K0 conformance predecessor global must keep Sigma semantic readiness blocked',
      { actual: nf.semanticOverlay?.semanticSigmaReady },
    );
  }
  return validationAccept0({
    kind: 'GlobalProofDAGSigmaPredecessorBoundary0NF',
    ...expected,
    activeFinalNodeIds: [],
...
function validateDevelopmentBundleBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ['KBundle', 'NF', field],
        'semantic Sigma KBundle did not preserve its development-only boundary',
        { field, expected: value, actual: nf[field] },
      );
    }
  }
  ...
  if (!sameCanonical0(nf.semanticKImplMissingRules, [])) {
    return validationReject0(
      ['KBundle', 'NF', 'semanticKImplMissingRules'],
      'semantic Sigma KBundle must expose an empty primitive missing-rule set',
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGSigmaKBundleBoundary0NF',
    ...expected,
