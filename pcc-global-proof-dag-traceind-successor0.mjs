...
const TRACEIND_RULES0 = Object.freeze([
  'Eq',
  'Subst',
  'Record',
  'DAGInd',
...
export const GLOBAL_DAG_TRACEIND_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const GLOBAL_DAG_TRACEIND_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'GlobalProofDAGSemanticTraceIndReleasePolicy0',
  version: CHECKER_VERSION,
  semanticKBundleInputMustRemainDevelopmentPurpose: true,
  predecessorGlobalGateMustRemainDevelopmentOnly: true,
  predecessorGlobalGateCannotImplyTraceIndReadiness: true,
...
export const GLOBAL_DAG_TRACEIND_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'GlobalProofDAGSemanticTraceIndCheckerBinding0',
  version: CHECKER_VERSION,
  predecessorGlobalChecker: 'CheckGlobalProofDAGOblTopoIndSuccessor0',
  semanticKBundleChecker: 'CheckKBundleTraceIndSuccessor0',
  semanticKBundleFinalChecker: 'CheckKBundleTraceIndFinalTheoremReadiness0',
});

export function makeGlobalProofDAGTraceIndSuccessor0({
  KBundle = makeKBundleTraceIndSuccessor0(),
  LegacyGlobalProofDAG = makeSyntheticGlobalProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!GLOBAL_DAG_TRACEIND_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeGlobalProofDAGTraceIndSuccessor0 Purpose must be one of ${GLOBAL_DAG_TRACEIND_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'GlobalProofDAGSemanticTraceIndSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KBundle,
    LegacyGlobalProofDAG,
    Binding: { ...GLOBAL_DAG_TRACEIND_SUCCESSOR_BINDING0 },
    Policy: { ...GLOBAL_DAG_TRACEIND_SUCCESSOR_POLICY0 },
  };
}

export async function CheckGlobalProofDAGTraceIndSuccessor0(input) {
  return checkGlobalInternal0(input, {
    checker: 'CheckGlobalProofDAGTraceIndSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckGlobalProofDAGTraceIndFinalTheoremReadiness0(input) {
  return checkGlobalInternal0(input, {
    checker: 'CheckGlobalProofDAGTraceIndFinalTheoremReadiness0',
...
async function checkGlobalInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'TraceInd semantic global-DAG final readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'GlobalProofDAGTraceIndPurpose0NF',
    purpose,
  }));
  ...
  const predecessorBundle = makeKBundleOblTopoIndSuccessor0({
  ...
  const predecessorGlobalInput = makeGlobalProofDAGOblTopoIndSuccessor0({
  ...
  const predecessorCall = await callChecker0(
    'CheckGlobalProofDAGOblTopoIndSuccessor0',
    () => CheckGlobalProofDAGOblTopoIndSuccessor0(predecessorGlobalInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckGlobalProofDAGOblTopoIndSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['PredecessorGlobal'],
      witness: {
        reason: 'OblTopoInd predecessor global gate rejected the six-rule semantic base',
  ...
  const developmentCall = await callChecker0(
    'CheckKBundleTraceIndSuccessor0',
    () => CheckKBundleTraceIndSuccessor0(developmentBundleInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKBundleTraceIndSuccessor0',
  ...
  if (developmentRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KBundle'],
      witness: {
        reason: 'TraceInd development semantic KBundle rejected',
  ...
  const finalCall = await callChecker0(
    'CheckKBundleTraceIndFinalTheoremReadiness0',
    () => CheckKBundleTraceIndFinalTheoremReadiness0(finalBundleInput),
  );
  if (!finalCall.ok) {
    ledger.push(makeLedgerEntry0(
      'CheckKBundleTraceIndFinalTheoremReadiness0',
  ...
  if (purpose === 'final-theorem' && !gate.finalTheoremReady) {
    return makeRejectRecord0({
      ...
      witness: {
        reason: 'TraceInd successor global proof DAG is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'GlobalProofDAGSemanticTraceIndSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticKBundleDevelopmentDigest: developmentRecord.Digest ?? developmentRecord.digest,
      ...
      semanticKBundleSupportedRules: developmentNF.semanticKImplSupportedRules ?? [],
      semanticKBundleMissingRules: developmentNF.semanticKImplMissingRules ?? [],
...
function validateAndBuildSemanticOverlay0({ dag, bundleNF }) {
  ...
  if (!Array.isArray(nodes)) {
    return validationReject0(
      ...
      'legacy global DAG must expose Nodes for TraceInd semantic overlay',
      { actual: typeof nodes },
    );
  }
  ...
  if (!sameCanonical0(supportedRules, TRACEIND_RULES0)) {
    return validationReject0(
      ['KBundle', 'NF', 'semanticKImplSupportedRules'],
      'TraceInd global overlay requires the exact expanded semantic rule set',
      { expected: TRACEIND_RULES0, actual: supportedRules },
    );
  }
  ...
  for (const rule of [...supportedRules, ...missingRules]) {
    if (!nodeById.has(`K.${rule}`)) {
      return validationReject0(
        ...
        'TraceInd semantic overlay requires a legacy kernel coordinate for every required primitive rule',
        { rule },
      );
    }
  }
  ...
  for (const id of requiredFinalNodeIds) {
    ...
    if (node === undefined || node.nodeKind !== 'final') {
      return validationReject0(
        ...
        'TraceInd semantic overlay requires every legacy final coordinate as a quarantined final node',
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGSemanticTraceIndOverlay0NF',
...
function makeComputedGlobalGate0({
  ...
  const nodes = [
    ...
    {
      id: 'Gate.KBundle.TraceIndDevelopmentAcceptance',
      premises: ['Gate.PredecessorGlobal.DevelopmentAcceptance'],
      ready: true,
      digest: bundleDevelopmentDigest,
    },
    {
      id: 'Gate.KBundle.TraceIndFinalReadiness',
      premises: ['Gate.KBundle.TraceIndDevelopmentAcceptance'],
  ...
  const blockers = [
    {
      coordinate: 'KBundle.TraceIndFinalReadiness',
      ready: kBundleFinalReady,
      reason: kBundleFinalReady
        ? null
        : 'TraceInd successor KBundle semantic readiness remains blocked',
      digest: bundleFinalDigest,
    },
    {
  ...
  return Object.freeze({
    kind: 'GlobalProofDAGComputedSemanticTraceIndGate0',
    version: CHECKER_VERSION,
    semanticKBundleComputedReadinessDigest:
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0([], 'TraceInd semantic global-DAG input must be an object', {
      actual: typeof input,
    });
  }

  if (input.kind !== 'GlobalProofDAGSemanticTraceIndSuccessor0') {
    return validationReject0(
      ['kind'],
      'TraceInd semantic global-DAG kind must be GlobalProofDAGSemanticTraceIndSuccessor0',
      { actual: input.kind },
    );
  }
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `TraceInd semantic global-DAG version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!GLOBAL_DAG_TRACEIND_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'TraceInd semantic global-DAG Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...GLOBAL_DAG_TRACEIND_SUCCESSOR_PURPOSES0],
      },
    );
  }
  ...
  for (const field of ['KBundle', 'LegacyGlobalProofDAG', 'Binding', 'Policy']) {
    if (!Object.prototype.hasOwnProperty.call(input, field)) {
      return validationReject0(
        [field],
        'TraceInd semantic global-DAG input is missing a required field',
        { field },
      );
    }
  }
  ...
  if (!isPlainObject0(input.KBundle)) {
    return validationReject0(
      ['KBundle'],
      'TraceInd semantic global-DAG KBundle must be an object',
  ...
  if (!isPlainObject0(input.LegacyGlobalProofDAG)) {
    return validationReject0(
      ['LegacyGlobalProofDAG'],
      'TraceInd semantic global-DAG must include a legacy global DAG object',
      { actual: typeof input.LegacyGlobalProofDAG },
    );
  }

  if (!sameCanonical0(input.Binding, GLOBAL_DAG_TRACEIND_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['Binding'],
      'TraceInd semantic global-DAG checker binding must match the executable checker boundary',
      {
        expected: GLOBAL_DAG_TRACEIND_SUCCESSOR_BINDING0,
        actual: input.Binding,
      },
    );
  }

  if (!sameCanonical0(input.Policy, GLOBAL_DAG_TRACEIND_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'TraceInd semantic global-DAG release policy must match the fail-closed policy',
      {
        expected: GLOBAL_DAG_TRACEIND_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  return validationAccept0({
    kind: 'GlobalProofDAGSemanticTraceIndSuccessorShape0NF',
...
function validatePredecessorGlobalBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'OblTopoInd predecessor global gate did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'OblTopoInd predecessor global semantic overlay mismatch',
      {
  ...
  if (!Array.isArray(nf.activeFinalNodeIds) || nf.activeFinalNodeIds.length !== 0) {
    return validationReject0(
      ...
      'OblTopoInd predecessor global gate must expose no active final node',
      { actual: nf.activeFinalNodeIds },
    );
  }

  return validationAccept0({
    kind: 'GlobalProofDAGTraceIndPredecessorBoundary0NF',
    ...expected,
...
function validateDevelopmentBundleBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ['KBundle', 'NF', field],
        'TraceInd semantic KBundle did not preserve its development-only boundary',
        { field, expected: value, actual: nf[field] },
      );
    }
  }

  if (!sameCanonical0(nf.semanticKImplSupportedRules, TRACEIND_RULES0)) {
    return validationReject0(
      ['KBundle', 'NF', 'semanticKImplSupportedRules'],
      'TraceInd semantic KBundle supported-rule set mismatch',
      { expected: TRACEIND_RULES0, actual: nf.semanticKImplSupportedRules },
    );
  }

  return validationAccept0({
    kind: 'GlobalProofDAGTraceIndKBundleDevelopmentBoundary0NF',
    ...expected,
    semanticKImplSupportedRules: [...TRACEIND_RULES0],
  });
}
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'GlobalProofDAGTraceIndProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  ...
  return validationAcceptWith0(
    { kind: 'GlobalProofDAGTraceIndProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
