...
export const KBUNDLE_INTARITH_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KBUNDLE_INTARITH_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KBundleSemanticIntArithReleasePolicy0',
  version: CHECKER_VERSION,
  semanticKImplInputMustRemainDevelopmentPurpose: true,
  predecessorKBundleMustRemainDevelopmentOnly: true,
  predecessorKBundleCannotImplyIntArithReadiness: true,
...
export const KBUNDLE_INTARITH_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KBundleSemanticIntArithCheckerBinding0',
  version: CHECKER_VERSION,
  predecessorKBundleChecker: 'CheckKBundleMinCounterexampleSuccessor0',
  semanticKImplChecker: 'CheckKImplIntArithSuccessor0',
  semanticKImplFinalChecker: 'CheckKImplIntArithFinalTheoremReadiness0',
});

export function makeKBundleIntArithSuccessor0({
  ...
  if (!KBUNDLE_INTARITH_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKBundleIntArithSuccessor0 Purpose must be one of ${KBUNDLE_INTARITH_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KBundleSemanticIntArithSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    SemanticKImpl: makeKImplIntArithSuccessor0({
      KImpl,
      SemanticProofDAG,
      Purpose: 'development',
    }),
    ...
    PSigma,
    ReflectionRegistry,
    Binding: { ...KBUNDLE_INTARITH_SUCCESSOR_BINDING0 },
    Policy: { ...KBUNDLE_INTARITH_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKBundleIntArithSuccessor0(input) {
  return checkKBundleInternal0(input, {
    checker: 'CheckKBundleIntArithSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKBundleIntArithFinalTheoremReadiness0(input) {
  return checkKBundleInternal0(input, {
    checker: 'CheckKBundleIntArithFinalTheoremReadiness0',
...
async function checkKBundleInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'IntArith semantic KBundle final readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KBundleIntArithPurpose0NF',
    purpose,
  }));
  ...
  const predecessorBundleInput = makeKBundleMinCounterexampleSuccessor0({
  ...
  const predecessorCall = await callChecker0(
    'CheckKBundleMinCounterexampleSuccessor0',
    () => CheckKBundleMinCounterexampleSuccessor0(predecessorBundleInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKBundleMinCounterexampleSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['PredecessorKBundle'],
      witness: {
        reason: 'MinCounterexample predecessor KBundle rejected the twelve-rule semantic base',
  ...
  const developmentCall = await callChecker0(
    'CheckKImplIntArithSuccessor0',
    () => CheckKImplIntArithSuccessor0(developmentKImplInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplIntArithSuccessor0',
  ...
  if (developmentRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKImpl'],
      witness: {
        reason: 'IntArith development semantic KImpl successor rejected',
  ...
  const finalCall = await callChecker0(
    'CheckKImplIntArithFinalTheoremReadiness0',
    () => CheckKImplIntArithFinalTheoremReadiness0(finalKImplInput),
  );
  if (!finalCall.ok) {
    ledger.push(makeLedgerEntry0(
      'CheckKImplIntArithFinalTheoremReadiness0',
  ...
  if (purpose === 'final-theorem' && !readiness.finalTheoremReady) {
    return makeRejectRecord0({
      ...
      witness: {
        reason: 'IntArith semantic KBundle is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KBundleSemanticIntArithSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticIntArithNodeCount: developmentNF.semanticIntArithNodeCount ?? null,
...
function makeComputedReadiness0({
  ...
  const blockers = [
    {
      ...
      checker: 'CheckKImplIntArithFinalTheoremReadiness0',
      reason: semanticKImplFinalReady
        ? null
        : 'semantic primitive-rule coverage remains incomplete after IntArith',
  ...
  return Object.freeze({
    kind: 'KBundleComputedSemanticIntArithReadiness0',
    version: CHECKER_VERSION,
    semanticKImplFinalReady,
    semanticConformanceReady: false,
    semanticSigmaReady: false,
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0(
      [],
      'IntArith semantic KBundle input must be an object',
      { actual: typeof input },
    );
  }

  if (input.kind !== 'KBundleSemanticIntArithSuccessor0') {
    return validationReject0(
      ['kind'],
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `IntArith semantic KBundle version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!KBUNDLE_INTARITH_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'IntArith semantic KBundle Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KBUNDLE_INTARITH_SUCCESSOR_PURPOSES0],
      },
    );
  }
  ...
  for (const field of [
    ...
    if (!Object.prototype.hasOwnProperty.call(input, field)) {
      return validationReject0(
        [field],
        'IntArith semantic KBundle is missing a required field',
        { field },
      );
    }
  }
  ...
  if (!isPlainObject0(input.SemanticKImpl)) {
    return validationReject0(
      ['SemanticKImpl'],
      'IntArith semantic KBundle SemanticKImpl must be an object',
  ...
  if (input.SemanticKImpl.Purpose !== 'development') {
    return validationReject0(
      ...
      'IntArith semantic KBundle input KImpl must remain development-purpose; final readiness is recomputed internally',
      { actual: input.SemanticKImpl.Purpose },
    );
  }

  if (!isPlainObject0(input.Binding)
      || !sameCanonical0(input.Binding, KBUNDLE_INTARITH_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['Binding'],
      'IntArith semantic KBundle checker binding must match the executable checker boundary',
      {
        expected: KBUNDLE_INTARITH_SUCCESSOR_BINDING0,
  ...
  if (!isPlainObject0(input.Policy)
      || !sameCanonical0(input.Policy, KBUNDLE_INTARITH_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'IntArith semantic KBundle release policy must match the fail-closed policy',
      {
        expected: KBUNDLE_INTARITH_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  return validationAccept0({
    kind: 'KBundleSemanticIntArithSuccessorShape0NF',
...
function validatePredecessorBundleBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'MinCounterexample predecessor KBundle did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'MinCounterexample predecessor KBundle semantic rule set mismatch',
      {
        expected: [...SEMANTIC_KERNEL_SUPPORTED_RULES_MINCOUNTEREXAMPLE0],
  ...
  return validationAccept0({
    kind: 'KBundleIntArithPredecessorBoundary0NF',
    ...expected,
...
function validateDevelopmentKImplBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'IntArith semantic KImpl did not preserve its development-only boundary',
        { field, expected: value, actual: nf[field] },
      );
    }
  }
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'IntArith semantic KImpl supported-rule set mismatch',
      {
  ...
  return validationAccept0({
    kind: 'KBundleIntArithKImplDevelopmentBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      {
        kind: 'KBundleIntArithProofInput0NF',
        form: 'array',
