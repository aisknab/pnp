...
export const KBUNDLE_LEDGERIND_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KBUNDLE_LEDGERIND_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KBundleSemanticLedgerIndReleasePolicy0',
  version: CHECKER_VERSION,
  semanticKImplInputMustRemainDevelopmentPurpose: true,
  predecessorKBundleMustRemainDevelopmentOnly: true,
  predecessorKBundleCannotImplyLedgerIndReadiness: true,
...
export const KBUNDLE_LEDGERIND_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KBundleSemanticLedgerIndCheckerBinding0',
  version: CHECKER_VERSION,
  predecessorKBundleChecker: 'CheckKBundleSemanticSuccessor0',
  semanticKImplChecker: 'CheckKImplLedgerIndSuccessor0',
  semanticKImplFinalChecker: 'CheckKImplLedgerIndFinalTheoremReadiness0',
});

export function makeKBundleLedgerIndSuccessor0({
  ...
  if (!KBUNDLE_LEDGERIND_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKBundleLedgerIndSuccessor0 Purpose must be one of ${KBUNDLE_LEDGERIND_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KBundleSemanticLedgerIndSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    SemanticKImpl: makeKImplLedgerIndSuccessor0({
      KImpl,
      SemanticProofDAG,
      Purpose: 'development',
    }),
    ...
    PSigma,
    ReflectionRegistry,
    Binding: { ...KBUNDLE_LEDGERIND_SUCCESSOR_BINDING0 },
    Policy: { ...KBUNDLE_LEDGERIND_SUCCESSOR_POLICY0 },
  };
}

/**
 * Development-facing successor KBundle after LedgerInd. The phase-6 KBundle is
 * rerun on the filtered Eq/Subst/Record/DAGInd predecessor sub-DAG, then the
 * full LedgerInd KImpl is checked independently. Legacy conformance, Sigma, and
 * reflection acceptance remains structural-only.
 */
export async function CheckKBundleLedgerIndSuccessor0(input) {
  return checkKBundleInternal0(input, {
    checker: 'CheckKBundleLedgerIndSuccessor0',
    requiredPurpose: null,
  });
}

/**
 * Explicit final-theorem gate. It rejects until the LedgerInd-expanded KImpl,
 * semantic conformance, Sigma derivations, and reflection soundness all accept.
 */
export async function CheckKBundleLedgerIndFinalTheoremReadiness0(input) {
  return checkKBundleInternal0(input, {
    checker: 'CheckKBundleLedgerIndFinalTheoremReadiness0',
...
async function checkKBundleInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'LedgerInd semantic KBundle final readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KBundleLedgerIndPurpose0NF',
    purpose,
  }));
  ...
  const predecessorBundleInput = makeKBundleSemanticSuccessor0({
  ...
  const predecessorBundleCall = await callChecker0(
    'CheckKBundleSemanticSuccessor0',
    () => CheckKBundleSemanticSuccessor0(predecessorBundleInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKBundleSemanticSuccessor0',
  ...
  if (predecessorBundleRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['PredecessorKBundle'],
      witness: {
        reason: 'phase-6 predecessor KBundle rejected the Eq/Subst/Record/DAGInd base',
  ...
  const developmentKImplCall = await callChecker0(
    'CheckKImplLedgerIndSuccessor0',
    () => CheckKImplLedgerIndSuccessor0(developmentKImplInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplLedgerIndSuccessor0',
  ...
  if (developmentKImplRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKImpl'],
      witness: {
        reason: 'LedgerInd development semantic KImpl successor rejected',
  ...
  const finalKImplCall = await callChecker0(
    'CheckKImplLedgerIndFinalTheoremReadiness0',
    () => CheckKImplLedgerIndFinalTheoremReadiness0(finalKImplInput),
  );
  ...
  if (purpose === 'final-theorem' && !readiness.finalTheoremReady) {
    return makeRejectRecord0({
      ...
      witness: {
        reason: 'LedgerInd semantic KBundle is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KBundleSemanticLedgerIndSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticLedgerIndNodeCount:
        developmentKImplNF.semanticLedgerIndNodeCount ?? null,
...
function makeComputedReadiness0({
  ...
  const blockers = [
    {
      ...
      reason: semanticKImplFinalReady
        ? null
        : 'semantic primitive-rule coverage remains incomplete after LedgerInd',
  ...
  return Object.freeze({
    kind: 'KBundleComputedSemanticLedgerIndReadiness0',
    version: CHECKER_VERSION,
    semanticKImplFinalReady,
    semanticConformanceReady: false,
    semanticSigmaReady: false,
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0([], 'LedgerInd semantic KBundle input must be an object', {
      actual: typeof input,
    });
  }

  if (input.kind !== 'KBundleSemanticLedgerIndSuccessor0') {
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `LedgerInd semantic KBundle version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!KBUNDLE_LEDGERIND_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'LedgerInd semantic KBundle Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KBUNDLE_LEDGERIND_SUCCESSOR_PURPOSES0],
      },
    );
  }
  ...
  for (const field of [
    ...
    if (!Object.prototype.hasOwnProperty.call(input, field)) {
      return validationReject0(
        [field],
        'LedgerInd semantic KBundle is missing a required field',
        { field },
      );
    }
  }
  ...
  if (!isPlainObject0(input.SemanticKImpl)) {
    return validationReject0(
      ['SemanticKImpl'],
      'LedgerInd semantic KBundle SemanticKImpl must be an object',
  ...
  if (!isPlainObject0(input.Binding)
      || !sameCanonical0(input.Binding, KBUNDLE_LEDGERIND_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['Binding'],
      'LedgerInd semantic KBundle checker binding must match the executable checker boundary',
      {
        expected: KBUNDLE_LEDGERIND_SUCCESSOR_BINDING0,
  ...
  if (!isPlainObject0(input.Policy)
      || !sameCanonical0(input.Policy, KBUNDLE_LEDGERIND_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'LedgerInd semantic KBundle release policy must match the fail-closed policy',
      {
        expected: KBUNDLE_LEDGERIND_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  return validationAccept0({
    kind: 'KBundleSemanticLedgerIndSuccessorShape0NF',
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'phase-6 predecessor KBundle did not preserve its development-only boundary',
        { field, expected: value, actual: nf[field] },
      );
    }
  }

  const expectedRules = ['Eq', 'Subst', 'Record', 'DAGInd'];
  if (!sameCanonical0(nf.semanticKImplSupportedRules, expectedRules)) {
    return validationReject0(
      ['PredecessorKBundle', 'NF', 'semanticKImplSupportedRules'],
      'phase-6 predecessor KBundle semantic rule set mismatch',
      { expected: expectedRules, actual: nf.semanticKImplSupportedRules },
    );
  }

  return validationAccept0({
    kind: 'KBundleLedgerIndPredecessorBoundary0NF',
    ...expected,
...
function validateDevelopmentKImplBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'LedgerInd semantic KImpl did not preserve its development-only boundary',
        { field, expected: value, actual: nf[field] },
      );
    }
  }
  ...
  if (!sameCanonical0(nf.supportedSemanticRules, expectedRules)) {
    return validationReject0(
      ...
      'LedgerInd semantic KImpl supported-rule set mismatch',
      { expected: expectedRules, actual: nf.supportedSemanticRules },
    );
  }

  return validationAccept0({
    kind: 'KBundleLedgerIndKImplDevelopmentBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'KBundleLedgerIndProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  ...
  return validationAcceptWith0(
    { kind: 'KBundleLedgerIndProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
