...
export const KBUNDLE_OBLTOPOIND_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KBUNDLE_OBLTOPOIND_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KBundleSemanticOblTopoIndReleasePolicy0',
  version: CHECKER_VERSION,
  semanticKImplInputMustRemainDevelopmentPurpose: true,
  predecessorKBundleMustRemainDevelopmentOnly: true,
  predecessorKBundleCannotImplyOblTopoIndReadiness: true,
...
export const KBUNDLE_OBLTOPOIND_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KBundleSemanticOblTopoIndCheckerBinding0',
  version: CHECKER_VERSION,
  predecessorKBundleChecker: 'CheckKBundleLedgerIndSuccessor0',
  semanticKImplChecker: 'CheckKImplOblTopoIndSuccessor0',
  semanticKImplFinalChecker: 'CheckKImplOblTopoIndFinalTheoremReadiness0',
});

export function makeKBundleOblTopoIndSuccessor0({
  ...
  if (!KBUNDLE_OBLTOPOIND_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKBundleOblTopoIndSuccessor0 Purpose must be one of ${KBUNDLE_OBLTOPOIND_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KBundleSemanticOblTopoIndSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    SemanticKImpl: makeKImplOblTopoIndSuccessor0({
      KImpl,
      SemanticProofDAG,
      Purpose: 'development',
    }),
    ...
    PSigma,
    ReflectionRegistry,
    Binding: { ...KBUNDLE_OBLTOPOIND_SUCCESSOR_BINDING0 },
    Policy: { ...KBUNDLE_OBLTOPOIND_SUCCESSOR_POLICY0 },
  };
}

/**
 * Development-facing successor KBundle after OblTopoInd. The LedgerInd KBundle
 * is rerun on the filtered predecessor sub-DAG, then the full OblTopoInd KImpl
 * is checked independently. Legacy conformance, Sigma, and reflection remain
 * structural-only until successor semantic checkers exist for those surfaces.
 */
export async function CheckKBundleOblTopoIndSuccessor0(input) {
  return checkKBundleInternal0(input, {
    checker: 'CheckKBundleOblTopoIndSuccessor0',
    requiredPurpose: null,
  });
}

/**
 * Explicit final-theorem gate. It rejects until the OblTopoInd-expanded KImpl,
 * semantic conformance, Sigma derivations, and reflection soundness all accept.
 */
export async function CheckKBundleOblTopoIndFinalTheoremReadiness0(input) {
  return checkKBundleInternal0(input, {
    checker: 'CheckKBundleOblTopoIndFinalTheoremReadiness0',
...
async function checkKBundleInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'OblTopoInd semantic KBundle final readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KBundleOblTopoIndPurpose0NF',
    purpose,
  }));
  ...
  const predecessorBundleInput = makeKBundleLedgerIndSuccessor0({
  ...
  const predecessorBundleCall = await callChecker0(
    'CheckKBundleLedgerIndSuccessor0',
    () => CheckKBundleLedgerIndSuccessor0(predecessorBundleInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKBundleLedgerIndSuccessor0',
    predecessorBundleCall.ok && predecessorBundleCall.record.tag === 'accept',
    predecessorBundleCall.ok
      ? predecessorBundleCall.record
      : predecessorBundleCall.witness,
  ));

  if (!predecessorBundleCall.ok) {
    return makeRejectRecord0({
      checker,
      coord: `${checker}.predecessorKBundle.exception`,
      path: ['PredecessorKBundle'],
      witness: predecessorBundleCall.witness,
      ledger,
    });
  }

  const predecessorBundleRecord = predecessorBundleCall.record;
  if (predecessorBundleRecord.tag !== 'accept') {
    return makeRejectRecord0({
      checker,
      coord: `${checker}.predecessorKBundle`,
      path: ['PredecessorKBundle'],
      witness: {
        reason: 'LedgerInd predecessor KBundle rejected the five-rule semantic base',
        predecessorNodeIds: predecessorNodes.map((node) => node?.id ?? null),
        inner: compactReject0(predecessorBundleRecord),
      },
      ledger,
    });
  }

  const predecessorBundleNF = predecessorBundleRecord.NF
    ?? predecessorBundleRecord.nf
    ?? {};
  const predecessorBoundary = validatePredecessorBundleBoundary0(
    predecessorBundleNF,
  );
  ledger.push(makeLedgerEntry0(
  ...
  const developmentKImplCall = await callChecker0(
    'CheckKImplOblTopoIndSuccessor0',
    () => CheckKImplOblTopoIndSuccessor0(developmentKImplInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplOblTopoIndSuccessor0',
  ...
  if (developmentKImplRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKImpl'],
      witness: {
        reason: 'OblTopoInd development semantic KImpl successor rejected',
  ...
  const finalKImplCall = await callChecker0(
    'CheckKImplOblTopoIndFinalTheoremReadiness0',
    () => CheckKImplOblTopoIndFinalTheoremReadiness0(finalKImplInput),
  );
  ...
  if (purpose === 'final-theorem' && !readiness.finalTheoremReady) {
    return makeRejectRecord0({
      ...
      witness: {
        reason: 'OblTopoInd semantic KBundle is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KBundleSemanticOblTopoIndSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      predecessorKBundleChecker: predecessorBundleRecord.checker,
      predecessorKBundleDigest: predecessorBundleRecord.Digest
        ?? predecessorBundleRecord.digest,
      ...
      predecessorKBundleSupportedRules:
        predecessorBundleNF.semanticKImplSupportedRules ?? [],
      predecessorKBundleMissingRules:
        predecessorBundleNF.semanticKImplMissingRules ?? [],
      ...
      semanticOblTopoIndNodeCount:
        developmentKImplNF.semanticOblTopoIndNodeCount ?? null,
      ...
      legacyBundleAccepted: predecessorBundleNF.legacyBundleAccepted === true,
      legacyBundleDigest: predecessorBundleNF.legacyBundleDigest ?? null,
      legacyKImplDigest: predecessorBundleNF.legacyKImplDigest ?? null,
      legacyConformanceDigest:
        predecessorBundleNF.legacyConformanceDigest ?? null,
      legacySigmaDigest: predecessorBundleNF.legacySigmaDigest ?? null,
      legacyReflectionDigest:
        predecessorBundleNF.legacyReflectionDigest ?? null,
...
function makeComputedReadiness0({
  ...
  const blockers = [
    {
      ...
      checker: 'CheckKImplOblTopoIndFinalTheoremReadiness0',
      reason: semanticKImplFinalReady
        ? null
        : 'semantic primitive-rule coverage remains incomplete after OblTopoInd',
  ...
  return Object.freeze({
    kind: 'KBundleComputedSemanticOblTopoIndReadiness0',
    version: CHECKER_VERSION,
    semanticKImplFinalReady,
    semanticConformanceReady: false,
    semanticSigmaReady: false,
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0([], 'OblTopoInd semantic KBundle input must be an object', {
      actual: typeof input,
    });
  }

  if (input.kind !== 'KBundleSemanticOblTopoIndSuccessor0') {
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `OblTopoInd semantic KBundle version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!KBUNDLE_OBLTOPOIND_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'OblTopoInd semantic KBundle Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KBUNDLE_OBLTOPOIND_SUCCESSOR_PURPOSES0],
      },
    );
  }
  ...
  for (const field of [
    ...
    if (!Object.prototype.hasOwnProperty.call(input, field)) {
      return validationReject0(
        [field],
        'OblTopoInd semantic KBundle is missing a required field',
        { field },
      );
    }
  }
  ...
  if (!isPlainObject0(input.SemanticKImpl)) {
    return validationReject0(
      ['SemanticKImpl'],
      'OblTopoInd semantic KBundle SemanticKImpl must be an object',
  ...
  if (!isPlainObject0(input.Binding)
      || !sameCanonical0(input.Binding, KBUNDLE_OBLTOPOIND_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['Binding'],
      'OblTopoInd semantic KBundle checker binding must match the executable checker boundary',
      {
        expected: KBUNDLE_OBLTOPOIND_SUCCESSOR_BINDING0,
  ...
  if (!isPlainObject0(input.Policy)
      || !sameCanonical0(input.Policy, KBUNDLE_OBLTOPOIND_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'OblTopoInd semantic KBundle release policy must match the fail-closed policy',
      {
        expected: KBUNDLE_OBLTOPOIND_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  return validationAccept0({
    kind: 'KBundleSemanticOblTopoIndSuccessorShape0NF',
...
function validatePredecessorBundleBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'LedgerInd predecessor KBundle did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(nf.semanticKImplSupportedRules, expectedRules)) {
    return validationReject0(
      ...
      'LedgerInd predecessor KBundle semantic rule set mismatch',
      { expected: expectedRules, actual: nf.semanticKImplSupportedRules },
    );
  }

  return validationAccept0({
    kind: 'KBundleOblTopoIndPredecessorBoundary0NF',
    ...expected,
...
function validateDevelopmentKImplBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'OblTopoInd semantic KImpl did not preserve its development-only boundary',
        { field, expected: value, actual: nf[field] },
      );
    }
  }
  ...
  if (!sameCanonical0(nf.supportedSemanticRules, expectedRules)) {
    return validationReject0(
      ['SemanticKImpl', 'NF', 'supportedSemanticRules'],
      'OblTopoInd semantic KImpl supported-rule set mismatch',
      { expected: expectedRules, actual: nf.supportedSemanticRules },
    );
  }

  return validationAccept0({
    kind: 'KBundleOblTopoIndKImplDevelopmentBoundary0NF',
    ...expected,
    supportedSemanticRules: expectedRules,
  });
}
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'KBundleOblTopoIndProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  ...
  return validationAcceptWith0(
    { kind: 'KBundleOblTopoIndProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
