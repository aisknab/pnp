...
export const KBUNDLE_RANKIND_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KBUNDLE_RANKIND_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KBundleSemanticRankIndReleasePolicy0',
  version: CHECKER_VERSION,
  semanticKImplInputMustRemainDevelopmentPurpose: true,
  predecessorKBundleMustRemainDevelopmentOnly: true,
  predecessorKBundleCannotImplyRankIndReadiness: true,
...
export const KBUNDLE_RANKIND_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KBundleSemanticRankIndCheckerBinding0',
  version: CHECKER_VERSION,
  predecessorKBundleChecker: 'CheckKBundleHallSuccessor0',
  semanticKImplChecker: 'CheckKImplRankIndSuccessor0',
  semanticKImplFinalChecker: 'CheckKImplRankIndFinalTheoremReadiness0',
});

export function makeKBundleRankIndSuccessor0({
  ...
  if (!KBUNDLE_RANKIND_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKBundleRankIndSuccessor0 Purpose must be one of ${KBUNDLE_RANKIND_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KBundleSemanticRankIndSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    SemanticKImpl: makeKImplRankIndSuccessor0({
      KImpl,
      SemanticProofDAG,
      Purpose: 'development',
    }),
    ...
    PSigma,
    ReflectionRegistry,
    Binding: { ...KBUNDLE_RANKIND_SUCCESSOR_BINDING0 },
    Policy: { ...KBUNDLE_RANKIND_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKBundleRankIndSuccessor0(input) {
  return checkKBundleInternal0(input, {
    checker: 'CheckKBundleRankIndSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKBundleRankIndFinalTheoremReadiness0(input) {
  return checkKBundleInternal0(input, {
    checker: 'CheckKBundleRankIndFinalTheoremReadiness0',
...
async function checkKBundleInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'RankInd semantic KBundle final readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  const predecessorBundleInput = makeKBundleHallSuccessor0({
  ...
  const predecessorCall = await callChecker0(
    'CheckKBundleHallSuccessor0',
    () => CheckKBundleHallSuccessor0(predecessorBundleInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKBundleHallSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['PredecessorKBundle'],
      witness: {
        reason: 'Hall predecessor KBundle rejected the ten-rule semantic base',
  ...
  const developmentCall = await callChecker0(
    'CheckKImplRankIndSuccessor0',
    () => CheckKImplRankIndSuccessor0(developmentKImplInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplRankIndSuccessor0',
  ...
  if (developmentRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKImpl'],
      witness: {
        reason: 'RankInd development semantic KImpl successor rejected',
  ...
  const finalCall = await callChecker0(
    'CheckKImplRankIndFinalTheoremReadiness0',
    () => CheckKImplRankIndFinalTheoremReadiness0(finalKImplInput),
  );
  if (!finalCall.ok) {
    ledger.push(makeLedgerEntry0(
      'CheckKImplRankIndFinalTheoremReadiness0',
  ...
  if (purpose === 'final-theorem' && !readiness.finalTheoremReady) {
    return makeRejectRecord0({
      ...
      witness: {
        reason: 'RankInd semantic KBundle is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KBundleSemanticRankIndSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticRankIndNodeCount: developmentNF.semanticRankIndNodeCount ?? null,
...
function makeComputedReadiness0({
  ...
  const blockers = [
    {
      ...
      checker: 'CheckKImplRankIndFinalTheoremReadiness0',
      reason: semanticKImplFinalReady
        ? null
        : 'semantic primitive-rule coverage remains incomplete after RankInd',
  ...
  return Object.freeze({
    kind: 'KBundleComputedSemanticRankIndReadiness0',
    version: CHECKER_VERSION,
    semanticKImplFinalReady,
    semanticConformanceReady: false,
    semanticSigmaReady: false,
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0([], 'RankInd semantic KBundle input must be an object', {
      actual: typeof input,
    });
  }

  if (input.kind !== 'KBundleSemanticRankIndSuccessor0') {
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `RankInd semantic KBundle version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!KBUNDLE_RANKIND_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'RankInd semantic KBundle Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KBUNDLE_RANKIND_SUCCESSOR_PURPOSES0],
      },
    );
  }
  ...
  for (const field of [
    ...
    if (!Object.prototype.hasOwnProperty.call(input, field)) {
      return validationReject0(
        [field],
        'RankInd semantic KBundle is missing a required field',
        { field },
      );
    }
  }
  ...
  if (!isPlainObject0(input.SemanticKImpl)) {
    return validationReject0(
      ['SemanticKImpl'],
      'RankInd semantic KBundle SemanticKImpl must be an object',
  ...
  if (input.SemanticKImpl.Purpose !== 'development') {
    return validationReject0(
      ...
      'RankInd semantic KBundle input KImpl must remain development-purpose; final readiness is recomputed internally',
      { actual: input.SemanticKImpl.Purpose },
    );
  }

  if (!isPlainObject0(input.Binding)
      || !sameCanonical0(input.Binding, KBUNDLE_RANKIND_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['Binding'],
      'RankInd semantic KBundle checker binding must match the executable checker boundary',
      {
        expected: KBUNDLE_RANKIND_SUCCESSOR_BINDING0,
  ...
  if (!isPlainObject0(input.Policy)
      || !sameCanonical0(input.Policy, KBUNDLE_RANKIND_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'RankInd semantic KBundle release policy must match the fail-closed policy',
      {
        expected: KBUNDLE_RANKIND_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  return validationAccept0({
    kind: 'KBundleSemanticRankIndSuccessorShape0NF',
...
function validatePredecessorBundleBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'Hall predecessor KBundle did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'Hall predecessor KBundle semantic rule set mismatch',
      {
  ...
  return validationAccept0({
    kind: 'KBundleRankIndPredecessorBoundary0NF',
    ...expected,
...
function validateDevelopmentKImplBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'RankInd semantic KImpl did not preserve its development-only boundary',
        { field, expected: value, actual: nf[field] },
      );
    }
  }
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'RankInd semantic KImpl supported-rule set mismatch',
      {
  ...
  return validationAccept0({
    kind: 'KBundleRankIndKImplDevelopmentBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      {
        kind: 'KBundleRankIndProofInput0NF',
        form: 'array',
