...
export const KIMPL_DPIND_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KIMPL_DPIND_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KImplSemanticDPIndReleasePolicy0',
  ...
  terminalJudgmentMustBeComputed: true,
...
export const KIMPL_DPIND_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KImplSemanticDPIndKernelBinding0',
  version: CHECKER_VERSION,
  predecessorSuccessorChecker: 'CheckKImplFiniteExhaustSuccessor0',
  predecessorProofChecker: 'CheckSemanticKernelProofFiniteExhaust0',
  proofChecker: 'CheckSemanticKernelProofDPInd0',
  readinessChecker: 'CheckSemanticKernelReadinessDPInd0',
});

export function makeKImplDPIndSuccessor0({
  KImpl = makeSyntheticKImpl0(),
  SemanticProofDAG = makeSemanticProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!KIMPL_DPIND_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKImplDPIndSuccessor0 Purpose must be one of ${KIMPL_DPIND_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KImplSemanticDPIndSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KImpl,
    SemanticKernel: {
      ...KIMPL_DPIND_SUCCESSOR_BINDING0,
      ProofDAG: SemanticProofDAG,
    },
    Policy: { ...KIMPL_DPIND_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKImplDPIndSuccessor0(input) {
  return checkSuccessorInternal0(input, {
    checker: 'CheckKImplDPIndSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKImplDPIndFinalTheoremReadiness0(input) {
  return checkSuccessorInternal0(input, {
...
async function checkSuccessorInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'DPInd final-theorem readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KImplDPIndSuccessorPurpose0NF',
    purpose,
  }));
  ...
  const predecessorInput = makeKImplFiniteExhaustSuccessor0({
    KImpl: input.KImpl,
  ...
  const predecessorCall = await callChecker0(
    'CheckKImplFiniteExhaustSuccessor0',
    () => CheckKImplFiniteExhaustSuccessor0(predecessorInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplFiniteExhaustSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KImpl'],
      witness: {
        reason: 'FiniteExhaust predecessor successor rejected the eight-rule semantic base',
  ...
  const proofCall = await callChecker0(
    'CheckSemanticKernelProofDPInd0',
    () => CheckSemanticKernelProofDPInd0(input.SemanticKernel.ProofDAG),
  );
  ledger.push(makeLedgerEntry0(
    'CheckSemanticKernelProofDPInd0',
  ...
  if (proofRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel', 'ProofDAG'],
      witness: {
        reason: 'DPInd-extended semantic proof checker rejected the proof DAG',
  ...
  if (purpose === 'final-theorem' && !semanticKernelReady) {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel'],
      witness: {
        reason: 'DPInd-extended successor KImpl is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KImplSemanticDPIndSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticDPIndNodeCount: proofNF.dpIndNodeCount ?? null,
      ...
      semanticKernelReady,
      requiredSemanticRules: [...SEMANTIC_KERNEL_REQUIRED_RULES_DPIND0],
      supportedSemanticRules: [...SEMANTIC_KERNEL_SUPPORTED_RULES_DPIND0],
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0([], 'DPInd successor KImpl input must be an object', {
      actual: typeof input,
    });
  }

  if (input.kind !== 'KImplSemanticDPIndSuccessor0') {
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `DPInd successor KImpl version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!KIMPL_DPIND_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'DPInd successor KImpl Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KIMPL_DPIND_SUCCESSOR_PURPOSES0],
      },
    );
  }
  ...
  if (!isPlainObject0(input.KImpl)) {
    return validationReject0(
      ['KImpl'],
      'DPInd successor KImpl must include a base KImpl object',
  ...
  if (!isPlainObject0(input.SemanticKernel)) {
    return validationReject0(
      ['SemanticKernel'],
      'DPInd successor KImpl must include SemanticKernel binding',
  ...
  if (!sameCanonical0(binding, KIMPL_DPIND_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['SemanticKernel'],
      'DPInd semantic-kernel binding must match the executable checker boundary',
      {
        expected: KIMPL_DPIND_SUCCESSOR_BINDING0,
        actual: binding,
      },
    );
  }
  ...
  if (!Object.prototype.hasOwnProperty.call(input.SemanticKernel, 'ProofDAG')) {
    return validationReject0(
      ...
      'DPInd semantic-kernel binding must include ProofDAG',
      null,
    );
  }
  ...
  if (!isPlainObject0(input.Policy)) {
    return validationReject0(
      ['Policy'],
      'DPInd successor KImpl must include semantic release policy',
      { actual: typeof input.Policy },
    );
  }

  if (!sameCanonical0(input.Policy, KIMPL_DPIND_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'DPInd successor release policy must match the fail-closed policy',
      {
        expected: KIMPL_DPIND_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  if (unexpected.length !== 0) {
    return validationReject0(
      [unexpected[0]],
      'DPInd successor rejects undeclared top-level readiness assertions',
  ...
  return validationAccept0({
    kind: 'KImplSemanticDPIndSuccessorShape0NF',
...
function validateRuleUniverse0() {
  ...
  const semantic = [...SEMANTIC_KERNEL_REQUIRED_RULES_DPIND0];
  if (!sameCanonical0(legacy, semantic)) {
    return validationReject0(
      ['SemanticKernel', 'requiredRules'],
      'DPInd semantic required-rule universe must match the legacy KImpl universe',
      { legacyRules: legacy, semanticRules: semantic },
    );
  }

  return validationAccept0({
    kind: 'KImplSemanticDPIndRuleUniverse0NF',
...
function validatePredecessorBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'FiniteExhaust predecessor successor did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(nf.supportedSemanticRules, expectedRules)) {
    return validationReject0(
      ...
      'FiniteExhaust predecessor successor semantic rule set mismatch',
      { expected: expectedRules, actual: nf.supportedSemanticRules },
    );
  }

  return validationAccept0({
    kind: 'KImplDPIndPredecessorBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'KImplDPIndProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  ...
  return validationAcceptWith0(
    { kind: 'KImplDPIndProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
...
function summarizeReadiness0(record) {
  ...
  return {
    missingRules: Array.isArray(witness.missingRules)
      ...
      : SEMANTIC_KERNEL_REQUIRED_RULES_DPIND0.filter(
