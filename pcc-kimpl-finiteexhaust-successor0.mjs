...
export const KIMPL_FINITEEXHAUST_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KIMPL_FINITEEXHAUST_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KImplSemanticFiniteExhaustReleasePolicy0',
  ...
  duplicateAndOmissionRejectionRequired: true,
  universalConclusionMustBeComputed: true,
...
export const KIMPL_FINITEEXHAUST_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KImplSemanticFiniteExhaustKernelBinding0',
  version: CHECKER_VERSION,
  predecessorSuccessorChecker: 'CheckKImplTraceIndSuccessor0',
  predecessorProofChecker: 'CheckSemanticKernelProofTraceInd0',
  proofChecker: 'CheckSemanticKernelProofFiniteExhaust0',
  readinessChecker: 'CheckSemanticKernelReadinessFiniteExhaust0',
});

export function makeKImplFiniteExhaustSuccessor0({
  KImpl = makeSyntheticKImpl0(),
  SemanticProofDAG = makeSemanticProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!KIMPL_FINITEEXHAUST_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKImplFiniteExhaustSuccessor0 Purpose must be one of ${KIMPL_FINITEEXHAUST_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KImplSemanticFiniteExhaustSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KImpl,
    SemanticKernel: {
      ...KIMPL_FINITEEXHAUST_SUCCESSOR_BINDING0,
      ProofDAG: SemanticProofDAG,
    },
    Policy: { ...KIMPL_FINITEEXHAUST_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKImplFiniteExhaustSuccessor0(input) {
  return checkSuccessorInternal0(input, {
    checker: 'CheckKImplFiniteExhaustSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKImplFiniteExhaustFinalTheoremReadiness0(input) {
  return checkSuccessorInternal0(input, {
...
async function checkSuccessorInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'FiniteExhaust final-theorem readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KImplFiniteExhaustSuccessorPurpose0NF',
    purpose,
  }));
  ...
  const predecessorInput = makeKImplTraceIndSuccessor0({
    KImpl: input.KImpl,
  ...
  const predecessorCall = await callChecker0(
    'CheckKImplTraceIndSuccessor0',
    () => CheckKImplTraceIndSuccessor0(predecessorInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplTraceIndSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KImpl'],
      witness: {
        reason: 'TraceInd predecessor successor rejected the seven-rule semantic base',
  ...
  const proofCall = await callChecker0(
    'CheckSemanticKernelProofFiniteExhaust0',
    () => CheckSemanticKernelProofFiniteExhaust0(input.SemanticKernel.ProofDAG),
  );
  ledger.push(makeLedgerEntry0(
    'CheckSemanticKernelProofFiniteExhaust0',
  ...
  if (proofRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel', 'ProofDAG'],
      witness: {
        reason: 'FiniteExhaust-extended semantic proof checker rejected the proof DAG',
  ...
  if (purpose === 'final-theorem' && !semanticKernelReady) {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel'],
      witness: {
        reason: 'FiniteExhaust-extended successor KImpl is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KImplSemanticFiniteExhaustSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticFiniteExhaustNodeCount: proofNF.finiteExhaustNodeCount ?? null,
      ...
      semanticKernelReady,
      requiredSemanticRules: [...SEMANTIC_KERNEL_REQUIRED_RULES_FINITEEXHAUST0],
      supportedSemanticRules: [...SEMANTIC_KERNEL_SUPPORTED_RULES_FINITEEXHAUST0],
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0([], 'FiniteExhaust successor KImpl input must be an object', {
      actual: typeof input,
    });
  }

  if (input.kind !== 'KImplSemanticFiniteExhaustSuccessor0') {
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `FiniteExhaust successor KImpl version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!KIMPL_FINITEEXHAUST_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'FiniteExhaust successor KImpl Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KIMPL_FINITEEXHAUST_SUCCESSOR_PURPOSES0],
      },
    );
  }
  ...
  if (!isPlainObject0(input.KImpl)) {
    return validationReject0(
      ['KImpl'],
      'FiniteExhaust successor KImpl must include a base KImpl object',
  ...
  if (!isPlainObject0(input.SemanticKernel)) {
    return validationReject0(
      ['SemanticKernel'],
      'FiniteExhaust successor KImpl must include SemanticKernel binding',
  ...
  if (!sameCanonical0(binding, KIMPL_FINITEEXHAUST_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['SemanticKernel'],
      'FiniteExhaust semantic-kernel binding must match the executable checker boundary',
      {
        expected: KIMPL_FINITEEXHAUST_SUCCESSOR_BINDING0,
        actual: binding,
      },
    );
  }
  ...
  if (!Object.prototype.hasOwnProperty.call(input.SemanticKernel, 'ProofDAG')) {
    return validationReject0(
      ...
      'FiniteExhaust semantic-kernel binding must include ProofDAG',
      null,
    );
  }
  ...
  if (!isPlainObject0(input.Policy)) {
    return validationReject0(
      ['Policy'],
      'FiniteExhaust successor KImpl must include semantic release policy',
      { actual: typeof input.Policy },
    );
  }

  if (!sameCanonical0(input.Policy, KIMPL_FINITEEXHAUST_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'FiniteExhaust successor release policy must match the fail-closed policy',
      {
        expected: KIMPL_FINITEEXHAUST_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  if (unexpected.length !== 0) {
    return validationReject0(
      [unexpected[0]],
      'FiniteExhaust successor rejects undeclared top-level readiness assertions',
  ...
  return validationAccept0({
    kind: 'KImplSemanticFiniteExhaustSuccessorShape0NF',
...
function validateRuleUniverse0() {
  ...
  const semantic = [...SEMANTIC_KERNEL_REQUIRED_RULES_FINITEEXHAUST0];
  if (!sameCanonical0(legacy, semantic)) {
    return validationReject0(
      ['SemanticKernel', 'requiredRules'],
      'FiniteExhaust semantic required-rule universe must match the legacy KImpl universe',
      { legacyRules: legacy, semanticRules: semantic },
    );
  }

  return validationAccept0({
    kind: 'KImplSemanticFiniteExhaustRuleUniverse0NF',
...
function validatePredecessorBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'TraceInd predecessor successor did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(nf.supportedSemanticRules, expectedRules)) {
    return validationReject0(
      ...
      'TraceInd predecessor successor semantic rule set mismatch',
      { expected: expectedRules, actual: nf.supportedSemanticRules },
    );
  }

  return validationAccept0({
    kind: 'KImplFiniteExhaustPredecessorBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'KImplFiniteExhaustProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  ...
  return validationAcceptWith0(
    { kind: 'KImplFiniteExhaustProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
...
function summarizeReadiness0(record) {
  ...
  return {
    missingRules: Array.isArray(witness.missingRules)
      ...
      : SEMANTIC_KERNEL_REQUIRED_RULES_FINITEEXHAUST0.filter(
