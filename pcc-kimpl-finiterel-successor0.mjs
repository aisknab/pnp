...
import {
  ...
  SEMANTIC_KERNEL_REQUIRED_RULES_FINITEREL0,
...
export const KIMPL_FINITEREL_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KIMPL_FINITEREL_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KImplSemanticFiniteRelReleasePolicy0',
...
export const KIMPL_FINITEREL_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KImplSemanticFiniteRelKernelBinding0',
  version: CHECKER_VERSION,
  predecessorSuccessorChecker: 'CheckKImplTruthVecSuccessor0',
  predecessorProofChecker: 'CheckSemanticKernelProofTruthVec0',
  proofChecker: 'CheckSemanticKernelProofFiniteRel0',
  readinessChecker: 'CheckSemanticKernelReadinessFiniteRel0',
});

export function makeKImplFiniteRelSuccessor0({
  KImpl = makeSyntheticKImpl0(),
  SemanticProofDAG = makeSemanticProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!KIMPL_FINITEREL_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKImplFiniteRelSuccessor0 Purpose must be one of ${KIMPL_FINITEREL_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }
  return {
    kind: 'KImplSemanticFiniteRelSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KImpl,
    SemanticKernel: {
      ...KIMPL_FINITEREL_SUCCESSOR_BINDING0,
      ProofDAG: SemanticProofDAG,
    },
    Policy: { ...KIMPL_FINITEREL_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKImplFiniteRelSuccessor0(input) {
  return checkSuccessorInternal0(input, {
    checker: 'CheckKImplFiniteRelSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKImplFiniteRelFinalTheoremReadiness0(input) {
  return checkSuccessorInternal0(input, {
...
async function checkSuccessorInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'FiniteRel final-theorem readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KImplFiniteRelSuccessorPurpose0NF',
    purpose,
  }));
  ...
  const predecessorInput = makeKImplTruthVecSuccessor0({
    KImpl: input.KImpl,
  ...
  const predecessorCall = await callChecker0(
    'CheckKImplTruthVecSuccessor0',
    () => CheckKImplTruthVecSuccessor0(predecessorInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplTruthVecSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KImpl'],
      witness: {
        reason: 'TruthVec predecessor successor rejected the fifteen-rule semantic base',
  ...
  const proofCall = await callChecker0(
    'CheckSemanticKernelProofFiniteRel0',
    () => CheckSemanticKernelProofFiniteRel0(input.SemanticKernel.ProofDAG),
  );
  ledger.push(makeLedgerEntry0(
    'CheckSemanticKernelProofFiniteRel0',
  ...
  if (proofRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel', 'ProofDAG'],
      witness: {
        reason: 'FiniteRel-extended semantic proof checker rejected the proof DAG',
  ...
  if (purpose === 'final-theorem' && !semanticKernelReady) {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel'],
      witness: {
        reason: 'FiniteRel-extended successor KImpl is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KImplSemanticFiniteRelSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticFiniteRelNodeCount: proofNF.finiteRelNodeCount ?? null,
      ...
      semanticKernelReady,
      requiredSemanticRules: [...SEMANTIC_KERNEL_REQUIRED_RULES_FINITEREL0],
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0(
      [],
      'FiniteRel successor KImpl input must be an object',
      { actual: typeof input },
    );
  }
  if (input.kind !== 'KImplSemanticFiniteRelSuccessor0') {
    return validationReject0(
      ['kind'],
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `FiniteRel successor KImpl version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }
  if (!KIMPL_FINITEREL_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'FiniteRel successor KImpl Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KIMPL_FINITEREL_SUCCESSOR_PURPOSES0],
      },
    );
  }
  if (!isPlainObject0(input.KImpl)) {
    return validationReject0(
      ['KImpl'],
      'FiniteRel successor KImpl must include a base KImpl object',
  ...
  if (!isPlainObject0(input.SemanticKernel)) {
    return validationReject0(
      ['SemanticKernel'],
      'FiniteRel successor KImpl must include SemanticKernel binding',
  ...
  if (!sameCanonical0(binding, KIMPL_FINITEREL_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['SemanticKernel'],
      'FiniteRel semantic-kernel binding must match the executable checker boundary',
      {
        expected: KIMPL_FINITEREL_SUCCESSOR_BINDING0,
        actual: binding,
      },
    );
  }
  if (!Object.prototype.hasOwnProperty.call(input.SemanticKernel, 'ProofDAG')) {
    return validationReject0(
      ...
      'FiniteRel semantic-kernel binding must include ProofDAG',
      null,
    );
  }
  if (!isPlainObject0(input.Policy)
      || !sameCanonical0(input.Policy, KIMPL_FINITEREL_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'FiniteRel successor release policy must match the fail-closed policy',
      {
        expected: KIMPL_FINITEREL_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  if (unexpected.length !== 0) {
    return validationReject0(
      [unexpected[0]],
      'FiniteRel successor rejects undeclared top-level readiness assertions',
  ...
  return validationAccept0({
    kind: 'KImplSemanticFiniteRelSuccessorShape0NF',
...
function validateRuleUniverse0() {
  ...
  const semantic = [...SEMANTIC_KERNEL_REQUIRED_RULES_FINITEREL0];
  if (!sameCanonical0(legacy, semantic)) {
    return validationReject0(
      ['SemanticKernel', 'requiredRules'],
      'FiniteRel semantic required-rule universe must match the legacy KImpl universe',
      { legacyRules: legacy, semanticRules: semantic },
    );
  }
  return validationAccept0({
    kind: 'KImplSemanticFiniteRelRuleUniverse0NF',
...
function validatePredecessorBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'TruthVec predecessor successor did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'TruthVec predecessor successor semantic rule set mismatch',
      {
  ...
  return validationAccept0({
    kind: 'KImplFiniteRelPredecessorBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      {
        kind: 'KImplFiniteRelProofInput0NF',
        form: 'array',
...
function summarizeReadiness0(record) {
  ...
  return {
    missingRules: Array.isArray(witness.missingRules)
      ...
      : SEMANTIC_KERNEL_REQUIRED_RULES_FINITEREL0.filter(
