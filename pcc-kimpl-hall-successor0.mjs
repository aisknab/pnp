...
export const KIMPL_HALL_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KIMPL_HALL_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KImplSemanticHallReleasePolicy0',
  ...
  completeMatchingMustBeLeftTotalInjectiveAndEdgeValid: true,
  deficientOutcomeMustCarryExactHallWitness: true,
...
export const KIMPL_HALL_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KImplSemanticHallKernelBinding0',
  version: CHECKER_VERSION,
  predecessorSuccessorChecker: 'CheckKImplDPIndSuccessor0',
  predecessorProofChecker: 'CheckSemanticKernelProofDPInd0',
  proofChecker: 'CheckSemanticKernelProofHall0',
  readinessChecker: 'CheckSemanticKernelReadinessHall0',
});

export function makeKImplHallSuccessor0({
  KImpl = makeSyntheticKImpl0(),
  SemanticProofDAG = makeSemanticProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!KIMPL_HALL_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKImplHallSuccessor0 Purpose must be one of ${KIMPL_HALL_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KImplSemanticHallSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KImpl,
    SemanticKernel: {
      ...KIMPL_HALL_SUCCESSOR_BINDING0,
      ProofDAG: SemanticProofDAG,
    },
    Policy: { ...KIMPL_HALL_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKImplHallSuccessor0(input) {
  return checkSuccessorInternal0(input, {
    checker: 'CheckKImplHallSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKImplHallFinalTheoremReadiness0(input) {
  return checkSuccessorInternal0(input, {
...
async function checkSuccessorInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'Hall final-theorem readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KImplHallSuccessorPurpose0NF',
    purpose,
  }));
  ...
  const predecessorInput = makeKImplDPIndSuccessor0({
    KImpl: input.KImpl,
  ...
  const predecessorCall = await callChecker0(
    'CheckKImplDPIndSuccessor0',
    () => CheckKImplDPIndSuccessor0(predecessorInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplDPIndSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KImpl'],
      witness: {
        reason: 'DPInd predecessor successor rejected the nine-rule semantic base',
  ...
  const proofCall = await callChecker0(
    'CheckSemanticKernelProofHall0',
    () => CheckSemanticKernelProofHall0(input.SemanticKernel.ProofDAG),
  );
  ledger.push(makeLedgerEntry0(
    'CheckSemanticKernelProofHall0',
  ...
  if (proofRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel', 'ProofDAG'],
      witness: {
        reason: 'Hall-extended semantic proof checker rejected the proof DAG',
  ...
  if (purpose === 'final-theorem' && !semanticKernelReady) {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel'],
      witness: {
        reason: 'Hall-extended successor KImpl is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KImplSemanticHallSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticHallNodeCount: proofNF.hallNodeCount ?? null,
      ...
      semanticKernelReady,
      requiredSemanticRules: [...SEMANTIC_KERNEL_REQUIRED_RULES_HALL0],
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0([], 'Hall successor KImpl input must be an object', {
      actual: typeof input,
    });
  }
  if (input.kind !== 'KImplSemanticHallSuccessor0') {
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `Hall successor KImpl version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }
  if (!KIMPL_HALL_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'Hall successor KImpl Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KIMPL_HALL_SUCCESSOR_PURPOSES0],
      },
    );
  }
  if (!isPlainObject0(input.KImpl)) {
    return validationReject0(
      ['KImpl'],
      'Hall successor KImpl must include a base KImpl object',
  ...
  if (!isPlainObject0(input.SemanticKernel)) {
    return validationReject0(
      ['SemanticKernel'],
      'Hall successor KImpl must include SemanticKernel binding',
  ...
  if (!sameCanonical0(binding, KIMPL_HALL_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['SemanticKernel'],
      'Hall semantic-kernel binding must match the executable checker boundary',
      {
        expected: KIMPL_HALL_SUCCESSOR_BINDING0,
        actual: binding,
      },
    );
  }
  if (!Object.prototype.hasOwnProperty.call(input.SemanticKernel, 'ProofDAG')) {
    return validationReject0(
      ...
      'Hall semantic-kernel binding must include ProofDAG',
      null,
    );
  }
  if (!isPlainObject0(input.Policy)) {
    return validationReject0(
      ['Policy'],
      'Hall successor KImpl must include semantic release policy',
      { actual: typeof input.Policy },
    );
  }
  if (!sameCanonical0(input.Policy, KIMPL_HALL_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'Hall successor release policy must match the fail-closed policy',
      {
        expected: KIMPL_HALL_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  if (unexpected.length !== 0) {
    return validationReject0(
      [unexpected[0]],
      'Hall successor rejects undeclared top-level readiness assertions',
  ...
  return validationAccept0({
    kind: 'KImplSemanticHallSuccessorShape0NF',
...
function validateRuleUniverse0() {
  ...
  const semantic = [...SEMANTIC_KERNEL_REQUIRED_RULES_HALL0];
  if (!sameCanonical0(legacy, semantic)) {
    return validationReject0(
      ['SemanticKernel', 'requiredRules'],
      'Hall semantic required-rule universe must match the legacy KImpl universe',
      { legacyRules: legacy, semanticRules: semantic },
    );
  }
  return validationAccept0({
    kind: 'KImplSemanticHallRuleUniverse0NF',
...
function validatePredecessorBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'DPInd predecessor successor did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'DPInd predecessor successor semantic rule set mismatch',
      {
  ...
  return validationAccept0({
    kind: 'KImplHallPredecessorBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'KImplHallProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  if (!isPlainObject0(input)) {
  ...
  return validationAcceptWith0(
    { kind: 'KImplHallProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
...
function summarizeReadiness0(record) {
  ...
  return {
    missingRules: Array.isArray(witness.missingRules)
      ...
      : SEMANTIC_KERNEL_REQUIRED_RULES_HALL0.filter(
