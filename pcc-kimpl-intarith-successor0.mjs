...
export const KIMPL_INTARITH_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KIMPL_INTARITH_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KImplSemanticIntArithReleasePolicy0',
...
export const KIMPL_INTARITH_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KImplSemanticIntArithKernelBinding0',
  version: CHECKER_VERSION,
  predecessorSuccessorChecker: 'CheckKImplMinCounterexampleSuccessor0',
  predecessorProofChecker: 'CheckSemanticKernelProofMinCounterexample0',
  proofChecker: 'CheckSemanticKernelProofIntArith0',
  readinessChecker: 'CheckSemanticKernelReadinessIntArith0',
});

export function makeKImplIntArithSuccessor0({
  KImpl = makeSyntheticKImpl0(),
  SemanticProofDAG = makeSemanticProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!KIMPL_INTARITH_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKImplIntArithSuccessor0 Purpose must be one of ${KIMPL_INTARITH_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KImplSemanticIntArithSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KImpl,
    SemanticKernel: {
      ...KIMPL_INTARITH_SUCCESSOR_BINDING0,
      ProofDAG: SemanticProofDAG,
    },
    Policy: { ...KIMPL_INTARITH_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKImplIntArithSuccessor0(input) {
  return checkSuccessorInternal0(input, {
    checker: 'CheckKImplIntArithSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKImplIntArithFinalTheoremReadiness0(input) {
  return checkSuccessorInternal0(input, {
...
async function checkSuccessorInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'IntArith final-theorem readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KImplIntArithSuccessorPurpose0NF',
    purpose,
  }));
  ...
  const predecessorInput = makeKImplMinCounterexampleSuccessor0({
    KImpl: input.KImpl,
  ...
  const predecessorCall = await callChecker0(
    'CheckKImplMinCounterexampleSuccessor0',
    () => CheckKImplMinCounterexampleSuccessor0(predecessorInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplMinCounterexampleSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KImpl'],
      witness: {
        reason: 'MinCounterexample predecessor successor rejected the twelve-rule semantic base',
  ...
  const proofCall = await callChecker0(
    'CheckSemanticKernelProofIntArith0',
    () => CheckSemanticKernelProofIntArith0(input.SemanticKernel.ProofDAG),
  );
  ledger.push(makeLedgerEntry0(
    'CheckSemanticKernelProofIntArith0',
  ...
  if (proofRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel', 'ProofDAG'],
      witness: {
        reason: 'IntArith-extended semantic proof checker rejected the proof DAG',
  ...
  if (purpose === 'final-theorem' && !semanticKernelReady) {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel'],
      witness: {
        reason: 'IntArith-extended successor KImpl is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KImplSemanticIntArithSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticIntArithNodeCount: proofNF.intArithNodeCount ?? null,
      ...
      semanticKernelReady,
      requiredSemanticRules: [...SEMANTIC_KERNEL_REQUIRED_RULES_INTARITH0],
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0(
      [],
      'IntArith successor KImpl input must be an object',
      { actual: typeof input },
    );
  }
  if (input.kind !== 'KImplSemanticIntArithSuccessor0') {
    return validationReject0(
      ['kind'],
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `IntArith successor KImpl version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }
  if (!KIMPL_INTARITH_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'IntArith successor KImpl Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KIMPL_INTARITH_SUCCESSOR_PURPOSES0],
      },
    );
  }
  if (!isPlainObject0(input.KImpl)) {
    return validationReject0(
      ['KImpl'],
      'IntArith successor KImpl must include a base KImpl object',
  ...
  if (!isPlainObject0(input.SemanticKernel)) {
    return validationReject0(
      ['SemanticKernel'],
      'IntArith successor KImpl must include SemanticKernel binding',
  ...
  if (!sameCanonical0(binding, KIMPL_INTARITH_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['SemanticKernel'],
      'IntArith semantic-kernel binding must match the executable checker boundary',
      {
        expected: KIMPL_INTARITH_SUCCESSOR_BINDING0,
        actual: binding,
      },
    );
  }
  if (!Object.prototype.hasOwnProperty.call(input.SemanticKernel, 'ProofDAG')) {
    return validationReject0(
      ...
      'IntArith semantic-kernel binding must include ProofDAG',
      null,
    );
  }
  if (!isPlainObject0(input.Policy)
      || !sameCanonical0(input.Policy, KIMPL_INTARITH_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'IntArith successor release policy must match the fail-closed policy',
      {
        expected: KIMPL_INTARITH_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  if (unexpected.length !== 0) {
    return validationReject0(
      [unexpected[0]],
      'IntArith successor rejects undeclared top-level readiness assertions',
  ...
  return validationAccept0({
    kind: 'KImplSemanticIntArithSuccessorShape0NF',
...
function validateRuleUniverse0() {
  ...
  const semantic = [...SEMANTIC_KERNEL_REQUIRED_RULES_INTARITH0];
  if (!sameCanonical0(legacy, semantic)) {
    return validationReject0(
      ['SemanticKernel', 'requiredRules'],
      'IntArith semantic required-rule universe must match the legacy KImpl universe',
      { legacyRules: legacy, semanticRules: semantic },
    );
  }
  return validationAccept0({
    kind: 'KImplSemanticIntArithRuleUniverse0NF',
...
function validatePredecessorBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'MinCounterexample predecessor successor did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'MinCounterexample predecessor successor semantic rule set mismatch',
      {
  ...
  return validationAccept0({
    kind: 'KImplIntArithPredecessorBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      {
        kind: 'KImplIntArithProofInput0NF',
        form: 'array',
...
function summarizeReadiness0(record) {
  ...
  return {
    missingRules: Array.isArray(witness.missingRules)
      ...
      : SEMANTIC_KERNEL_REQUIRED_RULES_INTARITH0.filter(
