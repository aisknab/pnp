...
export const KIMPL_OBLTOPOIND_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KIMPL_OBLTOPOIND_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KImplSemanticOblTopoIndReleasePolicy0',
  version: CHECKER_VERSION,
  developmentMayUsePartialSemanticKernel: true,
  predecessorSuccessorMustRemainDevelopmentOnly: true,
  exactObligationLifecycleScheduleRequired: true,
  localObligationCasesMustBeAcceptedRecordEvidence: true,
  onlyR5CreatesObligations: true,
  onlyR6R7R8DischargeObligations: true,
  fullModeDischargeWitnessRequired: true,
  crossModeDependenciesRequireExplicitTransport: true,
...
export const KIMPL_OBLTOPOIND_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KImplSemanticOblTopoIndKernelBinding0',
  version: CHECKER_VERSION,
  predecessorSuccessorChecker: 'CheckKImplLedgerIndSuccessor0',
  predecessorProofChecker: 'CheckSemanticKernelProofLedgerInd0',
  proofChecker: 'CheckSemanticKernelProofOblTopoInd0',
  readinessChecker: 'CheckSemanticKernelReadinessOblTopoInd0',
});

export function makeKImplOblTopoIndSuccessor0({
  KImpl = makeSyntheticKImpl0(),
  SemanticProofDAG = makeSemanticProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!KIMPL_OBLTOPOIND_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKImplOblTopoIndSuccessor0 Purpose must be one of ${KIMPL_OBLTOPOIND_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KImplSemanticOblTopoIndSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KImpl,
    SemanticKernel: {
      ...KIMPL_OBLTOPOIND_SUCCESSOR_BINDING0,
      ProofDAG: SemanticProofDAG,
    },
    Policy: { ...KIMPL_OBLTOPOIND_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKImplOblTopoIndSuccessor0(input) {
  return checkSuccessorInternal0(input, {
    checker: 'CheckKImplOblTopoIndSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKImplOblTopoIndFinalTheoremReadiness0(input) {
  return checkSuccessorInternal0(input, {
...
async function checkSuccessorInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'OblTopoInd final-theorem readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KImplOblTopoIndSuccessorPurpose0NF',
    purpose,
  }));
  ...
  const predecessorInput = makeKImplLedgerIndSuccessor0({
    KImpl: input.KImpl,
  ...
  const predecessorCall = await callChecker0(
    'CheckKImplLedgerIndSuccessor0',
    () => CheckKImplLedgerIndSuccessor0(predecessorInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplLedgerIndSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KImpl'],
      witness: {
        reason: 'LedgerInd predecessor successor rejected the Eq/Subst/Record/DAGInd/LedgerInd base',
  ...
  const proofCall = await callChecker0(
    'CheckSemanticKernelProofOblTopoInd0',
    () => CheckSemanticKernelProofOblTopoInd0(input.SemanticKernel.ProofDAG),
  );
  ledger.push(makeLedgerEntry0(
    'CheckSemanticKernelProofOblTopoInd0',
  ...
  if (proofRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel', 'ProofDAG'],
      witness: {
        reason: 'OblTopoInd-extended semantic proof checker rejected the proof DAG',
  ...
  if (purpose === 'final-theorem' && !semanticKernelReady) {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel'],
      witness: {
        reason: 'OblTopoInd-extended successor KImpl is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KImplSemanticOblTopoIndSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticOblTopoIndNodeCount: proofNF.oblTopoIndNodeCount ?? null,
      ...
      semanticKernelReady,
      requiredSemanticRules: [...SEMANTIC_KERNEL_REQUIRED_RULES_OBLTOPOIND0],
      supportedSemanticRules: [...SEMANTIC_KERNEL_SUPPORTED_RULES_OBLTOPOIND0],
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0([], 'OblTopoInd successor KImpl input must be an object', {
      actual: typeof input,
    });
  }

  if (input.kind !== 'KImplSemanticOblTopoIndSuccessor0') {
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `OblTopoInd successor KImpl version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!KIMPL_OBLTOPOIND_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'OblTopoInd successor KImpl Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KIMPL_OBLTOPOIND_SUCCESSOR_PURPOSES0],
      },
    );
  }
  ...
  if (!isPlainObject0(input.KImpl)) {
    return validationReject0(
      ['KImpl'],
      'OblTopoInd successor KImpl must include a base KImpl object',
  ...
  if (!isPlainObject0(input.SemanticKernel)) {
    return validationReject0(
      ['SemanticKernel'],
      'OblTopoInd successor KImpl must include SemanticKernel binding',
  ...
  if (!sameCanonical0(binding, KIMPL_OBLTOPOIND_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['SemanticKernel'],
      'OblTopoInd semantic-kernel binding must match the executable checker boundary',
      {
        expected: KIMPL_OBLTOPOIND_SUCCESSOR_BINDING0,
        actual: binding,
      },
    );
  }
  ...
  if (!Object.prototype.hasOwnProperty.call(input.SemanticKernel, 'ProofDAG')) {
    return validationReject0(
      ...
      'OblTopoInd semantic-kernel binding must include ProofDAG',
      null,
    );
  }
  ...
  if (!isPlainObject0(input.Policy)) {
    return validationReject0(
      ['Policy'],
      'OblTopoInd successor KImpl must include semantic release policy',
      { actual: typeof input.Policy },
    );
  }

  if (!sameCanonical0(input.Policy, KIMPL_OBLTOPOIND_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'OblTopoInd successor release policy must match the fail-closed policy',
      {
        expected: KIMPL_OBLTOPOIND_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  if (unexpected.length !== 0) {
    return validationReject0(
      [unexpected[0]],
      'OblTopoInd successor rejects undeclared top-level readiness assertions',
  ...
  return validationAccept0({
    kind: 'KImplSemanticOblTopoIndSuccessorShape0NF',
...
function validateRuleUniverse0() {
  ...
  const semantic = [...SEMANTIC_KERNEL_REQUIRED_RULES_OBLTOPOIND0];
  if (!sameCanonical0(legacy, semantic)) {
    return validationReject0(
      ['SemanticKernel', 'requiredRules'],
      'OblTopoInd semantic required-rule universe must match the legacy KImpl universe',
      { legacyRules: legacy, semanticRules: semantic },
    );
  }

  return validationAccept0({
    kind: 'KImplSemanticOblTopoIndRuleUniverse0NF',
...
function validatePredecessorBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'LedgerInd predecessor successor did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(nf.supportedSemanticRules, expectedRules)) {
    return validationReject0(
      ...
      'LedgerInd predecessor successor semantic rule set mismatch',
      { expected: expectedRules, actual: nf.supportedSemanticRules },
    );
  }

  return validationAccept0({
    kind: 'KImplOblTopoIndPredecessorBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'KImplOblTopoIndProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  ...
  return validationAcceptWith0(
    { kind: 'KImplOblTopoIndProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
...
function summarizeReadiness0(record) {
  ...
  return {
    missingRules: Array.isArray(witness.missingRules)
      ...
      : SEMANTIC_KERNEL_REQUIRED_RULES_OBLTOPOIND0.filter(
