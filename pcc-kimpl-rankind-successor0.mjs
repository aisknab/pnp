...
export const KIMPL_RANKIND_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KIMPL_RANKIND_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KImplSemanticRankIndReleasePolicy0',
...
export const KIMPL_RANKIND_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KImplSemanticRankIndKernelBinding0',
  version: CHECKER_VERSION,
  predecessorSuccessorChecker: 'CheckKImplHallSuccessor0',
  predecessorProofChecker: 'CheckSemanticKernelProofHall0',
  proofChecker: 'CheckSemanticKernelProofRankInd0',
  readinessChecker: 'CheckSemanticKernelReadinessRankInd0',
});

export function makeKImplRankIndSuccessor0({
  KImpl = makeSyntheticKImpl0(),
  SemanticProofDAG = makeSemanticProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!KIMPL_RANKIND_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKImplRankIndSuccessor0 Purpose must be one of ${KIMPL_RANKIND_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KImplSemanticRankIndSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KImpl,
    SemanticKernel: {
      ...KIMPL_RANKIND_SUCCESSOR_BINDING0,
      ProofDAG: SemanticProofDAG,
    },
    Policy: { ...KIMPL_RANKIND_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKImplRankIndSuccessor0(input) {
  return checkSuccessorInternal0(input, {
    checker: 'CheckKImplRankIndSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKImplRankIndFinalTheoremReadiness0(input) {
  return checkSuccessorInternal0(input, {
...
async function checkSuccessorInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'RankInd final-theorem readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KImplRankIndSuccessorPurpose0NF',
    purpose,
  }));
  ...
  const predecessorInput = makeKImplHallSuccessor0({
    KImpl: input.KImpl,
  ...
  const predecessorCall = await callChecker0(
    'CheckKImplHallSuccessor0',
    () => CheckKImplHallSuccessor0(predecessorInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplHallSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KImpl'],
      witness: {
        reason: 'Hall predecessor successor rejected the ten-rule semantic base',
  ...
  const proofCall = await callChecker0(
    'CheckSemanticKernelProofRankInd0',
    () => CheckSemanticKernelProofRankInd0(input.SemanticKernel.ProofDAG),
  );
  ledger.push(makeLedgerEntry0(
    'CheckSemanticKernelProofRankInd0',
  ...
  if (proofRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel', 'ProofDAG'],
      witness: {
        reason: 'RankInd-extended semantic proof checker rejected the proof DAG',
  ...
  if (purpose === 'final-theorem' && !semanticKernelReady) {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel'],
      witness: {
        reason: 'RankInd-extended successor KImpl is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KImplSemanticRankIndSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticRankIndNodeCount: proofNF.rankIndNodeCount ?? null,
      ...
      semanticKernelReady,
      requiredSemanticRules: [...SEMANTIC_KERNEL_REQUIRED_RULES_RANKIND0],
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0([], 'RankInd successor KImpl input must be an object', {
      actual: typeof input,
    });
  }
  if (input.kind !== 'KImplSemanticRankIndSuccessor0') {
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `RankInd successor KImpl version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }
  if (!KIMPL_RANKIND_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'RankInd successor KImpl Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KIMPL_RANKIND_SUCCESSOR_PURPOSES0],
      },
    );
  }
  if (!isPlainObject0(input.KImpl)) {
    return validationReject0(
      ['KImpl'],
      'RankInd successor KImpl must include a base KImpl object',
  ...
  if (!isPlainObject0(input.SemanticKernel)) {
    return validationReject0(
      ['SemanticKernel'],
      'RankInd successor KImpl must include SemanticKernel binding',
  ...
  if (!sameCanonical0(binding, KIMPL_RANKIND_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['SemanticKernel'],
      'RankInd semantic-kernel binding must match the executable checker boundary',
      {
        expected: KIMPL_RANKIND_SUCCESSOR_BINDING0,
        actual: binding,
      },
    );
  }
  if (!Object.prototype.hasOwnProperty.call(input.SemanticKernel, 'ProofDAG')) {
    return validationReject0(
      ...
      'RankInd semantic-kernel binding must include ProofDAG',
      null,
    );
  }
  if (!isPlainObject0(input.Policy)
      || !sameCanonical0(input.Policy, KIMPL_RANKIND_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'RankInd successor release policy must match the fail-closed policy',
      {
        expected: KIMPL_RANKIND_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  if (unexpected.length !== 0) {
    return validationReject0(
      [unexpected[0]],
      'RankInd successor rejects undeclared top-level readiness assertions',
  ...
  return validationAccept0({
    kind: 'KImplSemanticRankIndSuccessorShape0NF',
...
function validateRuleUniverse0() {
  ...
  const semantic = [...SEMANTIC_KERNEL_REQUIRED_RULES_RANKIND0];
  if (!sameCanonical0(legacy, semantic)) {
    return validationReject0(
      ['SemanticKernel', 'requiredRules'],
      'RankInd semantic required-rule universe must match the legacy KImpl universe',
      { legacyRules: legacy, semanticRules: semantic },
    );
  }
  return validationAccept0({
    kind: 'KImplSemanticRankIndRuleUniverse0NF',
...
function validatePredecessorBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'Hall predecessor successor did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'Hall predecessor successor semantic rule set mismatch',
      {
  ...
  return validationAccept0({
    kind: 'KImplRankIndPredecessorBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'KImplRankIndProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  if (!isPlainObject0(input)) {
  ...
  return validationAcceptWith0(
    { kind: 'KImplRankIndProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
...
function summarizeReadiness0(record) {
  ...
  return {
    missingRules: Array.isArray(witness.missingRules)
      ...
      : SEMANTIC_KERNEL_REQUIRED_RULES_RANKIND0.filter(
