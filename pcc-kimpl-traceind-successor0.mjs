...
export const KIMPL_TRACEIND_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KIMPL_TRACEIND_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KImplSemanticTraceIndReleasePolicy0',
  ...
  localTraceCasesMustBeAcceptedRecordEvidence: true,
...
export const KIMPL_TRACEIND_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KImplSemanticTraceIndKernelBinding0',
  version: CHECKER_VERSION,
  predecessorSuccessorChecker: 'CheckKImplOblTopoIndSuccessor0',
  predecessorProofChecker: 'CheckSemanticKernelProofOblTopoInd0',
  proofChecker: 'CheckSemanticKernelProofTraceInd0',
  readinessChecker: 'CheckSemanticKernelReadinessTraceInd0',
});

export function makeKImplTraceIndSuccessor0({
  KImpl = makeSyntheticKImpl0(),
  SemanticProofDAG = makeSemanticProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!KIMPL_TRACEIND_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKImplTraceIndSuccessor0 Purpose must be one of ${KIMPL_TRACEIND_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }

  return {
    kind: 'KImplSemanticTraceIndSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KImpl,
    SemanticKernel: {
      ...KIMPL_TRACEIND_SUCCESSOR_BINDING0,
      ProofDAG: SemanticProofDAG,
    },
    Policy: { ...KIMPL_TRACEIND_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKImplTraceIndSuccessor0(input) {
  return checkSuccessorInternal0(input, {
    checker: 'CheckKImplTraceIndSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKImplTraceIndFinalTheoremReadiness0(input) {
  return checkSuccessorInternal0(input, {
...
async function checkSuccessorInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'TraceInd final-theorem readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KImplTraceIndSuccessorPurpose0NF',
    purpose,
  }));
  ...
  const predecessorInput = makeKImplOblTopoIndSuccessor0({
    KImpl: input.KImpl,
  ...
  const predecessorCall = await callChecker0(
    'CheckKImplOblTopoIndSuccessor0',
    () => CheckKImplOblTopoIndSuccessor0(predecessorInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplOblTopoIndSuccessor0',
  ...
  if (predecessorRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KImpl'],
      witness: {
        reason: 'OblTopoInd predecessor successor rejected the six-rule semantic base',
  ...
  const proofCall = await callChecker0(
    'CheckSemanticKernelProofTraceInd0',
    () => CheckSemanticKernelProofTraceInd0(input.SemanticKernel.ProofDAG),
  );
  ledger.push(makeLedgerEntry0(
    'CheckSemanticKernelProofTraceInd0',
  ...
  if (proofRecord.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel', 'ProofDAG'],
      witness: {
        reason: 'TraceInd-extended semantic proof checker rejected the proof DAG',
  ...
  if (purpose === 'final-theorem' && !semanticKernelReady) {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel'],
      witness: {
        reason: 'TraceInd-extended successor KImpl is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KImplSemanticTraceIndSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticTraceIndNodeCount: proofNF.traceIndNodeCount ?? null,
      ...
      semanticKernelReady,
      requiredSemanticRules: [...SEMANTIC_KERNEL_REQUIRED_RULES_TRACEIND0],
      supportedSemanticRules: [...SEMANTIC_KERNEL_SUPPORTED_RULES_TRACEIND0],
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0([], 'TraceInd successor KImpl input must be an object', {
      actual: typeof input,
    });
  }

  if (input.kind !== 'KImplSemanticTraceIndSuccessor0') {
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `TraceInd successor KImpl version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }

  if (!KIMPL_TRACEIND_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'TraceInd successor KImpl Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KIMPL_TRACEIND_SUCCESSOR_PURPOSES0],
      },
    );
  }
  ...
  if (!isPlainObject0(input.KImpl)) {
    return validationReject0(
      ['KImpl'],
      'TraceInd successor KImpl must include a base KImpl object',
  ...
  if (!isPlainObject0(input.SemanticKernel)) {
    return validationReject0(
      ['SemanticKernel'],
      'TraceInd successor KImpl must include SemanticKernel binding',
  ...
  if (!sameCanonical0(binding, KIMPL_TRACEIND_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['SemanticKernel'],
      'TraceInd semantic-kernel binding must match the executable checker boundary',
      {
        expected: KIMPL_TRACEIND_SUCCESSOR_BINDING0,
        actual: binding,
      },
    );
  }
  ...
  if (!Object.prototype.hasOwnProperty.call(input.SemanticKernel, 'ProofDAG')) {
    return validationReject0(
      ...
      'TraceInd semantic-kernel binding must include ProofDAG',
      null,
    );
  }
  ...
  if (!isPlainObject0(input.Policy)) {
    return validationReject0(
      ['Policy'],
      'TraceInd successor KImpl must include semantic release policy',
      { actual: typeof input.Policy },
    );
  }

  if (!sameCanonical0(input.Policy, KIMPL_TRACEIND_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'TraceInd successor release policy must match the fail-closed policy',
      {
        expected: KIMPL_TRACEIND_SUCCESSOR_POLICY0,
        actual: input.Policy,
      },
    );
  }
  ...
  if (unexpected.length !== 0) {
    return validationReject0(
      [unexpected[0]],
      'TraceInd successor rejects undeclared top-level readiness assertions',
  ...
  return validationAccept0({
    kind: 'KImplSemanticTraceIndSuccessorShape0NF',
...
function validateRuleUniverse0() {
  ...
  const semantic = [...SEMANTIC_KERNEL_REQUIRED_RULES_TRACEIND0];
  if (!sameCanonical0(legacy, semantic)) {
    return validationReject0(
      ['SemanticKernel', 'requiredRules'],
      'TraceInd semantic required-rule universe must match the legacy KImpl universe',
      { legacyRules: legacy, semanticRules: semantic },
    );
  }

  return validationAccept0({
    kind: 'KImplSemanticTraceIndRuleUniverse0NF',
...
function validatePredecessorBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'OblTopoInd predecessor successor did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(nf.supportedSemanticRules, expectedRules)) {
    return validationReject0(
      ...
      'OblTopoInd predecessor successor semantic rule set mismatch',
      { expected: expectedRules, actual: nf.supportedSemanticRules },
    );
  }

  return validationAccept0({
    kind: 'KImplTraceIndPredecessorBoundary0NF',
    ...expected,
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'KImplTraceIndProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  ...
  return validationAcceptWith0(
    { kind: 'KImplTraceIndProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
...
function summarizeReadiness0(record) {
  ...
  return {
    missingRules: Array.isArray(witness.missingRules)
      ...
      : SEMANTIC_KERNEL_REQUIRED_RULES_TRACEIND0.filter(
