...
export const KIMPL_TRANSPORT_SUCCESSOR_PURPOSES0 = Object.freeze([
  'development',
  'final-theorem',
]);

export const KIMPL_TRANSPORT_SUCCESSOR_POLICY0 = Object.freeze({
  kind: 'KImplSemanticTransportReleasePolicy0',
...
export const KIMPL_TRANSPORT_SUCCESSOR_BINDING0 = Object.freeze({
  kind: 'KImplSemanticTransportKernelBinding0',
  version: CHECKER_VERSION,
  predecessorSuccessorChecker: 'CheckKImplIntArithSuccessor0',
  predecessorProofChecker: 'CheckSemanticKernelProofIntArith0',
  proofChecker: 'CheckSemanticKernelProofTransport0',
  readinessChecker: 'CheckSemanticKernelReadinessTransport0',
});

export function makeKImplTransportSuccessor0({
  KImpl = makeSyntheticKImpl0(),
  SemanticProofDAG = makeSemanticProofDAG0(),
  Purpose = 'development',
} = {}) {
  if (!KIMPL_TRANSPORT_SUCCESSOR_PURPOSES0.includes(Purpose)) {
    throw new TypeError(
      `makeKImplTransportSuccessor0 Purpose must be one of ${KIMPL_TRANSPORT_SUCCESSOR_PURPOSES0.join(', ')}`,
    );
  }
  return {
    kind: 'KImplSemanticTransportSuccessor0',
    version: CHECKER_VERSION,
    Purpose,
    KImpl,
    SemanticKernel: {
      ...KIMPL_TRANSPORT_SUCCESSOR_BINDING0,
      ProofDAG: SemanticProofDAG,
    },
    Policy: { ...KIMPL_TRANSPORT_SUCCESSOR_POLICY0 },
  };
}

export async function CheckKImplTransportSuccessor0(input) {
  return checkSuccessorInternal0(input, {
    checker: 'CheckKImplTransportSuccessor0',
    requiredPurpose: null,
  });
}

export async function CheckKImplTransportFinalTheoremReadiness0(input) {
  return checkSuccessorInternal0(input, {
    checker: 'CheckKImplTransportFinalTheoremReadiness0',
...
async function checkSuccessorInternal0(input, {
  ...
  if (requiredPurpose !== null && input.Purpose !== requiredPurpose) {
    const witness = {
      reason: 'Transport final-theorem readiness requires a final-theorem purpose record',
      requiredPurpose,
  ...
  ledger.push(makeLedgerEntry0('purpose', true, {
    kind: 'KImplTransportSuccessorPurpose0NF',
    purpose,
  }));

  const universe = validateRuleUniverse0();
  ledger.push(makeLedgerEntry0(
    'semanticRuleUniverse',
    universe.ok,
    universe.nf ?? universe.witness,
  ));
  if (!universe.ok) {
  ...
  const predecessorInput = makeKImplIntArithSuccessor0({
    KImpl: input.KImpl,
  ...
  const predecessorCall = await callChecker0(
    'CheckKImplIntArithSuccessor0',
    () => CheckKImplIntArithSuccessor0(predecessorInput),
  );
  ledger.push(makeLedgerEntry0(
    'CheckKImplIntArithSuccessor0',
  ...
  if (predecessorCall.record.tag !== 'accept') {
    return makeRejectRecord0({
      ...
      path: ['KImpl'],
      witness: {
        reason: 'IntArith predecessor successor rejected the thirteen-rule semantic base',
        predecessorNodeIds: predecessorNodes.map((node) => node?.id ?? null),
        inner: compactReject0(predecessorCall.record),
      },
      ledger,
    });
  }
  ...
  const proofCall = await callChecker0(
    'CheckSemanticKernelProofTransport0',
    () => CheckSemanticKernelProofTransport0(input.SemanticKernel.ProofDAG),
  );
  ledger.push(makeLedgerEntry0(
    'CheckSemanticKernelProofTransport0',
  ...
  if (proofCall.record.tag !== 'accept') {
    return makeRejectRecord0({
      checker,
      coord: `${checker}.semanticProof`,
      path: ['SemanticKernel', 'ProofDAG'],
      witness: {
        reason: 'Transport-extended semantic proof checker rejected the proof DAG',
        inner: compactReject0(proofCall.record),
      },
      ledger,
    });
  }
  ...
  if (purpose === 'final-theorem' && !semanticKernelReady) {
    return makeRejectRecord0({
      ...
      path: ['SemanticKernel'],
      witness: {
        reason: 'Transport-extended successor KImpl is not ready for final-theorem use',
  ...
  return makeAcceptRecord0({
    checker,
    nf: {
      kind: 'KImplSemanticTransportSuccessor0NF',
      checker,
      version: CHECKER_VERSION,
      purpose,
      ...
      semanticTransportNodeCount: proofNF.transportNodeCount ?? null,
      ...
      semanticKernelReady,
      requiredSemanticRules: [...SEMANTIC_KERNEL_REQUIRED_RULES_TRANSPORT0],
...
function validateShape0(input) {
  if (!isPlainObject0(input)) {
    return validationReject0(
      [],
      'Transport successor KImpl input must be an object',
      { actual: typeof input },
    );
  }
  if (input.kind !== 'KImplSemanticTransportSuccessor0') {
    return validationReject0(
      ['kind'],
  ...
  if (input.version !== CHECKER_VERSION) {
    return validationReject0(
      ['version'],
      `Transport successor KImpl version must be ${CHECKER_VERSION}`,
      { actual: input.version },
    );
  }
  if (!KIMPL_TRANSPORT_SUCCESSOR_PURPOSES0.includes(input.Purpose)) {
    return validationReject0(
      ['Purpose'],
      'Transport successor KImpl Purpose is unsupported',
      {
        actual: input.Purpose,
        supportedPurposes: [...KIMPL_TRANSPORT_SUCCESSOR_PURPOSES0],
      },
    );
  }
  if (!isPlainObject0(input.KImpl)) {
    return validationReject0(
      ['KImpl'],
      'Transport successor KImpl must include a base KImpl object',
  ...
  if (!isPlainObject0(input.SemanticKernel)) {
    return validationReject0(
      ['SemanticKernel'],
      'Transport successor KImpl must include SemanticKernel binding',
  ...
  if (!sameCanonical0(binding, KIMPL_TRANSPORT_SUCCESSOR_BINDING0)) {
    return validationReject0(
      ['SemanticKernel'],
      'Transport semantic-kernel binding must match the executable checker boundary',
      { expected: KIMPL_TRANSPORT_SUCCESSOR_BINDING0, actual: binding },
    );
  }
  if (!Object.prototype.hasOwnProperty.call(input.SemanticKernel, 'ProofDAG')) {
    return validationReject0(
      ['SemanticKernel', 'ProofDAG'],
      'Transport semantic-kernel binding must include ProofDAG',
      null,
    );
  }
  if (!isPlainObject0(input.Policy)
      || !sameCanonical0(input.Policy, KIMPL_TRANSPORT_SUCCESSOR_POLICY0)) {
    return validationReject0(
      ['Policy'],
      'Transport successor release policy must match the fail-closed policy',
      { expected: KIMPL_TRANSPORT_SUCCESSOR_POLICY0, actual: input.Policy },
    );
  }
  ...
  if (unexpected.length !== 0) {
    return validationReject0(
      [unexpected[0]],
      'Transport successor rejects undeclared top-level readiness assertions',
  ...
  return validationAccept0({
    kind: 'KImplSemanticTransportSuccessorShape0NF',
...
function validateRuleUniverse0() {
  ...
  const semantic = [...SEMANTIC_KERNEL_REQUIRED_RULES_TRANSPORT0];
  if (!sameCanonical0(legacy, semantic)) {
    return validationReject0(
      ['SemanticKernel', 'requiredRules'],
      'Transport semantic required-rule universe must match the legacy KImpl universe',
      { legacyRules: legacy, semanticRules: semantic },
    );
  }
  return validationAccept0({
    kind: 'KImplSemanticTransportRuleUniverse0NF',
...
function validatePredecessorBoundary0(nf) {
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (nf[field] !== value) {
      return validationReject0(
        ...
        'IntArith predecessor successor did not preserve its development-only boundary',
  ...
  if (!sameCanonical0(
    ...
    return validationReject0(
      ...
      'IntArith predecessor successor semantic rule set mismatch',
      {
  ...
  return validationAccept0({
    kind: 'KImplTransportPredecessorBoundary0NF',
    ...expected,
    supportedSemanticRules: [...SEMANTIC_KERNEL_SUPPORTED_RULES_INTARITH0],
  });
}
...
function extractNodes0(input) {
  if (Array.isArray(input)) {
    return validationAcceptWith0(
      { kind: 'KImplTransportProofInput0NF', form: 'array', nodeCount: input.length },
      { nodes: input },
    );
  }
  if (!isPlainObject0(input)) {
  ...
  return validationAcceptWith0(
    { kind: 'KImplTransportProofInput0NF', form: 'object', nodeCount: nodes.length },
    { nodes },
  );
}
...
function summarizeReadiness0(record) {
  ...
  return {
    missingRules: Array.isArray(witness.missingRules)
      ...
      : SEMANTIC_KERNEL_REQUIRED_RULES_TRANSPORT0.filter(
...
function makeAcceptRecord0({ checker, nf, ledger }) {
  ...
  return {
    tag: 'accept', kind: 'accept', checker, version: CHECKER_VERSION,
...
function makeRejectFromValidation0(checker, coord, result, ledger) {
  return makeRejectRecord0({
    checker, coord, path: result.path, witness: result.witness, ledger,
  });
}

function makeRejectRecord0({ checker, coord, path, witness, ledger }) {
  const nf = {
    kind: `${checker}RejectNF`, checker, version: CHECKER_VERSION,
    coord, path, witness, ledger,
  };
  const digest = digestCanonical0(nf);
  return {
    tag: 'reject', kind: 'reject', checker, version: CHECKER_VERSION,
