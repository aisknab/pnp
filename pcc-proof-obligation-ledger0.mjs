...
function validateLedger0(ledger) {
  ...
  if (!plain0(ledger.hashPolicy)) return reject0('ProofObligationLedger.HashPolicyShape', ['hashPolicy'], 'hashPolicy must be an object');
  if (ledger.hashPolicy.mode !== 'checker-computed-source-digests') return reject0('ProofObligationLedger.HashPolicyMode', ['hashPolicy', 'mode'], 'hash policy mode mismatch');
  if (ledger.hashPolicy.algorithm !== 'SHA256') return reject0('ProofObligationLedger.HashPolicyAlgorithm', ['hashPolicy', 'algorithm'], 'hash policy algorithm mismatch');
  if (!nonempty0(ledger.hashPolicy.rationale)) return reject0('ProofObligationLedger.HashPolicyRationale', ['hashPolicy', 'rationale'], 'hash policy rationale must be non-empty');
  ...
  for (const status of ALLOWED_STATUSES) {
    if (!nonempty0(ledger.obligationStatusLegend[status])) return reject0('ProofObligationLedger.StatusLegendMissing', ['obligationStatusLegend', status], 'status legend missing required status');
  }
  ...
  const expectedAudit = {
    checker: CHECKER,
    script: 'pcc-proof-obligation-ledger0.mjs',
    test: 'audits/proof-obligation-ledger0.test.mjs',
...
function validateObligation0(obligation, obligationPath, seen) {
  ...
  for (const field of ['id', 'statement', 'checker', 'status', 'hashMode']) {
    if (!nonempty0(obligation[field])) return reject0('ProofObligationLedger.ObligationField', [...obligationPath, field], 'obligation field must be a non-empty string');
  }
  ...
  const sourceCheck = validateStringArray0(obligation.sourceFiles, [...obligationPath, 'sourceFiles'], true);
  if (sourceCheck.tag === 'reject') return sourceCheck;
  const testCheck = validateStringArray0(obligation.testFiles, [...obligationPath, 'testFiles'], true);
  if (testCheck.tag === 'reject') return testCheck;
  const dependencyCheck = validateStringArray0(obligation.dependencies, [...obligationPath, 'dependencies'], false);
  if (dependencyCheck.tag === 'reject') return dependencyCheck;
  for (const dependency of obligation.dependencies) {
    if (!seen.has(dependency)) return reject0('ProofObligationLedger.ForwardDependency', [...obligationPath, 'dependencies'], 'obligation dependencies must point to earlier obligations', { dependency });
  }
  if (obligation.status === 'blocked-release-obligation' && !obligation.dependencies.some((id) => id.startsWith('OBL-013') || id.startsWith('OBL-010') || id.startsWith('OBL-011') || id.startsWith('OBL-012'))) {
    return reject0('ProofObligationLedger.BlockedDependency', [...obligationPath, 'dependencies'], 'blocked release obligations must depend on represented activation or release-ladder obligations');
  }
  return { tag: 'accept' };
}
...
async function digestObligations0({ root, obligations }) {
  ...
  for (const obligation of obligations) {
    ...
    obligationDigests.push({
      id: obligation.id,
      status: obligation.status,
      checker: obligation.checker,
      sourceDigest: sha256Text0(stableStringify0(sourceFiles)),
      testDigest: sha256Text0(stableStringify0(testFiles)),
      sourceFiles,
      testFiles,
      dependencyDigest: sha256Text0(stableStringify0(obligation.dependencies)),
    });
  }
...
function validateStringArray0(value, pathArray, nonEmpty) {
  if (!Array.isArray(value)) return reject0('ProofObligationLedger.StringArrayShape', pathArray, 'field must be an array of strings');
  if (nonEmpty && value.length === 0) return reject0('ProofObligationLedger.StringArrayEmpty', pathArray, 'field must not be empty');
  for (let index = 0; index < value.length; index += 1) {
    if (!nonempty0(value[index])) return reject0('ProofObligationLedger.StringArrayEntry', [...pathArray, index], 'array entry must be a non-empty string');
  }
  return { tag: 'accept' };
}
...
async function main0() {
  ...
  try { options = parseArgs0(process.argv.slice(2)); }
    ...
    const verdict = reject0('Cli.BadArgument', [], 'bad proof obligation ledger CLI argument', normalizeError0(error));
    console.error(JSON.stringify(verdict, null, 2));
    process.exit(2);
  }
  const verdict = await CheckProofObligationLedger0(options);
