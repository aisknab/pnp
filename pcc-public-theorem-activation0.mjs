...
const REQUIRED_OBLIGATIONS = Object.freeze(['PTW-001-CurrentEmissionDisabled', 'PTW-002-LegacyActivationSuperseded', 'PTW-003-FormalGateRequired', 'PTW-004-AssertionRecordsNonAuthoritative', 'PTW-005-NoHumanPremise']);
...
export async function CheckPublicTheoremActivation0(options = {}) {
  ...
  try {
    const status = await CheckFormalReconstructionStatus0({ root, writeOutput: false, statusOverride: options.statusOverride, siteOverride: options.siteOverride, legacyStatusOverride: options.legacyStatusOverride });
...
export function EvaluatePublicTheoremActivationExample0(input) {
  ...
  return { tag: 'accept', formalReleaseGatePassed: gate.formalReleaseGatePassed, publicTheoremEmissionAllowed: gate.publicTheoremEmissionAllowed, publicTheoremStatement: gate.publicTheoremEmissionAllowed ? 'P = NP' : null, remainingFormalRequirements: [...gate.missing, ...gate.forbidden] };
}
...
function validateManifest0(manifest) {
  ...
  const exact = { kind: 'PNPPublicTheoremActivationWithdrawal0', version: VERSION, coordinate: COORD, status: 'public-theorem-activation-withdrawn', supersedesCoordinate: HISTORICAL_COORD, supersededByStatusCoordinate: STATUS_COORD, publicTheoremActivationAccepted: false, publicTheoremActivationWithdrawn: true, publicTheoremEmissionAllowed: false, publicTheoremStatement: null, publicTheoremConclusion: null, publicTheoremUnderCheckerTrustModel: false, finalTheoremReady: false, internalFinalTheoremReady: false, formalReleaseGatePassed: false, externalReviewAcceptanceRequiredForEmission: false, externalReviewIsMathematicalPremise: false, humanReviewRequiredForMathematicalValidity: false };
  for (const [key, expected] of Object.entries(exact)) if (manifest[key] !== expected) return reject0('PublicTheoremActivation.WithdrawalBoolean', [key], 'withdrawal field mismatch', { expected, actual: manifest[key] });
  ...
  for (const key of ['requiresClosedLeanRootTheoremForFutureActivation', 'requiresConcreteMachineSemantics', 'requiresNoProjectSpecificAxioms', 'requiresNoSorryOrAdmit', 'requiresFormalPolynomialRuntimeProof']) if (policy[key] !== true) return reject0('PublicTheoremActivation.PolicyBoolean', ['withdrawalPolicy', key], 'withdrawal policy required field must be true', { actual: policy[key] });
  for (const key of ['allowsJsonBooleanActivation', 'allowsJavaScriptCheckerAcceptanceAsTheoremEvidence', 'usesExternalReviewAsPremise', 'usesHistoricalReportProseAsPremise', 'usesPublicSiteWordingAsPremise', 'allowsPublicTheoremEmissionNow']) if (policy[key] !== false) return reject0('PublicTheoremActivation.PolicyBoolean', ['withdrawalPolicy', key], 'withdrawal policy forbidden field must be false', { actual: policy[key] });
...
function validateExamples0(manifest) { for (let index = 0; index < manifest.positiveExamples.length; index += 1) { const example = manifest.positiveExamples[index]; const out = EvaluatePublicTheoremActivationExample0(example.input); if (out.tag !== 'accept') return reject0('PublicTheoremActivation.PositiveExampleRejected', ['positiveExamples', index], 'positive example rejected', { exampleId: example.id, reject: out }); for (const [key, expected] of Object.entries(example.expected)) if (out[key] !== expected) return reject0('PublicTheoremActivation.PositiveExampleMismatch', ['positiveExamples', index, 'expected', key], 'positive example mismatch', { expected, actual: out[key] }); } return { tag: 'accept' }; }
