...
export function makeReadmeReleaseBoundaryConfig0(overrides = {}) {
  return { kind: 'ReadmeReleaseBoundaryConfig0', version: CHECKER_VERSION, rootDir: REPO_ROOT, readmePath: null, readmeText: null, ...overrides };
}
...
export async function CheckReadmeReleaseBoundary0(config = makeReadmeReleaseBoundaryConfig0()) {
  ...
  const shape = validateConfig0(cfg); ledger.push(ledgerEntry0('config', shape));
  if (!shape.ok) return makeRejectRecord0(checker, `${checker}.config`, shape.path, shape.witness, ledger);
  const loaded = await loadReadmeText0(cfg); ledger.push(ledgerEntry0('load', loaded));
  if (!loaded.ok) return makeRejectRecord0(checker, `${checker}.load`, loaded.path, loaded.witness, ledger);
  const required = validateRequiredSnippets0(loaded.text); ledger.push(ledgerEntry0('requiredSnippets', required));
  if (!required.ok) return makeRejectRecord0(checker, `${checker}.requiredSnippet`, required.path, required.witness, ledger);
  const forbidden = validateForbiddenSnippets0(loaded.text); ledger.push(ledgerEntry0('forbiddenSnippets', forbidden));
  if (!forbidden.ok) return makeRejectRecord0(checker, `${checker}.forbiddenSnippet`, forbidden.path, forbidden.witness, ledger);

  const nf = {
    kind: 'ReadmeReleaseBoundary0NF', checker, version: CHECKER_VERSION, readmePath: loaded.readmePath,
...
async function loadReadmeText0(config) { if (config.readmeText !== null) return { ok: true, text: config.readmeText, readmePath: config.readmePath ?? '<inline>', nf: { kind: 'ReadmeReleaseBoundaryInlineText0NF', byteLength: Buffer.byteLength(config.readmeText, 'utf8') } }; const readmePath = config.readmePath ?? path.join(config.rootDir, 'README.md'); try { const text = await fs.readFile(readmePath, 'utf8'); return { ok: true, text, readmePath, nf: { kind: 'ReadmeReleaseBoundaryFile0NF', readmePath, byteLength: Buffer.byteLength(text, 'utf8') } }; } catch (error) { return validationReject0(['README.md'], 'README.md must be readable UTF-8 text', { readmePath, error: error.message }); } }
function validateRequiredSnippets0(text) { for (const snippet of README_RELEASE_BOUNDARY_REQUIRED_SNIPPETS0) if (!text.includes(snippet)) return validationReject0(['README.md'], 'README release boundary wording is missing a required snippet', { snippet }); return validationAccept0({ kind: 'ReadmeRequiredSnippets0NF', count: README_RELEASE_BOUNDARY_REQUIRED_SNIPPETS0.length }); }
function validateForbiddenSnippets0(text) { for (const snippet of README_RELEASE_BOUNDARY_FORBIDDEN_SNIPPETS0) if (text.includes(snippet)) return validationReject0(['README.md'], 'README release boundary wording contains a forbidden stale or overclaiming snippet', { snippet }); return validationAccept0({ kind: 'ReadmeForbiddenSnippets0NF', count: README_RELEASE_BOUNDARY_FORBIDDEN_SNIPPETS0.length }); }
function ledgerEntry0(phase, result) { return { phase, status: result.ok ? 'pass' : 'fail', digest: digestCanonical0(result.nf ?? result.witness ?? null) }; }
...
function makeRejectRecord0(checker, coord, pathArray, witness, ledger) { const rejectNF = { kind: `${checker}RejectNF`, checker, version: CHECKER_VERSION, coord, path: pathArray, witness, ledger }; const digest = digestCanonical0(rejectNF); return { tag: 'reject', kind: 'reject', checker, version: CHECKER_VERSION, Coord: coord, Path: pathArray, Witness: witness, Digest: digest, Ledger: ledger, coord, path: pathArray, witness, digest, ledger }; }
