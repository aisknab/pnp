...
export async function CheckUniformLockedNANDThreshold0(options = {}) {
  ...
  try {
    const inputFamily = await CheckUniformInputFamily0({ root, writeOutput: false });
    ...
    const construction = await CheckUniformLockedNANDConstruction0({ root, writeOutput: false });
...
async function main0() { let options; try { options = parseArgs0(process.argv.slice(2)); } catch (error) { const verdict = reject0('UniformLockedNANDThreshold.CliBadArgument', [], 'bad CLI argument', normalizeError0(error)); console.error(JSON.stringify(verdict, null, 2)); process.exit(2); } const verdict = await CheckUniformLockedNANDThreshold0(options); const rendered = JSON.stringify(verdict, null, 2); if (options.json || verdict.tag === 'accept') console.log(rendered); else console.error(rendered); process.exit(verdict.tag === 'accept' ? 0 : 1); }
