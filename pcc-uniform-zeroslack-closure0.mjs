...
export async function CheckUniformZeroSlackClosure0(options = {}) {
  ...
  try {
    const minimizer = await CheckUniformResidualBandMinimizer0({ root, writeOutput: false, historicalReplay: true });
...
async function main0() { EnforceHistoricalReplayCli0({ entrypoint: 'pcc-uniform-zeroslack-closure0.mjs' }); let options; try { options = parseArgs0(process.argv.slice(2)); } catch (error) { const verdict = reject0('UniformZeroSlackClosure.CliBadArgument', [], 'bad CLI argument', normalizeError0(error)); console.error(JSON.stringify(verdict, null, 2)); process.exit(2); } options.historicalReplay = true; const verdict = await CheckUniformZeroSlackClosure0(options); const rendered = JSON.stringify(verdict, null, 2); if (options.json || verdict.tag === 'accept') console.log(rendered); else console.error(rendered); process.exit(verdict.tag === 'accept' ? 0 : 1); }
