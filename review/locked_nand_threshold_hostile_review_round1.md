...
## Edward Savage round-1 reduction-boundary issue
...
```text
The locked NAND family is the reduction target.
General NAND circuits enter only as source circuits phi.
The construction phi -> (W^NAND_phi, B^NAND_phi) must be polynomial-time.
The locks must preserve SAT iff, not simplify the instance class.
The minimizer is applied only to W^NAND_phi, whose residual slack is at most four.
```

Reviewers should attack the newly named bridge:
...
```text
1. Does every Boolean formula/circuit reduce to a NAND circuit phi in polynomial time?
2. Does phi -> W^NAND_phi preserve all source assignments?
3. Do trace, occurrence, and lock slots preserve satisfiability iff?
4. Are duplicate source occurrences kept distinct before constraints are imposed?
5. In the unsat case, is the final output zero on the whole carrier rather than only coherent traces?
6. In the sat case, does the final lock force a new nonconstant nonprojection output?
7. Does Lambda(W^NAND_phi) <= 4 hold uniformly?
8. Does the SAT decision avoid assuming arbitrary-topology exact minimization?
```

Failure condition:

```text
A source SAT instance whose locked word is threshold-easy for reasons unrelated to satisfiability, or any place where the construction assumes the answer while building W^NAND_phi.
```
