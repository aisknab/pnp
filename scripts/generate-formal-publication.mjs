...
const REPORT_COORDINATE = 'PNP-CANONICAL-FORMAL-RECONSTRUCTION-REPORT-2026-08-05-104';
...
export async function BuildFormalPublication0(root) {
  ...
  if (publication.gate.passed !== false) throw new Error('Release-84 publication gate must remain intentionally fail-closed');
...
function texBreakableModuleName0(value) {
  return texEscape0(value)
    .replaceAll('.', '.\\allowbreak{}')
    .replace(/(?<=[a-z0-9])(?=[A-Z])/gu, '\\allowbreak{}');
}
