...
export async function RunPNPVerifyAll0(options = {}) {
  ...
  const nodeSyntaxTargets = [
    ...
    'pcc-direct-bind-base-semantics0.mjs',
  ...
  const requiredSteps = [
    ...
    { id: 'report-theorem-inventory-audit', command: process.execPath, args: ['pcc-report-theorem-inventory0.mjs', '--json'], kind: 'json', expectTag: 'accept' },
    ...
    { id: 'base-direct-binding-seed-audit', command: process.execPath, args: ['pcc-direct-bind-base-semantics0.mjs', '--json'], kind: 'json', expectTag: 'accept' },
    { id: 'base-direct-binding-seed-tests', command: process.execPath, args: ['--test', 'audits/direct-bind-base-semantics0.test.mjs'], kind: 'process' },
    ...
    { id: 'release-ladder-audit', command: process.execPath, args: ['pcc-release-ladder0.mjs', '--json'], kind: 'json', expectTag: 'accept' },
    { id: 'release-ladder-tests', command: process.execPath, args: ['--test', 'audits/release-ladder0.test.mjs'], kind: 'process' },
...
async function verifyStatusFile0(root) {
  ...
  requireEqual0(status.baseDirectBindingSeedCoordinate, 'PNP-DIRECT-BIND-BASE-SEMANTICS-SEED-2026-06-27-01', failures, ['baseDirectBindingSeedCoordinate']);
  ...
  requireEqual0(status.releaseLadderCoordinate, 'PNP-RELEASE-LADDER-2026-06-27-01', failures, ['releaseLadderCoordinate']);
...
function runStep0({ id, command, args, root, kind }) { return new Promise((resolve) => { const child = spawn(command, args, { cwd: root, stdio: ['ignore', 'pipe', 'pipe'] }); let stdout = ''; let stderr = ''; let settled = false; child.stdout.on('data', (chunk) => { stdout += chunk.toString('utf8'); }); child.stderr.on('data', (chunk) => { stderr += chunk.toString('utf8'); }); child.on('error', (error) => { if (settled) return; settled = true; resolve({ tag: 'reject', id, kind, coord: 'Process.SpawnFailed', path: [id], command: renderCommand0(command, args), witness: normalizeError0(error) }); }); child.on('close', (code) => { if (settled) return; settled = true; const base = { id, kind, command: renderCommand0(command, args), exitCode: code, stdoutSha256: sha256Hex0(Buffer.from(stdout, 'utf8')), stderrSha256: sha256Hex0(Buffer.from(stderr, 'utf8')), stdoutPreview: preview0(stdout), stderrPreview: preview0(stderr) }; if (code !== 0) { resolve({ tag: 'reject', ...base, coord: 'Process.NonZeroExit', path: [id], witness: { reason: 'process exited non-zero', code } }); return; } if (kind === 'json') { try { resolve({ tag: 'accept', ...base, json: JSON.parse(stdout) }); } catch (error) { resolve({ tag: 'reject', ...base, coord: 'Process.BadJson', path: [id], witness: { reason: 'process stdout was not parseable JSON', error: normalizeError0(error) } }); } return; } resolve({ tag: 'accept', ...base }); }); }); }
...
function parseArgs0(argv) { const options = { root: process.cwd(), outputPath: DEFAULT_OUTPUT_PATH, writeOutput: true, json: false, includeUnitTests: true, includeReleaseAudit: true, python: null }; for (let index = 0; index < argv.length; index += 1) { const arg = argv[index]; if (arg === '--json') options.json = true; else if (arg === '--no-write') options.writeOutput = false; else if (arg === '--skip-unit-tests') options.includeUnitTests = false; else if (arg === '--skip-release-audit') options.includeReleaseAudit = false; else if (arg === '--root') { index += 1; if (index >= argv.length) throw new Error('--root requires a value'); options.root = argv[index]; } else if (arg === '--output') { index += 1; if (index >= argv.length) throw new Error('--output requires a value'); options.outputPath = argv[index]; } else if (arg === '--python') { index += 1; if (index >= argv.length) throw new Error('--python requires a value'); options.python = argv[index]; } else if (arg === '--help' || arg === '-h') { printHelp0(); process.exit(0); } else throw new Error(`unknown argument: ${arg}`); } return options; }
