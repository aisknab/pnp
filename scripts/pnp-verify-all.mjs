...
const NODE_SYNTAX_TARGETS = [
  ...
  'scripts/audit-checker-cycles.mjs', 'scripts/audit-no-hidden-oracle.mjs', 'scripts/audit-determinism.mjs', 'scripts/audit-regeneration-ledger.mjs', 'scripts/pnp-verify-all.mjs',
];
...
export async function RunPNPVerifyAll0(options = {}) {
  ...
  return writeAndReturn0(root, outputPath, writeOutput, { tag: 'accept', kind: 'accept', checker: CHECKER, version: VERSION, claimStatus: 'internal-proof-certificate-stack-accepted-under-public-review-boundary', statusPath: PNP_STATUS_PATH, statusSha256: statusStep.statusSha256, ...Object.fromEntries(Object.entries(EXPECTED_STATUS_FIELDS).filter(([key]) => key.endsWith('Coordinate'))), publicTheoremEmissionAllowed: false, finalTheoremReady: false, activeFinalNodeIds: [], remainingBlockers: [...EXPECTED_BLOCKERS], oneCommand: 'npm run pnp:verify', stepCount: steps.length, acceptedStepCount: steps.filter((step) => step.tag === 'accept').length, steps, outputPath: writeOutput ? outputPath : null });
}

async function verifyStatusFile0(root) {
  const absolutePath = path.join(root, PNP_STATUS_PATH);
  let bytes; let status;
