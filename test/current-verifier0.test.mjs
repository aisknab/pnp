...
test('current verifier plan contains status, progress, surface, archive integrity, and current tests only', () => {
  ...
  assert.equal(CURRENT_VERIFICATION_TESTS0.includes(
    'audits/lean-residual-terminal-governed-support-completion0.test.mjs'), true);
  assert.equal(CURRENT_VERIFICATION_TESTS0.includes(
    'audits/lean-residual-terminal-frontier-pushout0.test.mjs'), true);
  assert.equal(CURRENT_VERIFICATION_TESTS0.includes(
    'audits/lean-residual-terminal-projection-square0.test.mjs'), true);
  assert.equal(CURRENT_VERIFICATION_TESTS0.includes(
    'audits/lean-residual-terminal-side-tight-minimum0.test.mjs'), true);
  assert.equal(CURRENT_VERIFICATION_TESTS0.includes(
