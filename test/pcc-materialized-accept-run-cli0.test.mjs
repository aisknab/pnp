...
test('bin/check-materialized-accept-run0.mjs accepts pending replay with no public conclusion', async (t) => {
...
test('bin/check-materialized-accept-run0.mjs accepts reject replay with one first failure and no public conclusion', async (t) => {
  const { packFilePath, aggregateDigest } = await writeAggregatePack0(t);
  const firstFailure = makeMaterializedReplayFirstFailure0({
    coord: 'Replay.first.bad.bounds',
    path: [
      'PackBytes',
      'Manifest',
      'artefactBounds',
...
test('bin/check-materialized-accept-run0.mjs accepts accepted replay and records public conclusion emission', async (t) => {
  const { packFilePath, aggregateDigest } = await writeAggregatePack0(t);
  const acceptRunFile = await writeTempJsonFile0(
    t,
...
test('bin/check-materialized-accept-run0.mjs --full emits the complete accept-run record', async (t) => {
  const { packFilePath, aggregateDigest } = await writeAggregatePack0(t);
  const acceptRunFile = await writeTempJsonFile0(
    t,
    'MaterializedAcceptRun0.accept.json',
...
test('bin/check-materialized-accept-run0.mjs rejects public conclusion before accepted replay', async (t) => {
  ...
  assert.equal(out.checker, 'CheckMaterializedAcceptRunFile0');
  assert.equal(out.coord, 'CheckMaterializedAcceptRunFile0.acceptRun');
  assert.equal(out.witness.detail.inner.coord, 'CheckMaterializedAcceptRun0.Verdict');
});

test('bin/check-materialized-accept-run0.mjs rejects aggregate path whose package fails', async (t) => {
...
test('npm run materialized:accept-run checks an accepted materialized accept run file', async (t) => {
  const { packFilePath, aggregateDigest } = await writeAggregatePack0(t);
  const acceptRunFile = await writeTempJsonFile0(
    t,
    'MaterializedAcceptRun0.accept.json',
...
test('bin/check-materialized-accept-run0.mjs rejects invalid JSON accept run file', async (t) => {
  ...
  assert.equal(out.tag, 'reject');
  assert.equal(out.checker, 'CheckMaterializedAcceptRunFile0');
  assert.equal(out.coord, 'CheckMaterializedAcceptRunFile0.load');
  assert.equal(out.witness.reason, 'materialized accept run file must parse as JSON');
});
...
test('bin/check-materialized-accept-run0.mjs exits nonzero without a file path', () => {
  ...
  assert.equal(out.tag, 'reject');
  assert.equal(out.checker, 'check-materialized-accept-run0');
  assert.equal(out.coord, 'check-materialized-accept-run0.args');
});
...
async function makeTempDir0(t) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'pnp-materialized-accept-run-cli-'));
